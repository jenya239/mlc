...
results.push_back(test_runner::assert_eq_str(mlc::String("print_expr CppAggregateInit"), cpp_printer::print_expr(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppAggregateInit(mlc::String("Point"), mlc::Array<std::shared_ptr<cpp_ast::CppExpr>>{std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInt(1)), std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInt(2))}))), mlc::String("Point{1, 2}")));
std::shared_ptr<cpp_ast::CppExpr> match_handler = std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppIdent(mlc::String("[&](const auto& __v) { return 1; }")));
results.push_back(test_runner::assert_eq_str(mlc::String("print_expr CppStdVisit"), cpp_printer::print_expr(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppStdVisit(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInt(0)), mlc::Array<std::shared_ptr<cpp_ast::CppExpr>>{match_handler}))), mlc::String("std::visit(overloaded{[&](const auto& __v) { return 1; }\n}, 0)")));
...
mlc::String loop_body = mlc::String("step();\n");
results.push_back(test_runner::assert_eq_str(mlc::String("print_expr CppInvokedWhile"), cpp_printer::print_expr(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInvokedWhile(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppBool(true)), loop_body))), mlc::String("[&]() {\nwhile (true) {\nstep();\n}\n}()")));
results.push_back(test_runner::assert_eq_str(mlc::String("print_expr CppInvokedFor"), cpp_printer::print_expr(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInvokedFor(mlc::String("item"), std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppIdent(mlc::String("items"))), loop_body))), mlc::String("[&]() {\nfor (auto item : items) {\nstep();\n}\n}()")));
results.push_back(test_runner::assert_eq_str(mlc::String("print_expr CppQuestionTry"), cpp_printer::print_expr(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppQuestionTry(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInt(7))))), mlc::String("({ auto __q = 7; if (std::get_if<1>(&__q)) return *std::get_if<1>(&__q); std::get<0>(__q).field0; })")));
results.push_back(test_runner::assert_eq_str(mlc::String("print_expr CppWithBlock"), cpp_printer::print_expr(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppWithBlock(std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppIdent(mlc::String("resource"))), mlc::String("handle"), mlc::String("handle.use();\n")))), mlc::String("{\nauto handle = (resource);\nhandle.use();\nhandle.drop();\n}\n")));
