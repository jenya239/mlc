...
mlc::Array<ast::Diagnostic> infer_undefined_record_method_diagnostics(std::shared_ptr<registry::Type> receiver_type, mlc::String method_name, ast::Span method_span, registry::TypeRegistry registry) noexcept{
return [&]() -> mlc::Array<ast::Diagnostic> { if (std::holds_alternative<registry::TNamed>((*receiver_type))) { auto _v_tnamed = std::get<registry::TNamed>((*receiver_type)); auto [type_name] = _v_tnamed; return infer_undefined_record_method_diagnostics_for_named(type_name, method_name, method_span, registry); } return {}; }();
}

mlc::Array<ast::Diagnostic> infer_undefined_record_method_diagnostics_for_named(mlc::String type_name, mlc::String method_name, ast::Span method_span, registry::TypeRegistry registry) noexcept{
return registry::TypeRegistry_has_fields(registry, type_name) && !registry::TypeRegistry_has_fn(registry, type_name + mlc::String("_") + method_name) && semantic_type_structure::type_is_unknown(semantic_type_structure::builtin_method_return_type(method_name)) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("undefined method: ") + method_name + mlc::String(" on ") + type_name, method_span, diagnostic_codes::diagnostic_code_e082())} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_push(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("push") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::type_is_array(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method push expects an array receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e021())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_set(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("set") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::type_is_array(receiver_type) && !semantic_type_structure::receiver_type_is_map(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method set expects an array or Map receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e022())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_length_or_size(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("length") || method_name == mlc::String("size") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::type_is_string(receiver_type) && !semantic_type_structure::type_is_array(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("methods length and size apply to string or array; receiver is ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e023())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_to_integer_string_only(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("to_i") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::type_is_string(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method to_i expects a string receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e024())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_string_only_methods(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("char_at") || method_name == mlc::String("substring") || method_name == mlc::String("to_lower") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::type_is_string(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method ") + method_name + mlc::String(" expects a string receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e025())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_join(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("join") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::type_is_array(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method join expects an array receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e026())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_has(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("has") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::receiver_type_is_map(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method has expects a Map receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e027())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_get_remove_keys_values(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("get") || method_name == mlc::String("remove") || method_name == mlc::String("keys") || method_name == mlc::String("values") ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::receiver_type_is_map(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method ") + method_name + mlc::String(" expects a Map receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e028())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_array_hof(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return result_option_method_types::should_skip_array_hof_for_receiver(receiver_type) ? mlc::Array<ast::Diagnostic>{} : array_method_types::is_array_hof_method(method_name) ? !semantic_type_structure::type_is_unknown(receiver_type) && !semantic_type_structure::type_is_array(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code(mlc::String("method ") + method_name + mlc::String(" expects an array receiver, got ") + semantic_type_structure::type_description(receiver_type), method_span, diagnostic_codes::diagnostic_code_e029())} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}
