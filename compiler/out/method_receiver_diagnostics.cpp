...
mlc::Array<ast::Diagnostic> infer_undefined_record_method_diagnostics(std::shared_ptr<registry::Type> receiver_type, mlc::String method_name, ast::Span method_span, registry::TypeRegistry registry) noexcept{
...
auto [type_name] = tNamed; return infer_undefined_record_method_diagnostics_for_named(type_name, method_name, method_span, registry);
}
return mlc::Array<ast::Diagnostic>{};
std::abort();
}();
}
mlc::Array<ast::Diagnostic> infer_undefined_record_method_diagnostics_for_named(mlc::String type_name, mlc::String method_name, ast::Span method_span, registry::TypeRegistry registry) noexcept{
  if (((registry::TypeRegistry_has_fields(registry, type_name) && (!registry::TypeRegistry_has_fn(registry, ((type_name + mlc::String("_", 1)) + method_name)))) && semantic_type_structure::type_is_unknown(semantic_type_structure::builtin_method_return_type(method_name))))   {
    return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((((mlc::String("undefined method: ", 18) + method_name) + mlc::String(" on ", 4)) + type_name), method_span, diagnostic_codes::diagnostic_code_e082())};
  } else   {
    return mlc::Array<ast::Diagnostic>{};
  }
}
mlc::Array<ast::Diagnostic> infer_method_receiver_push(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if ((method_name == mlc::String("push", 4)))   {
    if (((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::type_is_array(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((mlc::String("method push expects an array receiver, got ", 43) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e021())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_set(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if ((method_name == mlc::String("set", 3)))   {
    if ((((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::type_is_array(receiver_type))) && (!semantic_type_structure::receiver_type_is_map(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((mlc::String("method set expects an array or Map receiver, got ", 49) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e022())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_length_or_size(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if (((method_name == mlc::String("length", 6)) || (method_name == mlc::String("size", 4))))   {
    if ((((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::type_is_string(receiver_type))) && (!semantic_type_structure::type_is_array(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((mlc::String("methods length and size apply to string or array; receiver is ", 62) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e023())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_to_integer_string_only(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if ((method_name == mlc::String("to_i", 4)))   {
    if (((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::type_is_string(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((mlc::String("method to_i expects a string receiver, got ", 43) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e024())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_string_only_methods(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if ((((method_name == mlc::String("char_at", 7)) || (method_name == mlc::String("substring", 9))) || (method_name == mlc::String("to_lower", 8))))   {
    if (((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::type_is_string(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((((mlc::String("method ", 7) + method_name) + mlc::String(" expects a string receiver, got ", 32)) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e025())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_join(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if ((method_name == mlc::String("join", 4)))   {
    if (((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::type_is_array(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((mlc::String("method join expects an array receiver, got ", 43) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e026())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_has(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if ((method_name == mlc::String("has", 3)))   {
    if (((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::receiver_type_is_map(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((mlc::String("method has expects a Map receiver, got ", 39) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e027())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_get_remove_keys_values(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if (((((method_name == mlc::String("get", 3)) || (method_name == mlc::String("remove", 6))) || (method_name == mlc::String("keys", 4))) || (method_name == mlc::String("values", 6))))   {
    if (((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::receiver_type_is_map(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((((mlc::String("method ", 7) + method_name) + mlc::String(" expects a Map receiver, got ", 29)) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e028())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
...
mlc::Array<ast::Diagnostic> infer_method_receiver_array_hof(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
  if (result_option_method_types::should_skip_array_hof_for_receiver(receiver_type))   {
    ...
    if (((!semantic_type_structure::type_is_unknown(receiver_type)) && (!semantic_type_structure::type_is_array(receiver_type))))     {
      return mlc::Array<ast::Diagnostic>{ast::diagnostic_error_with_code((((mlc::String("method ", 7) + method_name) + mlc::String(" expects an array receiver, got ", 32)) + semantic_type_structure::type_description(receiver_type)), method_span, diagnostic_codes::diagnostic_code_e029())};
    } else     {
      return mlc::Array<ast::Diagnostic>{};
    }
  } else   {
