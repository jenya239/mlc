...
struct StatementsPartialPair {
  mlc::Array<std::shared_ptr<ast::Stmt>> statements;
  int next_serial;
};
struct StatementsBlockAccumulator {
  mlc::Array<std::shared_ptr<ast::Stmt>> accumulated_statements;
  int next_serial;
};
struct ArgumentsPartialPair {
  mlc::Array<std::shared_ptr<ast::Expr>> arguments;
  int next_serial;
};
struct ExpressionListAccumulator {
  mlc::Array<std::shared_ptr<ast::Expr>> mapped_expressions;
  int next_serial;
};
struct RecordLitPartsPartialPair {
  mlc::Array<ast::RecordLitPart> parts;
  int next_serial;
};
struct FieldValuesAccumulator {
  mlc::Array<std::shared_ptr<ast::FieldVal>> mapped_field_values;
  int next_serial;
};
struct PartialNamesAllocation {
  mlc::Array<mlc::String> names;
  int next_serial;
};
struct RecordLitPartsFoldState {
  mlc::Array<ast::RecordLitPart> output_parts;
  int next_serial;
};
struct StmtPartialPair {
  std::shared_ptr<ast::Stmt> statement;
  int next_serial;
};
struct MatchArmsPartialPair {
  mlc::Array<std::shared_ptr<ast::MatchArm>> match_arms;
  int next_serial;
};
