...
return (preds::TKind_is_let(kind) ? [&]() {
auto after_let = preds::Parser_advance(parser);
auto is_const = preds::TKind_is_const(preds::Parser_kind(after_let));
auto name_pos = (is_const ? preds::Parser_advance(after_let) : after_let);
...
}() : (preds::TKind_is_break(kind) ? preds::StmtResult{std::make_shared<ast::Stmt>(ast::StmtBreak{}), preds::Parser_advance(parser)} : (preds::TKind_is_continue(kind) ? preds::StmtResult{std::make_shared<ast::Stmt>(ast::StmtContinue{}), preds::Parser_advance(parser)} : (preds::TKind_is_return(kind) ? [&]() {
...
return (preds::TKind_is_equal(preds::Parser_kind(expr_result.parser)) ? [&]() {
...
auto kind = preds::Parser_kind(parser);
return (preds::TKind_is_ident(kind) ? [&]() {
auto name = preds::TKind_ident(kind);
return ((name == mlc::String("_", 1)) ? preds::PatResult{std::make_shared<ast::Pat>(ast::PatWild{}), preds::Parser_advance(parser)} : (preds::is_ctor_name(name) ? [&]() {
auto after = preds::Parser_advance(parser);
return (preds::TKind_is_lparen(preds::Parser_kind(after)) ? [&]() {
...
return (preds::TKind_is_rparen(preds::Parser_kind(parser)) ? preds::PatsResult{pats, preds::Parser_advance(parser)} : [&]() {
...
auto kind = preds::Parser_kind(parser);
return ((preds::TKind_is_op(kind) && ((preds::TKind_op_val(kind) == mlc::String("!", 1)) || (preds::TKind_op_val(kind) == mlc::String("-", 1)))) ? [&]() {
...
(preds::TKind_is_lparen(preds::Parser_kind(preds::Parser_advance_by(state, 2))) ? [&]() {
...
return (state = margs.parser);
}() : [&]() {
expr = std::make_shared<ast::Expr>(ast::ExprField(expr, field_name));
return (state = preds::Parser_advance_by(state, 2));
}());
...
return (preds::TKind_is_rparen(preds::Parser_kind(parser)) ? preds::ExprsResult{exprs, preds::Parser_advance(parser)} : [&]() {
...
}() : preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprUnit{}), preds::Parser_advance_by(parser, 2)}) : (looks_like_lambda_params(preds::Parser_advance(parser)) ? [&]() {
...
}() : (preds::TKind_is_ident(kind) ? [&]() {
auto name = preds::TKind_ident(kind);
return (preds::TKind_is_fat_arrow(preds::Parser_kind(preds::Parser_advance(parser))) ? [&]() {
...
((!preds::TKind_is_rparen(preds::Parser_kind(state))) ? (ok = false) : (ok = preds::TKind_is_fat_arrow(preds::Parser_kind(preds::Parser_advance(state)))));
}
return ok;
}
...
auto exprs = mlc::Array<std::shared_ptr<ast::Expr>>{};
return (preds::TKind_is_rbracket(preds::Parser_kind(parser)) ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprArray(exprs)), preds::Parser_advance(parser)} : [&]() {
auto first = parse_expr(parser);
exprs.push_back(first.expr);
auto state = first.parser;
while (preds::TKind_is_comma(preds::Parser_kind(state))) {
...
return (preds::TKind_is_else(preds::Parser_kind(then_result.parser)) ? [&]() {
auto after_else = preds::Parser_advance(then_result.parser);
return ((preds::TKind_is_if(preds::Parser_kind(after_else)) || preds::TKind_is_unless(preds::Parser_kind(after_else))) ? [&]() {
...
auto arms = parse_arms(preds::Parser_advance(subject.parser));
