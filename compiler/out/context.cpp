...
void mutate_context_for_let(mlc::String binding_name, std::shared_ptr<registry::Type> value_type, context::CodegenContext& context) noexcept{
return [&]() -> void { if (std::holds_alternative<registry::TShared>((*value_type))) { auto _v_tshared = std::get<registry::TShared>((*value_type)); auto [_w0] = _v_tshared; context.shared_params.push_back(binding_name); return; } if (std::holds_alternative<registry::TArray>((*value_type))) { auto _v_tarray = std::get<registry::TArray>((*value_type)); auto [inner] = _v_tarray; [&]() -> void { if (std::holds_alternative<registry::TShared>((*inner))) { auto _v_tshared = std::get<registry::TShared>((*inner)); auto [_w0] = _v_tshared; context.shared_array_params.push_back(binding_name); return; } context.value_params.push_back(binding_name); return; }(); return; } context.value_params.push_back(binding_name); return; }();
}
