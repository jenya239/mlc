...
mlc::HashMap<mlc::String, bool> collect_globals(ast::Program program) noexcept{
  ...
  names.set(mlc::String("__region_handle_new", 19), true);
...
bool diagnostic_is_warning(ast::Diagnostic diagnostic) noexcept{
  return (diagnostic.severity == mlc::String("warning", 7));
}
...
mlc::Array<ast::Diagnostic> warning_diagnostics_only(mlc::Array<ast::Diagnostic> diagnostics) noexcept{
  return diagnostics.filter([=](ast::Diagnostic diagnostic) mutable { return diagnostic_is_warning(diagnostic); });
}
ast::Result<CheckOut, mlc::Array<mlc::String>> check_program_against_full(ast::Program entry, ast::Program full_program) noexcept{
  ...
  auto warning_strings = ast::diagnostics_to_strings(warning_diagnostics_only(gathered.diagnostics));
  if ((error_diagnostics.length() > 0))   {
    return ast::Err<mlc::Array<mlc::String>>{ast::diagnostics_to_strings(error_diagnostics)};
  } else   {
    return ast::Ok<CheckOut>{CheckOut{{}, warning_strings, gathered.registry}};
  }
}
