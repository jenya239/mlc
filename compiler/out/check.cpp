...
names.set(mlc::String("spawn_task"), true);
names.set(mlc::String("make_channel"), true);
names.set(mlc::String("Channel"), true);
names.set(mlc::String("Arc"), true);
names.set(mlc::String("Mutex"), true);
names.set(mlc::String("File"), true);
names.set(mlc::String("Profile"), true);
names.set(mlc::String("Shared"), true);
names.set(mlc::String("Weak"), true);
...
bool type_is_checkable(std::shared_ptr<registry::Type> type_value, registry::TypeRegistry type_registry) noexcept{return [&]() { if (std::holds_alternative<registry::TI32>((*type_value))) {  return true; } if (std::holds_alternative<registry::TString>((*type_value))) {  return true; } if (std::holds_alternative<registry::TBool>((*type_value))) {  return true; } if (std::holds_alternative<registry::TUnit>((*type_value))) {  return true; } if (std::holds_alternative<registry::TI64>((*type_value))) {  return true; } if (std::holds_alternative<registry::TF64>((*type_value))) {  return true; } if (std::holds_alternative<registry::TU8>((*type_value))) {  return true; } if (std::holds_alternative<registry::TUsize>((*type_value))) {  return true; } if (std::holds_alternative<registry::TChar>((*type_value))) {  return true; } if (std::holds_alternative<registry::TArray>((*type_value))) { auto _v_tarray = std::get<registry::TArray>((*type_value)); auto [inner] = _v_tarray; return type_is_checkable(inner, type_registry); } if (std::holds_alternative<registry::TPair>((*type_value))) { auto _v_tpair = std::get<registry::TPair>((*type_value)); auto [a, b] = _v_tpair; return type_is_checkable(a, type_registry) && type_is_checkable(b, type_registry); } if (std::holds_alternative<registry::TTuple>((*type_value))) { auto _v_ttuple = std::get<registry::TTuple>((*type_value)); auto [type_parameters] = _v_ttuple; return type_parameters.all([type_registry](std::shared_ptr<registry::Type> type_parameter) mutable { return type_is_checkable(type_parameter, type_registry); }); } if (std::holds_alternative<registry::TGeneric>((*type_value))) { auto _v_tgeneric = std::get<registry::TGeneric>((*type_value)); auto [_w0, type_arguments] = _v_tgeneric; return type_arguments.all([type_registry](std::shared_ptr<registry::Type> type_argument) mutable { return type_is_checkable(type_argument, type_registry); }); } if (std::holds_alternative<registry::TFn>((*type_value))) { auto _v_tfn = std::get<registry::TFn>((*type_value)); auto [_w0, _w1] = _v_tfn; return true; } if (std::holds_alternative<registry::TShared>((*type_value))) { auto _v_tshared = std::get<registry::TShared>((*type_value)); auto [_w0] = _v_tshared; return true; } if (std::holds_alternative<registry::TNamed>((*type_value))) { auto _v_tnamed = std::get<registry::TNamed>((*type_value)); auto [name] = _v_tnamed; return type_registry.record_index.field_types.has(registry::resolve_nominal_type_name_for_fields(type_registry, name, {})); } if (std::holds_alternative<registry::TAssoc>((*type_value))) { auto _v_tassoc = std::get<registry::TAssoc>((*type_value)); auto [_w0, _w1] = _v_tassoc; return true; } return false; }();}

bool CheckOut_has_errors(check::CheckOut self) noexcept{return self.errors.size() > 0;}

bool CheckOut_has_warnings(check::CheckOut self) noexcept{return self.warnings.size() > 0;}
...
bool diagnostic_is_error(ast::Diagnostic diagnostic) noexcept{return diagnostic.severity == mlc::String("error");}

bool diagnostic_is_warning(ast::Diagnostic diagnostic) noexcept{return diagnostic.severity == mlc::String("warning");}

mlc::Array<ast::Diagnostic> error_diagnostics_only(mlc::Array<ast::Diagnostic> diagnostics) noexcept{return diagnostics.filter([](ast::Diagnostic diagnostic) mutable { return diagnostic_is_error(diagnostic); });}

mlc::Array<ast::Diagnostic> warning_diagnostics_only(mlc::Array<ast::Diagnostic> diagnostics) noexcept{return diagnostics.filter([](ast::Diagnostic diagnostic) mutable { return diagnostic_is_warning(diagnostic); });}

ast::Result<check::CheckOut, mlc::Array<mlc::String>> check_program_against_full(ast::Program entry, ast::Program full_program) noexcept{
check::Program_check_gathered gathered = gather_program_check(entry, full_program);
mlc::Array<ast::Diagnostic> error_diagnostics = error_diagnostics_only(gathered.diagnostics);
mlc::Array<mlc::String> warning_strings = ast::diagnostics_to_strings(warning_diagnostics_only(gathered.diagnostics));
return error_diagnostics.size() > 0 ? ast::Result<check::CheckOut, mlc::Array<mlc::String>>(ast::Err<mlc::Array<mlc::String>>(ast::diagnostics_to_strings(error_diagnostics))) : ast::Result<check::CheckOut, mlc::Array<mlc::String>>(ast::Ok<check::CheckOut>(check::CheckOut{{}, warning_strings, gathered.registry}));
}
