...
[&](const registry::TTuple& tTuple) { auto [parameter_types] = tTuple; return parameter_types.all([=](std::shared_ptr<registry::Type> entry) mutable { return type_is_send_safe(entry, registry); }); },
[&](const registry::TNamed& tNamed) { auto [record_name] = tNamed; return type_is_send_safe_named(record_name, registry); },
[&](const registry::TGeneric& tGeneric) { auto [type_name, type_arguments] = tGeneric; return type_is_send_safe_generic(type_name, type_arguments, registry); },
...
bool type_is_send_safe_field(mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> field_map, mlc::String field_name, registry::TypeRegistry registry) noexcept{
  if (field_map.has(field_name))   {
    return type_is_send_safe(field_map.get(field_name), registry);
  } else   {
    return false;
  }
}
bool type_is_send_safe_named(mlc::String record_name, registry::TypeRegistry registry) noexcept{
  ...
  return field_names.all([=](mlc::String field_name) mutable { return type_is_send_safe_field(field_map, field_name, registry); });
}
bool type_is_send_safe_generic(mlc::String type_name, mlc::Array<std::shared_ptr<registry::Type>> type_arguments, registry::TypeRegistry registry) noexcept{
  ...
  if ((((((((type_name == mlc::String("Channel", 7)) || (type_name == mlc::String("Task", 4))) || (type_name == mlc::String("Map", 3))) || (type_name == mlc::String("Shared", 6))) || (type_name == mlc::String("Arc", 3))) || (type_name == mlc::String("Mutex", 5))) || (type_name == mlc::String("Future", 6))))   {
    return false;
  }
  if ((type_arguments.length() == 0))   {
    return false;
  }
  return type_arguments.all([=](std::shared_ptr<registry::Type> argument) mutable { return type_is_send_safe(argument, registry); });
}
