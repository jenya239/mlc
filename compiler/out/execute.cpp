...
[&](const ast::Ok<mlc::Array<value::VmValue>>& ok) -> outcome::VmRunOutcome { auto [locals] = ok; return vm_run_frames(program, mlc::Array<frame::VmFrame>{frame::vm_frame_new(function, mir_types::mir_block_id(0), locals)}, trace_enabled); }
}, runtime::vm_bind_call_arguments(function, arguments));
}
outcome::VmRunOutcome vm_failed_outcome(mlc::String message) noexcept{
  return outcome::VmRunFailed{mlc::Array<mlc::String>{message}};
}
outcome::VmRunOutcome vm_run_frames_step(mir_types::MirProgram program, mlc::Array<frame::VmFrame> frames, bool trace_enabled) noexcept{
  ...
  auto frame_index = mlc::arith::checked_sub(frames.length(), 1);
  auto frame = frame::vm_frame_at(frames, frame_index);
  return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> outcome::VmRunOutcome { auto [errors] = err; return outcome::VmRunFailed{errors}; },
[&](const ast::Ok<mir_types::MirBlock>& ok) -> outcome::VmRunOutcome { auto [block] = ok; return ((frame.statement_index < block.stmts.length()) ? (vm_execute_statement(program, frames, frame_index, block.stmts[frame.statement_index], trace_enabled)) : (vm_execute_terminator(program, frames, frame_index, block.term, trace_enabled))); }
...
outcome::VmRunOutcome vm_run_frames(mir_types::MirProgram program, mlc::Array<frame::VmFrame> frames_initial, bool trace_enabled) noexcept{
  auto frames = frames_initial;
  auto done = false;
  auto final_outcome = vm_failed_outcome(mlc::String("vm: trampoline fell through", 27));
  while ((!done))   {
    auto step_outcome = vm_run_frames_step(program, frames, trace_enabled);
    std::visit(overloaded{[&](const outcome::VmRunContinue& vmRunContinue) -> void { auto [next_frames] = vmRunContinue; [&]() {
(frames = next_frames);
return std::make_tuple();
}(); },
[&](const outcome::VmRunReturn& vmRunReturn) -> void { auto [value] = vmRunReturn; [&]() {
(final_outcome = outcome::VmRunReturn{value});
(done = true);
return std::make_tuple();
}(); },
[&](const outcome::VmRunFailed& vmRunFailed) -> void { auto [errors] = vmRunFailed; [&]() {
(final_outcome = outcome::VmRunFailed{errors});
(done = true);
return std::make_tuple();
}(); }
}, step_outcome);
...
outcome::VmRunOutcome vm_execute_user_call(mir_types::MirProgram program, mlc::Array<frame::VmFrame> frames, int frame_index, mir_types::LocalId local_id, mir_types::MirFunction callee, mlc::Array<value::VmValue> argument_values, mlc::String callee_name, bool trace_enabled) noexcept{
  ...
  return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> outcome::VmRunOutcome { auto [errors] = err; return outcome::VmRunFailed{errors}; },
[&](const ast::Ok<mlc::Array<value::VmValue>>& ok) -> outcome::VmRunOutcome { auto [locals] = ok; return [&]() {
next_frames.push_back(frame::vm_frame_new(callee, mir_types::mir_block_id(0), locals));
if (trace_enabled) {
  mlc::io::println(((mlc::String("vm call ", 8) + mlc::to_string(callee_name)) + mlc::String("", 0)));
}
return outcome::VmRunContinue{next_frames};
}(); }
}, runtime::vm_bind_call_arguments(callee, argument_values));
}
...
mlc::Array<frame::VmFrame> vm_drop_last_frame(mlc::Array<frame::VmFrame> frames) noexcept{
  ...
  auto index = 0;
  while ((index < mlc::arith::checked_sub(frames.length(), 1)))   {
...
outcome::VmRunOutcome vm_pop_return(mir_types::MirProgram program, mlc::Array<frame::VmFrame> frames, int frame_index, value::VmValue return_value, bool trace_enabled) noexcept{
  ...
  auto caller_index = mlc::arith::checked_sub(caller_frames.length(), 1);
  ...
  return outcome::VmRunContinue{frame::vm_frame_advance_statement(caller_frames, caller_index)};
}
