...
return std::visit(overloaded{[&](const semantic_ir::SExprArray& sExprArray) { auto [elements, __1, array_span] = sExprArray; return ((elements.length() > 0) ? (eval_expr(else_expression, context, gen_stmts)) : (std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_then] = tArray; return eval_expr(std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprArray{elements, std::make_shared<registry::Type>(registry::TArray{inner_then}), array_span}), context, gen_stmts); },
...
return expr::ternary_conditional(eval_expr(condition, context, gen_stmts), eval_expr(then_expr, context, gen_stmts), eval_expression_else_branch_with_conditional_empty_array_coercion(then_expr, else_expr, context, gen_stmts));
}
