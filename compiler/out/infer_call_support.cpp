...
mlc::String owner_name = [&]() -> mlc::String { if (std::holds_alternative<registry::TNamed>((*constructor_type))) { auto _v_tnamed = std::get<registry::TNamed>((*constructor_type)); auto [type_name] = _v_tnamed; return type_name; } return mlc::String(""); }();
...
infer_result::InferResult infer_expr_call_non_constructor(infer_result::InferResult function_parsed, infer_result::InferResult with_arguments, mlc::Array<std::shared_ptr<ast::Expr>> call_arguments, ast::Span call_source_span, registry::TypeRegistry registry, mlc::String callee_name) noexcept{
std::shared_ptr<registry::Type> return_type = [&]() -> std::shared_ptr<registry::Type> { if (std::holds_alternative<registry::TFn>((*function_parsed.inferred_type))) { auto _v_tfn = std::get<registry::TFn>((*function_parsed.inferred_type)); auto [_w0, return_ty] = _v_tfn; return return_ty; } return std::make_shared<registry::Type>((registry::TUnknown{})); }();
