...
mlc::String cpp_generic_base_name(context::CodegenContext context, mlc::String type_name) noexcept{return type_name == mlc::String("Map") ? mlc::String("mlc::HashMap") : type_name == mlc::String("Shared") ? mlc::String("std::shared_ptr") : type_name == mlc::String("Weak") ? mlc::String("std::weak_ptr") : type_name == mlc::String("Task") ? mlc::String("mlc::Task") : type_name == mlc::String("Future") ? mlc::String("mlc::Task") : type_name == mlc::String("Channel") ? mlc::String("mlc::concurrency::Channel") : type_name == mlc::String("Arc") ? mlc::String("mlc::concurrency::Arc") : type_name == mlc::String("Mutex") ? mlc::String("mlc::concurrency::Mutex") : context::CodegenContext_resolve(context, type_name);}
...
mlc::String associated_type_name_cpp(context::CodegenContext context, mlc::String associated_type_name) noexcept{return context::trait_has_associated_types(context, context.self_type) ? mlc::String("typename Self::") + cpp_naming::cpp_safe(associated_type_name) : associated_type_name;}
...
mlc::String type_name_to_cpp(context::CodegenContext context, mlc::String type_name) noexcept{return type_alias_annotation_cpp(context, type_name) != mlc::String("") ? type_alias_annotation_cpp(context, type_name) : type_name == mlc::String("Self") || type_name == mlc::String("self") ? type_name_to_cpp_self(context) : context::trait_has_associated_types(context, context.self_type) && context.trait_associated_type_names.get(context.self_type).any([type_name](mlc::String entry) mutable { return entry == type_name; }) ? associated_type_name_cpp(context, type_name) : type_name_to_cpp_literal(type_name) != mlc::String("") ? type_name_to_cpp_literal(type_name) : resolved_type_name_cpp(context, type_name);}
...
return field_definition->has_default_expression && record_field_default_validate::record_field_default_expression_is_static_initializer(field_definition->default_expression) ? base + mlc::String(" = ") + record_field_default_emit::record_field_default_expression_cpp_initializer(field_definition->default_expression, context) : base;
}
