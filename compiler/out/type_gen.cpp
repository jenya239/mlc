...
mlc::String sem_type_to_cpp(context::CodegenContext context, std::shared_ptr<registry::Type> semantic_type) noexcept{return std::visit(overloaded{
  ...
  [&](const TTuple& ttuple) -> mlc::String { auto [types] = ttuple; return mlc::String("std::tuple<") + types.map([context](std::shared_ptr<registry::Type> t) mutable { return sem_type_to_cpp(context, t); }).join(mlc::String(", ")) + mlc::String(">"); },
  ...
  [&](const TFn& tfn) -> mlc::String { auto [param_types, return_type] = tfn; return expr::cpp_std_function_type(sem_type_to_cpp(context, return_type), param_types.map([context](std::shared_ptr<registry::Type> t) mutable { return sem_type_to_cpp(context, t); }).join(mlc::String(", "))); },
...
mlc::String type_to_cpp(context::CodegenContext context, std::shared_ptr<ast::TypeExpr> type_expr) noexcept{return std::visit(overloaded{
  ...
  [&](const TyFn& tyfn) -> mlc::String { auto [params, ret] = tyfn; return expr::cpp_std_function_type(type_to_cpp(context, ret), params.map([context](std::shared_ptr<ast::TypeExpr> p) mutable { return type_to_cpp(context, p); }).join(mlc::String(", "))); },
...
bool type_param_in_typeexpr_list(mlc::String param, mlc::Array<std::shared_ptr<ast::TypeExpr>> types) noexcept{return types.any([param](std::shared_ptr<ast::TypeExpr> t) mutable { return type_param_in_typeexpr(param, t); });}
...
mlc::Array<std::shared_ptr<ast::TypeExpr>> variant_field_typeexprs(std::shared_ptr<ast::TypeVariant> variant) noexcept{return std::visit(overloaded{
  ...
  [&](const VarRecord& varrecord) -> mlc::Array<std::shared_ptr<ast::TypeExpr>> { auto [_w0, fds, _w1] = varrecord; return fds.map([](std::shared_ptr<ast::FieldDef> fd) mutable { return fd->typ; }); },
...
return type_params.filter([field_types](mlc::String p) mutable { return type_param_in_typeexpr_list(p, field_types); });
}

mlc::Array<mlc::String> union_string_lists(mlc::Array<mlc::String> a, mlc::Array<mlc::String> b) noexcept{return a.concat(b.filter([a](mlc::String s) mutable { return !a.any([s](mlc::String x) mutable { return x == s; }); }));}

mlc::Array<mlc::String> type_phantom_params_for_variants(mlc::Array<mlc::String> type_params, mlc::Array<std::shared_ptr<ast::TypeVariant>> variants) noexcept{
auto all_fields = mlc::collections::flat_map(variants, [](std::shared_ptr<ast::TypeVariant> v) mutable { return variant_field_typeexprs(v); });
return type_params.filter([all_fields](mlc::String p) mutable { return !type_param_in_typeexpr_list(p, all_fields); });
}
...
mlc::String forward_decls = variants.map([context, type_params, phantom](std::shared_ptr<ast::TypeVariant> v) mutable { return variant_forward_decl_line(context, type_params, phantom, v); }).join(mlc::String(""));
mlc::String alias_parts = variants.map([context, type_params, phantom](std::shared_ptr<ast::TypeVariant> v) mutable { return variant_alias_part(context, type_params, phantom, v); }).join(mlc::String(", "));
...
mlc::String gen_adt_defs(context::CodegenContext context, mlc::String type_name, mlc::Array<mlc::String> type_params, mlc::Array<std::shared_ptr<ast::TypeVariant>> variants) noexcept{
mlc::Array<mlc::String> phantom = type_phantom_params_for_variants(type_params, variants);
return variants.map([context, type_name, type_params, phantom](std::shared_ptr<ast::TypeVariant> v) mutable { return variant_def_with_prefix(context, type_name, type_params, phantom, v); }).join(mlc::String(""));
}
...
mlc::String derive_display_record_fields(mlc::String type_name, mlc::Array<std::shared_ptr<ast::FieldDef>> field_defs) noexcept{return mlc::String("mlc::String(\"") + type_name + mlc::String(" { ") + field_defs[0]->name + mlc::String(": \") + ") + derive_field_display(field_defs[0]->name, field_defs[0]->typ) + field_defs.drop(1).fold(mlc::String(""), [](mlc::String acc, std::shared_ptr<ast::FieldDef> fd) mutable { return acc + mlc::String(" + mlc::String(\", ") + fd->name + mlc::String(": \") + ") + derive_field_display(fd->name, fd->typ); }) + mlc::String(" + mlc::String(\" }\")");}
...
return fds.size() == 0 ? mlc::String("  if (std::holds_alternative<") + name + mlc::String(">(self._)) return mlc::String(\"") + name + mlc::String("\");\n") : [&]() -> mlc::String { 
  mlc::String parts = mlc::String("mlc::to_string(std::get<") + name + mlc::String(">(self._).") + fds[0]->name + mlc::String(")") + fds.drop(1).fold(mlc::String(""), [name](mlc::String acc, std::shared_ptr<ast::FieldDef> fd) mutable { return acc + mlc::String(" + mlc::String(\", \") + mlc::to_string(std::get<") + name + mlc::String(">(self._).") + fd->name + mlc::String(")"); });
...
mlc::String derive_display_sum(mlc::String type_name, mlc::Array<std::shared_ptr<ast::TypeVariant>> variants) noexcept{
mlc::String cases = variants.map([](std::shared_ptr<ast::TypeVariant> v) mutable { return derive_display_variant_case(v); }).join(mlc::String(""));
...
return field_defs.size() == 0 ? sig + mlc::String(" { return true; }\n") : [&]() -> mlc::String { 
  mlc::String cond = mlc::String("a.") + field_defs[0]->name + mlc::String(" == b.") + field_defs[0]->name + field_defs.drop(1).fold(mlc::String(""), [](mlc::String acc, std::shared_ptr<ast::FieldDef> fd) mutable { return acc + mlc::String(" && a.") + fd->name + mlc::String(" == b.") + fd->name; });
...
mlc::String derive_hash_record_variant_inner(mlc::String variant_struct_name, mlc::Array<std::shared_ptr<ast::FieldDef>> field_defs, int discriminant_index) noexcept{return field_defs.fold(derive_hash_combine_line(mlc::String("size_t"), mlc::to_string(discriminant_index)), [variant_struct_name](mlc::String acc, std::shared_ptr<ast::FieldDef> fd) mutable { return acc + derive_hash_combine_line(derive_hash_std_cpp_type(fd->typ), mlc::String("std::get<") + variant_struct_name + mlc::String(">(self._).") + fd->name); });}
