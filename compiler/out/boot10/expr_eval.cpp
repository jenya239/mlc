...
return expr::ternary_conditional(eval_expr(condition, context, gen_stmts), eval_expr(then_expr, context, gen_stmts), std::visit(overloaded{[&](const semantic_ir::SExprArray& sExprArray) { auto [elements, __1, array_span] = sExprArray; return ((elements.length() > 0) ? (eval_expr(else_expr, context, gen_stmts)) : (std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_from_conditional] = tArray; return eval_expr(std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprArray{elements, std::make_shared<registry::Type>(registry::TArray{inner_from_conditional}), array_span}), context, gen_stmts); },
...
}, (*semantic_ir::sexpr_type(then_expr))); }
}, (*if_semantic_type)))); },
[&](const auto& __v) { return eval_expr(else_expr, context, gen_stmts); }
}, (*else_expr)));
}
