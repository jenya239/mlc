...
}() : (((preds::TKind_is_ident(first_kind) && preds::is_ctor_name(preds::TKind_ident(first_kind))) && preds::TKind_is_lbrace(preds::Parser_kind(preds::Parser_advance(after_eq)))) ? [&]() {
auto field_defs_result = parse_field_defs(preds::Parser_advance_by(after_eq, 2));
auto variants = mlc::Array<std::shared_ptr<ast::TypeVariant>>{};
