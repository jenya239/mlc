...
predicates::Parser parser_expect_generic_close(predicates::Parser state, bool opened) noexcept{return !opened ? state : predicates::TokenKind_is_op(predicates::Parser_kind(state)) && predicates::TokenKind_op_val(predicates::Parser_kind(state)) == mlc::String(">") ? predicates::Parser_advance(state) : predicates::Parser_record_parse_error(state, mlc::String("parse: unclosed generic parameter list"));}
...
{
state = predicates::Parser_advance(state);
while (!predicates::Parser_at_eof(state) && !(predicates::TokenKind_is_op(predicates::Parser_kind(state)) && predicates::TokenKind_op_val(predicates::Parser_kind(state)) == mlc::String(">"))){
{
state = predicates::Parser_advance(state);
}
}
