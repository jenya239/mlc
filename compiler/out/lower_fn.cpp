...
MirLowerBlockStep mir_lower_allocate_block_index(MirLowerState state) noexcept{
  ...
  return MirLowerBlockStep{MirLowerState{state.name_environment, state.locals, state.finished_blocks, state.current_block_id, state.current_label, state.current_stmts, block_step.pool}, block_step.block_id};
}
MirLowerState mir_lower_finish_current_block(MirLowerState state, mir_types::MirTerminator terminator) noexcept{
  ...
  return MirLowerState{state.name_environment, state.locals, finished_blocks, state.current_block_id, state.current_label, {}, state.id_pool};
}
MirLowerState mir_lower_start_block(MirLowerState state, mir_types::BlockId block_id, mlc::String label) noexcept{
  return MirLowerState{state.name_environment, state.locals, state.finished_blocks, block_id, label, {}, state.id_pool};
}
MirLowerState mir_lower_bind_params(MirLowerState state, mlc::Array<std::shared_ptr<ast::Param>> params) noexcept{
  ...
  return MirLowerState{environment, state.locals, state.finished_blocks, state.current_block_id, state.current_label, state.current_stmts, mir_ids::mir_id_pool_locals_reserved(state.id_pool, index)};
}
MirLowerLocalStep mir_lower_allocate_local(MirLowerState state, mlc::String name, std::shared_ptr<registry::Type> type_value) noexcept{
  ...
  return MirLowerLocalStep{MirLowerState{state.name_environment, locals, state.finished_blocks, state.current_block_id, state.current_label, state.current_stmts, local_step.pool}, local_step.local_id};
}
MirLowerState mir_lower_bind_name(MirLowerState state, mlc::String name, mir_types::LocalId local_id) noexcept{
  ...
  return MirLowerState{environment, state.locals, state.finished_blocks, state.current_block_id, state.current_label, state.current_stmts, state.id_pool};
}
...
MirLowerState mir_lower_emit_assign(MirLowerState state, mir_types::LocalId local_id, mir_types::MirRvalue rvalue) noexcept{
  ...
  return MirLowerState{state.name_environment, state.locals, state.finished_blocks, state.current_block_id, state.current_label, statements, state.id_pool};
}
...
bool mir_lower_is_mutating_method(mlc::String method_name) noexcept{
  return ((method_name == mlc::String("push", 4)) || (method_name == mlc::String("set", 3)));
}
...
if (std::holds_alternative<semantic_ir::SemanticExpressionIdent>((*__match_subject))) {
const semantic_ir::SemanticExpressionIdent& semanticExpressionIdent = std::get<semantic_ir::SemanticExpressionIdent>((*__match_subject));
auto [type_name, __1, __2] = semanticExpressionIdent; return ((((type_name == mlc::String("Map", 3)) && (method_name == mlc::String("new", 3))) && (arguments.length() == 0)) ? (mir_lower_map_new_local(state)) : (mir_lower_method_call_on_operand(state, object, method_name, arguments)));
}
...
[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerOperandStep, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; }
...
[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerRvalueStep, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; }
}, mir_lower_lookup_local(state, name));
}
if (std::holds_alternative<semantic_ir::SemanticExpressionBin>((*__match_subject))) {
const semantic_ir::SemanticExpressionBin& semanticExpressionBin = std::get<semantic_ir::SemanticExpressionBin>((*__match_subject));
...
auto branch_state = mir_lower_finish_current_block(then_block_step.state, mir_types::MirCondJump{condition_step.operand, then_block_step.block_id, else_block_step.block_id});
...
[&](const ast::Ok<MirLowerOperandStep>& ok) -> ast::Result<MirLowerOperandStep, mlc::Array<mlc::String>> { auto [and_step] = ok; return mir_lower_ctor_subpattern_tests(and_step.state, scrutinee, sub_patterns, and_step.operand, (index + 1)); }
...
auto [__0] = patternWild; return mir_lower_ctor_subpattern_tests(state, scrutinee, sub_patterns, accumulated, (index + 1));
}
...
auto [__0, __1] = patternIdent; return mir_lower_ctor_subpattern_tests(state, scrutinee, sub_patterns, accumulated, (index + 1));
}
...
auto [__0] = patternUnit; return mir_lower_ctor_subpattern_tests(state, scrutinee, sub_patterns, accumulated, (index + 1));
}
...
ast::Result<MirLowerState, mlc::Array<mlc::String>> mir_lower_bind_ctor_subpatterns(MirLowerState state, mir_types::MirOperand scrutinee, mlc::Array<std::shared_ptr<ast::Pattern>> sub_patterns, int index) noexcept{
  if ((index >= sub_patterns.length()))   {
    ...
    return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
[&](const ast::Ok<MirLowerState>& ok) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [bound_state] = ok; return mir_lower_bind_ctor_subpatterns(bound_state, scrutinee, sub_patterns, (index + 1)); }
...
ast::Result<MirLowerState, mlc::Array<mlc::String>> mir_lower_match_arm_body_with_binding(MirLowerState state, mir_types::MirOperand scrutinee, std::shared_ptr<semantic_ir::SemanticMatchArm> arm) noexcept{
  if (arm->has_guard)   {
    return ast::Err<mlc::Array<mlc::String>>{mlc::Array<mlc::String>{mlc::String("mir lower: match guards not supported in staged lowering", 56)}};
  } else   {
    return [&]() -> ast::Result<MirLowerState, mlc::Array<mlc::String>> {
...
if (std::holds_alternative<ast::PatternCtor>((*__match_subject))) {
const ast::PatternCtor& patternCtor = std::get<ast::PatternCtor>((*__match_subject));
auto [__0, sub_patterns, __2] = patternCtor; return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
[&](const ast::Ok<MirLowerState>& ok) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [bound_state] = ok; return mir_lower_return_expression(bound_state, arm->body); }
...
ast::Result<MirLowerState, mlc::Array<mlc::String>> mir_lower_match_arms_return(MirLowerState state, mir_types::MirOperand scrutinee, mlc::Array<std::shared_ptr<semantic_ir::SemanticMatchArm>> arms, int index) noexcept{
  ...
  } else if ((index == (arms.length() - 1)))   {
    return mir_lower_match_arm_body_with_binding(state, scrutinee, arms[index]);
...
return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
[&](const ast::Ok<MirLowerState>& ok) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [arm_done_state] = ok; return mir_lower_match_arms_return(mir_lower_start_block(arm_done_state, next_test_step.block_id, mlc::String("match_test", 10)), scrutinee, arms, (index + 1)); }
...
ast::Result<MirLowerState, mlc::Array<mlc::String>> mir_lower_question_return(MirLowerState state, std::shared_ptr<semantic_ir::SemanticExpression> inner) noexcept{
  return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
[&](const ast::Ok<MirLowerLocalStep>& ok) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [result_step] = ok; return [&]() {
...
auto error_value_step = mir_lower_allocate_local(branch_state, mlc::String("", 0), mir_lower_unknown_type());
(branch_state = mir_lower_emit_call_assign(error_value_step.state, error_value_step.local_id, mlc::String("__mir_result_err_value", 22), mlc::Array<mir_types::MirOperand>{mir_types::MirOperandLocal{result_step.local_id}}));
...
return ast::Ok<MirLowerState>{mir_lower_finish_return(branch_state, mir_types::MirOperandLocal{ok_value_step.local_id})};
}(); }
...
auto [name, __1, value, type_value, __4] = semanticStatementLet; return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
...
auto [name, value, type_value, __3] = semanticStatementLetConst; return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
[&](const ast::Ok<MirLowerRvalueStep>& ok) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [rvalue_step] = ok; return [&]() {
auto local_step = mir_lower_allocate_local(rvalue_step.state, name, type_value);
auto next_state = mir_lower_bind_name(local_step.state, name, local_step.local_id);
return ast::Ok<MirLowerState>{mir_lower_emit_assign(next_state, local_step.local_id, rvalue_step.rvalue)};
...
if (std::holds_alternative<semantic_ir::SemanticExpressionIf>((*__match_subject))) {
const semantic_ir::SemanticExpressionIf& semanticExpressionIf = std::get<semantic_ir::SemanticExpressionIf>((*__match_subject));
auto [__0, __1, __2, __3, __4] = semanticExpressionIf; return ast::Err<mlc::Array<mlc::String>>{mlc::Array<mlc::String>{mlc::String("mir lower: if expression statement requires return context", 58)}};
}
return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
[&](const ast::Ok<MirLowerOperandStep>& ok) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [operand_step] = ok; return ast::Ok<MirLowerState>{operand_step.state}; }
}, mir_lower_operand_from_expression(state, expression));
...
ast::Result<MirLowerState, mlc::Array<mlc::String>> mir_lower_statements_from_index(MirLowerState state, mlc::Array<std::shared_ptr<semantic_ir::SemanticStatement>> statements, int index) noexcept{
  if ((index >= statements.length()))   {
    ...
    return std::visit(overloaded{[&](const ast::Err<mlc::Array<mlc::String>>& err) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [errors] = err; return ast::Err<mlc::Array<mlc::String>>{errors}; },
[&](const ast::Ok<MirLowerState>& ok) -> ast::Result<MirLowerState, mlc::Array<mlc::String>> { auto [next_state] = ok; return mir_lower_statements_from_index(next_state, statements, (index + 1)); }
...
auto [name, __1, __2, params, return_type, body, __6, __7] = semanticDeclarationFn; return [&]() {
