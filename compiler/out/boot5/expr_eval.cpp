...
return expr::ternary_conditional(eval_expr(condition, context, gen_stmts), eval_expr(then_expr, context, gen_stmts), expression_support::generate_conditional_else_with_empty_array_coercion(then_expr, else_expr, if_semantic_type, context, [=](auto expression, auto evaluation_context) { return eval_expr(expression, evaluation_context, gen_stmts); }));
}
