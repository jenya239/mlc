...
[&](const semantic_ir::SStmtLetConst& sStmtLetConst) { auto [name, value, binding_semantic_type, __3] = sStmtLetConst; return context::GenStmtResult{expr::constexpr_auto_binding_statement(cpp_naming::cpp_safe(name), std::visit(overloaded{[&](const semantic_ir::SExprIf& sExprIf) { auto [condition, then_branch, else_branch, __3, __4] = sExprIf; return expr::ternary_conditional(gen_expr_fn(condition, context), gen_expr_fn(then_branch, context), expression_support::generate_conditional_else_with_empty_array_coercion(then_branch, else_branch, binding_semantic_type, context, gen_expr_fn)); },
[&](const auto& __v) { return gen_expr_fn(value, context); }
}, (*value))), try_counter}; },
