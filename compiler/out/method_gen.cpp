...
if ((__match_subject == mlc::String("ok", 2))) {
  return (is_option ? (method_name) : (mlc::String("ok_into_optional", 16)));
} else {
  return method_name;
}
}();
...
auto [generic_name, __1] = tGeneric; return [&]() -> mlc::String {
auto __match_subject = generic_name;
if ((__match_subject == mlc::String("Result", 6))) {
  return gen_result_option_combinator_call_using_fragments(false, receiver_fragment, method_name, trailing_arguments_joined);
} else if ((__match_subject == mlc::String("Option", 6))) {
  return gen_result_option_combinator_call_using_fragments(true, receiver_fragment, method_name, trailing_arguments_joined);
} else {
  return mlc::String("", 0);
}
}();
...
if (std::holds_alternative<registry::TNamed>((*__match_subject))) {
const registry::TNamed& tNamed = std::get<registry::TNamed>((*__match_subject));
auto [receiver_named] = tNamed; return [&]() -> mlc::String {
auto __match_subject = receiver_named;
if ((__match_subject == mlc::String("Result", 6))) {
  return gen_result_option_combinator_call_using_fragments(false, receiver_fragment, method_name, trailing_arguments_joined);
} else if ((__match_subject == mlc::String("Option", 6))) {
  return gen_result_option_combinator_call_using_fragments(true, receiver_fragment, method_name, trailing_arguments_joined);
...
std::abort();
  ...
  auto invoke_expression = [&]() -> mlc::String {
...
auto [parameter_names, body, __2, __3] = semanticExpressionLambda; return [&]() -> auto {
...
auto lock_callback = std::make_shared<cpp_ast::CppExpression>(cpp_ast::CppMutableLambda{mlc::Array{std::make_shared<cpp_ast::CppCapture>(cpp_ast::CppCapture{mlc::String("", 0), true})}, mlc::Array{std::make_shared<cpp_ast::CppParam>(cpp_ast::CppParam{parameter_name, std::make_shared<cpp_ast::CppType>(cpp_ast::CppTypeName{((mlc::String("", 0) + mlc::to_string(inner_type_cpp)) + mlc::String("&", 1))})})}, body_expression});
return std::make_shared<cpp_ast::CppExpression>(cpp_ast::CppCall{std::make_shared<cpp_ast::CppExpression>(cpp_ast::CppMember{receiver_expression, mlc::String("lock", 4), false}), mlc::Array{lock_callback}});
}();
  } else   {
...
if (std::holds_alternative<registry::TGeneric>((*__match_subject))) {
const registry::TGeneric& tGeneric = std::get<registry::TGeneric>((*__match_subject));
auto [generic_name, __1] = tGeneric; return [&]() -> std::shared_ptr<cpp_ast::CppExpression> {
auto __match_subject = generic_name;
if ((__match_subject == mlc::String("Result", 6))) {
  return gen_result_option_combinator_call_cpp(false, receiver_expression, method_name, trailing_argument_expressions);
} else if ((__match_subject == mlc::String("Option", 6))) {
  return gen_result_option_combinator_call_cpp(true, receiver_expression, method_name, trailing_argument_expressions);
} else {
...
if (std::holds_alternative<registry::TNamed>((*__match_subject))) {
const registry::TNamed& tNamed = std::get<registry::TNamed>((*__match_subject));
auto [receiver_named] = tNamed; return [&]() -> std::shared_ptr<cpp_ast::CppExpression> {
auto __match_subject = receiver_named;
if ((__match_subject == mlc::String("Result", 6))) {
  return gen_result_option_combinator_call_cpp(false, receiver_expression, method_name, trailing_argument_expressions);
} else if ((__match_subject == mlc::String("Option", 6))) {
