...
auto type_errors = (type_mismatch ? (mlc::Array<ast::Diagnostic>{ast::diagnostic_error((((((mlc::String("field ", 6) + field_value->name) + mlc::String(": expected ", 11)) + semantic_type_structure::type_description(expected_type)) + mlc::String(", got ", 6)) + semantic_type_structure::type_description(actual_type)), ast::expr_span(field_value->val))}) : ({}));
...
auto private_errors = ((registry::TypeRegistry_is_private_ctor(inference_context.registry, type_name) && (inference_context.current_extend_type != type_name)) ? (mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("private constructor: cannot construct ", 38) + type_name) + mlc::String(" outside its extend block", 25)), span)}) : ({}));
auto base = infer_result::InferResult{std::make_shared<registry::Type>(registry::TNamed{type_name}), private_errors};
...
[&](const ast::ExprRecord& exprRecord) { auto [name, field_vals, span] = exprRecord; return infer_expr_record(name, field_vals, inference_context, span); },
...
collected_errors = ast::diagnostics_append(collected_errors, value_result.errors);
auto is_refutable = std::visit(overloaded{[&](const ast::PatCtor& patCtor) { auto [__0, __1, __2] = patCtor; return true; },
[&](const auto& __v) { return false; }
}, (*pattern));
if ((is_refutable && (!has_else))) {
collected_errors = ast::diagnostics_append(collected_errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("refutable pattern in let requires `else` branch", 47), source_span)});
}
if (has_else) {
auto else_result = infer_expr(else_body, check_context::check_context_new(current_environment, inference_context.registry));
