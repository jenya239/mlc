...
return std::visit(overloaded{[&](const semantic_ir::SExprArray& sExprArray) { auto [elements, __1, array_span] = sExprArray; return ((elements.length() > 0) ? (eval_expr(else_expression, context, gen_stmts)) : (std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_from_conditional] = tArray; return eval_expr(std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprArray{elements, std::make_shared<registry::Type>(registry::TArray{inner_from_conditional}), array_span}), context, gen_stmts); },
[&](const auto& __v) { return std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_then] = tArray; return eval_expr(std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprArray{elements, std::make_shared<registry::Type>(registry::TArray{inner_then}), array_span}), context, gen_stmts); },
...
return (((((((type_name + mlc::String("_", 1)) + method) + mlc::String("(", 1)) + eval_expr(left_expr, context, gen_stmts)) + mlc::String(", ", 2)) + eval_expr(right_expr, context, gen_stmts)) + mlc::String(")", 1));
} else {
...
return expr::ternary_conditional(eval_expr(condition, context, gen_stmts), eval_expr(then_expr, context, gen_stmts), eval_expression_else_branch_with_conditional_empty_array_coercion(then_expr, else_expr, if_semantic_type, context, gen_stmts));
}
