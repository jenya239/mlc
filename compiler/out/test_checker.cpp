...
results.push_back(test_runner::assert_true(mlc::String("E4: record field default call expression rejected for codegen"), first_checker_error_line(mlc::String("fn zero() -> i32 = 0\ntype R = { x: i32 = zero() }\nfn main() -> i32 = R {} .x")).contains(mlc::String("cannot be lowered"))));
results.push_back(test_runner::assert_true(mlc::String("E4: record field default still type-checked when shape ok"), first_checker_error_line(mlc::String("type Bad = { x: i32 = \"wrong\" }\nfn main() -> i32 = 0")).contains(mlc::String("type mismatch"))));
...
results.push_back(test_runner::assert_true(mlc::String("D3 trait and type same name — conflict diagnostic"), first_checker_error_line(mlc::String("type Ambiguous { fn method(self: Self) -> unit }\ntype Ambiguous = VariantOne | VariantTwo")).contains(mlc::String("both a type and a trait"))));
...
results.push_back(test_runner::assert_true(mlc::String("D4 where clause unknown type parameter — diagnostic"), first_checker_error_line(mlc::String("fn f<T>(x: T) -> unit where U: Display = ()")).contains(mlc::String("unknown type parameter"))));
...
results.push_back(test_runner::assert_eq_int(mlc::String("mutation: assign inside fold reducer to outer let mut — 0 errors"), check_error_count(mlc::String("fn fold_mut_touch(visited: [i32]) -> i32 = do let mut progress_count = 0; let summed = visited.fold(0, (accumulator, element_under_fold) => do progress_count = progress_count + 1; accumulator + element_under_fold end); summed + progress_count end")), 0));
