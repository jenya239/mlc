...
infer_result::InferResult infer_method_from_object_and_arguments(infer_result::InferResult object_result, infer_result::InferResult with_arguments, mlc::String method_name, ast::Span method_span, int argument_count, registry::TypeRegistry registry) noexcept{
  ...
  auto missing_method_errors = ((semantic_type_structure::type_is_unknown(builtin_type) && semantic_type_structure::type_is_unknown(inferred_type)) ? (method_receiver_diagnostics::infer_undefined_record_method_diagnostics(object_result.inferred_type, method_name, method_span, registry)) : (mlc::Array<ast::Diagnostic>{}));
  return infer_result::InferResult{inferred_type, ast::diagnostics_append(ast::diagnostics_append(ast::diagnostics_append(with_arguments.errors, receiver_errors), arity_errors), missing_method_errors)};
}
