...
{
[&]() -> void { if (std::holds_alternative<ast::DeclImport>((*program.decls[index]))) { auto _v_declimport = std::get<ast::DeclImport>((*program.decls[index])); auto [import_path, symbols] = _v_declimport; mlc::String resolved = path_normalize::resolve_dotdot(path_normalize::resolve_import_path(path, import_path)); my_imports.push_back(resolved); if (symbols.size() >= 2 && symbols[0] == mlc::String("*")){
{
my_namespace_import_aliases.push_back(load_item::NamespaceImportAlias{symbols[1], resolved});
}
} module_loader::LoadResult dependency_parsed = load_module_impl(resolved, loaded, cache, profile_enabled); all_errors = ast::errs_append(all_errors, dependency_parsed.errors); int dep_item_index = 0; [&]() { 
...
dep_item_index = dep_item_index + 1;
 ...
 }(); return; } if (std::holds_alternative<ast::DeclExported>((*program.decls[index]))) { auto _v_declexported = std::get<ast::DeclExported>((*program.decls[index])); auto [_w0] = _v_declexported; module_declarations.push_back(program.decls[index]); return; } module_declarations.push_back(program.decls[index]); return; }();
index = index + 1;
}
}
