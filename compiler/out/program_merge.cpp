...
mlc::HashMap<mlc::String, module_loader::LoadResult> cache = mlc::HashMap<mlc::String, module_loader::LoadResult>();
int index = 0;
...
[&]() -> void { if (std::holds_alternative<ast::DeclImport>((*program.decls[index]))) { auto _v_declimport = std::get<ast::DeclImport>((*program.decls[index])); auto [path, symbols] = _v_declimport; return [&]() { 
  mlc::String resolved = path_normalize::resolve_dotdot(path_normalize::resolve_import_path(entry_path, path));
...
entry_namespace_import_aliases.push_back(load_item::NamespaceImportAlias{symbols[1], resolved});
  ...
  module_loader::LoadResult dependency_parsed = module_loader::load_module(resolved, cache, profile_enabled);
