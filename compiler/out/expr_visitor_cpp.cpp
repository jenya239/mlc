...
mlc::Array<std::shared_ptr<cpp_ast::CppExpr>> record_field_expressions(mlc::Array<std::shared_ptr<semantic_ir::SFieldVal>> field_values, mlc::String type_name, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept;
...
std::shared_ptr<cpp_ast::CppExpr> gen_match_via_cpp_visitor(std::shared_ptr<semantic_ir::SExpr> subject, mlc::Array<std::shared_ptr<semantic_ir::SMatchArm>> arms, std::shared_ptr<registry::Type> match_semantic_type, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept{
mlc::Array<std::shared_ptr<semantic_ir::SMatchArm>> expanded_arms = match_gen::expand_or_arms(arms);
return match_gen::expanded_any_guard(expanded_arms) ? cpp_expr_from_rendered_fragment(match_gen::gen_match(subject, arms, match_semantic_type, context, gen_stmts, eval_expr_cpp_as_string)) : [&]() -> std::shared_ptr<cpp_ast::CppExpr> { 
  bool visit_void = match_gen::match_visit_uses_void_lambdas(match_semantic_type);
  mlc::Array<std::shared_ptr<cpp_ast::CppExpr>> handlers = expanded_arms.map([context, gen_stmts, visit_void](std::shared_ptr<semantic_ir::SMatchArm> arm) mutable { return cpp_expr_from_rendered_fragment(match_gen::gen_arm(arm, context, gen_stmts, eval_expr_cpp_as_string, visit_void)); });
  std::shared_ptr<cpp_ast::CppExpr> subject_expression = eval_expr_cpp(subject, context, gen_stmts);
  bool needs_dereference = expanded_arms.size() > 0 ? match_analysis::first_arm_needs_deref(expanded_arms[0], subject, context) : true;
  std::shared_ptr<cpp_ast::CppExpr> visit_subject = needs_dereference ? std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppUnary(mlc::String("*"), subject_expression)) : subject_expression;
  return std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppStdVisit(visit_subject, handlers));
 }();
}

mlc::Array<std::shared_ptr<cpp_ast::CppExpr>> record_field_expressions(mlc::Array<std::shared_ptr<semantic_ir::SFieldVal>> field_values, mlc::String type_name, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept{
mlc::Array<mlc::String> field_order = context::lookup_fields_for_context(context, type_name);
return field_order.size() > 0 ? field_order.map([field_values, context, gen_stmts](mlc::String field_name) mutable { return eval_expr_cpp(expression_support::find_field_value(field_values, field_name), context, gen_stmts); }) : field_values.map([context, gen_stmts](std::shared_ptr<semantic_ir::SFieldVal> field_value) mutable { return eval_expr_cpp(field_value->val, context, gen_stmts); });
}
...
return std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppAggregateInit(cpp_name, record_field_expressions(field_values, type_name, context, gen_stmts)));
}
...
std::shared_ptr<cpp_ast::CppExpr> gen_while_via_cpp_visitor(std::shared_ptr<semantic_ir::SExpr> condition, mlc::Array<std::shared_ptr<semantic_ir::SStmt>> statements, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept{return std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInvokedWhile(eval_expr_cpp(condition, context, gen_stmts), gen_stmts(statements, context)));}

std::shared_ptr<cpp_ast::CppExpr> gen_for_via_cpp_visitor(mlc::String variable, std::shared_ptr<semantic_ir::SExpr> iterator, mlc::Array<std::shared_ptr<semantic_ir::SStmt>> statements, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept{return std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppInvokedFor(cpp_naming::cpp_safe(variable), eval_expr_cpp(iterator, context, gen_stmts), gen_stmts(statements, context)));}
...
std::shared_ptr<cpp_ast::CppExpr> gen_tuple_via_cpp_visitor(mlc::Array<std::shared_ptr<semantic_ir::SExpr>> elements, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept{return std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppCall(emit_helpers::make_identifier_cpp_expression(mlc::String("std::make_tuple")), elements.map([context, gen_stmts](std::shared_ptr<semantic_ir::SExpr> element) mutable { return eval_expr_cpp(element, context, gen_stmts); })));}

std::shared_ptr<cpp_ast::CppExpr> gen_question_via_cpp_visitor(std::shared_ptr<semantic_ir::SExpr> inner_expression, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept{return std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppQuestionTry(eval_expr_cpp(inner_expression, context, gen_stmts)));}

std::shared_ptr<cpp_ast::CppExpr> gen_with_via_cpp_visitor(std::shared_ptr<semantic_ir::SExpr> resource, mlc::String binder, mlc::Array<std::shared_ptr<semantic_ir::SStmt>> statements, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SStmt>>, context::CodegenContext)> gen_stmts) noexcept{return std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppWithBlock(eval_expr_cpp(resource, context, gen_stmts), binder, gen_stmts(statements, context)));}
...
return method != mlc::String("") && type_name != mlc::String("") ? std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppCall(emit_helpers::make_identifier_cpp_expression(type_name + mlc::String("_") + method), mlc::Array<std::shared_ptr<cpp_ast::CppExpr>>{evaluate_expression(left_expression, context, gen_stmts), evaluate_expression(right_expression, context, gen_stmts)})) : std::make_shared<cpp_ast::CppExpr>(cpp_ast::CppBinary(operation, evaluate_expression(left_expression, context, gen_stmts), evaluate_expression(right_expression, context, gen_stmts)));
}
