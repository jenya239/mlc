...
mlc::String result_wrapper_cpp_type_argument(mlc::String wrapped_cpp_type) noexcept{return wrapped_cpp_type == mlc::String("auto") || wrapped_cpp_type.length() == 0 ? mlc::String("") : mlc::String("<") + wrapped_cpp_type + mlc::String(">");}
...
return cpp_argument_parts.size() == 0 ? mlc::String("") : mlc::String("<") + cpp_argument_parts.join(mlc::String(", ")) + mlc::String(">");
}

mlc::String owner_type_name_from_semantic_type(std::shared_ptr<registry::Type> type_value) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<registry::TNamed>((*type_value))) { auto _v_tnamed = std::get<registry::TNamed>((*type_value)); auto [type_name] = _v_tnamed; return type_name; } if (std::holds_alternative<registry::TGeneric>((*type_value))) { auto _v_tgeneric = std::get<registry::TGeneric>((*type_value)); auto [type_name, _w0] = _v_tgeneric; return type_name; } return mlc::String(""); }();}

mlc::String generic_sum_variant_ctor_type_argument_for_enclosing(mlc::String owner_name, mlc::String enclosing_name, mlc::Array<std::shared_ptr<registry::Type>> type_arguments, mlc::String constructor_name, context::CodegenContext context) noexcept{
return owner_name == mlc::String("") || enclosing_name != owner_name ? mlc::String("") : context.variant_used_type_parameter_names.has(constructor_name) && context.sum_type_parameter_names.has(owner_name) ? generic_sum_variant_ctor_type_argument_from_enclosing(owner_name, context.sum_type_parameter_names.get(owner_name), context.variant_used_type_parameter_names.get(constructor_name), type_arguments, context) : type_arguments.size() == 1 && !semantic_type_structure::type_is_unknown(type_arguments[0]) ? result_wrapper_cpp_type_argument(type_gen::sem_type_to_cpp(context, type_arguments[0])) : mlc::String("");
}

mlc::String generic_sum_variant_ctor_type_argument(mlc::String constructor_name, std::shared_ptr<registry::Type> call_semantic_type, context::CodegenContext context) noexcept{
mlc::String owner_name = owner_type_name_from_semantic_type(call_semantic_type);
if (owner_name == mlc::String("") && context.variant_types.has(constructor_name)){
{
owner_name = context.variant_types.get(constructor_name);
}
}
mlc::String from_enclosing = [&]() -> mlc::String { if (std::holds_alternative<registry::TGeneric>((*context.enclosing_function_return_type))) { auto _v_tgeneric = std::get<registry::TGeneric>((*context.enclosing_function_return_type)); auto [enclosing_name, type_arguments] = _v_tgeneric; return generic_sum_variant_ctor_type_argument_for_enclosing(owner_name, enclosing_name, type_arguments, constructor_name, context); } return mlc::String(""); }();
return from_enclosing.length() > 0 ? from_enclosing : [&]() -> mlc::String { if (std::holds_alternative<registry::TGeneric>((*call_semantic_type))) { auto _v_tgeneric = std::get<registry::TGeneric>((*call_semantic_type)); auto [call_owner_name, type_arguments] = _v_tgeneric; return generic_sum_variant_ctor_type_argument_for_enclosing(call_owner_name, call_owner_name, type_arguments, constructor_name, context); } return mlc::String(""); }();
}

mlc::String variant_constructor_template_suffix(mlc::String constructor_name, std::shared_ptr<registry::Type> call_semantic_type, context::CodegenContext context) noexcept{
mlc::String result_wrapper = result_wrapper_ctor_type_argument(constructor_name, context);
return result_wrapper.length() > 0 ? result_wrapper : generic_sum_variant_ctor_type_argument(constructor_name, call_semantic_type, context);
}

mlc::String result_wrapper_ctor_type_argument(mlc::String constructor_name, context::CodegenContext context) noexcept{
std::shared_ptr<registry::Type> enclosing_type = context.enclosing_function_return_type;
return !result_option_method_types::is_result_generic(enclosing_type) ? mlc::String("") : constructor_name == mlc::String("Ok") ? result_wrapper_cpp_type_argument(type_gen::sem_type_to_cpp(context, result_option_method_types::result_ok_type(enclosing_type))) : constructor_name == mlc::String("Err") ? result_wrapper_cpp_type_argument(type_gen::sem_type_to_cpp(context, result_option_method_types::result_err_type(enclosing_type))) : mlc::String("");
}
...
std::shared_ptr<cpp_ast::CppExpression> gen_constructor_call_cpp_expression(std::shared_ptr<semantic_ir::SemanticExpression> function_expression, mlc::Array<std::shared_ptr<cpp_ast::CppExpression>> argument_expressions, std::shared_ptr<registry::Type> call_semantic_type, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SemanticStatement>>, context::CodegenContext)> gen_stmts) noexcept{return [&]() -> std::shared_ptr<cpp_ast::CppExpression> { if (std::holds_alternative<semantic_ir::SemanticExpressionIdent>((*function_expression)._)) { auto _v_semanticexpressionident = std::get<semantic_ir::SemanticExpressionIdent>((*function_expression)._); auto [name, _w0, _w1] = _v_semanticexpressionident; return [&]() -> std::shared_ptr<cpp_ast::CppExpression> { 
  mlc::String callee_code = context::qualify_function_callee(cpp_naming::map_builtin(name), context) + variant_constructor_template_suffix(name, call_semantic_type, context);
...
std::shared_ptr<cpp_ast::CppExpression> invoke_expression = expression_support::is_constructor_call(function_expression) ? gen_constructor_call_cpp_expression(function_expression, materialization_outcome.materialized_argument_expressions, call_semantic_type, context, gen_stmts) : std::make_shared<cpp_ast::CppExpression>(cpp_ast::CppCall(gen_call_function_cpp_expression(function_expression, context, gen_stmts), materialization_outcome.materialized_argument_expressions));
...
bool if_branch_type_matches_result(std::shared_ptr<semantic_ir::SemanticExpression> branch_expression, mlc::String result_cpp, context::CodegenContext context) noexcept{return type_gen::sem_type_to_cpp(context, semantic_ir::sexpr_type(branch_expression)) == result_cpp;}

mlc::String if_branch_constructor_name(std::shared_ptr<semantic_ir::SemanticExpression> branch_expression) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<semantic_ir::SemanticExpressionCall>((*branch_expression)._)) { auto _v_semanticexpressioncall = std::get<semantic_ir::SemanticExpressionCall>((*branch_expression)._); auto [function_expression, _w0, _w1, _w2, _w3] = _v_semanticexpressioncall; return [&]() -> mlc::String { if (std::holds_alternative<semantic_ir::SemanticExpressionIdent>((*function_expression)._)) { auto _v_semanticexpressionident = std::get<semantic_ir::SemanticExpressionIdent>((*function_expression)._); auto [name, _w0, _w1] = _v_semanticexpressionident; return expression_support::is_constructor_call(function_expression) ? name : mlc::String(""); } return mlc::String(""); }(); } return mlc::String(""); }();}

bool if_ternary_is_safe(std::shared_ptr<semantic_ir::SemanticExpression> then_expression, std::shared_ptr<semantic_ir::SemanticExpression> else_expression, std::shared_ptr<registry::Type> if_semantic_type, context::CodegenContext context) noexcept{
mlc::String then_ctor = if_branch_constructor_name(then_expression);
mlc::String else_ctor = if_branch_constructor_name(else_expression);
return (then_ctor.length() > 0 || else_ctor.length() > 0) && then_ctor != else_ctor ? false : [&]() -> bool { 
  mlc::String result_cpp = match_gen::expression_result_cpp_type_for_codegen(context, if_semantic_type);
  return result_cpp == mlc::String("auto") || result_cpp.length() == 0 ? true : if_branch_type_matches_result(then_expression, result_cpp, context) && if_branch_type_matches_result(else_expression, result_cpp, context);
 }();
}
...
mlc::String return_type = result_cpp.length() > 0 ? result_cpp : mlc::String("auto");
...
std::shared_ptr<cpp_ast::CppExpression> gen_block_via_cpp_visitor(mlc::Array<std::shared_ptr<semantic_ir::SemanticStatement>> statements, std::shared_ptr<semantic_ir::SemanticExpression> result_expression, context::CodegenContext context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SemanticStatement>>, context::CodegenContext)> gen_stmts) noexcept{return statements.size() == 0 ? eval_expr_cpp(result_expression, context, gen_stmts) : [&]() -> std::shared_ptr<cpp_ast::CppExpression> { 
  context::CodegenContext final_context = statement_context::stmts_final_ctx(statements, context);
  mlc::String body_fragment = gen_stmts(statements, context) + mlc::String("return ") + print::print_expr(eval_expr_cpp(result_expression, final_context, gen_stmts)) + mlc::String(";\n");
...
mlc::String inner_body = match_gen::gen_match_guarded_body(subject, expanded_arms, match_semantic_type, context, gen_stmts, [](std::shared_ptr<semantic_ir::SemanticExpression> expression, context::CodegenContext eval_context, std::function<mlc::String(mlc::Array<std::shared_ptr<semantic_ir::SemanticStatement>>, context::CodegenContext)> gen_stmts_fn) mutable { return print::print_expr(eval_expr_cpp(expression, eval_context, gen_stmts_fn)); });
...
std::shared_ptr<cpp_ast::CppExpression> CodegenPass_visit_call(expr_visitor_cpp::CodegenPass self, std::shared_ptr<semantic_ir::SemanticExpression> function_expression, mlc::Array<std::shared_ptr<semantic_ir::SemanticExpression>> arguments, mlc::Array<int> mutability_flags, std::shared_ptr<registry::Type> semantic_type) noexcept{return gen_call_via_cpp_visitor(function_expression, arguments, mutability_flags, semantic_type, self.context, self.gen_stmts);}
