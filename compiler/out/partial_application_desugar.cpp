...
return MatchArmsPartialPair{accumulated.match_arms.concat(mlc::Array{std::make_shared<ast::MatchArm>(ast::MatchArm{arm_shared_under_match->pattern, arm_shared_under_match->has_guard, guard_pair.expression, body_pair.expression})}), body_pair.next_serial};
}(); });
}
...
[&](const ast::ExprExtern& exprExtern) -> PartialPair { auto [__0] = exprExtern; return partial_application_leaf_partial_pair(expression, serial); },
