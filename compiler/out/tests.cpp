...
mlc::Array<std::shared_ptr<FieldOrder>> empty_orders = {};
results.push_back(assert_eq_str(mlc::String("ExprInt(42) → '42'"), gen_expr(std::make_shared<Expr>(ExprInt(42)), empty_orders), mlc::String("42")));
results.push_back(assert_eq_str(mlc::String("ExprBool(true) → 'true'"), gen_expr(std::make_shared<Expr>(ExprBool(true)), empty_orders), mlc::String("true")));
results.push_back(assert_eq_str(mlc::String("ExprStr('hi') → 'mlc::String(\"hi\")'"), gen_expr(std::make_shared<Expr>(ExprStr(mlc::String("hi"))), empty_orders), mlc::String("mlc::String(\"hi\")")));
results.push_back(assert_eq_str(mlc::String("ExprUnit → '/* unit */'"), gen_expr(std::make_shared<Expr>(ExprUnit{}), empty_orders), mlc::String("/* unit */")));
results.push_back(assert_eq_str(mlc::String("ExprIdent('x') → 'x'"), gen_expr(std::make_shared<Expr>(ExprIdent(mlc::String("x"))), empty_orders), mlc::String("x")));
results.push_back(assert_eq_str(mlc::String("ExprIdent('print') → 'mlc::io::print'"), gen_expr(std::make_shared<Expr>(ExprIdent(mlc::String("print"))), empty_orders), mlc::String("mlc::io::print")));
results.push_back(assert_eq_str(mlc::String("ExprIdent('println') → 'mlc::io::println'"), gen_expr(std::make_shared<Expr>(ExprIdent(mlc::String("println"))), empty_orders), mlc::String("mlc::io::println")));
results.push_back(assert_eq_str(mlc::String("ExprBin add → '(1 + 2)'"), gen_expr(std::make_shared<Expr>(ExprBin(mlc::String("+"), std::make_shared<Expr>(ExprInt(1)), std::make_shared<Expr>(ExprInt(2)))), empty_orders), mlc::String("(1 + 2)")));
results.push_back(assert_eq_str(mlc::String("ExprUn neg → '(-42)'"), gen_expr(std::make_shared<Expr>(ExprUn(mlc::String("-"), std::make_shared<Expr>(ExprInt(42)))), empty_orders), mlc::String("(-42)")));
...
results.push_back(assert_eq_str(mlc::String("ExprLambda single param → '[=](auto x) { return ... }'"), gen_expr(std::make_shared<Expr>(ExprLambda(single_params, std::make_shared<Expr>(ExprBin(mlc::String("+"), std::make_shared<Expr>(ExprIdent(mlc::String("x"))), std::make_shared<Expr>(ExprInt(1)))))), empty_orders), mlc::String("[=](auto x) { return (x + 1); }")));
mlc::Array<mlc::String> two_params = mlc::Array<mlc::String>{mlc::String("a"), mlc::String("b")};
results.push_back(assert_eq_str(mlc::String("ExprLambda two params → '[=](auto a, auto b) { return ... }'"), gen_expr(std::make_shared<Expr>(ExprLambda(two_params, std::make_shared<Expr>(ExprBin(mlc::String("+"), std::make_shared<Expr>(ExprIdent(mlc::String("a"))), std::make_shared<Expr>(ExprIdent(mlc::String("b"))))))), empty_orders), mlc::String("[=](auto a, auto b) { return (a + b); }")));
mlc::Array<mlc::String> zero_params = {};
results.push_back(assert_eq_str(mlc::String("ExprLambda zero params → '[]() { return ... }'"), gen_expr(std::make_shared<Expr>(ExprLambda(zero_params, std::make_shared<Expr>(ExprInt(42)))), empty_orders), mlc::String("[]() { return 42; }")));
...
results.push_back(assert_eq_str(mlc::String("pipe x |> double → double(x)"), gen_expr(plain_pipe, empty_orders), mlc::String("double(x)")));
...
results.push_back(assert_eq_str(mlc::String("pipe x |> add(5) → add(x, 5)"), gen_expr(partial_pipe, empty_orders), mlc::String("add(x, 5)")));
...
results.push_back(assert_eq_str(mlc::String("pipe x |> clamp(3, 4) → clamp(x, 3, 4)"), gen_expr(multi_partial_pipe, empty_orders), mlc::String("clamp(x, 3, 4)")));
mlc::Array<mlc::String> point_fields = mlc::Array<mlc::String>{mlc::String("x"), mlc::String("y")};
std::shared_ptr<FieldOrder> point_order = std::make_shared<FieldOrder>(FieldOrder{mlc::String("Point"), point_fields});
mlc::Array<std::shared_ptr<FieldOrder>> orders = mlc::Array<std::shared_ptr<FieldOrder>>{point_order};
...
results.push_back(assert_eq_str(mlc::String("Point { ...p, x: 10 } → Point{10, p.y}"), gen_expr(update_expr, orders), mlc::String("Point{10, p.y}")));
...
results.push_back(assert_eq_str(mlc::String("Point { ...p, x: 0, y: 0 } → Point{0, 0}"), gen_expr(all_overrides_expr, orders), mlc::String("Point{0, 0}")));
