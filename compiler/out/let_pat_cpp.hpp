...
mlc::Array<std::shared_ptr<cpp_ast::CppStatement>> constructor_mismatch_else_statements(bool has_else, std::shared_ptr<semantic_ir::SemanticExpression> else_body, context::CodegenContext context, __F3 expression_codegen) noexcept;
...
mlc::Array<std::shared_ptr<cpp_ast::CppStatement>> constructor_mismatch_else_statements(bool has_else, std::shared_ptr<semantic_ir::SemanticExpression> else_body, context::CodegenContext context, __F3 expression_codegen) noexcept{
  if (has_else)   {
    return mlc::Array{emit_helpers::make_expression_cpp_statement(expression_codegen(else_body, context))};
  } else   {
    return mlc::Array{emit_helpers::make_expression_cpp_statement(std::make_shared<cpp_ast::CppExpression>(cpp_ast::CppCall{std::make_shared<cpp_ast::CppExpression>(cpp_ast::CppIdent{mlc::String("std::abort", 10)}), {}}))};
  }
}
