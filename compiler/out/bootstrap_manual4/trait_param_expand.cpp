...
auto declaration_stack = mlc::Array<std::shared_ptr<ast::Decl>>{};
declaration_stack.push_back(program.decls[declaration_index]);
auto stack_index = 0;
while ((stack_index < declaration_stack.length())) {
auto current_declaration = declaration_stack[stack_index];
std::visit(overloaded{[&](const ast::DeclExported& declExported) { auto [inner] = declExported; return [&]() {
declaration_stack.push_back(inner);
/* unit */;
return /* unit */;
}(); },
[&](const ast::DeclType& declType) { auto [type_name, __1, __2, __3] = declType; return [&]() {
if (trait_declaration_names.has(type_name)) {
conflicts.push_back(ast::diagnostic_error(((mlc::String("the name \"", 10) + type_name) + mlc::String("\" is declared as both a type and a trait", 40)), ast::span_unknown()));
}
...
conflicts.push_back(ast::diagnostic_error(((mlc::String("the name \"", 10) + trait_name) + mlc::String("\" is declared as both a type and a trait", 40)), ast::span_unknown()));
}
