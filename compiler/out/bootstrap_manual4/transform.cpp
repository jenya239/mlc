...
return std::visit(overloaded{[&](const semantic_ir::SExprArray& sExprArray) { auto [elements, __1, span_else] = sExprArray; return ((elements.length() > 0) ? (typed_else) : ([&]() {
std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_then] = tArray; return std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_else] = tArray; return (semantic_type_structure::type_is_unknown(inner_else) ? (std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprArray{elements, std::make_shared<registry::Type>(registry::TArray{inner_then}), span_else})) : (typed_else)); },
[&](const auto& __v) { return typed_else; }
}, (*semantic_ir::sexpr_type(typed_else))); },
[&](const auto& __v) { return typed_else; }
}, (*semantic_ir::sexpr_type(typed_then)));
/* unit */;
[=](auto _) { return typed_else; };
/* unit */;
export_;
/* unit */;
TransformContext{} = TransformContext{(Map{} < string), Shared{}, , registry::Type{}, , , registry::TypeRegistry{}};
/* unit */;
transform_context_new((Map{} < string), (Shared{} < registry::Type{}), (/* unit */ > /* unit */), registry::TypeRegistry{});
/* unit */;
TransformContext{} = TransformContext{type_env, registry};
export_;
/* unit */;
empty_transform_context();
/* unit */;
TransformContext{} = TransformContext{{}, registry::empty_registry()};
export_;
/* unit */;
transform_context_with_env(TransformContext{}, (Map{} < string), (Shared{} < registry::Type{}), (/* unit */ > /* unit */), /* unit */, TransformContext{}, /* unit */, TransformContext{type_env, base.registry}, /* unit */, tunk_args(i32), /* unit */[(Shared{} < registry::Type{})], /* unit */, /* unit */, [&]() {
...
return args;
}(), export_, /* unit */, coerce_expr_to_type((Shared{} < semantic_ir::SExpr{}), /* unit */, (Shared{} < registry::Type{}), /* unit */), /* unit */, (Shared{} < semantic_ir::SExpr{}), /* unit */, /* unit */, std::visit(overloaded{[&](const semantic_ir::SExprArray& sExprArray) { auto [elements, __1, source_span] = sExprArray; return ((elements.length() == 0) ? (std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprArray{elements, target_type, source_span})) : (expression)); },
...
[&](const semantic_ir::SExprIf& sExprIf) { auto [condition, then_branch, else_branch, __3, source_span] = sExprIf; return std::visit(overloaded{[&](const registry::TArray& tArray) { auto [__0] = tArray; return std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprIf{condition, then_branch, else_branch, target_type, source_span}); },
[&](const auto& __v) { return expression; }
}, target_type); },
[&](const auto& __v) { return expression; }
}, expression), export_, /* unit */, TransformStmtsResult{}, /* unit */, TransformStmtsResult{mlc::Array<bool>{(Shared{} < semantic_ir::SStmt{})}, , (Map{} < string), Shared{}, , registry::Type{}, , }, /* unit */, transform_has_named_args(mlc::Array<bool>{(Shared{} < ast::Expr{})}, /* unit */), /* unit */, bool_, /* unit */, [&]() {
auto i = 0;
auto found = false;
while ((i < args.length())) {
...
return found;
}(), /* unit */, transform_find_param_slot(mlc::Array{string}, string), /* unit */, i32, /* unit */, [&]() {
auto i = 0;
auto slot = (-1);
...
return slot;
}(), /* unit */, transform_strip_labels(mlc::Array<bool>{(Shared{} < ast::Expr{})}, /* unit */), /* unit */[(Shared{} < ast::Expr{})], /* unit */, /* unit */, [&]() {
...
return stripped;
}(), /* unit */, transform_reorder_to_positional(mlc::Array<bool>{(Shared{} < ast::Expr{})}, /* unit */, mlc::Array{string}), /* unit */[(Shared{} < ast::Expr{})], /* unit */, /* unit */, [&]() {
...
return compact;
}(), /* unit */, reorder_named_args_for_transform(mlc::Array<bool>{(Shared{} < ast::Expr{})}, /* unit */, string, registry::TypeRegistry{}), /* unit */[(Shared{} < ast::Expr{})], /* unit */, /* unit */, ((!transform_has_named_args(args)) ? (args) : ([&]() {
auto param_names = registry.parameter_names_for(callee_name);
return ((param_names.length() == 0) ? (transform_strip_labels(args)) : (transform_reorder_to_positional(args, param_names)));
}())), /* unit */, transform_exprs(mlc::Array<bool>{(Shared{} < ast::Expr{})}, /* unit */, TransformContext{}, mlc::Array<bool>{(Shared{} < ast::Stmt{})}, TransformContext{}), /* unit */, TransformStmtsResult{});
...
/* unit */;
transform_field_vals(mlc::Array<bool>{(Shared{} < ast::FieldVal{})}, /* unit */, TransformContext{}, mlc::Array<bool>{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
TransformStmtsResult{};
/* unit */;
/* unit */[(Shared{} < semantic_ir::SFieldVal{})];
/* unit */ = [&]() {
...
/* unit */;
build_transform_substitution((Shared{} < registry::Type{}), /* unit */, registry::TypeRegistry{});
/* unit */;
(Map{} < string);
/* unit */;
...
auto param_names = registry.algebraic_decl_type_parameter_names_for(type_name);
auto i = 0;
return [&]() {
...
substitution;
/* unit */;
transform_match_arms(mlc::Array<bool>{(Shared{} < ast::MatchArm{})}, /* unit */, TransformContext{}, (Map{} < string), (Shared{} < registry::Type{}), (/* unit */ > /* unit */), mlc::Array<bool>{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
TransformStmtsResult{};
/* unit */;
/* unit */[(Shared{} < semantic_ir::SMatchArm{})];
/* unit */ = [&]() {
...
/* unit */;
transform_expr_lambda_with_param_types(mlc::Array{string}, mlc::Array<bool>{(Shared{} < registry::Type{})}, /* unit */, (Shared{} < ast::Expr{}), /* unit */, ast::Span{}, TransformContext{}, mlc::Array<bool>{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
TransformStmtsResult{};
/* unit */;
/* unit */;
...
/* unit */;
transform_expr_maybe_lambda_context((Shared{} < ast::Expr{}), /* unit */, mlc::Array<bool>{(Shared{} < registry::Type{})}, /* unit */, TransformContext{}, mlc::Array<bool>{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
TransformStmtsResult{};
/* unit */;
/* unit */;
...
/* unit */;
transform_array_hof_method_arguments((Shared{} < semantic_ir::SExpr{}), /* unit */, string, mlc::Array<bool>{(Shared{} < ast::Expr{})}, /* unit */, TransformContext{}, mlc::Array<bool>{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
TransformStmtsResult{};
/* unit */;
/* unit */[(Shared{} < semantic_ir::SExpr{})];
...
export_;
/* unit */;
transform_expr((Shared{} < ast::Expr{}), /* unit */, TransformContext{}, mlc::Array<bool>{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
TransformStmtsResult{};
/* unit */;
/* unit */;
(Shared{} < semantic_ir::SExpr{});
/* unit */ = std::visit(overloaded{[&](const ast::ExprInt& exprInt) { auto [value, source_span] = exprInt; return std::make_shared<semantic_ir::SExpr>(semantic_ir::SExprInt{value, std::make_shared<registry::Type>(registry::TI32{}), source_span}); },
...
auto resolved_type = (transform_context.type_env.has(name) ? (transform_context.type_env.get(name)) : ((transform_context.registry.has_fn(name) ? (transform_context.registry.fn_type(name)) : ((transform_context.registry.has_ctor(name) ? (transform_context.registry.ctor_type(name)) : (std::make_shared<registry::Type>(registry::TUnknown{})))))));
...
auto result_type = (((callee_name != mlc::String("", 0)) && transform_context.registry.has_ctor(callee_name)) ? (transform_context.registry.ctor_type(callee_name)) : (std::visit(overloaded{[&](const registry::TFn& tFn) { auto [__0, return_type] = tFn; return return_type; },
...
auto phantom = transform_context.registry.phantom_type_params_for(type_name);
auto all_params = transform_context.registry.algebraic_decl_type_parameter_names_for(type_name);
