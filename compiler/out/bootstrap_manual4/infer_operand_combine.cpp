...
return std::visit(overloaded{[&](const ast::ExprArray& exprArray) { auto [elements, __1] = exprArray; return (elements.length() == 0); },
...
std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_then] = tArray; return std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_else] = tArray; return (semantic_type_structure::type_is_unknown(inner_else) ? (infer_result::InferResult{std::make_shared<registry::Type>(registry::TArray{inner_then}), else_result.errors}) : (else_result)); },
[&](const auto& __v) { return else_result; }
}, (*else_result.inferred_type)); },
[&](const auto& __v) { return else_result; }
}, (*then_result.inferred_type));
export_;
/* unit */;
infer_conditional_from_branch_results(infer_result::InferResult{}, infer_result::InferResult{}, infer_result::InferResult{}, (Shared{} < ast::Expr{}), /* unit */);
/* unit */;
infer_result::InferResult{} = [&]() {
auto else_coerced = conditional_else_result_coerced_for_empty_unknown_array(then_result, else_result, else_expression);
auto merged = InferResult_absorb(InferResult_absorb(then_result, condition_result), else_coerced);
...
export_;
/* unit */;
infer_field_from_object_result(infer_result::InferResult{}, string, ast::Span{}, registry::TypeRegistry{});
/* unit */;
infer_result::InferResult{} = [&]() {
...
export_;
/* unit */;
infer_method_from_object_and_arguments(infer_result::InferResult{}, infer_result::InferResult{}, string, ast::Span{}, i32, registry::TypeRegistry{});
/* unit */;
infer_result::InferResult{} = [&]() {
