...
pipeline::ModularCompileInput pipeline_input_for_source(mlc::String source, bool check_only, bool verify_each_pass) noexcept;
...
}, pipeline::run_modular_compiler_pipeline(pipeline_input_for_source(source, true, false)));}
...
load_item::LoadItem load_item = load_item::LoadItem{mlc::String("check_only_probe.mlc"), program.decls, {}, {}};
return pipeline::ModularCompileInput{mlc::Array<load_item::LoadItem>{load_item}, program, program, mlc::String(""), false, check_only, false, verify_each_pass, false, false, false};
}
...
compile_options::CompileOptions verify_each_parsed = compile_options::parse_compile_options(mlc::Array<mlc::String>{mlc::String("--verify-each"), mlc::String("--check-only"), mlc::String("entry.mlc")});
results.push_back(test_runner::assert_true(mlc::String("parse_compile_options --verify-each flag"), verify_each_parsed.verify_each_pass));
results.push_back(test_runner::assert_true(mlc::String("check_only pipeline with --verify-each valid program"), std::visit(overloaded{
  [&](const ast::Ok<mlc::String>& ok) -> bool { auto [_w0] = ok; return true; },
  [&](const ast::Err<mlc::Array<mlc::String>>& err) -> bool { auto [_w0] = err; return false; }
}, pipeline::run_modular_compiler_pipeline(pipeline_input_for_source(mlc::String("fn f() -> i32 = 42"), true, true)))));
return results;
}
