...
mlc::Array<mlc::String> mutable_scope_from_parameters(mlc::Array<std::shared_ptr<ast::Param>> parameters, registry::TypeRegistry registry) noexcept{
  return parameters.fold(empty_names(), [=](mlc::Array<mlc::String> scope, std::shared_ptr<ast::Param> parameter) mutable { return [&]() {
if ((!parameter->is_mut)) {
  return scope;
}
auto parameter_type = registry::type_from_annotation_with_registry(parameter->type_value, registry);
return (type_is_sync_safe_shared(parameter_type, registry) ? (scope) : (names_add(scope, parameter->name)));
}(); });
}
...
[&](const ast::ExprExtern& exprExtern) { auto [__0, __1, __2] = exprExtern; return empty_diagnostics(); },
...
return ast::diagnostics_append(statements_state.diagnostics, walk_spawn_sites_in_expr(result_expression, statements_state.scope, SpawnCaptureContext{capture_context.registry, statements_state.type_environment}));
}(); },
...
auto scope_after = ((is_mut && (!type_is_sync_safe_shared(binding_type, registry))) ? (names_add(scope, name)) : (scope));
...
auto binding_type = resolve_binding_type(type_expression, value, type_environment, registry);
type_environment.set(name, binding_type);
return SpawnCaptureMutableWalkState{empty_diagnostics(), scope, type_environment};
}();
}
return SpawnCaptureMutableWalkState{empty_diagnostics(), scope, type_environment};
std::abort();
...
auto value_diagnostics = std::visit(overloaded{[&](const ast::StmtLet& stmtLet) { auto [__0, __1, __2, value, __4] = stmtLet; return walk_spawn_sites_in_expr(value, state.scope, nested_context); },
[&](const ast::StmtLetPattern& stmtLetPattern) { auto [__0, __1, __2, value, __4, else_expression, __6] = stmtLetPattern; return ast::diagnostics_append(walk_spawn_sites_in_expr(value, state.scope, nested_context), walk_spawn_sites_in_expr(else_expression, state.scope, nested_context)); },
[&](const ast::StmtLetConst& stmtLetConst) { auto [__0, __1, value, __3] = stmtLetConst; return walk_spawn_sites_in_expr(value, state.scope, nested_context); },
[&](const ast::StmtExpr& stmtExpr) { auto [expression, __1] = stmtExpr; return walk_spawn_sites_in_expr(expression, state.scope, nested_context); },
[&](const ast::StmtBreak& stmtBreak) { auto [__0] = stmtBreak; return empty_diagnostics(); },
[&](const ast::StmtContinue& stmtContinue) { auto [__0] = stmtContinue; return empty_diagnostics(); },
[&](const ast::StmtReturn& stmtReturn) { auto [expression, __1] = stmtReturn; return walk_spawn_sites_in_expr(expression, state.scope, nested_context); }
}, (*statement));
auto after = apply_statement_to_mutable_scope(statement, state.scope, state.type_environment, capture_context.registry);
return SpawnCaptureMutableWalkState{ast::diagnostics_append(state.diagnostics, value_diagnostics), after.scope, after.type_environment};
}(); });
}
...
mlc::Array<ast::Diagnostic> spawn_mutable_capture_diagnostics(mlc::Array<std::shared_ptr<ast::Param>> parameters, std::shared_ptr<ast::Expr> body, registry::TypeRegistry registry) noexcept{
  ...
  return walk_spawn_sites_in_expr(body, mutable_scope_from_parameters(parameters, registry), capture_context);
}
