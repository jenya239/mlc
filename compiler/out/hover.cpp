...
[&]() -> void { if (std::holds_alternative<ast::ExprBlock>((*desugared_body)._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*desugared_body)._); auto [statements, _w0, _w1] = _v_exprblock; int statement_index = 0; [&]() { 
  while (statement_index < statements.size()){
{
[&]() -> void { if (std::holds_alternative<ast::StmtLet>((*statements[statement_index])._)) { auto _v_stmtlet = std::get<ast::StmtLet>((*statements[statement_index])._); auto [name, _w0, _w1, value, statement_span] = _v_stmtlet; if (statement_span.line <= target_line_one_based){
{
  ...
  while (false){
{
 ...
 }(); return; } if (std::holds_alternative<ast::StmtExpr>((*statements[statement_index])._)) { auto _v_stmtexpr = std::get<ast::StmtExpr>((*statements[statement_index])._); auto [expression, _w0] = _v_stmtexpr; if (ast::stmt_span(statements[statement_index]).line <= target_line_one_based){
{
