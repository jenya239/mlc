...
predicates::Parser parser_new_with_source_path(mlc::Array<ast_tokens::Token> tokens, mlc::String source_path) noexcept{return predicates::Parser{tokens, 0, source_path, false, {}, false};}
...
ast_tokens::TokenKind Parser_kind(predicates::Parser self) noexcept{return self.has_unget_angle_close ? ast_tokens::TokenKind(ast_tokens::Op(mlc::String(">"))) : ast_tokens::TokenKind(self.position >= self.tokens.size() ? ast_tokens::TokenKind((ast_tokens::Eof{})) : ast_tokens::TokenKind([&]() -> ast_tokens::TokenKind { 
...
predicates::Parser Parser_with_lambda_shorthand_suppressed(predicates::Parser self, bool value) noexcept{return predicates::Parser{self.tokens, self.position, self.source_path, value, self.errors, self.has_unget_angle_close};}

predicates::Parser Parser_record_parse_error(predicates::Parser self, mlc::String message) noexcept{
mlc::Array<mlc::String> updated_errors = self.errors;
updated_errors.push_back(message);
return predicates::Parser{self.tokens, self.position, self.source_path, self.suppress_lambda_shorthand, updated_errors, self.has_unget_angle_close};
}

bool Parser_has_parse_errors(predicates::Parser self) noexcept{return self.errors.size() > 0;}

predicates::Parser Parser_advance(predicates::Parser self) noexcept{return self.has_unget_angle_close ? predicates::Parser{self.tokens, self.position, self.source_path, self.suppress_lambda_shorthand, self.errors, false} : predicates::Parser{self.tokens, self.position + 1, self.source_path, self.suppress_lambda_shorthand, self.errors, false};}

predicates::Parser Parser_advance_shift_right_as_angle_close(predicates::Parser self) noexcept{return predicates::Parser{self.tokens, self.position + 1, self.source_path, self.suppress_lambda_shorthand, self.errors, true};}

predicates::Parser Parser_advance_by(predicates::Parser self, int count) noexcept{return predicates::Parser{self.tokens, self.position + count, self.source_path, self.suppress_lambda_shorthand, self.errors, self.has_unget_angle_close};}
