...
mlc::Array<ast::Diagnostic> infer_binary_diagnostic_for_addition(mlc::String operation, std::shared_ptr<registry::Type> left_type, std::shared_ptr<registry::Type> right_type, ast::Span source_span) noexcept{return operation == mlc::String("+") && !type_utils::type_is_unknown(left_type) && !type_utils::type_is_unknown(right_type) && !type_utils::types_allowed_for_binary_plus(left_type, right_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("incompatible operand types for +: ") + type_utils::type_description(left_type) + mlc::String(" and ") + type_utils::type_description(right_type), source_span)} : mlc::Array<ast::Diagnostic>{};}

mlc::Array<ast::Diagnostic> infer_binary_diagnostic_for_int_arithmetic(mlc::String operation, std::shared_ptr<registry::Type> left_type, std::shared_ptr<registry::Type> right_type, ast::Span source_span) noexcept{
return operation == mlc::String("-") || operation == mlc::String("*") || operation == mlc::String("/") || operation == mlc::String("%") ? !type_utils::type_is_unknown(left_type) && !type_utils::type_is_unknown(right_type) && !type_utils::types_allowed_for_binary_int_only(left_type, right_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("numeric operands required for ") + operation + mlc::String(", got ") + type_utils::type_description(left_type) + mlc::String(" and ") + type_utils::type_description(right_type), source_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_binary_diagnostic_for_equality(mlc::String operation, std::shared_ptr<registry::Type> left_type, std::shared_ptr<registry::Type> right_type, ast::Span source_span) noexcept{
return operation == mlc::String("==") || operation == mlc::String("!=") ? !type_utils::type_is_unknown(left_type) && !type_utils::type_is_unknown(right_type) && !type_utils::types_structurally_equal(left_type, right_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("incompatible types for ") + operation + mlc::String(": ") + type_utils::type_description(left_type) + mlc::String(" and ") + type_utils::type_description(right_type), source_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_binary_diagnostic_for_ordering(mlc::String operation, std::shared_ptr<registry::Type> left_type, std::shared_ptr<registry::Type> right_type, ast::Span source_span) noexcept{
return operation == mlc::String("<") || operation == mlc::String(">") || operation == mlc::String("<=") || operation == mlc::String(">=") ? !type_utils::type_is_unknown(left_type) && !type_utils::type_is_unknown(right_type) && !type_utils::types_allowed_for_ordering(left_type, right_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("ordering requires i32 or string operands of the same kind, got ") + type_utils::type_description(left_type) + mlc::String(" and ") + type_utils::type_description(right_type), source_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_binary_diagnostic_for_logical(mlc::String operation, std::shared_ptr<registry::Type> left_type, std::shared_ptr<registry::Type> right_type, ast::Span source_span) noexcept{
return operation == mlc::String("&&") || operation == mlc::String("||") ? !type_utils::type_is_unknown(left_type) && !type_utils::type_is_unknown(right_type) && !type_utils::types_allowed_for_logical_and_or(left_type, right_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("logical operators require bool operands, got ") + type_utils::type_description(left_type) + mlc::String(" and ") + type_utils::type_description(right_type), source_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_binary_diagnostic_for_bitwise(mlc::String operation, std::shared_ptr<registry::Type> left_type, std::shared_ptr<registry::Type> right_type, ast::Span source_span) noexcept{
return operation == mlc::String("&") || operation == mlc::String("|") || operation == mlc::String("^") ? !type_utils::type_is_unknown(left_type) && !type_utils::type_is_unknown(right_type) && !type_utils::types_allowed_for_binary_int_only(left_type, right_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("bitwise ") + operation + mlc::String(" requires i32 operands, got ") + type_utils::type_description(left_type) + mlc::String(" and ") + type_utils::type_description(right_type), source_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_binary_diagnostic_for_shift(mlc::String operation, std::shared_ptr<registry::Type> left_type, std::shared_ptr<registry::Type> right_type, ast::Span source_span) noexcept{
return operation == mlc::String("<<") || operation == mlc::String(">>") ? !type_utils::type_is_unknown(left_type) && !type_utils::type_is_unknown(right_type) && !type_utils::types_allowed_for_binary_int_only(left_type, right_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("shift requires i32 operands, got ") + type_utils::type_description(left_type) + mlc::String(" and ") + type_utils::type_description(right_type), source_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}
...
mlc::Array<ast::Diagnostic> unary_minus_diagnostic(mlc::String operation, std::shared_ptr<registry::Type> inner_type, ast::Span source_span) noexcept{
return operation == mlc::String("-") && !type_utils::type_is_unknown(inner_type) && !type_utils::type_is_i32(inner_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("unary - expects i32, got ") + type_utils::type_description(inner_type), source_span)} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> unary_bang_diagnostic(mlc::String operation, std::shared_ptr<registry::Type> inner_type, ast::Span source_span) noexcept{
return operation == mlc::String("!") && !type_utils::type_is_unknown(inner_type) && !type_utils::type_is_bool(inner_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("operator ! expects bool, got ") + type_utils::type_description(inner_type), source_span)} : mlc::Array<ast::Diagnostic>{};
}
...
mlc::Array<ast::Diagnostic> infer_method_receiver_to_integer_string_only(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("to_i") ? !type_utils::type_is_unknown(receiver_type) && !type_utils::type_is_string(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method to_i expects a string receiver, got ") + type_utils::type_description(receiver_type), method_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_string_only_methods(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("char_at") || method_name == mlc::String("substring") || method_name == mlc::String("to_lower") ? !type_utils::type_is_unknown(receiver_type) && !type_utils::type_is_string(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method ") + method_name + mlc::String(" expects a string receiver, got ") + type_utils::type_description(receiver_type), method_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}
...
mlc::Array<ast::Diagnostic> infer_method_receiver_has(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("has") ? !type_utils::type_is_unknown(receiver_type) && !type_utils::receiver_type_is_map(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method has expects a Map receiver, got ") + type_utils::type_description(receiver_type), method_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> infer_method_receiver_get_remove_keys_values(mlc::String method_name, std::shared_ptr<registry::Type> receiver_type, ast::Span method_span) noexcept{
return method_name == mlc::String("get") || method_name == mlc::String("remove") || method_name == mlc::String("keys") || method_name == mlc::String("values") ? !type_utils::type_is_unknown(receiver_type) && !type_utils::receiver_type_is_map(receiver_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method ") + method_name + mlc::String(" expects a Map receiver, got ") + type_utils::type_description(receiver_type), method_span)} : mlc::Array<ast::Diagnostic>{} : mlc::Array<ast::Diagnostic>{};
}
...
mlc::Array<ast::Diagnostic> infer_builtin_method_arity_diagnostics(mlc::String method_name, int argument_count, ast::Span method_span) noexcept{
int expected = type_utils::builtin_method_expected_argument_count(method_name);
...
mlc::Array<ast::Diagnostic> index_not_i32_diagnostic(std::shared_ptr<registry::Type> index_type, ast::Span bracket_source_span) noexcept{
return !type_utils::type_is_unknown(index_type) && !type_utils::type_is_i32(index_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("array index must be i32, got ") + type_utils::type_description(index_type), bracket_source_span)} : mlc::Array<ast::Diagnostic>{};
}

mlc::Array<ast::Diagnostic> if_branch_mismatch_diagnostic(bool branches_mismatch, std::shared_ptr<registry::Type> then_type, std::shared_ptr<registry::Type> else_type, std::shared_ptr<ast::Expr> else_expression) noexcept{
return branches_mismatch ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("if branch types differ: then ") + type_utils::type_description(then_type) + mlc::String(", else ") + type_utils::type_description(else_type), ast::expr_span(else_expression))} : mlc::Array<ast::Diagnostic>{};
}
...
mlc::Array<ast::Diagnostic> match_arm_type_mismatch_diagnostic(int arm_index, std::shared_ptr<registry::Type> first_arm_type, std::shared_ptr<registry::Type> arm_result_type, std::shared_ptr<ast::Expr> arm_body) noexcept{
return arm_index > 0 && !type_utils::type_is_unknown(first_arm_type) && !type_utils::type_is_unknown(arm_result_type) && !type_utils::types_structurally_equal(first_arm_type, arm_result_type) ? mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("match arm result type ") + type_utils::type_description(arm_result_type) + mlc::String(" does not match first arm type ") + type_utils::type_description(first_arm_type), ast::expr_span(arm_body))} : mlc::Array<ast::Diagnostic>{};
}
