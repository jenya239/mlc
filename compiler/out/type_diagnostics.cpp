...
mlc::Array<ast::Diagnostic> infer_expr_field_diagnostics(std::shared_ptr<registry::Type> object_type, mlc::String field_name, ast::Span field_source_span, registry::TypeRegistry registry) noexcept{
std::shared_ptr<registry::Type> inner_object_type = [&]() -> std::shared_ptr<registry::Type> { if (std::holds_alternative<registry::TShared>((*object_type))) { auto _v_tshared = std::get<registry::TShared>((*object_type)); auto [inner] = _v_tshared; return inner; } return object_type; }();
mlc::String type_name = [&]() -> mlc::String { if (std::holds_alternative<registry::TNamed>((*inner_object_type))) { auto _v_tnamed = std::get<registry::TNamed>((*inner_object_type)); auto [name] = _v_tnamed; return name; } if (std::holds_alternative<registry::TGeneric>((*inner_object_type))) { auto _v_tgeneric = std::get<registry::TGeneric>((*inner_object_type)); auto [name, _w0] = _v_tgeneric; return name; } return mlc::String(""); }();
