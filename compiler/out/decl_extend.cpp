...
mlc::String trait_dispatch_name(mlc::String trait_name, mlc::String method_name) noexcept{
  return ((trait_name + mlc::String("_", 1)) + method_name);
}
...
mlc::String parameter_type_cpp(context::CodegenContext context, std::shared_ptr<ast::Param> parameter) noexcept{
  if (context.param_template_type_names.has(parameter->name))   {
    return context.param_template_type_names.get(parameter->name);
  } else   {
    return [&]() -> mlc::String {
...
mlc::String generic_trait_static_assert_line(mlc::String trait_name, mlc::String type_name, mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> methods, context::CodegenContext extend_context, mlc::String implementor_type_cpp) noexcept{
  if ((trait_name == mlc::String("ExprVisitor", 11)))   {
    return expr::static_assert_concept_for_result_and_implementor_line(trait_concept_cpp_name_for_static_assert(trait_name), extend_method_return_type_cpp(extend_context, methods), implementor_type_cpp, ((type_name + mlc::String(" does not implement ", 20)) + trait_name));
  } else   {
    return expr::static_assert_concept_for_type_line(trait_concept_cpp_name_for_static_assert(trait_name), implementor_type_cpp, ((type_name + mlc::String(" does not implement ", 20)) + trait_name));
  }
}
...
if (std::holds_alternative<semantic_ir::SemanticDeclarationFn>((*__match_subject))) {
const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn = std::get<semantic_ir::SemanticDeclarationFn>((*__match_subject));
auto [mangled, __1, __2, parameters, return_type, __5, __6, __7, __8] = semanticDeclarationFn; return trait_method_field_line(trait_context, extract_method_name(mangled, trait_name), parameters, return_type);
}
return mlc::String("", 0);
std::abort();
}();
...
if (std::holds_alternative<semantic_ir::SemanticDeclarationFn>((*__match_subject))) {
const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn = std::get<semantic_ir::SemanticDeclarationFn>((*__match_subject));
auto [mangled, __1, __2, parameters, return_type, __5, __6, __7, __8] = semanticDeclarationFn; return gen_extend_trait_wrapper(type_name, mangled, parameters, return_type, context, context_resolve_fn);
}
return mlc::String("", 0);
std::abort();
}();
...
if (std::holds_alternative<semantic_ir::SemanticDeclarationFn>((*__match_subject))) {
const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn = std::get<semantic_ir::SemanticDeclarationFn>((*__match_subject));
auto [mangled, __1, __2, parameters, return_type, __5, __6, __7, __8] = semanticDeclarationFn; return gen_extend_trait_concept_adapter(type_name, trait_name, mangled, parameters, return_type, context, context_resolve_fn);
}
return mlc::String("", 0);
std::abort();
}();
...
mlc::String trait_concept_dispatch_forward_proto_line(mlc::String trait_name, mlc::String mangled, mlc::String return_type_cpp) noexcept{
  return ((((mlc::String("template<typename Self>\n", 24) + return_type_cpp) + mlc::String(" ", 1)) + cpp_naming::cpp_safe(trait_dispatch_name(trait_name, extract_method_name(mangled, trait_name)))) + mlc::String("(const Self& self) noexcept;\n", 29));
}
mlc::String trait_concept_dispatch_forward_protos(context::CodegenContext context, mlc::String trait_name, mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> methods) noexcept{
  if (trait_skips_concept_dispatch_forward_protos(trait_name))   {
    ...
    auto trait_context = context::CodegenContext_for_type_body(context, trait_name);
    auto forward_protos = mlc::String("", 0);
    auto method_index = 0;
...
mlc::String gen_trait_decl_with_dispatch_forward_protos(context::CodegenContext context, mlc::String trait_name, mlc::Array<mlc::String> type_parameters, mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> methods) noexcept{
  return (trait_concept_dispatch_forward_protos(context, trait_name, methods) + gen_trait_decl(context, trait_name, type_parameters, methods));
}
...
mlc::String gen_decl_extend_trait_suffix(mlc::String type_name, mlc::String trait_name, mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> methods, context::CodegenContext context, std::function<mlc::String(context::CodegenContext, mlc::String)> context_resolve_fn) noexcept{
  if (context::trait_has_associated_types(context, registry::trait_base_name(trait_name)))   {
    return gen_extend_trait_wrappers(type_name, methods, context, context_resolve_fn);
  } else   {
...
mlc::String gen_decl_extend_methods_only(mlc::String type_name, mlc::String trait_name, mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> methods, context::CodegenContext context, std::function<mlc::String(context::CodegenContext, mlc::String)> context_resolve_fn, std::function<mlc::String(std::shared_ptr<semantic_ir::SemanticDeclaration>, context::CodegenContext)> gen_decl_fn) noexcept{
  auto bare_trait_name = registry::trait_base_name(trait_name);
  auto extend_context = context::CodegenContext_for_type_body(context, type_name);
  return gen_extend_method_parts(type_name, bare_trait_name, methods, extend_context, context, context_resolve_fn, gen_decl_fn).join(mlc::String("", 0));
}
mlc::String gen_decl_extend(mlc::String type_name, mlc::String trait_name, mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> methods, context::CodegenContext context, std::function<mlc::String(context::CodegenContext, mlc::String)> context_resolve_fn, std::function<mlc::String(std::shared_ptr<semantic_ir::SemanticDeclaration>, context::CodegenContext)> gen_decl_fn) noexcept{
  auto bare_trait_name = registry::trait_base_name(trait_name);
  auto methods_str = gen_decl_extend_methods_only(type_name, bare_trait_name, methods, context, context_resolve_fn, gen_decl_fn);
  if ((bare_trait_name.length() > 0))   {
    return (methods_str + gen_decl_extend_trait_suffix(type_name, bare_trait_name, methods, context, context_resolve_fn));
  } else   {
    return methods_str;
  }
}
