...
mlc::String print_expr(std::shared_ptr<cpp_ast::CppExpr> expression) noexcept{return std::visit(overloaded{
  ...
  [&](const CppCall& cppcall) -> mlc::String { auto [callee, arguments] = cppcall; return function_call_parentheses(print_expr(callee), print_comma_separated_expressions(arguments)); },
  ...
  [&](const CppIndex& cppindex) -> mlc::String { auto [object, index] = cppindex; return index_subscript(print_expr(object), print_expr(index)); },
  [&](const CppBinary& cppbinary) -> mlc::String { auto [operation, left, right] = cppbinary; return parenthesized_binary(print_expr(left), operation, print_expr(right)); },
  [&](const CppUnary& cppunary) -> mlc::String { auto [operation, inner] = cppunary; return parenthesized_unary(operation, print_expr(inner)); },
  [&](const CppTernary& cppternary) -> mlc::String { auto [condition, then_branch, else_branch] = cppternary; return ternary_conditional(print_expr(condition), print_expr(then_branch), print_expr(else_branch)); },
