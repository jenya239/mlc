...
results.push_back(codegen_test_helpers::assert_code_contains(mlc::String("partial: codegen emits lambda for bound call"), codegen_harness::gen_program(decls::parse_program(lexer::tokenize(mlc::String("fn add(left: i32, right: i32) -> i32 = left + right\nfn main() -> i32 = do\n  let bound = add(1, _)\n  bound(2)\nend")).tokens)), mlc::String("[=]")));
return results;
}
