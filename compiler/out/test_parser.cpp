...
mlc::Array<mlc::String> parse_source_errors(mlc::String source) noexcept;

std::shared_ptr<ast::Expr> parse_expr_source(mlc::String source) noexcept;

std::shared_ptr<ast::TypeExpr> parse_type_source(mlc::String source) noexcept;

mlc::Array<mlc::String> parse_expr_errors(mlc::String source) noexcept;

bool parser_garbage_smoke(mlc::String source) noexcept;

bool parser_expr_garbage_smoke(mlc::String source) noexcept;

mlc::String parser_deep_nesting_source(int depth) noexcept;
...
mlc::String expr_pat_ident_binding_name(std::shared_ptr<ast::Pattern> pattern) noexcept;

int expr_pat_or_alt_count(std::shared_ptr<ast::Pattern> pattern) noexcept;
...
bool type_alias_words_is_array_string(std::shared_ptr<ast::Decl> declaration) noexcept;

bool type_alias_named_is_bar(std::shared_ptr<ast::Decl> declaration) noexcept;

bool type_sum_foo_bar_baz(std::shared_ptr<ast::Decl> declaration) noexcept;
...
mlc::Array<mlc::String> parse_source_errors(mlc::String source) noexcept{return decls::parse_program_with_errors(lexer::tokenize(source).tokens, mlc::String("")).errors;}

std::shared_ptr<ast::Expr> parse_expr_source(mlc::String source) noexcept{return exprs::parse_expr(predicates::parser_new(lexer::tokenize(source).tokens)).value;}

std::shared_ptr<ast::TypeExpr> parse_type_source(mlc::String source) noexcept{return types::parse_type(predicates::parser_new(lexer::tokenize(source).tokens)).value;}

mlc::Array<mlc::String> parse_expr_errors(mlc::String source) noexcept{return exprs::parse_expr(predicates::parser_new(lexer::tokenize(source).tokens)).parser.errors;}

bool parser_garbage_smoke(mlc::String source) noexcept{
ast_tokens::LexOut lex = lexer::tokenize(source);
return ast_tokens::LexOut_has_errors(lex) ? true : [&]() -> bool { 
  decls::parse_program_with_errors(lex.tokens, mlc::String("smoke.mlc"));
  return true;
 }();
}
...
ast_tokens::LexOut lex = lexer::tokenize(source);
return ast_tokens::LexOut_has_errors(lex) ? true : [&]() -> bool { 
  exprs::parse_expr(predicates::parser_new(lex.tokens));
  return true;
 }();
}
...
return mlc::String("fn smoke_entry() -> i32 = ") + body;
}
...
mlc::String expr_pat_ident_binding_name(std::shared_ptr<ast::Pattern> pattern) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<ast::PatternIdent>((*pattern))) { auto _v_patternident = std::get<ast::PatternIdent>((*pattern)); auto [binding_name, _w0] = _v_patternident; return binding_name; } return mlc::String(""); }();}

int expr_pat_or_alt_count(std::shared_ptr<ast::Pattern> pattern) noexcept{return [&]() { if (std::holds_alternative<ast::PatternOr>((*pattern))) { auto _v_patternor = std::get<ast::PatternOr>((*pattern)); auto [alternatives, _w0] = _v_patternor; return [&]() -> int { 
  int count = 0;
  while (count < alternatives.size()){
{
count = count + 1;
}
}
...
bool type_alias_words_is_array_string(std::shared_ptr<ast::Decl> declaration) noexcept{return [&]() { if (std::holds_alternative<ast::DeclTypeAlias>((*declaration))) { auto _v_decltypealias = std::get<ast::DeclTypeAlias>((*declaration)); auto [name, _w0, type_expression, _w1] = _v_decltypealias; return name != mlc::String("Words") ? false : [&]() { if (std::holds_alternative<ast::TyArray>((*type_expression))) { auto _v_tyarray = std::get<ast::TyArray>((*type_expression)); auto [inner] = _v_tyarray; return [&]() { if (std::holds_alternative<ast::TyString>((*inner))) {  return true; } return false; }(); } return false; }(); } return false; }();}

bool type_alias_named_is_bar(std::shared_ptr<ast::Decl> declaration) noexcept{return [&]() { if (std::holds_alternative<ast::DeclTypeAlias>((*declaration))) { auto _v_decltypealias = std::get<ast::DeclTypeAlias>((*declaration)); auto [name, _w0, type_expression, _w1] = _v_decltypealias; return name != mlc::String("Alias") ? false : [&]() { if (std::holds_alternative<ast::TyNamed>((*type_expression))) { auto _v_tynamed = std::get<ast::TyNamed>((*type_expression)); auto [type_name] = _v_tynamed; return type_name == mlc::String("Bar"); } return false; }(); } return false; }();}

bool type_sum_foo_bar_baz(std::shared_ptr<ast::Decl> declaration) noexcept{return [&]() { if (std::holds_alternative<ast::DeclType>((*declaration))) { auto _v_decltype = std::get<ast::DeclType>((*declaration)); auto [name, _w0, variants, _w1, _w2] = _v_decltype; return name == mlc::String("Foo") && variants.size() == 2; } return false; }();}
...
bool trait_display_decl(ast::Program program) noexcept{return [&]() { if (std::holds_alternative<ast::DeclTrait>((*program.decls[0]))) { auto _v_decltrait = std::get<ast::DeclTrait>((*program.decls[0])); auto [name, _w0, methods, _w1] = _v_decltrait; return name == mlc::String("Display") && methods.size() == 1; } return false; }();}
...
results.push_back(test_runner::assert_true(mlc::String("type decl is DeclType"), [&]() { if (std::holds_alternative<ast::DeclType>((*first_decl(mlc::String("type Color = Red | Green"))))) { auto _v_decltype = std::get<ast::DeclType>((*first_decl(mlc::String("type Color = Red | Green")))); auto [_w0, _w1, _w2, _w3, _w4] = _v_decltype; return true; } return false; }()));
results.push_back(test_runner::assert_true(mlc::String("type alias is DeclTypeAlias"), type_alias_words_is_array_string(first_decl(mlc::String("type Words = [string]")))));
results.push_back(test_runner::assert_true(mlc::String("type alias named type"), type_alias_named_is_bar(first_decl(mlc::String("type Alias = Bar")))));
results.push_back(test_runner::assert_true(mlc::String("type Foo = Bar | Baz stays sum DeclType"), type_sum_foo_bar_baz(first_decl(mlc::String("type Foo = Bar | Baz")))));
results.push_back(test_runner::assert_true(mlc::String("exported type alias is DeclTypeAlias"), [&]() { if (std::holds_alternative<ast::DeclExported>((*first_decl(mlc::String("export type Words = [string]"))))) { auto _v_declexported = std::get<ast::DeclExported>((*first_decl(mlc::String("export type Words = [string]")))); auto [inner] = _v_declexported; return type_alias_words_is_array_string(inner); } return false; }()));
...
results.push_back(test_runner::assert_eq_int(mlc::String("fn where multi-parameter — 0 parse errors"), parse_source_errors(mlc::String("fn f<T, U>() -> unit where T: Display, U: Clone = ()")).size(), 0));
results.push_back(test_runner::assert_true(mlc::String("parse_type tuple fn uses TypesResult value"), [&]() { if (std::holds_alternative<ast::TyFn>((*parse_type_source(mlc::String("(i32, string) -> bool"))))) { auto _v_tyfn = std::get<ast::TyFn>((*parse_type_source(mlc::String("(i32, string) -> bool")))); auto [parameters, _w0] = _v_tyfn; return parameters.size() == 2; } return false; }()));
...
results.push_back(test_runner::assert_true(mlc::String("or-pattern brace-style: PatternOr produced"), [&]() { if (std::holds_alternative<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { A(r) | B(r) => r }")))._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { A(r) | B(r) => r }")))._); auto [_w0, arms, _w1] = _v_exprmatch; return arms.size() == 1 ? [&]() { if (std::holds_alternative<ast::PatternOr>((*arms[0]->pattern))) { auto _v_patternor = std::get<ast::PatternOr>((*arms[0]->pattern)); auto [alternatives, _w0] = _v_patternor; return alternatives.size() == 2; } return false; }() : false; } return false; }()));
results.push_back(test_runner::assert_true(mlc::String("or-pattern pipe-style: PatternOr produced"), [&]() { if (std::holds_alternative<ast::ExprMatch>((*parse_expr_source(mlc::String("match x | A(r) | B(r) => r end")))._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*parse_expr_source(mlc::String("match x | A(r) | B(r) => r end")))._); auto [_w0, arms, _w1] = _v_exprmatch; return arms.size() == 1 ? [&]() { if (std::holds_alternative<ast::PatternOr>((*arms[0]->pattern))) { auto _v_patternor = std::get<ast::PatternOr>((*arms[0]->pattern)); auto [alternatives, _w0] = _v_patternor; return alternatives.size() == 2; } return false; }() : false; } return false; }()));
results.push_back(test_runner::assert_true(mlc::String("unit or-pattern: PatternOr produced"), [&]() { if (std::holds_alternative<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { Red | Green => 1, Blue => 2 }")))._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { Red | Green => 1, Blue => 2 }")))._); auto [_w0, arms, _w1] = _v_exprmatch; return arms.size() == 2 ? [&]() { if (std::holds_alternative<ast::PatternOr>((*arms[0]->pattern))) { auto _v_patternor = std::get<ast::PatternOr>((*arms[0]->pattern)); auto [alternatives, _w0] = _v_patternor; return alternatives.size() == 2; } return false; }() : false; } return false; }()));
results.push_back(test_runner::assert_true(mlc::String("single pattern: no PatternOr, no guard"), [&]() { if (std::holds_alternative<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { A(r) => r }")))._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { A(r) => r }")))._); auto [_w0, arms, _w1] = _v_exprmatch; return arms.size() == 1 ? arms[0]->has_guard ? false : [&]() { if (std::holds_alternative<ast::PatternCtor>((*arms[0]->pattern))) { auto _v_patternctor = std::get<ast::PatternCtor>((*arms[0]->pattern)); auto [name, _w0, _w1] = _v_patternctor; return name == mlc::String("A"); } return false; }() : false; } return false; }()));
...
results.push_back(test_runner::assert_true(mlc::String("match catch-all: else => synonyms wildcard PatternWild"), [&]() { if (std::holds_alternative<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { 1 => 0, else => 1 }")))._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*parse_expr_source(mlc::String("match x { 1 => 0, else => 1 }")))._); auto [_w0, arms, _w1] = _v_exprmatch; return arms.size() == 2 ? [&]() { if (std::holds_alternative<ast::PatternWild>((*arms[1]->pattern))) { auto _v_patternwild = std::get<ast::PatternWild>((*arms[1]->pattern)); auto [_w0] = _v_patternwild; return true; } return false; }() : false; } return false; }()));
results.push_back(test_runner::assert_true(mlc::String("PatternStringLit pattern_span"), ast::pattern_span(std::make_shared<ast::Pattern>(ast::PatternStringLit(mlc::String("x"), ast::span_unknown()))).column >= 0));
results.push_back(test_runner::assert_true(mlc::String("match string literal arm parses PatternStringLit"), [&]() { if (std::holds_alternative<ast::ExprMatch>((*parse_expr_source(mlc::String("match s { | \"x\" -> 1 | _ -> 0 }")))._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*parse_expr_source(mlc::String("match s { | \"x\" -> 1 | _ -> 0 }")))._); auto [_w0, arms, _w1] = _v_exprmatch; return arms.size() == 2 ? [&]() { if (std::holds_alternative<ast::PatternStringLit>((*arms[0]->pattern))) { auto _v_patternstringlit = std::get<ast::PatternStringLit>((*arms[0]->pattern)); auto [value, _w0] = _v_patternstringlit; return value == mlc::String("x"); } return false; }() : false; } return false; }()));
results.push_back(test_runner::assert_true(mlc::String("match string or-pattern parses PatternOr of PatternStringLit"), [&]() { if (std::holds_alternative<ast::ExprMatch>((*parse_expr_source(mlc::String("match word { | \"a\" | \"b\" -> 1 | _ -> 0 }")))._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*parse_expr_source(mlc::String("match word { | \"a\" | \"b\" -> 1 | _ -> 0 }")))._); auto [_w0, arms, _w1] = _v_exprmatch; return arms.size() != 2 ? false : [&]() { if (std::holds_alternative<ast::PatternOr>((*arms[0]->pattern))) { auto _v_patternor = std::get<ast::PatternOr>((*arms[0]->pattern)); auto [alternatives, _w0] = _v_patternor; return alternatives.size() == 2; } return false; }(); } return false; }()));
results.push_back(test_runner::assert_eq_int(mlc::String("match string arms in function — 1 decl"), decl_count(mlc::String("fn f(word: string) -> i32 = match word { | \"fn\" => 1 | _ => 0 }")), 1));
...
results.push_back(test_runner::assert_true(mlc::String("let-else: has_else=true when else present"), [&]() { if (std::holds_alternative<ast::ExprBlock>((*parse_expr_source(mlc::String("do\n  let Some(v) = opt else return 0 end\n  0\nend")))._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*parse_expr_source(mlc::String("do\n  let Some(v) = opt else return 0 end\n  0\nend")))._); auto [statements, _w0, _w1] = _v_exprblock; return statements.size() >= 1 ? [&]() { if (std::holds_alternative<ast::StmtLetPattern>((*statements[0])._)) { auto _v_stmtletpattern = std::get<ast::StmtLetPattern>((*statements[0])._); auto [_w0, _w1, _w2, _w3, has_else, _w4, _w5] = _v_stmtletpattern; return has_else; } return false; }() : false; } return false; }()));
...
results.push_back(test_runner::assert_eq_int(mlc::String("typed lambda - 0 parse errors"), parse_expr_errors(mlc::String("(x: i32) => x")).size(), 0));
results.push_back(test_runner::assert_eq_int(mlc::String("type derive clause - 1 decl"), parse_source(mlc::String("type Point = { x: i32 } derive { Hash }")).decls.size(), 1));
...
results.push_back(test_runner::assert_true(mlc::String("parse decl: invalid function name yields parse error"), parse_source_errors(mlc::String("fn @@@() -> i32 = 0")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse decl: empty type body after = yields parse error"), parse_source_errors(mlc::String("type EmptyType =")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: unclosed trait body yields parse error"), parse_source_errors(mlc::String("trait BrokenTrait {\n  fn missing_body(self: i32) -> i32")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse decl: trait without { yields parse error"), parse_source_errors(mlc::String("trait Broken")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse decl: unexpected token at top level yields parse error"), parse_source_errors(mlc::String("123")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse expr: unexpected token yields parse error"), parse_expr_errors(mlc::String("@@@")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse expr: unclosed ( yields parse error"), parse_expr_errors(mlc::String("(1")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse expr: unclosed [ yields parse error"), parse_expr_errors(mlc::String("[1")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse expr: unclosed argument list yields parse error"), parse_expr_errors(mlc::String("f(1")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: unclosed do block yields parse error"), parse_source_errors(mlc::String("fn f() -> i32 = do 1")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: multiline unclosed do block yields parse error"), parse_source_errors(mlc::String("fn f() -> i32 = do\n  let x = 1\n  x")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: unclosed record type yields parse error"), parse_source_errors(mlc::String("type OpenRecord = { field_name: string")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: unclosed match brace yields parse error"), parse_source_errors(mlc::String("fn f() -> i32 = match x { A => 1")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: inline if-then-else without end parses"), parse_source_errors(mlc::String("fn f() -> i32 = if true then 1 else 2")).size() == 0));
results.push_back(test_runner::assert_true(mlc::String("parse program: invalid let pattern yields parse error"), parse_source_errors(mlc::String("fn f() -> i32 = do let @@@ = 1 end")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: invalid function name noise yields parse error"), parse_source_errors(mlc::String("fn @@@")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parse program: orphan end yields parse error"), parse_source_errors(mlc::String("end")).size() >= 1));
results.push_back(test_runner::assert_true(mlc::String("parser smoke: deep nesting completes"), parser_garbage_smoke(parser_deep_nesting_source(16))));
results.push_back(test_runner::assert_true(mlc::String("parser smoke: expr garbage completes"), parser_expr_garbage_smoke(mlc::String("@@@ + (["))));
return results;
}
