...
results.push_back(test_runner::assert_true(mlc::String("fn decl is DeclFn"), [&]() -> bool { if (std::holds_alternative<ast::DeclFn>((*first_decl(mlc::String("fn f() -> i32 = 42"))))) { auto _v_declfn = std::get<ast::DeclFn>((*first_decl(mlc::String("fn f() -> i32 = 42")))); auto [_w0, _w1, _w2, _w3] = _v_declfn; return true; } return false; }()));
results.push_back(test_runner::assert_true(mlc::String("type decl is DeclType"), [&]() -> bool { if (std::holds_alternative<ast::DeclType>((*first_decl(mlc::String("type Color = Red | Green"))))) { auto _v_decltype = std::get<ast::DeclType>((*first_decl(mlc::String("type Color = Red | Green")))); auto [_w0, _w1] = _v_decltype; return true; } return false; }()));
