...
return cache.has(norm_path) ? cache.get(norm_path) : loaded.has(norm_path) ? LoadResult{{}, mlc::Array<mlc::String>{mlc::String("circular: ") + norm_path}} : header_import::is_cpp_header_path(norm_path) ? [&]() -> LoadResult { 
  ...
  return header_loaded.errors.size() > 0 ? LoadResult{{}, header_loaded.errors} : [&]() -> LoadResult { 
  mlc::Array<decl_index::LoadItem> header_items = mlc::Array<decl_index::LoadItem>{decl_index::LoadItem{norm_path, header_loaded.declarations, {}, {}}};
  LoadResult load_parsed = LoadResult{header_items, {}};
