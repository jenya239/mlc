...
return expr::ternary_conditional(eval_expr(condition, context, gen_stmts), eval_expr(then_expr, context, gen_stmts), std::visit(overloaded{[&](const semantic_ir::SExprArray& sExprArray) { auto [elements, __1, array_span] = sExprArray; return ((elements.length() > 0) ? (eval_expr(else_expr, context, gen_stmts)) : ([&]() {
