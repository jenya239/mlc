...
return ((((((mlc::String("(", 1) + condition_code) + mlc::String(" ? (", 4)) + then_code) + mlc::String(") : (", 5)) + else_code) + mlc::String("))", 2));
}
...
return ((((mlc::String("mlc::Array<", 11) + element_type_cpp) + mlc::String(">{", 2)) + comma_separated_elements) + mlc::String("}", 1));
}
...
return (((((((mlc::String("{\nauto ", 7) + binder) + mlc::String(" = (", 4)) + resource_code) + mlc::String(");\n", 3)) + body_statements) + binder) + mlc::String(".drop();\n}\n", 11));
}
