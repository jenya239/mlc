...
auto initial_context = TransformContext{param_env, registry};
auto typed_body = transform_expr(body, initial_context, [=](auto stmts, auto ctx) { return transform_stmts::transform_stmts(stmts, ctx); });
auto coerced_body = coerce_expr_to_type(typed_body, return_type);
...
[&](const ast::DeclAssocType& declAssocType) { auto [__0, __1] = declAssocType; return std::make_shared<semantic_ir::SDecl>(semantic_ir::SDeclImport{mlc::String("", 0), {}}); },
[&](const ast::DeclAssocBind& declAssocBind) { auto [name, type_expr, span] = declAssocBind; return std::make_shared<semantic_ir::SDecl>(semantic_ir::SDeclAssocBind{name, registry::type_from_annotation(type_expr), span}); }
}, (*declaration));
}
