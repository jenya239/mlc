...
[&](const semantic_ir::SExprIf& sExprIf) { auto [condition, then_branch, else_branch, __3, __4] = sExprIf; return expr::ternary_conditional(gen_expr_fn(condition, context), gen_expr_fn(then_branch, context), expression_support::generate_conditional_else_with_empty_array_coercion(then_branch, else_branch, binding_semantic_type, context, gen_expr_fn)); },
...
auto value_code = ((elements.length() == 0) ? (std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_type] = tArray; return expr::typed_array_empty_or_untyped_empty(type_gen::sem_type_to_cpp(context, inner_type)); },
