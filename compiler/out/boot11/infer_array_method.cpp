...
extra = mlc::Array<ast::Diagnostic>{ast::diagnostic_error((mlc::String("method zip expects an array argument, got ", 42) + semantic_type_structure::type_description(arg0_result.inferred_type)), ast::expr_span(method_arguments[0]))};
}
} else if (((!semantic_type_structure::type_is_unknown(element_type)) && (!semantic_type_structure::type_is_string(element_type)))) {
extra = mlc::Array<ast::Diagnostic>{ast::diagnostic_error((mlc::String("method join expects an array of string elements, got ", 53) + semantic_type_structure::type_description(element_type)), method_span)};
}
...
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error((mlc::String("method join expects string separator, got ", 42) + semantic_type_structure::type_description(arg0_result.inferred_type)), ast::expr_span(method_arguments[0]))});
}
}
