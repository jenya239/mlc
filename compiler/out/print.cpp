...
mlc::String using_alias(mlc::String alias, mlc::String type_code) noexcept{
  return ((((mlc::String("using ", 6) + mlc::to_string(alias)) + mlc::String(" = ", 3)) + mlc::to_string(type_code)) + mlc::String(";\n", 2));
}
...
mlc::String namespace_block(mlc::String name, mlc::String contents_code) noexcept{
  return ((((mlc::String("namespace ", 10) + mlc::to_string(name)) + mlc::String(" {\n", 3)) + mlc::to_string(contents_code)) + mlc::String("}", 1));
}
...
mlc::String question_try_expression(std::shared_ptr<cpp_ast::CppExpression> inner, mlc::String from_converter_name) noexcept{
  if ((from_converter_name.length() > 0))   {
    return ((((mlc::String("({ auto __q = ", 14) + mlc::to_string(print_expr(inner))) + mlc::String("; if (std::get_if<1>(&__q)) return ", 35)) + mlc::to_string(from_converter_name)) + mlc::String("(*std::get_if<1>(&__q)); std::get<0>(__q).field0; })", 52));
  } else   {
...
[&](const cpp_ast::CppQuestionTry& cppQuestionTry) { auto [inner, from_converter_name] = cppQuestionTry; return question_try_expression(inner, from_converter_name); },
