...
mlc::String print_capture_entry(std::shared_ptr<cpp_ast::CppCapture> capture_entry) noexcept{return cpp_ast::cpp_capture_by_reference(capture_entry) ? mlc::String("&") + cpp_ast::cpp_capture_name(capture_entry) : cpp_ast::cpp_capture_name(capture_entry);}
...
mlc::String print_parameter_entry(std::shared_ptr<cpp_ast::CppParam> parameter) noexcept{return print_type(cpp_ast::cpp_param_type(parameter)) + mlc::String(" ") + cpp_ast::cpp_param_name(parameter);}
...
mlc::String print_struct_field_line(std::shared_ptr<cpp_ast::CppField> field_entry) noexcept{return indent_text(1) + struct_field_line(cpp_ast::cpp_field_type(field_entry), cpp_ast::cpp_field_name(field_entry));}
...
mlc::String print_access_specifier_for_base(cpp_ast::CppAccessLevel access) noexcept{return std::visit(overloaded{
  [&](const CppPublic& cpppublic) -> mlc::String { return mlc::String("public"); },
  [&](const CppProtected& cppprotected) -> mlc::String { return mlc::String("protected"); },
  [&](const CppPrivate& cppprivate) -> mlc::String { return mlc::String("private"); }
}, access);}

mlc::String print_access_label(cpp_ast::CppAccessLevel access) noexcept{return std::visit(overloaded{
  [&](const CppPublic& cpppublic) -> mlc::String { return mlc::String("public:"); },
  [&](const CppProtected& cppprotected) -> mlc::String { return mlc::String("protected:"); },
  [&](const CppPrivate& cppprivate) -> mlc::String { return mlc::String("private:"); }
}, access);}

mlc::String print_base_class_entry(cpp_ast::CppBaseClass base_class) noexcept{return print_access_specifier_for_base(base_class.access) + mlc::String(" ") + base_class.name;}

mlc::String print_base_class_list(mlc::Array<cpp_ast::CppBaseClass> base_classes) noexcept{return base_classes.size() == 0 ? mlc::String("") : mlc::String(" : ") + base_classes.map([](cpp_ast::CppBaseClass base_class) mutable { return print_base_class_entry(base_class); }).join(mlc::String(", "));}

mlc::String print_function_modifiers_prefix(cpp_ast::CppFnModifiers modifiers) noexcept{
mlc::Array<mlc::String> parts = {};
if (modifiers.is_static){
{
parts.push_back(mlc::String("static"));
}
}
if (modifiers.is_virtual){
{
parts.push_back(mlc::String("virtual"));
}
}
if (modifiers.is_explicit){
{
parts.push_back(mlc::String("explicit"));
}
}
return parts.size() == 0 ? mlc::String("") : parts.join(mlc::String(" ")) + mlc::String(" ");
}

mlc::String print_function_modifiers_suffix(cpp_ast::CppFnModifiers modifiers) noexcept{
return modifiers.is_pure_virtual ? mlc::String(" = 0") : modifiers.is_deleted ? mlc::String(" = delete") : modifiers.is_defaulted ? mlc::String(" = default") : [&]() -> mlc::String { 
  mlc::String const_part = modifiers.is_const ? mlc::String(" const") : mlc::String("");
  mlc::String override_part = modifiers.is_override ? mlc::String(" override") : mlc::String("");
  return const_part + override_part;
 }();
}

mlc::String print_function_prototype_core(std::shared_ptr<cpp_ast::CppFunctionPrototype> prototype, int indent_depth) noexcept{
mlc::String modifiers_prefix = print_function_modifiers_prefix(prototype->modifiers);
mlc::String return_type_code = print_type(prototype->return_type);
mlc::String parameters_code = print_parameter_list(prototype->parameters);
mlc::String modifiers_suffix = print_function_modifiers_suffix(prototype->modifiers);
return indent_text(indent_depth) + modifiers_prefix + return_type_code + mlc::String(" ") + prototype->name + mlc::String("(") + parameters_code + mlc::String(")") + modifiers_suffix + mlc::String(" noexcept;");
}

mlc::String print_function_prototype_line(std::shared_ptr<cpp_ast::CppFunctionPrototype> prototype) noexcept{return prototype->template_prefix + print_function_prototype_core(prototype, 0) + mlc::String("\n");}

mlc::String print_member_field_line(std::shared_ptr<cpp_ast::CppType> field_type, mlc::String field_name, mlc::String default_value) noexcept{return default_value != mlc::String("") ? indent_text(1) + print_type(field_type) + mlc::String(" ") + field_name + mlc::String(" = ") + default_value + mlc::String(";") : indent_text(1) + print_type(field_type) + mlc::String(" ") + field_name + mlc::String(";");}

mlc::String print_class_member_line(cpp_ast::CppClassMember member) noexcept{return std::visit(overloaded{
  [&](const CppClassMemberAccess& cppclassmemberaccess) -> mlc::String { auto [access] = cppclassmemberaccess; return indent_text(1) + print_access_label(access); },
  [&](const CppClassMemberField& cppclassmemberfield) -> mlc::String { auto [field_type, field_name, default_value] = cppclassmemberfield; return print_member_field_line(field_type, field_name, default_value); },
  [&](const CppClassMemberTypeAlias& cppclassmembertypealias) -> mlc::String { auto [alias, type_node] = cppclassmembertypealias; return indent_text(1) + mlc::String("using ") + alias + mlc::String(" = ") + print_type(type_node) + mlc::String(";"); },
  [&](const CppClassMemberFunction& cppclassmemberfunction) -> mlc::String { auto [prototype] = cppclassmemberfunction; return print_function_prototype_core(prototype, 1); }
}, member);}

mlc::String print_class_body_members(mlc::Array<cpp_ast::CppClassMember> members) noexcept{return members.map([](cpp_ast::CppClassMember member) mutable { return print_class_member_line(member); }).join(mlc::String("\n"));}

mlc::String print_class_definition(cpp_ast::CppClassDefinition definition) noexcept{
mlc::String kind_word = definition.is_struct ? mlc::String("struct") : mlc::String("class");
mlc::String bases = print_base_class_list(definition.base_classes);
mlc::String body = print_class_body_members(definition.members);
return body.length() == 0 ? definition.template_prefix + kind_word + mlc::String(" ") + definition.name + bases + mlc::String(" {};\n") : definition.template_prefix + kind_word + mlc::String(" ") + definition.name + bases + mlc::String(" {\n") + body + mlc::String("\n};\n");
}

mlc::String typedef_declaration(mlc::String name, mlc::String type_code) noexcept{return mlc::String("typedef ") + type_code + mlc::String(" ") + name + mlc::String(";\n");}

mlc::String extern_block_declaration(mlc::String linkage, mlc::String body_code) noexcept{return mlc::String("extern ") + linkage + mlc::String(" {\n") + body_code + mlc::String("\n}\n");}

mlc::String print_template_declaration(mlc::String template_parameters, std::shared_ptr<cpp_ast::CppDeclaration> inner) noexcept{return mlc::String("template<") + template_parameters + mlc::String(">\n") + print_decl_node(inner);}

mlc::Array<mlc::String> variant_arm_type_strings(std::shared_ptr<cpp_ast::CppVariantArm> arm) noexcept{
mlc::Array<std::shared_ptr<cpp_ast::CppType>> types = cpp_ast::cpp_variant_arm_types(arm);
return types.size() == 0 ? mlc::Array<mlc::String>{cpp_ast::cpp_variant_arm_name(arm)} : types.map([](std::shared_ptr<cpp_ast::CppType> type_node) mutable { return print_type(type_node); });
}
...
mlc::String print_decl_node(std::shared_ptr<cpp_ast::CppDeclaration> declaration) noexcept{return std::visit(overloaded{
  ...
  [&](const CppClassDeclaration& cppclassdeclaration) -> mlc::String { auto [definition] = cppclassdeclaration; return print_class_definition(definition); },
  [&](const CppTypedefDeclaration& cpptypedefdeclaration) -> mlc::String { auto [name, type_node] = cpptypedefdeclaration; return typedef_declaration(name, print_type(type_node)); },
  [&](const CppTemplateDeclaration& cpptemplatedeclaration) -> mlc::String { auto [template_parameters, inner] = cpptemplatedeclaration; return print_template_declaration(template_parameters, inner); },
  [&](const CppFunctionPrototypeDecl& cppfunctionprototypedecl) -> mlc::String { auto [prototype] = cppfunctionprototypedecl; return print_function_prototype_line(std::make_shared<cpp_ast::CppFunctionPrototype>(prototype)); },
  [&](const CppExternBlock& cppexternblock) -> mlc::String { auto [linkage, declarations] = cppexternblock; return extern_block_declaration(linkage, print_decls(declarations)); },
...
mlc::String print_expr(std::shared_ptr<cpp_ast::CppExpression> expression) noexcept{return std::visit(overloaded{
  ...
  [&](const CppCharLiteral& cppcharliteral) -> mlc::String { auto [character_value] = cppcharliteral; return mlc::String("'") + escape_cpp_character(character_value) + mlc::String("'"); },
  [&](const CppFloatLiteral& cppfloatliteral) -> mlc::String { auto [float_text] = cppfloatliteral; return float_text; },
