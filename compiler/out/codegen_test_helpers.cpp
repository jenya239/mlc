...
test_runner::TestResult assert_expr_generates(mlc::String test_name, context::CodegenContext context, std::shared_ptr<semantic_ir::SemanticExpression> expression, mlc::String expected_code) noexcept{return test_runner::assert_eq_str(test_name, eval::gen_expr(expression, context), expected_code);}
