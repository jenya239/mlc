...
mlc::Array<mlc::String> merged_function_type_parameters(mlc::Array<mlc::String> type_parameters, semantic_ir::FnEscapeInfo escape_info) noexcept{
  return type_parameters.concat(escape_info.synthetic_type_params);
}
bool function_emits_template(mlc::Array<mlc::String> type_parameters, semantic_ir::FnEscapeInfo escape_info) noexcept{
  return (merged_function_type_parameters(type_parameters, escape_info).length() > 0);
}
context::CodegenContext context_with_fn_escape(context::CodegenContext context, semantic_ir::FnEscapeInfo escape_info) noexcept{
...
mlc::String gen_fn_proto(mlc::String name, mlc::Array<mlc::String> type_parameters, mlc::Array<mlc::Array<mlc::String>> type_bounds, mlc::Array<std::shared_ptr<ast::Param>> parameters, std::shared_ptr<registry::Type> return_type, context::CodegenContext context) noexcept{
  return gen_fn_proto_with_escape(name, type_parameters, type_bounds, parameters, return_type, semantic_ir::empty_fn_escape_info(), context);
}
mlc::String gen_fn_proto_with_escape(mlc::String name, mlc::Array<mlc::String> type_parameters, mlc::Array<mlc::Array<mlc::String>> type_bounds, mlc::Array<std::shared_ptr<ast::Param>> parameters, std::shared_ptr<registry::Type> return_type, semantic_ir::FnEscapeInfo escape_info, context::CodegenContext context) noexcept{
  auto all_type_parameters = merged_function_type_parameters(type_parameters, escape_info);
  auto prefix = (cpp_naming::template_prefix(all_type_parameters) + type_gen::requires_clause(type_parameters, type_bounds));
  auto escape_context = context_with_fn_escape(context, escape_info);
  auto safe_name = context::CodegenContext_resolve(escape_context, name);
  auto prototype_context = prototype_context_for_function(escape_context, parameters);
...
mlc::String gen_fn_decl(mlc::String name, mlc::Array<mlc::String> type_parameters, mlc::Array<mlc::Array<mlc::String>> type_bounds, mlc::Array<std::shared_ptr<ast::Param>> parameters, std::shared_ptr<registry::Type> return_type, std::shared_ptr<semantic_ir::SemanticExpression> body, context::CodegenContext context) noexcept{
  return gen_fn_decl_with_escape(name, type_parameters, type_bounds, parameters, return_type, body, semantic_ir::empty_fn_escape_info(), context);
}
mlc::String gen_fn_decl_with_escape(mlc::String name, mlc::Array<mlc::String> type_parameters, mlc::Array<mlc::Array<mlc::String>> type_bounds, mlc::Array<std::shared_ptr<ast::Param>> parameters, std::shared_ptr<registry::Type> return_type, std::shared_ptr<semantic_ir::SemanticExpression> body, semantic_ir::FnEscapeInfo escape_info, context::CodegenContext context) noexcept{
  auto all_type_parameters = merged_function_type_parameters(type_parameters, escape_info);
  auto prefix = (cpp_naming::template_prefix(all_type_parameters) + type_gen::requires_clause(type_parameters, type_bounds));
  auto escape_context = context_with_fn_escape(context, escape_info);
  auto safe_name = context::CodegenContext_resolve(escape_context, name);
  auto body_context = context::CodegenContext_with_enclosing_function_return_type(compute_fn_body_context(name, parameters, escape_context), return_type);
  auto prototype_context = decl_prototype_context_for_body(parameters, body_context, escape_context);
  if (((name == mlc::String("main", 4)) && (parameters.length() == 0)))   {
...
[&](const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn) { auto [name, type_parameters, type_bounds, parameters, return_type, body, __6, escape_info, __8] = semanticDeclarationFn; return [&]() -> mlc::String {
...
auto [name, __1, __2, __3, __4, __5, __6, __7, __8] = semanticDeclarationFn; return (name == mlc::String("main", 4));
}
return false;
std::abort();
}();
...
return (decl_extend::gen_extend_wrapper_protos(type_name, bare_trait_name, methods, codegen_context) + methods.map([=](std::shared_ptr<semantic_ir::SemanticDeclaration> method) mutable { return gen_proto(method, extend_context); }).join(mlc::String("", 0)));
}();
}
...
mlc::String decl_segment_for_trait(mlc::String name, mlc::Array<mlc::String> type_parameters, mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> methods, context::CodegenContext codegen_context, int phase) noexcept{
  if ((phase == 0))   {
    return decl_extend::gen_trait_decl_with_dispatch_forward_protos(codegen_context, name, type_parameters, methods);
  } else if ((phase == 4))   {
...
if (std::holds_alternative<semantic_ir::SemanticDeclarationFn>((*__match_subject))) {
const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn = std::get<semantic_ir::SemanticDeclarationFn>((*__match_subject));
auto [__0, type_parameters, __2, __3, __4, body, __6, escape_info, __8] = semanticDeclarationFn; return (extern_body_is_extern(body) ? (mlc::String("", 0)) : ((function_emits_template(type_parameters, escape_info) ? (mlc::String("", 0)) : (gen_decl(declaration, codegen_context)))));
}
return mlc::String("", 0);
std::abort();
}();
...
mlc::String decl_segment_for_exported_function_phase_two(std::shared_ptr<semantic_ir::SemanticDeclaration> declaration, std::shared_ptr<semantic_ir::SemanticExpression> body, context::CodegenContext codegen_context) noexcept{
  if (extern_body_is_extern(body))   {
    return gen_proto(declaration, codegen_context);
  } else   {
    return gen_proto(declaration, codegen_context);
  }
}
mlc::String decl_segment_for_exported_function_phase_three(std::shared_ptr<semantic_ir::SemanticDeclaration> declaration, mlc::Array<mlc::String> type_parameters, std::shared_ptr<semantic_ir::SemanticExpression> body, semantic_ir::FnEscapeInfo escape_info, context::CodegenContext codegen_context) noexcept{
  if (function_emits_template(type_parameters, escape_info))   {
    return mlc::String("", 0);
...
mlc::String decl_segment_for_exported_function(std::shared_ptr<semantic_ir::SemanticDeclaration> declaration, mlc::Array<mlc::String> type_parameters, std::shared_ptr<semantic_ir::SemanticExpression> body, semantic_ir::FnEscapeInfo escape_info, context::CodegenContext codegen_context, int phase) noexcept{
  if ((phase == 2))   {
    return decl_segment_for_exported_function_phase_two(declaration, body, codegen_context);
  } else if ((phase == 3))   {
    return decl_segment_for_exported_function_phase_three(declaration, type_parameters, body, escape_info, codegen_context);
  } else   {
    return mlc::String("", 0);
  }
}
...
[&](const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn) { auto [__0, type_parameters, __2, __3, __4, body, __6, escape_info, __8] = semanticDeclarationFn; return decl_segment_for_exported_function(declaration, type_parameters, body, escape_info, codegen_context, phase); },
...
[&](const semantic_ir::SemanticDeclarationTypeAlias& semanticDeclarationTypeAlias) { auto [__0, __1, __2, __3] = semanticDeclarationTypeAlias; return mlc::String("", 0); },
[&](const semantic_ir::SemanticDeclarationTrait& semanticDeclarationTrait) { auto [name, type_parameters, methods, __3] = semanticDeclarationTrait; return decl_segment_for_trait(name, type_parameters, methods, codegen_context, phase); },
[&](const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn) { auto [__0, __1, __2, __3, __4, __5, __6, __7, __8] = semanticDeclarationFn; return decl_segment_for_function(declaration, codegen_context, phase); },
[&](const semantic_ir::SemanticDeclarationExtend& semanticDeclarationExtend) { auto [__0, __1, __2, __3] = semanticDeclarationExtend; return decl_segment_for_extend(declaration, codegen_context, phase); },
[&](const semantic_ir::SemanticDeclarationImport& semanticDeclarationImport) { auto [__0, __1] = semanticDeclarationImport; return mlc::String("", 0); },
[&](const semantic_ir::SemanticDeclarationExternLib& semanticDeclarationExternLib) { auto [__0, __1] = semanticDeclarationExternLib; return mlc::String("", 0); },
[&](const semantic_ir::SemanticDeclarationExternType& semanticDeclarationExternType) { auto [__0, __1, __2, __3, __4, __5] = semanticDeclarationExternType; return mlc::String("", 0); },
...
if (std::holds_alternative<semantic_ir::SemanticDeclarationExported>((*__match_subject))) {
const semantic_ir::SemanticDeclarationExported& semanticDeclarationExported = std::get<semantic_ir::SemanticDeclarationExported>((*__match_subject));
auto [inner] = semanticDeclarationExported; return exported_type_name_from_declaration(inner);
}
...
if (std::holds_alternative<semantic_ir::SemanticDeclarationExported>((*__match_subject))) {
const semantic_ir::SemanticDeclarationExported& semanticDeclarationExported = std::get<semantic_ir::SemanticDeclarationExported>((*__match_subject));
auto [inner] = semanticDeclarationExported; return extend_helper_prototypes_for_declaration(exported_types, context, inner);
}
return [&]() {
auto empty_prototypes = mlc::Array<mlc::String>{};
return empty_prototypes;
...
DeclPartsBundle collect_all_decl_parts(mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> declarations, context::CodegenContext context) noexcept{
  ...
  (fn_protos = fn_protos.concat(collect_template_fn_bodies(declarations, context)));
  return DeclPartsBundle{type_fwds, type_defs, fn_protos, fn_defs};
}
mlc::String template_fn_body_for_declaration(std::shared_ptr<semantic_ir::SemanticDeclaration> declaration, context::CodegenContext context) noexcept{
  return [&]() -> mlc::String {
auto __match_subject = declaration;
if (std::holds_alternative<semantic_ir::SemanticDeclarationFn>((*__match_subject))) {
const semantic_ir::SemanticDeclarationFn& semanticDeclarationFn = std::get<semantic_ir::SemanticDeclarationFn>((*__match_subject));
auto [__0, type_parameters, __2, __3, __4, body, __6, escape_info, __8] = semanticDeclarationFn; return (extern_body_is_extern(body) ? (mlc::String("", 0)) : ((function_emits_template(type_parameters, escape_info) ? (gen_decl(declaration, context)) : (mlc::String("", 0)))));
}
if (std::holds_alternative<semantic_ir::SemanticDeclarationExported>((*__match_subject))) {
const semantic_ir::SemanticDeclarationExported& semanticDeclarationExported = std::get<semantic_ir::SemanticDeclarationExported>((*__match_subject));
auto [inner_declaration] = semanticDeclarationExported; return template_fn_body_for_declaration(semantic_ir::sdecl_inner(inner_declaration), context);
}
return mlc::String("", 0);
std::abort();
}();
}
mlc::Array<mlc::String> collect_template_fn_bodies(mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> declarations, context::CodegenContext context) noexcept{
  return declarations.map([=](std::shared_ptr<semantic_ir::SemanticDeclaration> declaration) mutable { return template_fn_body_for_declaration(declaration, context); }).filter([=](mlc::String segment) mutable { return (segment.length() > 0); });
}
...
if (std::holds_alternative<semantic_ir::SemanticDeclarationExported>((*__match_subject))) {
const semantic_ir::SemanticDeclarationExported& semanticDeclarationExported = std::get<semantic_ir::SemanticDeclarationExported>((*__match_subject));
auto [inner_declaration] = semanticDeclarationExported; return extend_trait_suffix_header_for_declaration(inner_declaration, context);
}
return mlc::String("", 0);
std::abort();
}();
...
mlc::Array<mlc::String> collect_extend_trait_suffix_header(mlc::Array<std::shared_ptr<semantic_ir::SemanticDeclaration>> declarations, context::CodegenContext context) noexcept{
  return declarations.map([=](std::shared_ptr<semantic_ir::SemanticDeclaration> declaration) mutable { return extend_trait_suffix_header_for_declaration(declaration, context); }).filter([=](mlc::String segment) mutable { return (segment.length() > 0); });
}
