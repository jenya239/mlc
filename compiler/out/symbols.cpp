...
int pattern_index = 0;
return [&]() { 
  while (pattern_index < patterns.size()){
{
...
return [&]() { 
  while (expression_index < expressions.size()){
{
...
void collect_pattern_definition_spans(std::shared_ptr<ast::Pattern> pattern, mlc::String source_file, mlc::HashMap<mlc::String, ast::Span>& table) noexcept{
return [&]() -> void { if (std::holds_alternative<ast::PatternIdent>((*pattern))) { auto _v_patternident = std::get<ast::PatternIdent>((*pattern)); auto [name, pattern_source_span] = _v_patternident; return symbol_table_set(table, name, source_file, pattern_source_span); } if (std::holds_alternative<ast::PatternCtor>((*pattern))) { auto _v_patternctor = std::get<ast::PatternCtor>((*pattern)); auto [_w0, sub_patterns, _w1] = _v_patternctor; return collect_pattern_list_definition_spans(sub_patterns, source_file, table); } if (std::holds_alternative<ast::PatternRecord>((*pattern))) { auto _v_patternrecord = std::get<ast::PatternRecord>((*pattern)); auto [_w0, field_patterns, _w1] = _v_patternrecord; return collect_pattern_list_definition_spans(field_patterns, source_file, table); } if (std::holds_alternative<ast::PatternTuple>((*pattern))) { auto _v_patterntuple = std::get<ast::PatternTuple>((*pattern)); auto [sub_patterns, _w0] = _v_patterntuple; return collect_pattern_list_definition_spans(sub_patterns, source_file, table); } if (std::holds_alternative<ast::PatternArray>((*pattern))) { auto _v_patternarray = std::get<ast::PatternArray>((*pattern)); auto [sub_patterns, rest_name, _w0] = _v_patternarray; return [&]() { 
 ...
 }(); } if (std::holds_alternative<ast::PatternOr>((*pattern))) { auto _v_patternor = std::get<ast::PatternOr>((*pattern)); auto [alternatives, _w0] = _v_patternor; return collect_first_pattern_alternative(alternatives, source_file, table); }  }();
}
...
index = index + 1;
 ...
 }(); } if (std::holds_alternative<ast::ExprMatch>((*expression)._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*expression)._); auto [scrutinee, arms, _w0] = _v_exprmatch; collect_expression_definition_spans(scrutinee, source_file, table); int index = 0; [&]() { 
...
index = index + 1;
 ...
 }(); return; } if (std::holds_alternative<ast::ExprWith>((*expression)._)) { auto _v_exprwith = std::get<ast::ExprWith>((*expression)._); auto [resource_expression, binder_name, body_statements, with_span] = _v_exprwith; return [&]() { 
 ...
 }(); } if (std::holds_alternative<ast::ExprRecord>((*expression)._)) { auto _v_exprrecord = std::get<ast::ExprRecord>((*expression)._); auto [_w0, parts, _w1] = _v_exprrecord; return collect_record_parts_definition_spans(parts, source_file, table); } if (std::holds_alternative<ast::ExprRecordUpdate>((*expression)._)) { auto _v_exprrecordupdate = std::get<ast::ExprRecordUpdate>((*expression)._); auto [_w0, base_record, field_values, _w1] = _v_exprrecordupdate; return [&]() { 
 ...
 }(); } if (std::holds_alternative<ast::ExprArray>((*expression)._)) { auto _v_exprarray = std::get<ast::ExprArray>((*expression)._); auto [elements, _w0] = _v_exprarray; return collect_expression_list_definition_spans(elements, source_file, table); } if (std::holds_alternative<ast::ExprTuple>((*expression)._)) { auto _v_exprtuple = std::get<ast::ExprTuple>((*expression)._); auto [elements, _w0] = _v_exprtuple; return collect_expression_list_definition_spans(elements, source_file, table); } if (std::holds_alternative<ast::ExprQuestion>((*expression)._)) { auto _v_exprquestion = std::get<ast::ExprQuestion>((*expression)._); auto [inner_expression, _w0] = _v_exprquestion; return collect_expression_definition_spans(inner_expression, source_file, table); } if (std::holds_alternative<ast::ExprNamedArg>((*expression)._)) { auto _v_exprnamedarg = std::get<ast::ExprNamedArg>((*expression)._); auto [_w0, argument_expression, _w1] = _v_exprnamedarg; return collect_expression_definition_spans(argument_expression, source_file, table); }  }();
}
...
index = index + 1;
 ...
 }(); } if (std::holds_alternative<ast::DeclExtend>((*declaration))) { auto _v_declextend = std::get<ast::DeclExtend>((*declaration)); auto [_w0, _w1, members, _w2] = _v_declextend; return collect_declaration_list_definition_spans(members, source_file, table); } if (std::holds_alternative<ast::DeclExported>((*declaration))) { auto _v_declexported = std::get<ast::DeclExported>((*declaration)); auto [inner_declaration] = _v_declexported; return collect_declaration_definition_spans(inner_declaration, source_file, table); }  }();
}
