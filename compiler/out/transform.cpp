...
std::shared_ptr<semantic_ir::SemanticExpression> transform_shared_static_new_call(mlc::Array<std::shared_ptr<ast::Expr>> method_arguments, transform::TransformContext transform_context, ast::Span source_span, std::function<transform::TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, transform::TransformContext)> stmts_fn) noexcept;
...
std::shared_ptr<registry::Type> method_result_type_for_dispatch(std::shared_ptr<registry::Type> receiver_type, mlc::String method_name, registry::TypeRegistry registry) noexcept{return infer_weak_method::is_shared_weak_method_on_receiver(receiver_type, method_name) ? std::make_shared<registry::Type>(registry::TGeneric(mlc::String("Weak"), mlc::Array<std::shared_ptr<registry::Type>>{semantic_type_structure::shared_pointer_inner_type(receiver_type)})) : infer_weak_method::is_weak_upgrade_method_on_receiver(receiver_type, method_name) ? [&]() -> std::shared_ptr<registry::Type> { if (std::holds_alternative<registry::TGeneric>((*receiver_type))) { auto _v_tgeneric = std::get<registry::TGeneric>((*receiver_type)); auto [_w0, type_arguments] = _v_tgeneric; return type_arguments.size() == 1 ? std::make_shared<registry::Type>(registry::TGeneric(mlc::String("Option"), mlc::Array<std::shared_ptr<registry::Type>>{std::make_shared<registry::Type>(registry::TShared(type_arguments[0]))})) : std::make_shared<registry::Type>((registry::TUnknown{})); } return std::make_shared<registry::Type>((registry::TUnknown{})); }() : semantic_type_structure::type_is_unknown(semantic_type_structure::builtin_method_return_type(method_name)) ? registry::method_return_type_from_object(receiver_type, method_name, registry) : semantic_type_structure::builtin_method_return_type(method_name);}
...
std::shared_ptr<registry::Type> argument_type = typed_arguments.size() == 1 ? semantic_ir::sexpr_type(typed_arguments[0]) : std::make_shared<registry::Type>((registry::TUnknown{}));
std::shared_ptr<registry::Type> result_type = std::make_shared<registry::Type>(registry::TShared(argument_type));
std::shared_ptr<semantic_ir::SemanticExpression> typed_receiver = std::make_shared<semantic_ir::SemanticExpression>(semantic_ir::SemanticExpressionIdent(mlc::String("Shared"), std::make_shared<registry::Type>((registry::TUnknown{})), source_span));
return std::make_shared<semantic_ir::SemanticExpression>(semantic_ir::SemanticExpressionMethod(typed_receiver, mlc::String("new"), typed_arguments, mlc::Array<int>{0}, result_type, source_span));
}
...
std::shared_ptr<semantic_ir::SemanticExpression> coerce_expr_to_type(std::shared_ptr<semantic_ir::SemanticExpression> expression, std::shared_ptr<registry::Type> target_type) noexcept{return std::visit(overloaded{
  ...
  [&](const SemanticExpressionCall& semanticexpressioncall) -> std::shared_ptr<semantic_ir::SemanticExpression> { auto [function, arguments, flags, _w0, span] = semanticexpressioncall; return [&]() -> std::shared_ptr<semantic_ir::SemanticExpression> { if (std::holds_alternative<registry::TGeneric>((*target_type))) { auto _v_tgeneric = std::get<registry::TGeneric>((*target_type)); auto [_w0, _w1] = _v_tgeneric; return std::make_shared<semantic_ir::SemanticExpression>(semantic_ir::SemanticExpressionCall(function, arguments, flags, target_type, span)); } return expression; }(); },
...
std::shared_ptr<semantic_ir::SemanticExpression> TransformPass_visit_method(transform::TransformPass self, std::shared_ptr<ast::Expr> object, mlc::String method_name, mlc::Array<std::shared_ptr<ast::Expr>> method_arguments, ast::Span source_span, std::function<transform::TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, transform::TransformContext)> stmts_fn) noexcept{return infer_weak_method::is_shared_static_new(object, method_name) ? transform_shared_static_new_call(method_arguments, self.transform_context, source_span, stmts_fn) : transform_method_call_after_object(self.transform_context.registry, object, method_name, method_arguments, self, source_span, stmts_fn);}
...
std::shared_ptr<semantic_ir::SemanticExpression> transform_method_call_after_object(registry::TypeRegistry registry, std::shared_ptr<ast::Expr> object, mlc::String method_name, mlc::Array<std::shared_ptr<ast::Expr>> method_arguments, transform::TransformPass transform_pass, ast::Span source_span, std::function<transform::TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, transform::TransformContext)> stmts_fn) noexcept{
std::shared_ptr<semantic_ir::SemanticExpression> typed_object = dispatch_transform_pass(transform_pass, object, stmts_fn);
std::shared_ptr<registry::Type> receiver_type = semantic_ir::sexpr_type(typed_object);
return hof_method_spec::is_result_option_hof_method(receiver_type, method_name) ? transform_result_option_hof_method_call(registry, typed_object, method_name, method_arguments, receiver_type, transform_pass.transform_context, source_span, stmts_fn) : hof_method_spec::is_array_hof_method_on_receiver(receiver_type, method_name) ? transform_array_hof_method_call(registry, typed_object, method_name, method_arguments, receiver_type, transform_pass.transform_context, source_span, stmts_fn) : transform_regular_method_call(registry, typed_object, method_name, method_arguments, receiver_type, transform_pass.transform_context, source_span, stmts_fn);
}
