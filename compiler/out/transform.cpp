...
[&](const ast::ExprExtern& exprExtern) { auto [__0] = exprExtern; return mlc::String("", 0); },
...
[&](const ast::ExprExtern& exprExtern) { auto [__0] = exprExtern; return false; },
...
std::shared_ptr<registry::Type> method_result_type_for_dispatch(std::shared_ptr<registry::Type> receiver_type, mlc::String method_name, registry::TypeRegistry registry) noexcept{
  ...
  if (semantic_type_structure::type_is_unknown(builtin_type))   {
...
[&](const semantic_ir::SemanticExpressionExtern& semanticExpressionExtern) { auto [__0, __1] = semanticExpressionExtern; return typed_else; },
...
[&](const semantic_ir::SemanticExpressionExtern& semanticExpressionExtern) { auto [__0, __1] = semanticExpressionExtern; return expression; },
...
[&](const semantic_ir::SemanticExpressionCall& semanticExpressionCall) { auto [__0, __1, __2, __3, __4] = semanticExpressionCall; return expression; },
...
[&](const ast::ExprExtern& exprExtern) { auto [__0] = exprExtern; return coerce_expr_to_type(transform_expr(argument_expression_shared, transform_context, stmts_fn), expected_formal_parameter_type); },
...
std::shared_ptr<semantic_ir::SemanticMatchArm> transform_single_match_arm(std::shared_ptr<ast::MatchArm> match_arm, TransformContext transform_context, mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> substitution, std::shared_ptr<registry::Type> scrutinee_type, std::function<TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, TransformContext)> stmts_fn) noexcept{
  ...
  auto typed_guard_expression = [&]() -> std::shared_ptr<semantic_ir::SemanticExpression> {
  if (match_arm->has_guard)   {
    return transform_expr(match_arm->when_condition, arm_transform_context, stmts_fn);
  } else   {
    return std::make_shared<semantic_ir::SemanticExpression>(semantic_ir::SemanticExpressionBool{true, std::make_shared<registry::Type>(registry::TBool{}), ast::pattern_span(match_arm->pattern)});
  }
}();
...
auto [source_span] = exprExtern; return TransformPass_visit_extern(transform_pass, source_span, stmts_fn);
}
...
std::shared_ptr<semantic_ir::SemanticExpression> TransformPass_visit_extern(TransformPass self, ast::Span source_span, std::function<TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, TransformContext)> stmts_fn) noexcept{
return std::make_shared<semantic_ir::SemanticExpression>(semantic_ir::SemanticExpressionExtern{std::make_shared<registry::Type>(registry::TUnit{}), source_span});
}
std::shared_ptr<semantic_ir::SemanticExpression> TransformPass_visit_ident(TransformPass self, mlc::String name, ast::Span source_span, std::function<TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, TransformContext)> stmts_fn) noexcept{
auto resolved_type = (self.transform_context.type_env.has(name) ? (self.transform_context.type_env.get(name)) : ((registry::TypeRegistry_has_fn(self.transform_context.registry, name) ? (registry::TypeRegistry_fn_type(self.transform_context.registry, name)) : ([&]() -> std::shared_ptr<registry::Type> {
  if (registry::TypeRegistry_has_ctor(self.transform_context.registry, name))   {
    return registry::TypeRegistry_ctor_type(self.transform_context.registry, name);
  } else   {
    return std::make_shared<registry::Type>(registry::TUnknown{});
  }
}()))));
...
if (hof_method_spec::is_result_option_hof_method(receiver_type, method_name)) {
  return transform_result_option_hof_method_call(self.transform_context.registry, typed_object, method_name, method_arguments, receiver_type, self.transform_context, source_span, stmts_fn);
} else if (hof_method_spec::is_array_hof_method_on_receiver(receiver_type, method_name)) {
  return transform_array_hof_method_call(self.transform_context.registry, typed_object, method_name, method_arguments, receiver_type, self.transform_context, source_span, stmts_fn);
} else {
  return transform_regular_method_call(self.transform_context.registry, typed_object, method_name, method_arguments, receiver_type, self.transform_context, source_span, stmts_fn);
}
}
std::shared_ptr<semantic_ir::SemanticExpression> TransformPass_visit_field(TransformPass self, std::shared_ptr<ast::Expr> object, mlc::String field_name, ast::Span source_span, std::function<TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, TransformContext)> stmts_fn) noexcept{
auto typed_object = dispatch_transform_pass(self, object, stmts_fn);
...
return std::make_shared<semantic_ir::SemanticExpression>(semantic_ir::SemanticExpressionCall{spawn_callee, mlc::Array{lambda}, mlc::Array<int>{0}, std::make_shared<registry::Type>(registry::TGeneric{mlc::String("Task", 4), mlc::Array<std::shared_ptr<registry::Type>>{result_type}}), source_span});
}
std::shared_ptr<semantic_ir::SemanticExpression> TransformPass_visit_for(TransformPass self, mlc::String variable_name, std::shared_ptr<ast::Expr> iterator, mlc::Array<std::shared_ptr<ast::Stmt>> statements, ast::Span source_span, std::function<TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, TransformContext)> stmts_fn) noexcept{
...
auto result_type = [&]() -> std::shared_ptr<registry::Type> {
  if ((typed_arms.length() > 0))   {
    return semantic_ir::sexpr_type(typed_arms[0]->body);
  } else   {
    return std::make_shared<registry::Type>(registry::TUnknown{});
  }
}();
...
auto element_type = [&]() -> std::shared_ptr<registry::Type> {
  if ((typed_elements.length() > 0))   {
    return semantic_ir::sexpr_type(typed_elements[0]);
  } else   {
    return std::make_shared<registry::Type>(registry::TUnknown{});
  }
}();
