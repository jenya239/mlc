...
std::shared_ptr<registry::Type> element_type_for_for_iterator(std::shared_ptr<registry::Type> iterator_type) noexcept{return [&]() -> std::shared_ptr<registry::Type> { if (std::holds_alternative<registry::TArray>((*iterator_type))) { auto _v_tarray = std::get<registry::TArray>((*iterator_type)); auto [element_type] = _v_tarray; return element_type; } return std::make_shared<registry::Type>((registry::TUnknown{})); }();}
