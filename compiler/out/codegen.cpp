...
mlc::HashMap<mlc::String, mlc::String> build_method_owners_for(codegen::LoadItem item, mlc::Array<codegen::LoadItem> all_items) noexcept;
...
int ai = 0;
while (ai < all_items.size()){
{
codegen::LoadItem ai_item = all_items[ai];
if (ai_item.path == imp_path){
{
m = add_method_owners_from_decls(ai_item.decls, m);
int ti = 0;
[&]() { 
  while (ti < ai_item.imports.size()){
{
mlc::String t_path = ai_item.imports[ti];
int bi = 0;
while (bi < all_items.size()){
{
codegen::LoadItem bi_item = all_items[bi];
...
codegen::CodegenContext ctx = codegen::CodegenContext{build_field_orders(full_prog), mlc::String(""), qualified, mlc::String(""), build_method_owners_for(item, all_items), {}, build_variant_types_from_decls(full_prog.decls)};
