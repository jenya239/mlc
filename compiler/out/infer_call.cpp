...
mlc::Array<std::shared_ptr<registry::Type>> generic_type_arguments_from_substitution(mlc::Array<mlc::String> type_parameter_names, mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> substitution) noexcept{
mlc::Array<std::shared_ptr<registry::Type>> type_arguments = {};
int index = 0;
while (index < type_parameter_names.size()){
{
mlc::String parameter_name = type_parameter_names[index];
...
std::shared_ptr<registry::Type> instantiated_algebraic_constructor_type(mlc::String owner_name, mlc::Array<mlc::String> type_parameter_names, mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> substitution) noexcept{return type_parameter_names.size() == 0 ? std::make_shared<registry::Type>(registry::TNamed(owner_name)) : std::make_shared<registry::Type>(registry::TGeneric(owner_name, generic_type_arguments_from_substitution(type_parameter_names, substitution)));}
...
mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> substitution = call_argument_unify::build_call_type_substitution(constructor_parameter_types, argument_inferred_types, constructor_type_parameter_names, type_parameter_source_argument_index, effective_args, call_source_span);
std::shared_ptr<registry::Type> inferred_type = algebraic_type_name != mlc::String("") && constructor_type_parameter_names.size() > 0 ? instantiated_algebraic_constructor_type(algebraic_type_name, constructor_type_parameter_names, substitution) : constructor_output.inferred_type;
