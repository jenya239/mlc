...
return callee_name != mlc::String("") && registry::TypeRegistry_has_ctor(inference_context.registry, callee_name) ? [&]() -> infer_result::InferResult { 
  ...
  int required_arity = registry::TypeRegistry_required_arity_for_fn(inference_context.registry, callee_name);
  ...
  return semantic_type_structure::type_is_function(function_parsed.inferred_type) && argument_count >= min_arity && argument_count <= max_arity ? [&]() -> infer_result::InferResult { 
