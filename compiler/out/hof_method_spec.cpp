...
std::shared_ptr<registry::Type> hof_call_result_type(std::shared_ptr<registry::Type> receiver_type, mlc::String method_name, mlc::Array<std::shared_ptr<registry::Type>> argument_inferred_types) noexcept{
  if (is_result_option_hof_method(receiver_type, method_name))   {
    return result_option_method_types::result_option_hof_call_result_type(receiver_type, method_name, argument_inferred_types);
  } else if (is_array_hof_method_on_receiver(receiver_type, method_name))   {
    return array_method_types::array_hof_call_result_type(semantic_type_structure::array_element_type_from_array_type(receiver_type), method_name, argument_inferred_types);
  } else   {
