...
results.push_back(test_runner::assert_eq_str(mlc::String("pipe x |> double - double_(x)"), eval::gen_expr_via_string(transform::transform_expr(plain_pipe, tctx, [](mlc::Array<std::shared_ptr<ast::Stmt>> stmts, transform::TransformContext ctx2) mutable { return transform_stmts::transform_stmts(stmts, ctx2); }), ctx), mlc::String("double_(x)")));
...
results.push_back(test_runner::assert_eq_str(mlc::String("pipe x |> add(5) - add(x, 5)"), eval::gen_expr_via_string(transform::transform_expr(partial_pipe, tctx, [](mlc::Array<std::shared_ptr<ast::Stmt>> stmts, transform::TransformContext ctx2) mutable { return transform_stmts::transform_stmts(stmts, ctx2); }), ctx), mlc::String("add(x, 5)")));
...
results.push_back(test_runner::assert_eq_str(mlc::String("Point { ...p, x: 10 } - Point{10, p.y}"), eval::gen_expr_via_string(update_expr, ctx_point), mlc::String("Point{10, p.y}")));
...
results.push_back(test_runner::assert_eq_str(mlc::String("Point { ...p, x: 0, y: 0 } - Point{0, 0}"), eval::gen_expr_via_string(all_overrides_expr, ctx_point), mlc::String("Point{0, 0}")));
...
results.push_back(test_runner::assert_true(mlc::String("record update without field order registry uses lazy block"), eval::gen_expr_via_string(lazy_update, ctx).contains(mlc::String("__upd"))));
