...
results.push_back(test_runner::assert_eq_str(mlc::String("pipe x |> double - double_(x)"), eval::gen_expr(transform::transform_expr(plain_pipe, tctx, [](mlc::Array<std::shared_ptr<ast::Stmt>> stmts, transform::TransformContext ctx2) mutable { return transform_stmts::transform_stmts(stmts, ctx2); }), ctx), mlc::String("double_(x)")));
...
results.push_back(test_runner::assert_eq_str(mlc::String("pipe x |> add(5) - add(x, 5)"), eval::gen_expr(transform::transform_expr(partial_pipe, tctx, [](mlc::Array<std::shared_ptr<ast::Stmt>> stmts, transform::TransformContext ctx2) mutable { return transform_stmts::transform_stmts(stmts, ctx2); }), ctx), mlc::String("add(x, 5)")));
...
results.push_back(test_runner::assert_eq_str(mlc::String("pipe x |> clamp(3, 4) - clamp(x, 3, 4)"), eval::gen_expr(transform::transform_expr(multi_partial_pipe, tctx, [](mlc::Array<std::shared_ptr<ast::Stmt>> stmts, transform::TransformContext ctx2) mutable { return transform_stmts::transform_stmts(stmts, ctx2); }), ctx), mlc::String("clamp(x, 3, 4)")));
