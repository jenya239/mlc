...
return std::visit(overloaded{[&](const ast::ExprLambda& exprLambda) { auto [parameter_names, __1, __2] = exprLambda; return ((parameter_names.length() != expected) ? (ast::diagnostics_append(errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((((mlc::String("method ", 7) + mlc::to_string(method_name)) + mlc::String(" expects a lambda with ", 23)) + mlc::to_string(mlc::to_string(expected))) + mlc::String(" parameter(s)", 13)), ast::expr_span(expression))})) : (errors)); },
...
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method unwrap_or value must match Ok type (expected ", 52) + mlc::to_string(semantic_type_structure::type_description(t_t))) + mlc::String(")", 1)), ast::expr_span(method_arguments[0]))})};
}
} else if (((method_name == mlc::String("unwrap_or_else", 14)) && (method_arguments.length() > 0))) {
auto arg0 = method_arguments[0];
auto ar = append_lambda_arity(mlc::Array<ast::Diagnostic>{}, arg0, 1, method_name);
auto lam_r = std::visit(overloaded{[&](const ast::ExprLambda& exprLambda) { auto [parameter_names, body, __2] = exprLambda; return infer_lambda_context::infer_lambda_with_param_types(parameter_names, mlc::Array<std::shared_ptr<registry::Type>>{e_t}, body, inference_context, infer_expr_fn); },
...
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method unwrap_or_else must return the Ok type (expected ", 56) + mlc::to_string(semantic_type_structure::type_description(t_t))) + mlc::String(")", 1)), ast::expr_span(arg0))})};
}
arg_types.push_back(lam_r.inferred_type);
} else if (((method_name == mlc::String("map", 3)) && (method_arguments.length() > 0))) {
auto arg0 = method_arguments[0];
auto ar = append_lambda_arity(mlc::Array<ast::Diagnostic>{}, arg0, 1, method_name);
auto lam_r = std::visit(overloaded{[&](const ast::ExprLambda& exprLambda) { auto [parameter_names, body, __2] = exprLambda; return infer_lambda_context::infer_lambda_with_param_types(parameter_names, mlc::Array<std::shared_ptr<registry::Type>>{t_t}, body, inference_context, infer_expr_fn); },
[&](const auto& __v) { return infer_expr_fn(arg0, inference_context); }
}, (*arg0));
ar = append_lambda_arity(ar, arg0, 1, method_name);
merged = InferResult_absorb(merged, lam_r);
...
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method and_then callback must return Result with the same error type (expected ", 79) + mlc::to_string(semantic_type_structure::type_description(e_t))) + mlc::String(")", 1)), ast::expr_span(arg0))});
}
} else {
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method and_then must return a Result type", 41), ast::expr_span(arg0))});
}
...
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method unwrap_or value must match Some type (expected ", 54) + mlc::to_string(semantic_type_structure::type_description(o_t))) + mlc::String(")", 1)), ast::expr_span(method_arguments[0]))})};
}
} else if (((method_name == mlc::String("or_else", 7)) && (method_arguments.length() > 0))) {
auto arg0 = method_arguments[0];
auto ar = append_lambda_arity(mlc::Array<ast::Diagnostic>{}, arg0, 0, mlc::String("or_else", 7));
auto lam_r = std::visit(overloaded{[&](const ast::ExprLambda& exprLambda) { auto [parameter_names, body, __2] = exprLambda; return infer_lambda_context::infer_lambda_with_param_types(parameter_names, mlc::Array<std::shared_ptr<registry::Type>>{}, body, inference_context, infer_expr_fn); },
[&](const auto& __v) { return infer_expr_fn(arg0, inference_context); }
}, (*arg0));
...
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method or_else must return Option with the same inner type (expected ", 69) + mlc::to_string(semantic_type_structure::type_description(o_t))) + mlc::String(")", 1)), ast::expr_span(arg0))});
}
} else {
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method or_else must return Option", 33), ast::expr_span(arg0))});
}
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, extra)};
arg_types.push_back(lam_r.inferred_type);
...
auto arg0 = method_arguments[0];
auto ar = append_lambda_arity(mlc::Array<ast::Diagnostic>{}, arg0, 1, method_name);
auto lam_r = std::visit(overloaded{[&](const ast::ExprLambda& exprLambda) { auto [parameter_names, body, __2] = exprLambda; return infer_lambda_context::infer_lambda_with_param_types(parameter_names, mlc::Array<std::shared_ptr<registry::Type>>{o_t}, body, inference_context, infer_expr_fn); },
[&](const auto& __v) { return infer_expr_fn(arg0, inference_context); }
}, (*arg0));
ar = append_lambda_arity(ar, arg0, 1, method_name);
merged = InferResult_absorb(merged, lam_r);
...
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method and_then must return Option", 34), ast::expr_span(arg0))});
}
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, extra)};
arg_types.push_back(lam_r.inferred_type);
...
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(mlc::String("method filter expects a predicate returning bool", 48), ast::expr_span(arg0))});
}
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, extra)};
arg_types.push_back(lam_r.inferred_type);
}
