...
return std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_else] = tArray; return [&]() -> infer_result::InferResult {
if (semantic_type_structure::type_is_unknown(inner_else)) {
return infer_result::InferResult{std::make_shared<registry::Type>(registry::TArray{then_inner}), else_result.errors};
} else {
return else_result;
}
}(); },
...
return std::visit(overloaded{[&](const registry::TArray& tArray) { auto [inner_then] = tArray; return infer_merge_unknown_else_array_inner(inner_then, else_result); },
[&](const auto& __v) { return else_result; }
