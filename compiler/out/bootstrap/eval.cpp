...
mlc::String value_string = [&]() -> mlc::String { if (std::holds_alternative<ast::ExprUnit>((*override_value)._)) {  return gen_expr(std::make_shared<ast::Expr>(ast::ExprField(base_expr, field_name)), context); } return gen_expr(override_value, context); }();
...
bool subject_needs_deref(std::shared_ptr<ast::Expr> subject, context::CodegenContext context) noexcept{
return context::expr_returns_shared_sum_visit_ptr(subject) ? true : [&]() { if (std::holds_alternative<ast::ExprIdent>((*subject)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*subject)._); auto [name] = _v_exprident; return context.match_deref_params.contains(name) ? true : context.shared_params.contains(name) ? true : context.value_params.contains(name) ? false : true; } if (std::holds_alternative<ast::ExprIndex>((*subject)._)) { auto _v_exprindex = std::get<ast::ExprIndex>((*subject)._); auto [array_expr, _w0] = _v_exprindex; return [&]() { if (std::holds_alternative<ast::ExprIdent>((*array_expr)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*array_expr)._); auto [array_name] = _v_exprident; return context.shared_array_params.contains(array_name); } return true; }(); } if (std::holds_alternative<ast::ExprMethod>((*subject)._)) { auto _v_exprmethod = std::get<ast::ExprMethod>((*subject)._); auto [_w0, _w1, _w2] = _v_exprmethod; return false; } return true; }();
}

bool first_arm_needs_deref(std::shared_ptr<ast::MatchArm> first_arm, std::shared_ptr<ast::Expr> subject, context::CodegenContext context) noexcept{return [&]() { if (std::holds_alternative<ast::PatCtor>((*first_arm->pat))) { auto _v_patctor = std::get<ast::PatCtor>((*first_arm->pat)); auto [name, _w0] = _v_patctor; return [&]() -> bool { 
...
{
mlc::String bind_name = [&]() -> mlc::String { if (std::holds_alternative<ast::PatIdent>((*sub_patterns[index]))) { auto _v_patident = std::get<ast::PatIdent>((*sub_patterns[index])); auto [name] = _v_patident; return context::cpp_safe(name); } return mlc::String("__") + mlc::to_string(index); }();
...
while (index < sub_patterns.size()){
{
[&]() -> void { if (std::holds_alternative<ast::PatIdent>((*sub_patterns[index]))) { auto _v_patident = std::get<ast::PatIdent>((*sub_patterns[index])); auto [pattern_name] = _v_patident; return [&]() { 
...
mlc::String gen_arm(std::shared_ptr<ast::MatchArm> arm, context::CodegenContext context) noexcept{return std::visit(overloaded{
  ...
  [&](const PatIdent& patident) -> mlc::String { auto [name] = patident; return mlc::String("[&](const auto& ") + context::cpp_safe(name) + mlc::String(") { return ") + gen_expr(arm->body, context) + mlc::String("; }"); },
  [&](const PatCtor& patctor) -> mlc::String { auto [name, sub_patterns] = patctor; return gen_arm_ctor(name, sub_patterns, arm->body, context); },
  [&](const PatRecord& patrecord) -> mlc::String { auto [name, field_patterns] = patrecord; return gen_arm_record_pattern(name, field_patterns, arm->body, context); }
}, (*arm->pat));}
...
mlc::String gen_call_function_code(std::shared_ptr<ast::Expr> function_expr, context::CodegenContext context) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIdent>((*function_expr)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*function_expr)._); auto [name] = _v_exprident; return context::context_resolve(context, context::map_builtin(name)); } return gen_expr(function_expr, context); }();}

bool is_constructor_call(std::shared_ptr<ast::Expr> function_expr) noexcept{return [&]() { if (std::holds_alternative<ast::ExprIdent>((*function_expr)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*function_expr)._); auto [name] = _v_exprident; return name.length() > 0 && name.char_at(0) >= mlc::String("A") && name.char_at(0) <= mlc::String("Z"); } return false; }();}
...
mlc::String gen_object_code(std::shared_ptr<ast::Expr> object, context::CodegenContext context) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIdent>((*object)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*object)._); auto [name] = _v_exprident; return context.self_type.length() > 0 ? resolve_object_code_in_self_context(name, context) : context::context_resolve(context, context::map_builtin(name)); } return gen_expr(object, context); }();}

mlc::String infer_shared_new_type_name(std::shared_ptr<ast::Expr> argument, context::CodegenContext context) noexcept{
mlc::String raw_type_name = [&]() -> mlc::String { if (std::holds_alternative<ast::ExprRecord>((*argument)._)) { auto _v_exprrecord = std::get<ast::ExprRecord>((*argument)._); auto [name, _w0] = _v_exprrecord; return name; } if (std::holds_alternative<ast::ExprCall>((*argument)._)) { auto _v_exprcall = std::get<ast::ExprCall>((*argument)._); auto [function_expr, _w0] = _v_exprcall; return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIdent>((*function_expr)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*function_expr)._); auto [name] = _v_exprident; return name; } return mlc::String(""); }(); } if (std::holds_alternative<ast::ExprIdent>((*argument)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*argument)._); auto [name] = _v_exprident; return name; } if (std::holds_alternative<ast::ExprIndex>((*argument)._)) { auto _v_exprindex = std::get<ast::ExprIndex>((*argument)._); auto [array_expr, _w0] = _v_exprindex; return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIdent>((*array_expr)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*array_expr)._); auto [array_name] = _v_exprident; return context.array_elem_types.has(array_name) ? context.array_elem_types.get(array_name) : mlc::String(""); } return mlc::String(""); }(); } return mlc::String(""); }();
...
mlc::String field_access_operator(std::shared_ptr<ast::Expr> object, context::CodegenContext context) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIndex>((*object)._)) { auto _v_exprindex = std::get<ast::ExprIndex>((*object)._); auto [inner_expr, _w0] = _v_exprindex; return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIdent>((*inner_expr)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*inner_expr)._); auto [array_name] = _v_exprident; return context.shared_array_params.contains(array_name) ? mlc::String("->") : mlc::String("."); } return mlc::String("."); }(); } if (std::holds_alternative<ast::ExprIdent>((*object)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*object)._); auto [name] = _v_exprident; return context.shared_params.contains(name) ? mlc::String("->") : mlc::String("."); } if (std::holds_alternative<ast::ExprMethod>((*object)._)) { auto _v_exprmethod = std::get<ast::ExprMethod>((*object)._); auto [map_object, method_name, _w0] = _v_exprmethod; return method_name == mlc::String("get") ? [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIdent>((*map_object)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*map_object)._); auto [map_name] = _v_exprident; return context.shared_map_params.contains(map_name) ? mlc::String("->") : mlc::String("."); } return mlc::String("."); }() : mlc::String("."); } return mlc::String("."); }();}
...
mlc::String gen_expr(std::shared_ptr<ast::Expr> expr, context::CodegenContext context) noexcept{return std::visit(overloaded{
  [&](const ExprInt& exprint) -> mlc::String { auto [integer_value] = exprint; return gen_integer_literal(integer_value); },
  [&](const ExprStr& exprstr) -> mlc::String { auto [string_value] = exprstr; return gen_string_literal(string_value); },
  [&](const ExprBool& exprbool) -> mlc::String { auto [boolean_value] = exprbool; return gen_boolean_literal(boolean_value); },
  [&](const ExprUnit& exprunit) -> mlc::String { return gen_unit_literal(); },
  [&](const ExprExtern& exprextern) -> mlc::String { return gen_extern_placeholder(); },
  [&](const ExprIdent& exprident) -> mlc::String { auto [name] = exprident; return gen_identifier(name, context); },
  [&](const ExprBin& exprbin) -> mlc::String { auto [operation, left_expr, right_expr] = exprbin; return gen_binary_expr(operation, left_expr, right_expr, context); },
  [&](const ExprUn& exprun) -> mlc::String { auto [operation, inner_expr] = exprun; return gen_unary_expr(operation, inner_expr, context); },
  [&](const ExprCall& exprcall) -> mlc::String { auto [function_expr, arguments] = exprcall; return gen_call_expr(function_expr, arguments, context); },
  [&](const ExprMethod& exprmethod) -> mlc::String { auto [object, method_name, arguments] = exprmethod; return gen_method_expr(object, method_name, arguments, context); },
  [&](const ExprField& exprfield) -> mlc::String { auto [object, field_name] = exprfield; return gen_field_expr(object, field_name, context); },
  [&](const ExprIndex& exprindex) -> mlc::String { auto [object, index_expr] = exprindex; return gen_index_expr(object, index_expr, context); },
  [&](const ExprIf& exprif) -> mlc::String { auto [condition, then_expr, else_expr] = exprif; return gen_if_expr(condition, then_expr, else_expr, context); },
  [&](const ExprBlock& exprblock) -> mlc::String { auto [statements, result_expr] = exprblock; return gen_block_expr(statements, result_expr, context); },
  [&](const ExprWhile& exprwhile) -> mlc::String { auto [condition, statements] = exprwhile; return gen_while_expr(condition, statements, context); },
  [&](const ExprFor& exprfor) -> mlc::String { auto [variable, iterator, statements] = exprfor; return gen_for_expr(variable, iterator, statements, context); },
  [&](const ExprMatch& exprmatch) -> mlc::String { auto [subject, arms] = exprmatch; return gen_match(subject, arms, context); },
  [&](const ExprRecord& exprrecord) -> mlc::String { auto [type_name, field_values] = exprrecord; return gen_record_expr(type_name, field_values, context); },
  [&](const ExprRecordUpdate& exprrecordupdate) -> mlc::String { auto [type_name, base_expr, overrides] = exprrecordupdate; return gen_record_update_expr(type_name, base_expr, overrides, context); },
  [&](const ExprArray& exprarray) -> mlc::String { auto [elements] = exprarray; return gen_array_expr(elements, context); },
  [&](const ExprQuestion& exprquestion) -> mlc::String { auto [inner_expr] = exprquestion; return gen_question_expr(inner_expr, context); },
  [&](const ExprLambda& exprlambda) -> mlc::String { auto [parameters, body_expr] = exprlambda; return gen_lambda_expr(parameters, body_expr, context); }
}, (*expr)._);}
...
context::GenStmtResult gen_stmt_with_try(std::shared_ptr<ast::Stmt> stmt, context::CodegenContext context, int try_counter) noexcept{return std::visit(overloaded{
  [&](const StmtLet& stmtlet) -> context::GenStmtResult { auto [name, _w0, typ, value] = stmtlet; return [&]() -> context::GenStmtResult { 
  mlc::String try_identifier = mlc::String("__try_") + mlc::to_string(try_counter);
  int next_counter = try_counter + 1;
  context::GenStmtResult result = [&]() -> context::GenStmtResult { if (std::holds_alternative<ast::ExprQuestion>((*value)._)) { auto _v_exprquestion = std::get<ast::ExprQuestion>((*value)._); auto [inner_expr] = _v_exprquestion; return context::GenStmtResult{gen_try_unwrap(inner_expr, context, try_identifier, mlc::String("auto ") + context::cpp_safe(name) + mlc::String(" = std::get<0>(") + try_identifier + mlc::String(").field0;\n")), next_counter}; } if (std::holds_alternative<ast::ExprArray>((*value)._)) { auto _v_exprarray = std::get<ast::ExprArray>((*value)._); auto [elements] = _v_exprarray; return [&]() -> context::GenStmtResult { 
 ...
 }(); } if (std::holds_alternative<ast::ExprMethod>((*value)._)) { auto _v_exprmethod = std::get<ast::ExprMethod>((*value)._); auto [map_object, method_name, _w0] = _v_exprmethod; return [&]() -> context::GenStmtResult { 
  mlc::String value_code = method_name == mlc::String("new") ? [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIdent>((*map_object)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*map_object)._); auto [object_name] = _v_exprident; return object_name == mlc::String("Map") ? [&]() -> mlc::String { if (std::holds_alternative<ast::TyGeneric>((*typ))) { auto _v_tygeneric = std::get<ast::TyGeneric>((*typ)); auto [type_name, type_arguments] = _v_tygeneric; return type_name == mlc::String("Map") && type_arguments.size() == 2 ? mlc::String("mlc::HashMap<") + type_gen::type_to_cpp(context, type_arguments[0]) + mlc::String(", ") + type_gen::type_to_cpp(context, type_arguments[1]) + mlc::String(">()") : gen_expr(value, context); } return gen_expr(value, context); }() : gen_expr(value, context); } return gen_expr(value, context); }() : gen_expr(value, context);
  return context::GenStmtResult{mlc::String("auto ") + context::cpp_safe(name) + mlc::String(" = ") + value_code + mlc::String(";\n"), try_counter};
 }(); } return context::GenStmtResult{mlc::String("auto ") + context::cpp_safe(name) + mlc::String(" = ") + gen_expr(value, context) + mlc::String(";\n"), try_counter}; }();
  return result;
 }(); },
  [&](const StmtExpr& stmtexpr) -> context::GenStmtResult { auto [expression] = stmtexpr; return [&]() -> context::GenStmtResult { if (std::holds_alternative<ast::ExprQuestion>((*expression)._)) { auto _v_exprquestion = std::get<ast::ExprQuestion>((*expression)._); auto [inner_expr] = _v_exprquestion; return [&]() -> context::GenStmtResult { 
  mlc::String try_identifier = mlc::String("__try_") + mlc::to_string(try_counter);
  ...
  [&](const StmtBreak& stmtbreak) -> context::GenStmtResult { return context::GenStmtResult{mlc::String("break;\n"), try_counter}; },
  [&](const StmtContinue& stmtcontinue) -> context::GenStmtResult { return context::GenStmtResult{mlc::String("continue;\n"), try_counter}; },
  [&](const StmtReturn& stmtreturn) -> context::GenStmtResult { auto [expression] = stmtreturn; return [&]() -> context::GenStmtResult { if (std::holds_alternative<ast::ExprQuestion>((*expression)._)) { auto _v_exprquestion = std::get<ast::ExprQuestion>((*expression)._); auto [inner_expr] = _v_exprquestion; return [&]() -> context::GenStmtResult { 
...
output = output + else_string;
 ...
 }(); } if (std::holds_alternative<ast::ExprWhile>((*expression)._)) { auto _v_exprwhile = std::get<ast::ExprWhile>((*expression)._); auto [condition, statements] = _v_exprwhile; return mlc::String("while (") + gen_expr(condition, context) + mlc::String(") {\n") + gen_stmts_str(statements, context) + mlc::String("}\n"); } if (std::holds_alternative<ast::ExprFor>((*expression)._)) { auto _v_exprfor = std::get<ast::ExprFor>((*expression)._); auto [variable, iterator, statements] = _v_exprfor; return mlc::String("for (auto ") + context::cpp_safe(variable) + mlc::String(" : ") + gen_expr(iterator, context) + mlc::String(") {\n") + gen_stmts_str(statements, context) + mlc::String("}\n"); } if (std::holds_alternative<ast::ExprBlock>((*expression)._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*expression)._); auto [statements, result_expr] = _v_exprblock; return [&]() -> mlc::String { 
...
mlc::String gen_block_body(std::shared_ptr<ast::Expr> expr, context::CodegenContext context) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprBlock>((*expr)._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*expr)._); auto [statements, result_expr] = _v_exprblock; return [&]() -> mlc::String { 
  mlc::String statements_code = gen_stmts_str(statements, context);
  context::CodegenContext final_context = stmts_final_ctx(statements, context);
  return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIf>((*result_expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*result_expr)._); auto [_w0, _w1, _w2] = _v_exprif; return statements_code + gen_stmt_expr(result_expr, final_context); } if (std::holds_alternative<ast::ExprUnit>((*result_expr)._)) {  return statements_code; } return [&]() -> mlc::String { 
...
mlc::String gen_return_if_stmt(std::shared_ptr<ast::Expr> expr, context::CodegenContext context) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprIf>((*expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*expr)._); auto [condition, then_expr, else_expr] = _v_exprif; return [&]() -> mlc::String { 
  mlc::String output = mlc::String("if (") + gen_expr(condition, context) + mlc::String(") {\n") + gen_return_body(then_expr, context) + mlc::String("}");
  mlc::String else_string = [&]() -> mlc::String { if (std::holds_alternative<ast::ExprUnit>((*else_expr)._)) {  return mlc::String(""); } if (std::holds_alternative<ast::ExprIf>((*else_expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*else_expr)._); auto [_w0, _w1, _w2] = _v_exprif; return mlc::String(" else ") + gen_return_if_stmt(else_expr, context); } return mlc::String(" else {\n") + gen_return_body(else_expr, context) + mlc::String("}"); }();
  if (else_string.length() > 0){
{
output = output + else_string;
}
...
mlc::String gen_return_body(std::shared_ptr<ast::Expr> expr, context::CodegenContext context) noexcept{return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprBlock>((*expr)._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*expr)._); auto [statements, result_expr] = _v_exprblock; return [&]() -> mlc::String { 
  context::CodegenContext final_context = stmts_final_ctx(statements, context);
  mlc::String statements_code = gen_stmts_str(statements, context);
  return [&]() -> mlc::String { if (std::holds_alternative<ast::ExprQuestion>((*result_expr)._)) { auto _v_exprquestion = std::get<ast::ExprQuestion>((*result_expr)._); auto [inner_expr] = _v_exprquestion; return [&]() -> mlc::String { 
 ...
 }(); } if (std::holds_alternative<ast::ExprUnit>((*result_expr)._)) {  return statements_code; } if (std::holds_alternative<ast::ExprIf>((*result_expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*result_expr)._); auto [_w0, _w1, _w2] = _v_exprif; return statements_code + gen_return_body(result_expr, final_context); } if (std::holds_alternative<ast::ExprMatch>((*result_expr)._)) { auto _v_exprmatch = std::get<ast::ExprMatch>((*result_expr)._); auto [_w0, _w1] = _v_exprmatch; return statements_code + gen_return_body(result_expr, final_context); } if (std::holds_alternative<ast::ExprBlock>((*result_expr)._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*result_expr)._); auto [_w0, _w1] = _v_exprblock; return statements_code + gen_return_body(result_expr, final_context); } return statements_code + mlc::String("return ") + gen_expr(result_expr, final_context) + mlc::String(";\n"); }();
 }(); } if (std::holds_alternative<ast::ExprIf>((*expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*expr)._); auto [condition, then_expr, else_expr] = _v_exprif; return [&]() -> mlc::String { 
  mlc::String output = mlc::String("if (") + gen_expr(condition, context) + mlc::String(") {\n") + gen_return_body(then_expr, context) + mlc::String("}");
  mlc::String else_string = [&]() -> mlc::String { if (std::holds_alternative<ast::ExprUnit>((*else_expr)._)) {  return mlc::String(""); } if (std::holds_alternative<ast::ExprIf>((*else_expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*else_expr)._); auto [_w0, _w1, _w2] = _v_exprif; return mlc::String(" else ") + gen_return_if_stmt(else_expr, context); } return mlc::String(" else {\n") + gen_return_body(else_expr, context) + mlc::String("}"); }();
  if (else_string.length() > 0){
{
output = output + else_string;
  ...
  return output + mlc::String("\n");
 }(); } if (std::holds_alternative<ast::ExprQuestion>((*expr)._)) { auto _v_exprquestion = std::get<ast::ExprQuestion>((*expr)._); auto [inner_expr] = _v_exprquestion; return [&]() -> mlc::String { 
  mlc::String try_identifier = mlc::String("__try_ret");
