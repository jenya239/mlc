...
return LoadResult{{}, mlc::Array<mlc::String>{(mlc::String("circular: ", 10) + norm_path)}};
} else {
...
return LoadResult{{}, mlc::Array<mlc::String>{(mlc::String("file not found: ", 16) + path)}};
} else {
auto lex = lexer::tokenize(source);
if (ast_tokens::LexOut_has_errors(lex)) {
return LoadResult{{}, mlc::Array<mlc::String>{(((mlc::String("lex ", 4) + path) + mlc::String(": ", 2)) + lex.errors[0])}};
} else {
...
if (ast_tokens::LexOut_has_errors(lex)) {
return ast::Err{mlc::Array<mlc::String>{(mlc::String("lex: ", 5) + lex.errors[0])}};
} else {
auto source_path = resolve_dotdot(entry_path);
auto prog = decls::parse_program_with_source_path(lex.tokens, source_path);
...
auto __try_0 = check::check_with_context(entry_prog, merged.prog);
...
auto registry = registry::build_registry(full_prog);
auto s_items = transform_decl::transform_load_items(items, registry, full_prog);
auto precomp = module::precompute(full_prog, items);
auto i = 0;
while ((i < s_items.length())) {
auto s_item = s_items[i];
auto out = module::gen_module(s_item, items, full_prog, precomp);
