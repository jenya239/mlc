...
TransformStmtsResult transform_stmts(mlc::Array<std::shared_ptr<ast::Stmt>> statements, TransformContext transform_context) noexcept{
...
auto current_context = transform_context_with_env(transform_context, current_env);
...
auto typed_value = transform_expr(value_expression, current_context, transform_stmts);
...
auto coerced_value = coerce_expr_to_type(typed_value, value_type);
...
[&](const ast::StmtLetPat& stmtLetPat) { auto [pattern, is_mut, annotation, value_expression, has_else, else_body, source_span] = stmtLetPat; return [&]() {
auto annotated_type = registry::type_from_annotation(annotation);
auto typed_value = transform_expr(value_expression, current_context, transform_stmts);
auto inferred_type = semantic_ir::sexpr_type(typed_value);
auto value_type = std::visit(overloaded{[&](const registry::TUnit& tUnit) { return inferred_type; },
...
auto typed_else_body = transform_expr(else_body, current_context, transform_stmts);
...
auto typed_expr = transform_expr(expression, current_context, transform_stmts);
...
auto typed_return = transform_expr(return_expression, current_context, transform_stmts);
...
return TransformStmtsResult{typed_statements, current_env};
}
