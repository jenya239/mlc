...
auto arity_errors = ((expected_arity >= 0) ? (((method_arguments.length() != expected_arity) ? (mlc::Array<ast::Diagnostic>{ast::diagnostic_error((((mlc::String("expected ", 9) + mlc::to_string(expected_arity)) + mlc::String(" arguments, got ", 16)) + mlc::to_string(method_arguments.length())), method_span)}) : (mlc::Array<ast::Diagnostic>{}))) : (mlc::Array<ast::Diagnostic>{}));
...
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method unwrap_or value must match Ok type (expected ", 52) + semantic_type_structure::type_description(t_t)) + mlc::String(")", 1)), ast::expr_span(method_arguments[0]))})};
}
...
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method unwrap_or_else must return the Ok type (expected ", 56) + semantic_type_structure::type_description(t_t)) + mlc::String(")", 1)), ast::expr_span(arg0))})};
}
...
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method and_then callback must return Result with the same error type (expected ", 79) + semantic_type_structure::type_description(e_t)) + mlc::String(")", 1)), ast::expr_span(arg0))});
}
} else {
...
merged = infer_result::InferResult{merged.inferred_type, ast::diagnostics_append(merged.errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method unwrap_or value must match Some type (expected ", 54) + semantic_type_structure::type_description(o_t)) + mlc::String(")", 1)), ast::expr_span(method_arguments[0]))})};
}
} else if (((method_name == mlc::String("or_else", 7)) && (method_arguments.length() > 0))) {
auto arg0 = method_arguments[0];
...
extra = ast::diagnostics_append(extra, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("method or_else must return Option with the same inner type (expected ", 69) + semantic_type_structure::type_description(o_t)) + mlc::String(")", 1)), ast::expr_span(arg0))});
}
} else {
