...
constexpr bool LexState_eof(lexer::LexState self) noexcept;
...
constexpr bool LexState_eof(lexer::LexState self) noexcept{return self.pos >= self.src.length();}

mlc::String LexState_current(lexer::LexState self) noexcept{return lexer::LexState_eof(self) ? mlc::String("\0") : self.src.char_at(self.pos);}
...
lexer::LexState LexState_advance(lexer::LexState self) noexcept{return lexer::LexState_current(self) == mlc::String("\n") ? lexer::LexState{self.src, self.pos + 1, self.line + 1, 1} : lexer::LexState{self.src, self.pos + 1, self.line, self.col + 1};}
...
while (i < count){
{
state = lexer::LexState_advance(state);
i = i + 1;
}
}
return state;
...
constexpr bool is_alnum(mlc::String character) noexcept{return lexer::is_alpha(character) || lexer::is_digit(character);}
...
{
current = lexer::LexState_advance(current);
}
}
mlc::String word = source.substring(start, current.pos - start);
return lexer::ScanResult{current, ast_tokens::Token{lexer::keyword_kind(word), token_line, token_col}};
}
...
int token_line = state.line;
int token_col = state.col;
lexer::LexState current = lexer::LexState_advance(state);
...
current = lexer::LexState_advance_by(current, 2);
[&]() { 
  while (!lexer::LexState_eof(current) && lexer::LexState_current(current) != mlc::String("\n")){
{
current = lexer::LexState_advance(current);
...
mlc::String next = lexer::LexState_peek(state, 1);
return character == mlc::String("-") && next == mlc::String(">") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, (ast_tokens::Arrow{}))} : character == mlc::String("=") && next == mlc::String(">") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, (ast_tokens::FatArrow{}))} : character == mlc::String("|") && next == mlc::String(">") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, (ast_tokens::Pipe{}))} : character == mlc::String("=") && next == mlc::String("=") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, ast_tokens::Op(mlc::String("==")))} : character == mlc::String("!") && next == mlc::String("=") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, ast_tokens::Op(mlc::String("!=")))} : character == mlc::String("<") && next == mlc::String("=") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, ast_tokens::Op(mlc::String("<=")))} : character == mlc::String(">") && next == mlc::String("=") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, ast_tokens::Op(mlc::String(">=")))} : character == mlc::String("&") && next == mlc::String("&") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, ast_tokens::Op(mlc::String("&&")))} : character == mlc::String("|") && next == mlc::String("|") ? lexer::ScanResult{lexer::LexState_advance_by(state, 2), lexer::LexState_token(state, ast_tokens::Op(mlc::String("||")))} : character == mlc::String("=") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::Equal{}))} : character == mlc::String("|") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::Bar{}))} : character == mlc::String("?") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::Question{}))} : character == mlc::String(".") && next == mlc::String(".") && lexer::LexState_peek(state, 2) == mlc::String(".") ? lexer::ScanResult{lexer::LexState_advance_by(state, 3), lexer::LexState_token(state, (ast_tokens::Spread{}))} : character == mlc::String(".") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::Dot{}))} : character == mlc::String("(") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::LParen{}))} : character == mlc::String(")") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::RParen{}))} : character == mlc::String("{") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::LBrace{}))} : character == mlc::String("}") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::RBrace{}))} : character == mlc::String("[") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::LBracket{}))} : character == mlc::String("]") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::RBracket{}))} : character == mlc::String(",") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::Comma{}))} : character == mlc::String(";") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::Semicolon{}))} : character == mlc::String(":") ? lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, (ast_tokens::Colon{}))} : lexer::ScanResult{lexer::LexState_advance(state), lexer::LexState_token(state, ast_tokens::Op(character))};
}
...
while (!lexer::LexState_eof(state)){
{
state = lexer::skip_whitespace(state);
if (lexer::LexState_eof(state)){
{
break;
}
}
...
{
lexer::ScanResult result = lexer::scan_ident(state);
...
lexer::ScanResult result = lexer::scan_int(state);
tokens.push_back(result.token);
state = result.state;
} else {
if (character == mlc::String("\"")){
lexer::ScanStrResult result = lexer::scan_string(state);
tokens.push_back(result.token);
state = result.state;
...
lexer::ScanResult result = lexer::scan_op(state);
tokens.push_back(result.token);
state = result.state;
}
}
...
tokens.push_back(lexer::LexState_token(state, (ast_tokens::Eof{})));
