...
constexpr mlc::Array<mlc::String> NameCheckResult_append_expr_errors(names::NameCheckResult self, mlc::Array<mlc::String> expr_errors) noexcept;
...
return names::collect_pattern_bindings(pattern, accumulator);
}
...
{
accumulator = names::collect_pattern_bindings(sub_patterns[index], accumulator);
index = index + 1;
}
}
  return accumulator;
...
return [&]() -> mlc::Array<mlc::String> { if (std::holds_alternative<ast::ExprIdent>((*expr)._)) { auto _v_exprident = std::get<ast::ExprIdent>((*expr)._); auto [name] = _v_exprident; return [&]() -> mlc::Array<mlc::String> { 
  ...
  if (!names::scope_contains(locals, name) && !names::scope_contains(globals, name)){
{
errors.push_back(mlc::String("undefined: ") + name);
}
}
  return errors;
 }(); } if (std::holds_alternative<ast::ExprBin>((*expr)._)) { auto _v_exprbin = std::get<ast::ExprBin>((*expr)._); auto [_w0, left, right] = _v_exprbin; return ast::errs_append(names::check_names_expr(left, locals, globals), names::check_names_expr(right, locals, globals)); } if (std::holds_alternative<ast::ExprUn>((*expr)._)) { auto _v_exprun = std::get<ast::ExprUn>((*expr)._); auto [_w0, inner] = _v_exprun; return names::check_names_expr(inner, locals, globals); } if (std::holds_alternative<ast::ExprCall>((*expr)._)) { auto _v_exprcall = std::get<ast::ExprCall>((*expr)._); auto [func, args] = _v_exprcall; return [&]() -> mlc::Array<mlc::String> { 
  mlc::Array<mlc::String> errors = names::check_names_expr(func, locals, globals);
  int i = 0;
  while (i < args.size()){
{
errors = ast::errs_append(errors, names::check_names_expr(args[i], locals, globals));
i = i + 1;
}
}
  return errors;
 }(); } if (std::holds_alternative<ast::ExprMethod>((*expr)._)) { auto _v_exprmethod = std::get<ast::ExprMethod>((*expr)._); auto [obj, _w0, args] = _v_exprmethod; return [&]() -> mlc::Array<mlc::String> { 
  mlc::Array<mlc::String> errors = names::check_names_expr(obj, locals, globals);
  int i = 0;
  while (i < args.size()){
{
errors = ast::errs_append(errors, names::check_names_expr(args[i], locals, globals));
i = i + 1;
  ...
  return errors;
 }(); } if (std::holds_alternative<ast::ExprField>((*expr)._)) { auto _v_exprfield = std::get<ast::ExprField>((*expr)._); auto [obj, _w0] = _v_exprfield; return names::check_names_expr(obj, locals, globals); } if (std::holds_alternative<ast::ExprIndex>((*expr)._)) { auto _v_exprindex = std::get<ast::ExprIndex>((*expr)._); auto [obj, idx] = _v_exprindex; return ast::errs_append(names::check_names_expr(obj, locals, globals), names::check_names_expr(idx, locals, globals)); } if (std::holds_alternative<ast::ExprIf>((*expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*expr)._); auto [cond, then_expr, else_expr] = _v_exprif; return ast::errs_append(ast::errs_append(names::check_names_expr(cond, locals, globals), names::check_names_expr(then_expr, locals, globals)), names::check_names_expr(else_expr, locals, globals)); } if (std::holds_alternative<ast::ExprBlock>((*expr)._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*expr)._); auto [stmts, result] = _v_exprblock; return [&]() -> mlc::Array<mlc::String> { 
  names::NameCheckResult stmt_result = names::check_names_stmts(stmts, locals, globals);
  return names::NameCheckResult_append_expr_errors(stmt_result, names::check_names_expr(result, stmt_result.scope, globals));
  ...
  names::NameCheckResult check_result = names::check_names_stmts(stmts, locals, globals);
  return ast::errs_append(names::check_names_expr(cond, locals, globals), check_result.errors);
  ...
  inner.push_back(var);
  names::NameCheckResult check_result = names::check_names_stmts(stmts, inner, globals);
  return ast::errs_append(names::check_names_expr(iter, locals, globals), check_result.errors);
  ...
  mlc::Array<mlc::String> errors = names::check_names_expr(subject, locals, globals);
  int i = 0;
  while (i < arms.size()){
{
mlc::Array<mlc::String> bindings = names::pattern_bindings(arms[i]->pat);
...
errors = ast::errs_append(errors, names::check_names_expr(arms[i]->body, arm_scope, globals));
i = i + 1;
}
}
  return errors;
...
{
errors = ast::errs_append(errors, names::check_names_expr(field_vals[i]->val, locals, globals));
i = i + 1;
}
}
  return errors;
  ...
  mlc::Array<mlc::String> errors = names::check_names_expr(base, locals, globals);
  int i = 0;
  while (i < field_vals.size()){
{
errors = ast::errs_append(errors, names::check_names_expr(field_vals[i]->val, locals, globals));
i = i + 1;
  ...
  while (i < exprs.size()){
{
errors = ast::errs_append(errors, names::check_names_expr(exprs[i], locals, globals));
i = i + 1;
}
}
  return errors;
 }(); } if (std::holds_alternative<ast::ExprQuestion>((*expr)._)) { auto _v_exprquestion = std::get<ast::ExprQuestion>((*expr)._); auto [inner] = _v_exprquestion; return names::check_names_expr(inner, locals, globals); } if (std::holds_alternative<ast::ExprLambda>((*expr)._)) { auto _v_exprlambda = std::get<ast::ExprLambda>((*expr)._); auto [params, body] = _v_exprlambda; return [&]() -> mlc::Array<mlc::String> { 
...
j = j + 1;
  ...
  return names::check_names_expr(body, lambda_locals, globals);
 }(); } return {}; }();
}

constexpr mlc::Array<mlc::String> NameCheckResult_append_expr_errors(names::NameCheckResult self, mlc::Array<mlc::String> expr_errors) noexcept{return ast::errs_append(self.errors, expr_errors);}
...
std::visit(overloaded{
  ...
  errors = ast::errs_append(errors, names::check_names_expr(value, scope, globals));
  scope.push_back(name);
  return std::make_tuple();
 }(); },
  [&](const StmtExpr& stmtexpr) { auto [expr] = stmtexpr; return [&]() -> std::tuple<> { 
  errors = ast::errs_append(errors, names::check_names_expr(expr, scope, globals));
  return std::make_tuple();
 }(); },
