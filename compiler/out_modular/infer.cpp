...
infer::InferResult infer_args_errors(infer::InferResult initial, mlc::Array<std::shared_ptr<ast::Expr>> args, mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> type_env, registry::TypeRegistry registry) noexcept;
...
constexpr bool CheckOut_has_errors(infer::CheckOut self) noexcept;
...
{
result = infer::InferResult_absorb(result, infer::infer_expr(args[index], type_env, registry));
index = index + 1;
}
}
return result;
...
{
result = infer::InferResult_absorb(result, infer::infer_expr(field_vals[index]->val, type_env, registry));
index = index + 1;
}
}
return result;
...
return std::visit(overloaded{
  [&](const ExprInt& exprint) { auto [_w0] = exprint; return infer::infer_ok(std::make_shared<registry::Type>((registry::TI32{}))); },
  [&](const ExprStr& exprstr) { auto [_w0] = exprstr; return infer::infer_ok(std::make_shared<registry::Type>((registry::TString{}))); },
  [&](const ExprBool& exprbool) { auto [_w0] = exprbool; return infer::infer_ok(std::make_shared<registry::Type>((registry::TBool{}))); },
  [&](const ExprUnit& exprunit) { return infer::infer_ok(std::make_shared<registry::Type>((registry::TUnit{}))); },
  [&](const ExprIdent& exprident) { auto [name] = exprident; return type_env.has(name) ? infer::infer_ok(type_env.get(name)) : registry::TypeRegistry_has_fn(registry, name) ? infer::infer_ok(registry::TypeRegistry_fn_type(registry, name)) : registry::TypeRegistry_has_ctor(registry, name) ? infer::infer_ok(registry::TypeRegistry_ctor_type(registry, name)) : infer::infer_ok(std::make_shared<registry::Type>((registry::TUnknown{}))); },
  [&](const ExprBin& exprbin) { auto [op, left, right] = exprbin; return [&]() -> infer::InferResult { 
  infer::InferResult left_result = infer::infer_expr(left, type_env, registry);
  infer::InferResult right_result = infer::infer_expr(right, type_env, registry);
  return infer::InferResult_with_type(infer::InferResult_absorb(left_result, right_result), infer::binary_op_type(op, left_result.inferred_type));
 }(); },
  [&](const ExprUn& exprun) { auto [op, inner] = exprun; return [&]() -> infer::InferResult { 
  infer::InferResult inner_result = infer::infer_expr(inner, type_env, registry);
  auto result_type = op == mlc::String("!") ? std::make_shared<registry::Type>((registry::TBool{})) : inner_result.inferred_type;
  return infer::InferResult_with_type(inner_result, result_type);
 }(); },
  [&](const ExprCall& exprcall) { auto [func, args] = exprcall; return [&]() -> infer::InferResult { 
  infer::InferResult func_result = infer::infer_expr(func, type_env, registry);
  infer::InferResult with_args = infer::infer_args_errors(func_result, args, type_env, registry);
  ...
  return infer::InferResult_with_type(with_args, return_type);
 }(); },
  [&](const ExprMethod& exprmethod) { auto [obj, method_name, args] = exprmethod; return [&]() -> infer::InferResult { 
  infer::InferResult obj_result = infer::infer_expr(obj, type_env, registry);
  return infer::InferResult_with_type(infer::infer_args_errors(obj_result, args, type_env, registry), infer::method_return_type(method_name));
 }(); },
  [&](const ExprField& exprfield) { auto [obj, field_name] = exprfield; return [&]() -> infer::InferResult { 
  infer::InferResult obj_result = infer::infer_expr(obj, type_env, registry);
  return infer::InferResult_with_type(obj_result, infer::field_lookup_type(obj_result.inferred_type, field_name, registry));
 }(); },
  [&](const ExprIndex& exprindex) { auto [obj, idx] = exprindex; return [&]() -> infer::InferResult { 
  infer::InferResult obj_result = infer::infer_expr(obj, type_env, registry);
  infer::InferResult idx_result = infer::infer_expr(idx, type_env, registry);
  ...
  return infer::InferResult_with_type(infer::InferResult_absorb(obj_result, idx_result), elem_type);
 }(); },
  [&](const ExprIf& exprif) { auto [cond, then_expr, else_expr] = exprif; return [&]() -> infer::InferResult { 
  infer::InferResult cond_result = infer::infer_expr(cond, type_env, registry);
  infer::InferResult then_result = infer::infer_expr(then_expr, type_env, registry);
  infer::InferResult else_result = infer::infer_expr(else_expr, type_env, registry);
  return infer::InferResult_absorb(infer::InferResult_absorb(then_result, cond_result), else_result);
 }(); },
  [&](const ExprBlock& exprblock) { auto [stmts, result] = exprblock; return [&]() -> infer::InferResult { 
  infer::StmtInferResult stmts_result = infer::infer_stmts(stmts, type_env, registry);
  infer::InferResult result_infer = infer::infer_expr(result, stmts_result.type_env, registry);
  return infer::InferResult_absorb_stmt(result_infer, stmts_result);
 }(); },
  [&](const ExprWhile& exprwhile) { auto [cond, stmts] = exprwhile; return [&]() -> infer::InferResult { 
  infer::InferResult cond_result = infer::infer_expr(cond, type_env, registry);
  infer::StmtInferResult stmts_result = infer::infer_stmts(stmts, type_env, registry);
  return infer::InferResult_with_type(infer::InferResult_absorb_stmt(cond_result, stmts_result), std::make_shared<registry::Type>((registry::TUnit{})));
 }(); },
  [&](const ExprFor& exprfor) { auto [var, iter, stmts] = exprfor; return [&]() -> infer::InferResult { 
  infer::InferResult iter_result = infer::infer_expr(iter, type_env, registry);
  ...
  infer::StmtInferResult stmts_result = infer::infer_stmts(stmts, inner_env, registry);
  return infer::InferResult_with_type(infer::InferResult_absorb_stmt(iter_result, stmts_result), std::make_shared<registry::Type>((registry::TUnit{})));
 }(); },
  [&](const ExprMatch& exprmatch) { auto [subject, arms] = exprmatch; return [&]() -> infer::InferResult { 
  infer::InferResult subject_result = infer::infer_expr(subject, type_env, registry);
  ...
  while (i < arms.size()){
{
mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> arm_env = infer::env_for_pat(type_env, arms[i]->pat, registry);
infer::InferResult arm_result = infer::infer_expr(arms[i]->body, arm_env, registry);
...
i = i + 1;
  ...
  [&](const ExprRecord& exprrecord) { auto [name, field_vals] = exprrecord; return infer::infer_field_vals_errors(infer::infer_ok(std::make_shared<registry::Type>(registry::TNamed(name))), field_vals, type_env, registry); },
  [&](const ExprRecordUpdate& exprrecordupdate) { auto [name, base, field_vals] = exprrecordupdate; return [&]() -> infer::InferResult { 
  infer::InferResult base_result = infer::infer_expr(base, type_env, registry);
  return infer::infer_field_vals_errors(infer::InferResult_with_type(base_result, std::make_shared<registry::Type>(registry::TNamed(name))), field_vals, type_env, registry);
 }(); },
  [&](const ExprArray& exprarray) { auto [exprs] = exprarray; return [&]() -> infer::InferResult { 
  std::shared_ptr<registry::Type> first_type = exprs.size() > 0 ? infer::infer_expr(exprs[0], type_env, registry).inferred_type : std::make_shared<registry::Type>((registry::TUnknown{}));
  return infer::infer_args_errors(infer::infer_ok(std::make_shared<registry::Type>(registry::TArray(first_type))), exprs, type_env, registry);
 }(); },
  [&](const ExprQuestion& exprquestion) { auto [inner] = exprquestion; return infer::infer_expr(inner, type_env, registry); },
  [&](const ExprLambda& exprlambda) { auto [params, body] = exprlambda; return [&]() -> infer::InferResult { 
...
i = i + 1;
  ...
  infer::InferResult ret_result = infer::infer_expr(body, lambda_env, registry);
  return infer::infer_ok(std::make_shared<registry::Type>(registry::TFn({}, ret_result.inferred_type)));
 }(); }
}, (*expr)._);
}
...
{
infer::InferResult value_result = infer::infer_expr(value, current_env, registry);
...
auto [expr] = stmtexpr;
{
infer::InferResult expr_res = infer::infer_expr(expr, current_env, registry);
...
{
infer::InferResult ret_res = infer::infer_expr(expr, current_env, registry);
...
current_env = infer::env_for_pat_with_type(current_env, sub_pats[i], sub_type, registry);
i = i + 1;
}
}
  return current_env;
...
return [&]() -> mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> { if (std::holds_alternative<ast::PatIdent>((*pat))) { auto _v_patident = std::get<ast::PatIdent>((*pat)); auto [name] = _v_patident; return [&]() -> mlc::HashMap<mlc::String, std::shared_ptr<registry::Type>> { 
  ...
  return env;
 }(); } return infer::env_for_pat(type_env, pat, registry); }();
}

constexpr bool CheckOut_has_errors(infer::CheckOut self) noexcept{return self.errors.size() > 0;}

infer::CheckOut check(ast::Program prog) noexcept{
mlc::Array<mlc::String> globals = names::collect_globals(prog);
...
type_env.set(params[j]->name, registry::type_from_annotation(params[j]->typ));
j = j + 1;
}
}
all_errors = ast::errs_append(all_errors, names::check_names_expr(body, locals, globals));
infer::InferResult infer_result = infer::infer_expr(body, type_env, registry);
