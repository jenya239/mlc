...
{
preds::StmtResult result = exprs::parse_stmt(state);
...
return preds::TKind_is_let(kind) ? [&]() -> preds::StmtResult { 
  ...
  bool is_const = preds::TKind_is_const(preds::Parser_kind(after_let));
  preds::Parser name_pos = is_const ? preds::Parser_advance(after_let) : after_let;
  ...
  preds::ExprResult value_result = exprs::parse_expr(preds::Parser_advance(type_result.parser));
  return preds::StmtResult{std::make_shared<ast::Stmt>(ast::StmtLet(var_name, !is_const, type_result.type_expr, value_result.expr)), value_result.parser};
  ...
  preds::ExprResult value_result = exprs::parse_expr(preds::Parser_advance(parser));
  return preds::StmtResult{std::make_shared<ast::Stmt>(ast::StmtReturn(value_result.expr)), value_result.parser};
 }() : [&]() -> preds::StmtResult { 
  preds::ExprResult expr_result = exprs::parse_expr(parser);
  return preds::TKind_is_equal(preds::Parser_kind(expr_result.parser)) ? [&]() -> preds::StmtResult { 
  preds::ExprResult rhs = exprs::parse_expr(preds::Parser_advance(expr_result.parser));
...
return preds::TKind_is_ident(kind) ? [&]() -> preds::PatResult { 
  ...
  preds::PatsResult sub_pats = exprs::parse_pat_args(preds::Parser_advance(after));
...
return preds::TKind_is_rparen(preds::Parser_kind(parser)) ? preds::PatsResult{pats, preds::Parser_advance(parser)} : [&]() -> preds::PatsResult { 
  preds::PatResult first = exprs::parse_pat(parser);
  pats.push_back(first.pat);
  preds::Parser state = first.parser;
  while (preds::TKind_is_comma(preds::Parser_kind(state))){
{
preds::PatResult next = exprs::parse_pat(preds::Parser_advance(state));
...
preds::ExprResult parse_expr(preds::Parser parser) noexcept{return exprs::parse_pipe(parser);}
...
preds::ExprResult parse_pipe(preds::Parser parser) noexcept{
preds::ExprResult left = exprs::parse_or(parser);
std::shared_ptr<ast::Expr> expr = left.expr;
preds::Parser state = left.parser;
while (preds::TKind_is_pipe(preds::Parser_kind(state))){
{
preds::ExprResult right = exprs::parse_or(preds::Parser_advance(state));
expr = exprs::pipe_desugar(expr, right.expr);
state = right.parser;
}
}
...
preds::ExprResult parse_or(preds::Parser parser) noexcept{
preds::ExprResult left = exprs::parse_and(parser);
std::shared_ptr<ast::Expr> expr = left.expr;
preds::Parser state = left.parser;
while (preds::TKind_is_op(preds::Parser_kind(state)) && preds::TKind_op_val(preds::Parser_kind(state)) == mlc::String("||")){
{
preds::ExprResult right = exprs::parse_and(preds::Parser_advance(state));
...
preds::ExprResult parse_and(preds::Parser parser) noexcept{
preds::ExprResult left = exprs::parse_cmp(parser);
std::shared_ptr<ast::Expr> expr = left.expr;
preds::Parser state = left.parser;
while (preds::TKind_is_op(preds::Parser_kind(state)) && preds::TKind_op_val(preds::Parser_kind(state)) == mlc::String("&&")){
{
preds::ExprResult right = exprs::parse_cmp(preds::Parser_advance(state));
expr = std::make_shared<ast::Expr>(ast::ExprBin(mlc::String("&&"), expr, right.expr));
state = right.parser;
}
}
...
preds::ExprResult parse_cmp(preds::Parser parser) noexcept{
preds::ExprResult left = exprs::parse_add(parser);
std::shared_ptr<ast::Expr> expr = left.expr;
preds::Parser state = left.parser;
ast_tokens::TKind kind = preds::Parser_kind(state);
if (preds::TKind_is_op(kind) && exprs::is_cmp_op(preds::TKind_op_val(kind))){
{
preds::ExprResult right = exprs::parse_add(preds::Parser_advance(state));
...
preds::ExprResult parse_add(preds::Parser parser) noexcept{
preds::ExprResult left = exprs::parse_mul(parser);
std::shared_ptr<ast::Expr> expr = left.expr;
preds::Parser state = left.parser;
bool go = true;
while (go){
...
{
preds::ExprResult right = exprs::parse_mul(preds::Parser_advance(state));
expr = std::make_shared<ast::Expr>(ast::ExprBin(preds::TKind_op_val(kind), expr, right.expr));
state = right.parser;
}
} else {
...
preds::ExprResult parse_mul(preds::Parser parser) noexcept{
preds::ExprResult left = exprs::parse_unary(parser);
std::shared_ptr<ast::Expr> expr = left.expr;
preds::Parser state = left.parser;
bool go = true;
while (go){
...
{
preds::ExprResult right = exprs::parse_unary(preds::Parser_advance(state));
expr = std::make_shared<ast::Expr>(ast::ExprBin(preds::TKind_op_val(kind), expr, right.expr));
state = right.parser;
}
} else {
...
return preds::TKind_is_op(kind) && preds::TKind_op_val(kind) == mlc::String("!") || preds::TKind_op_val(kind) == mlc::String("-") ? [&]() -> preds::ExprResult { 
  preds::ExprResult inner = exprs::parse_unary(preds::Parser_advance(parser));
  return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprUn(preds::TKind_op_val(kind), inner.expr)), inner.parser};
 }() : exprs::parse_postfix(parser);
}

preds::ExprResult parse_postfix(preds::Parser parser) noexcept{
preds::ExprResult base = exprs::parse_primary(parser);
...
preds::ExprsResult args = exprs::parse_arg_list(preds::Parser_advance_by(state, 3));
expr = std::make_shared<ast::Expr>(ast::ExprMethod(expr, field_name, args.exprs));
state = args.parser;
} else {
...
preds::ExprsResult args = exprs::parse_arg_list(preds::Parser_advance(state));
expr = std::make_shared<ast::Expr>(ast::ExprCall(expr, args.exprs));
state = args.parser;
} else {
if (preds::TKind_is_lbracket(kind)){
preds::ExprResult index = exprs::parse_expr(preds::Parser_advance(state));
...
return preds::TKind_is_rparen(preds::Parser_kind(parser)) ? preds::ExprsResult{exprs, preds::Parser_advance(parser)} : [&]() -> preds::ExprsResult { 
  preds::ExprResult first = exprs::parse_expr(parser);
  exprs.push_back(first.expr);
  preds::Parser state = first.parser;
  while (preds::TKind_is_comma(preds::Parser_kind(state))){
{
preds::ExprResult next = exprs::parse_expr(preds::Parser_advance(state));
...
return preds::TKind_is_int(kind) ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprInt(preds::TKind_int_val(kind))), preds::Parser_advance(parser)} : preds::TKind_is_str(kind) ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprStr(preds::TKind_str_val(kind))), preds::Parser_advance(parser)} : preds::TKind_is_true(kind) ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprBool(true)), preds::Parser_advance(parser)} : preds::TKind_is_false(kind) ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprBool(false)), preds::Parser_advance(parser)} : preds::TKind_is_lparen(kind) ? preds::TKind_is_rparen(preds::Parser_kind(preds::Parser_advance(parser))) ? preds::TKind_is_fat_arrow(preds::Parser_kind(preds::Parser_advance_by(parser, 2))) ? [&]() -> preds::ExprResult { 
  ...
  preds::ExprResult body = exprs::parse_expr(preds::Parser_advance_by(parser, 3));
  return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprLambda(params, body.expr)), body.parser};
 }() : preds::ExprResult{std::make_shared<ast::Expr>((ast::ExprUnit{})), preds::Parser_advance_by(parser, 2)} : exprs::looks_like_lambda_params(preds::Parser_advance(parser)) ? [&]() -> preds::ExprResult { 
  preds::NamesResult param_result = exprs::parse_lambda_params(preds::Parser_advance(parser));
  preds::ExprResult body = exprs::parse_expr(preds::Parser_advance(param_result.parser));
  return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprLambda(param_result.exprs, body.expr)), body.parser};
 }() : [&]() -> preds::ExprResult { 
  preds::ExprResult inner = exprs::parse_expr(preds::Parser_advance(parser));
  return preds::ExprResult{inner.expr, preds::Parser_advance(inner.parser)};
 }() : preds::TKind_is_lbracket(kind) ? exprs::parse_array_lit(preds::Parser_advance(parser)) : preds::TKind_is_if(kind) || preds::TKind_is_unless(kind) ? exprs::parse_if_expr(parser) : preds::TKind_is_do(kind) ? exprs::parse_block(preds::Parser_advance(parser)) : preds::TKind_is_while(kind) ? exprs::parse_while_expr(preds::Parser_advance(parser)) : preds::TKind_is_for(kind) ? exprs::parse_for_expr(preds::Parser_advance(parser)) : preds::TKind_is_match(kind) ? exprs::parse_match_expr(preds::Parser_advance(parser)) : preds::TKind_is_return(kind) ? [&]() -> preds::ExprResult { 
  preds::ExprResult value = exprs::parse_expr(preds::Parser_advance(parser));
  mlc::Array<std::shared_ptr<ast::Stmt>> stmts = {};
  ...
  preds::ExprResult body = exprs::parse_expr(preds::Parser_advance_by(parser, 2));
  return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprLambda(params, body.expr)), body.parser};
 }() : preds::is_ctor_name(name) && preds::TKind_is_lbrace(preds::Parser_kind(preds::Parser_advance(parser))) ? exprs::parse_record_expr(preds::Parser_advance_by(parser, 2), name) : preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprIdent(name)), preds::Parser_advance(parser)};
...
preds::ExprResult cond_result = exprs::parse_expr(preds::Parser_advance(parser));
...
preds::StmtsResult then_result = exprs::parse_stmts_until_else_end(preds::Parser_advance(cond_result.parser));
std::shared_ptr<ast::Expr> then_expr = exprs::StmtsResult_to_block_expr(then_result);
return preds::TKind_is_else(preds::Parser_kind(then_result.parser)) ? [&]() -> preds::ExprResult { 
  ...
  preds::ExprResult else_result = exprs::parse_if_expr(after_else);
  return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprIf(condition, then_expr, else_result.expr)), else_result.parser};
 }() : [&]() -> preds::ExprResult { 
  preds::StmtsResult else_stmts = exprs::parse_stmts_until_end(after_else);
  return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprIf(condition, then_expr, exprs::StmtsResult_to_block_expr(else_stmts))), else_stmts.parser};
 }();
...
preds::ExprResult parse_block(preds::Parser parser) noexcept{
preds::StmtsResult result = exprs::parse_stmts_until_end(parser);
return preds::ExprResult{exprs::StmtsResult_to_block_expr(result), result.parser};
}

bool is_unit_expr(std::shared_ptr<ast::Expr> expr) noexcept{return [&]() -> bool { if (std::holds_alternative<ast::ExprUnit>((*expr)._)) {  return true; } if (std::holds_alternative<ast::ExprBlock>((*expr)._)) { auto _v_exprblock = std::get<ast::ExprBlock>((*expr)._); auto [_w0, result] = _v_exprblock; return exprs::is_unit_expr(result); } if (std::holds_alternative<ast::ExprIf>((*expr)._)) { auto _v_exprif = std::get<ast::ExprIf>((*expr)._); auto [_w0, _w1, else_expr] = _v_exprif; return exprs::is_unit_expr(else_expr); } return false; }();}

std::shared_ptr<ast::Expr> block_result(mlc::Array<std::shared_ptr<ast::Stmt>> stmts) noexcept{return stmts.size() == 0 ? std::make_shared<ast::Expr>((ast::ExprUnit{})) : [&]() { if (std::holds_alternative<ast::StmtExpr>((*stmts[stmts.size() - 1])._)) { auto _v_stmtexpr = std::get<ast::StmtExpr>((*stmts[stmts.size() - 1])._); auto [expr] = _v_stmtexpr; return exprs::is_unit_expr(expr) ? std::make_shared<ast::Expr>((ast::ExprUnit{})) : expr; } return std::make_shared<ast::Expr>((ast::ExprUnit{})); }();}

mlc::Array<std::shared_ptr<ast::Stmt>> block_body(mlc::Array<std::shared_ptr<ast::Stmt>> stmts) noexcept{
return stmts.size() == 0 ? stmts : [&]() -> mlc::Array<std::shared_ptr<ast::Stmt>> { if (std::holds_alternative<ast::StmtExpr>((*stmts[stmts.size() - 1])._)) { auto _v_stmtexpr = std::get<ast::StmtExpr>((*stmts[stmts.size() - 1])._); auto [expr] = _v_stmtexpr; return exprs::is_unit_expr(expr) ? stmts : [&]() -> mlc::Array<std::shared_ptr<ast::Stmt>> { 
...
std::shared_ptr<ast::Expr> StmtsResult_to_block_expr(preds::StmtsResult self) noexcept{return std::make_shared<ast::Expr>(ast::ExprBlock(exprs::block_body(self.stmts), exprs::block_result(self.stmts)));}

preds::ExprResult parse_while_expr(preds::Parser parser) noexcept{
preds::ExprResult cond = exprs::parse_expr(parser);
preds::StmtsResult body = exprs::parse_stmts_until_end(preds::Parser_advance(cond.parser));
...
preds::ExprResult iter = exprs::parse_expr(preds::Parser_advance_by(parser, 2));
preds::StmtsResult body = exprs::parse_stmts_until_end(preds::Parser_advance(iter.parser));
return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprFor(var_name, iter.expr, body.stmts)), body.parser};
}

preds::ExprResult parse_match_expr(preds::Parser parser) noexcept{
preds::ExprResult subject = exprs::parse_expr(parser);
preds::ArmsResult arms = exprs::parse_arms(preds::Parser_advance(subject.parser));
...
{
preds::PatResult pat_result = exprs::parse_pat(state);
preds::ExprResult body_result = exprs::parse_expr(preds::Parser_advance(pat_result.parser));
...
{
preds::ExprResult value_result = exprs::parse_expr(preds::Parser_advance(after_name));
...
preds::ExprResult parse_record_expr(preds::Parser parser, mlc::String record_name) noexcept{return preds::TKind_is_spread(preds::Parser_kind(parser)) ? [&]() -> preds::ExprResult { 
  preds::ExprResult base_result = exprs::parse_expr(preds::Parser_advance(parser));
...
after_base = preds::Parser_advance(after_base);
  ...
  preds::FieldValsResult fields_result = exprs::parse_record_fields(after_base);
  ...
  preds::FieldValsResult fields_result = exprs::parse_record_fields(parser);
