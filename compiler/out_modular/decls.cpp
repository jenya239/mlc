...
{
preds::DeclResult method_result = decls::parse_extend_method(methods_state, type_name);
...
preds::ParamsResult rest_params = preds::TKind_is_rparen(preds::Parser_kind(params_state)) ? preds::ParamsResult{{}, preds::Parser_advance(params_state)} : decls::parse_params(params_state);
int index = 0;
...
return preds::TKind_is_ident(kind) && preds::TKind_ident(kind) == mlc::String("export") ? [&]() -> preds::DeclResult { 
  preds::DeclResult inner = decls::parse_decl(preds::Parser_advance(parser));
  return preds::DeclResult{std::make_shared<ast::Decl>(ast::DeclExported(inner.decl)), inner.parser};
 }() : preds::TKind_is_fn(kind) || preds::TKind_is_extern(kind) && preds::TKind_is_fn(preds::Parser_kind(preds::Parser_advance(parser))) ? decls::parse_fn_decl(parser) : preds::TKind_is_type(kind) ? decls::parse_type_decl(preds::Parser_advance(parser)) : preds::TKind_is_extend(kind) ? decls::parse_extend_decl(preds::Parser_advance(parser)) : preds::TKind_is_import(kind) ? decls::parse_import_decl(preds::Parser_advance(parser)) : preds::DeclResult{std::make_shared<ast::Decl>(ast::DeclFn(mlc::String("__skip__"), {}, std::make_shared<ast::TypeExpr>((ast::TyUnit{})), std::make_shared<ast::Expr>((ast::ExprUnit{})))), preds::Parser_advance(parser)};
}

preds::DeclResult parse_fn_decl(preds::Parser parser) noexcept{
preds::Parser fn_start = preds::TKind_is_extern(preds::Parser_kind(parser)) ? preds::Parser_advance(parser) : parser;
mlc::String fn_name = preds::TKind_ident(preds::Parser_kind(preds::Parser_advance(fn_start)));
preds::ParamsResult params_result = decls::parse_params(preds::Parser_advance_by(fn_start, 3));
...
return preds::TKind_is_rparen(preds::Parser_kind(parser)) ? preds::ParamsResult{params, preds::Parser_advance(parser)} : [&]() -> preds::ParamsResult { 
  preds::ParamResult first = decls::parse_param(parser);
  params.push_back(first.param);
  preds::Parser state = first.parser;
  while (preds::TKind_is_comma(preds::Parser_kind(state))){
{
preds::ParamResult next = decls::parse_param(preds::Parser_advance(state));
...
return preds::TKind_is_lbrace(first_kind) ? [&]() -> preds::DeclResult { 
  preds::FieldDefsResult field_defs_result = decls::parse_field_defs(preds::Parser_advance(after_eq));
  ...
  preds::FieldDefsResult field_defs_result = decls::parse_field_defs(preds::Parser_advance_by(after_eq, 2));
  mlc::Array<std::shared_ptr<ast::TypeVariant>> variants = {};
  ...
  preds::VariantsResult variants_result = decls::parse_variants(after_eq);
...
bool parse_variants_continue(preds::Parser state) noexcept{return preds::Parser_at_eof(state) ? false : decls::is_decl_start(preds::Parser_kind(state)) ? false : preds::TKind_is_bar(preds::Parser_kind(state)) || preds::TKind_is_ident(preds::Parser_kind(state));}

preds::VariantsResult parse_variants(preds::Parser parser) noexcept{
mlc::Array<std::shared_ptr<ast::TypeVariant>> variants = {};
preds::Parser state = preds::TKind_is_bar(preds::Parser_kind(parser)) ? preds::Parser_advance(parser) : parser;
while (decls::parse_variants_continue(state)){
{
preds::VariantResult variant_result = decls::parse_variant(state);
...
return preds::TKind_is_lparen(preds::Parser_kind(after_name)) ? [&]() -> preds::VariantResult { 
  preds::TypesResult types_result = decls::parse_tuple_types(preds::Parser_advance(after_name));
  ...
  preds::FieldDefsResult field_defs_result = decls::parse_field_defs(preds::Parser_advance(after_name));
...
{
preds::DeclResult result = decls::parse_decl(state);
