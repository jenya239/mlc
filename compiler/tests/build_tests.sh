...
echo "[compiler tests] phase 1/4: compile tests_main.mlc → ${OUT_DIR}/run_tests" >&2
...
echo "[compiler tests] phase 2/4: execute run_tests" >&2
"$OUT_DIR/run_tests"

echo "[compiler tests] phase 3/4: fuzz smoke (mlcc --check-only)" >&2
bash "$COMPILER_DIR/tests/fuzz/run_fuzz_smoke.sh" "$COMPILER_DIR/out/mlcc"

echo "[compiler tests] phase 4/4: negative corpus (mlcc --check-only)" >&2
