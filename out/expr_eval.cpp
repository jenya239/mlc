...
auto materialization_outcome = mut_actual_argument::materialize_positional_actual_arguments_maybe_holding_mut_references(arguments, call_parameter_mutability_flags, context, gen_stmts, evaluate_expression);
auto argument_list = materialization_outcome.materialized_argument_fragments.join(mlc::String(", ", 2));
auto invoke_expression = (expression_support::is_constructor_call(function_expr) ? (expr::constructor_call_braces(function_code, argument_list)) : (expr::function_call_parentheses(function_code, argument_list)));
