...
return std::visit(overloaded{[&](const ast_tokens::KWhere& kWhere) { return true; },
[&](const auto& __v) { return false; }
}, self);
}
...
return Parser{self.tokens, self.pos, self.source_path, value};
}
Parser Parser_advance(Parser self) noexcept{
return Parser{self.tokens, (self.pos + 1), self.source_path, self.suppress_lambda_shorthand};
}
Parser Parser_advance_by(Parser self, int count) noexcept{
return Parser{self.tokens, (self.pos + count), self.source_path, self.suppress_lambda_shorthand};
}
