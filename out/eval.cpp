...
return expr_eval::eval_expr(expr, context, [=](auto statements, auto ctx) mutable { return gen_stmts_str(statements, ctx); });
}
mlc::String gen_stmts_str(mlc::Array<std::shared_ptr<semantic_ir::SStmt>> statements, context::CodegenContext context) noexcept{
return stmt_eval::eval_stmts_str(statements, context, [=](auto expression, auto ctx) mutable { return gen_expr(expression, ctx); });
}
...
return expr_eval::eval_argument_list(expressions, context, [=](auto statements, auto ctx) mutable { return gen_stmts_str(statements, ctx); });
}
...
return stmt_eval::eval_try_unwrap(inner_expr, context, try_identifier, success_line, [=](auto expression, auto ctx) mutable { return gen_expr(expression, ctx); });
}
...
return stmt_eval::eval_stmt_with_try(stmt, context, try_counter, [=](auto expression, auto ctx) mutable { return gen_expr(expression, ctx); });
}
...
return stmt_eval::eval_stmts_str_with_try(statements, context, try_counter, [=](auto expression, auto ctx) mutable { return gen_expr(expression, ctx); });
}
