...
return std::visit(overloaded{[&](const registry::TFn& tFn) { auto [expected_formal_parameter_types, __1] = tFn; return [&]() {
...
auto expected_placeholder_type = ((argument_index_under_call < expected_formal_parameter_types.length()) ? (expected_formal_parameter_types[argument_index_under_call]) : (standalone_unknown_cell()));
...
[&](const auto& __v) { return transform_exprs(argument_expressions_under_call, transform_context, stmts_fn); }
}, (*semantic_ir::sexpr_type(callee_semantic_expression)));
}
std::shared_ptr<semantic_ir::SFieldVal> semantic_field_val_from_source_field_after_transform(std::shared_ptr<ast::FieldVal> field_value_under_transform, TransformContext transform_context, std::function<TransformStmtsResult(mlc::Array<std::shared_ptr<ast::Stmt>>, TransformContext)> stmts_fn) noexcept{
return std::make_shared<semantic_ir::SFieldVal>(semantic_ir::SFieldVal{field_value_under_transform->name, transform_expr(field_value_under_transform->val, transform_context, stmts_fn)});
}
...
std::visit(overloaded{[&](const registry::TGeneric& tGeneric) { auto [type_name, type_arguments] = tGeneric; return [&]() {
auto algebraic_parameter_names = registry::TypeRegistry_algebraic_decl_type_parameter_names_for(registry, type_name);
