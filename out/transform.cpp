...
auto i = 0;
while ((i < count)) {
mlc::io::args.push_back(std::make_shared<registry::Type>(registry::TUnknown{}));
i = (i + 1);
}
return mlc::io::args;
}
...
[&](const auto& __v) { return stripped.push_back(mlc::io::args[i]); }
}, (*mlc::io::args[i]));
i = (i + 1);
}
return stripped;
...
if ((pos_slot < n)) {
result.set(pos_slot, mlc::io::args[i]);
...
if ((!transform_has_named_args(mlc::io::args))) {
return mlc::io::args;
} else {
auto param_names = registry::TypeRegistry_parameter_names_for(registry, callee_name);
if ((param_names.length() == 0)) {
return transform_strip_labels(mlc::io::args);
} else {
return transform_reorder_to_positional(mlc::io::args, param_names);
}
}
}
...
/* unit */;
transform_expr_maybe_lambda_context((Shared{} < ast::Expr{}), /* unit */, mlc::Array{(Shared{} < registry::Type{})}, /* unit */, TransformContext{}, mlc::Array{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
...
/* unit */;
transform_array_hof_method_arguments((Shared{} < semantic_ir::SExpr{}), /* unit */, string, mlc::Array{(Shared{} < ast::Expr{})}, /* unit */, TransformContext{}, mlc::Array{(Shared{} < ast::Stmt{})}, TransformContext{});
/* unit */;
TransformStmtsResult{};
/* unit */;
