...
auto state = parser;
while (((!preds::Parser_kind(state).is_end()) && (!state.at_eof()))) {
auto result = parse_stmt(state);
stmts.push_back(result.stmt);
state = result.parser.skip_semi();
}
return preds::StmtsResult{stmts, state.advance()};
}
preds::StmtsResult parse_stmts_until_else_end(preds::Parser parser) noexcept{
auto stmts = mlc::Array<std::shared_ptr<ast::Stmt>>{};
auto state = parser;
while ((((!preds::Parser_kind(state).is_end()) && (!preds::Parser_kind(state).is_else())) && (!state.at_eof()))) {
auto result = parse_stmt(state);
stmts.push_back(result.stmt);
state = result.parser;
}
...
auto is_mut = preds::Parser_kind(after_let).is_mut();
auto name_pos = (is_mut ? after_let.advance() : after_let);
auto var_name = preds::Parser_kind(name_pos).ident();
auto type_result = (preds::Parser_kind(name_pos.advance()).is_colon() ? types::parse_type(name_pos.advance_by(2)) : preds::TypeResult{std::make_shared<ast::TypeExpr>(ast::TyUnit{}), name_pos.advance()});
...
}() : (kind.is_break() ? preds::StmtResult{std::make_shared<ast::Stmt>(ast::StmtBreak{}), parser.advance()} : (kind.is_continue() ? preds::StmtResult{std::make_shared<ast::Stmt>(ast::StmtContinue{}), parser.advance()} : (kind.is_return() ? [&]() {
...
return (preds::Parser_kind(expr_result.parser).is_equal() ? [&]() {
...
return ((name == mlc::String("_", 1)) ? preds::PatResult{std::make_shared<ast::Pat>(ast::PatWild{}), parser.advance()} : (preds::is_ctor_name(name) ? [&]() {
auto after = parser.advance();
return (preds::Parser_kind(after).is_lparen() ? [&]() {
...
}() : (preds::Parser_kind(after).is_lbrace() ? [&]() {
auto field_result = parse_record_pat_fields(after.advance());
...
}() : preds::PatResult{std::make_shared<ast::Pat>(ast::PatIdent(name)), parser.advance()}));
}() : (kind.is_int() ? preds::PatResult{std::make_shared<ast::Pat>(ast::PatInt(kind.int_val())), parser.advance()} : (kind.is_str() ? preds::PatResult{std::make_shared<ast::Pat>(ast::PatStr(kind.str_val())), parser.advance()} : (kind.is_true() ? preds::PatResult{std::make_shared<ast::Pat>(ast::PatBool(true)), parser.advance()} : (kind.is_false() ? preds::PatResult{std::make_shared<ast::Pat>(ast::PatBool(false)), parser.advance()} : preds::PatResult{std::make_shared<ast::Pat>(ast::PatWild{}), parser.advance()})))));
}
preds::PatsResult parse_pat_args(preds::Parser parser) noexcept{
auto pats = mlc::Array<std::shared_ptr<ast::Pat>>{};
return (preds::Parser_kind(parser).is_rparen() ? preds::PatsResult{pats, parser.advance()} : [&]() {
...
return preds::PatsResult{pats, state.advance()};
}());
}
...
pats.push_back(std::make_shared<ast::Pat>(ast::PatIdent(preds::Parser_kind(state).ident())));
}
state = state.advance();
...
while (preds::Parser_kind(state).is_pipe()) {
...
auto expr = left.expr;
auto state = left.parser;
while ((preds::Parser_kind(state).is_op() && (preds::Parser_kind(state).op_val() == mlc::String("||", 2)))) {
...
expr = std::make_shared<ast::Expr>(ast::ExprBin(kind.op_val(), expr, right.expr));
state = right.parser;
}
return preds::ExprResult{expr, state};
}
...
while (go) {
auto kind = preds::Parser_kind(state);
if ((kind.is_op() && ((kind.op_val() == mlc::String("+", 1)) || (kind.op_val() == mlc::String("-", 1))))) {
...
while (go) {
auto kind = preds::Parser_kind(state);
if ((kind.is_op() && (((kind.op_val() == mlc::String("*", 1)) || (kind.op_val() == mlc::String("/", 1))) || (kind.op_val() == mlc::String("%", 1))))) {
...
auto kind = preds::Parser_kind(parser);
return ((kind.is_op() && ((kind.op_val() == mlc::String("!", 1)) || (kind.op_val() == mlc::String("-", 1)))) ? [&]() {
auto inner = parse_unary(parser.advance());
return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprUn(kind.op_val(), inner.expr)), inner.parser};
...
auto kind = preds::Parser_kind(state);
if (kind.is_dot()) {
auto field_name = preds::Parser_kind(state.advance()).ident();
if (preds::Parser_kind(state.advance_by(2)).is_lparen()) {
auto margs = parse_arg_list(state.advance_by(3));
...
auto call_args = parse_arg_list(state.advance());
...
return (preds::Parser_kind(parser).is_rparen() ? preds::ExprsResult{exprs, parser.advance()} : [&]() {
...
return preds::ExprsResult{exprs, state.advance()};
}());
}
...
auto kind = preds::Parser_kind(parser);
return (kind.is_int() ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprInt(kind.int_val())), parser.advance()} : (kind.is_str() ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprStr(kind.str_val())), parser.advance()} : (kind.is_fstr() ? preds::ExprResult{build_fstr_expr(kind.fstr_parts()), parser.advance()} : (kind.is_true() ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprBool(true)), parser.advance()} : (kind.is_false() ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprBool(false)), parser.advance()} : (kind.is_lparen() ? (preds::Parser_kind(parser.advance()).is_rparen() ? (preds::Parser_kind(parser.advance_by(2)).is_fat_arrow() ? [&]() {
...
}() : preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprUnit{}), parser.advance_by(2)}) : (looks_like_lambda_params(parser.advance()) ? [&]() {
...
return preds::ExprResult{inner.expr, inner.parser.advance()};
...
auto name = kind.ident();
return (preds::Parser_kind(parser.advance()).is_fat_arrow() ? [&]() {
auto params = mlc::Array{name};
auto body = parse_expr(parser.advance_by(2));
return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprLambda(params, body.expr)), body.parser};
}() : ((preds::is_ctor_name(name) && preds::Parser_kind(parser.advance()).is_lbrace()) ? parse_record_expr(parser.advance_by(2), name) : preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprIdent(name)), parser.advance()}));
}() : preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprUnit{}), parser.advance()}))))))))))))));
}
...
if ((!preds::Parser_kind(state).is_rparen())) {
ok = false;
} else {
ok = preds::Parser_kind(state.advance()).is_fat_arrow();
}
}
return ok;
}
...
auto state = parser;
names.push_back(preds::Parser_kind(state).ident());
state = state.advance();
while (preds::Parser_kind(state).is_comma()) {
state = state.advance();
names.push_back(preds::Parser_kind(state).ident());
...
return preds::NamesResult{names, state.advance()};
}
preds::ExprResult parse_array_lit(preds::Parser parser) noexcept{
auto exprs = mlc::Array<std::shared_ptr<ast::Expr>>{};
return (preds::Parser_kind(parser).is_rbracket() ? preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprArray(exprs)), parser.advance()} : [&]() {
auto first = parse_expr(parser);
exprs.push_back(first.expr);
auto state = first.parser;
while (preds::Parser_kind(state).is_comma()) {
...
return preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprArray(exprs)), state.advance()};
}());
}
preds::ExprResult parse_if_expr(preds::Parser parser) noexcept{
auto is_negated = preds::Parser_kind(parser).is_unless();
...
return (preds::Parser_kind(then_result.parser).is_else() ? [&]() {
auto after_else = then_result.parser.advance();
return ((preds::Parser_kind(after_else).is_if() || preds::Parser_kind(after_else).is_unless()) ? [&]() {
...
}() : preds::ExprResult{std::make_shared<ast::Expr>(ast::ExprIf(condition, then_expr, std::make_shared<ast::Expr>(ast::ExprUnit{}))), then_result.parser.advance()});
}
...
return ((stmts.length() == 0) ? stmts : std::visit(overloaded{[&](const ast::StmtExpr& stmtExpr) { auto [expr] = stmtExpr; return (is_unit_expr(expr) ? stmts : [&]() {
...
auto var_name = preds::Parser_kind(parser).ident();
...
return (next.is_lbrace() ? [&]() {
auto arms = parse_arms_brace(subject.parser.advance());
...
return preds::ArmsResult{arms, state.advance()};
}
preds::ArmsResult parse_arms_pipe(preds::Parser parser) noexcept{
auto arms = mlc::Array<std::shared_ptr<ast::MatchArm>>{};
auto state = parser;
while (preds::Parser_kind(state).is_bar()) {
auto pat_result = parse_pat(state.advance());
auto body_result = parse_expr(pat_result.parser.advance());
arms.push_back(std::make_shared<ast::MatchArm>(ast::MatchArm{pat_result.pat, body_result.expr}));
state = body_result.parser;
}
if (preds::Parser_kind(state).is_end()) {
state = state.advance();
}
...
if (preds::Parser_kind(after_name).is_colon()) {
