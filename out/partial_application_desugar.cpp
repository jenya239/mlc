...
return std::visit(overloaded{[&](const ast::ExprNamedArg& exprNamedArg) { auto [label, inner, span_named] = exprNamedArg; return [&]() {
...
auto next_serial_after_fold = parts.fold(serial, [=](int next_serial_under_fold, ast::RecordLitPart part_under_scan) mutable { return std::visit(overloaded{[&](const ast::RecordLitFields& recordLitFields) { auto [field_values_under_part] = recordLitFields; return [&]() {
...
return std::visit(overloaded{[&](const ast::StmtLet& stmtLet) { auto [name, is_mut, typ, value_expression, span_statement] = stmtLet; return [&]() {
...
return arms.fold(MatchArmsPartialPair{{}, serial}, [=](MatchArmsPartialPair accumulated, std::shared_ptr<ast::MatchArm> arm_shared_under_match) mutable { return [&]() {
...
return std::visit(overloaded{[&](const ast::ExprInt& exprInt) { auto [__0, __1] = exprInt; return partial_application_leaf_partial_pair(expression, serial); },
