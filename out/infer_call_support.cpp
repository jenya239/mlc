...
auto ctor_type = registry::TypeRegistry_ctor_type(inference_context.registry, constructor_name);
auto owner_name = std::visit(overloaded{[&](const registry::TNamed& tNamed) { auto [n] = tNamed; return n; },
[&](const auto& __v) { return mlc::String("", 0); }
}, (*ctor_type));
auto private_errors = ((registry::TypeRegistry_is_private_ctor(inference_context.registry, constructor_name) && (inference_context.current_extend_type != owner_name)) ? (mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("private constructor: cannot construct ", 38) + constructor_name) + mlc::String(" outside its extend block", 25)), call_source_span)}) : (mlc::Array<ast::Diagnostic>{}));
...
return infer_result::InferResult{std::make_shared<registry::Type>(registry::TUnknown{}), ast::diagnostics_append(base.errors, mlc::Array<ast::Diagnostic>{ast::diagnostic_error(((mlc::String("called value is not a function (got ", 36) + semantic_type_structure::type_description(function_result.inferred_type)) + mlc::String(")", 1)), call_source_span)})};
}
}
