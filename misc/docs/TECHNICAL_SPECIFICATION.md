...
- Версия: 2.1.0
- Тесты: 2577 runs, 6561 assertions, 0 failures, 2 skips (full suite, 2026-02-17)
- Coverage: Line 84.27%, Branch 58.82%
- Фаза разработки: Spread operator, block syntax complete (2025-11-28)
...
```bash
# Быстрые unit тесты (~50 сек) - использовать по умолчанию
rake test_unit

# Быстрые тесты без E2E (~2-3 мин)
rake test_fast

# Полный набор (~20 мин) - перед коммитом
rake test

# Только integration тесты
rake test_integration

# Только E2E тесты (компиляция бинарников)
rake test_e2e
...
```
2577 runs, 6561 assertions, 0 failures, 0 errors, 2 skips (full suite, 2026-02-17)
```
...
*Версия компилятора: 2.1.0 (обновлено 2026-02-17)*

**Примечание**: Ownership model (move semantics, borrow checking), async lowering, FFI — описаны в LANGUAGE_STRATEGY.md и SAFETY_GUIDE.md, но не реализованы в семантическом анализаторе.
