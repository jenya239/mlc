...
**Канон строковых литералов (см. § F1):** интерполяция только в шаблонах `` `...${expr}...` ``. Литералы в **`"..."`** в mlcc — всегда без интерполяции (только escape через `\`). В Ruby-пайплайне **`"..."`** могут давать интерполяцию `${` — при переносе фрагментов из эталона в mlcc шаблоны переносите на `` ` ``.
...
#### Целочисленная арифметика: переполнение и деление на 0

Зафиксировано 2026-07-10 (`TRACK_LANG_INT_OVERFLOW` STEP=1–2). Операторы
`+` `-` `*` `/` `%` на целочисленных типах:

| Класс | Операторы | Поведение |
|-------|-----------|-----------|
| Signed (`i32`, `i64`, …) | `+` `-` `*` | **Debug:** panic с сообщением при overflow/underflow. **Release:** как в C++ — signed overflow = UB (детектируется UBSan; CI может включать `-fsanitize=undefined`). Без тихого wrap. |
...
**Приоритет (как в эталонном парсере Ruby):** ниже `==` / `!=`, ниже `&&`, ниже `||`; выше операций сравнения `<` `>` `<=` `>=`. Без скобок `a || b |> c` читается как `a || (b |> c)`, а не `(a || b) |> c`.

Пример с `filter` / `map` / `fold` выше — идиоматический стиль; сами методы высшего порядка для `[T]` относятся к фазе **A1** и могут отсутствовать до её закрытия; оператор `|>` достаточно описать на уже существующих функциях и вызовах.
...
Сахар над лямбдами: `mul(2, _)` → лямбда с одним параметром и телом `mul(2, partial_application_parameter)` (имена параметров внутренние, свежие).

**Правила:**
- `_` допустим только как **непосредственный** аргумент вызова (в том числе значение именованного аргумента `label: _`). Выражение вида `let x = _` или операнд вроде `x + _` без обёртки вызова — не сахар частичного применения (обычная семантика идентификатора `_`).
- Несколько дыр в одном вызове: `f(_, _)` → лямбда с параметрами слева направо по списку аргументов.
- Вложенные вызовы десахарятся снизу вверх: сначала внутренний `g(_)`, затем внешний.
- В паттернах `match` символ `_` по-прежнему wildcard; в **выражениях** в позиции аргумента вызова — только заполнитель частичного применения.
- Сочетание с E5 (`|>`): после десахара `_` правая часть пайпа может быть лямбдой.

**C++ трансляция:** обычная лямбда/замыкание по уже существующему lowering лямбд.
...
| Форма | Интерполяция `${…}` | Комментарий |
|-------|---------------------|-------------|
| `'…'` | Нет | Обычная строка; минимальные escape (как в Ruby): `\\`, `\'`, `\n`, `\t`, `\r`, `\0`; неизвестный `\x` — в эталонном Ruby-лексере сохраняются `\` и `x`. Перевод строки внутри литерала допускается. |
| `"…"` | **В mlcc — нет** (только `\`-escape). **В Ruby-пайплайне — может быть** (`${`) | В self-hosted исходниках предпочитать `'…'` для литералов без `\`; смешанные escape (`\{`, `\"`, …) оставлять в `"…"` или переписывать явно под правила `'…'`. |
| `` `…${expr}…` `` | Да | Шаблон (§ A5); escape `` \` ``, `\${`, стандартные `\n` / `\t` / `\r` / `\\`. |

**Тип `char` vs `string` (mlcc):** литерал **`'a'`** из ровно одного символа токенизируется как **символ** (`char`), не как `string`. Нужна строка из одного символа — используйте **`"a"`** (или шаблон `` `"${ ... }"` `` не применимо; достаточно `"a"`).

**Конвенция стиля в репозитории `compiler/`:** тексты без интерполяции и без проблемных escape — в **`'…'`**; интерполяция — только **шаблоны**; **`"…"`** допускаются для совместимости (односимвольные строки, сложные escape, литералы с внутренним `'` без `\'`).
...
**Цель:** альтернатива скобкам и pipe-стилю для ограничения списка рук матча и для многострочного тела ветки.

**Грамматика после скрутини (ровно одна форма на один `match`):**

| Оболочка рук | Вид |
|--------------|-----|
| `{` … `}` | Руки через запятую, как сейчас (`pattern => body`, …). |
| `do` … `end` | Последовательность рук до ключевого слова `end`; между руками допускается необязательный `;`. Перед рукой допускается необязательный `\|` (как у `{ … }`). |
| без `{` и без начального `do` | Pipe-стиль: каждая рука начинается с `\|`, завершение — необязательный `end` перед следующим выражением или EOF (как у эталонного Ruby-парсера). |

**Тело ветки после `=>`:** произвольное выражение — в том числе **`do` … `end`** (блок как при первичном `do`) или однострочное выражение. Литералы записей `{ поле: … }` после `=>` по-прежнему различаются контекстом и lookahead (см. правила матча в парсере).
