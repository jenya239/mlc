...
2. **Thread safety ([TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) MVP +
   ...
   **Send / Sync (checker predicates in `send_safe.mlc`):**
   ...
   | `TestRuntime` | no | no | !Send/!Sync; MLC facade over `TestScheduler`; `TestRuntime.new(seed)` / `.spawn` / `.join` / `.log_event` / `.events_joined` / `.seed` ([TRACK_CONCURRENCY_TESTRUNTIME_MLC_SURFACE](agent/TRACK_CONCURRENCY_TESTRUNTIME_MLC_SURFACE.md)) |
   ...
   **TestRuntime (MLC):** `TestRuntime.new(seed)` → `TestScheduler`; `.spawn do … end`
   ...
   | `TestRuntime` | N/A | N/A | `testing/scheduler.hpp` (`TestScheduler`) + MLC `TestRuntime.new` / `.spawn` / `.join` / `.log_event` / `.events_joined` / `.seed`; deterministic seed; !Send/!Sync; ([TRACK_CONCURRENCY_TESTRUNTIME_MLC_SURFACE](agent/TRACK_CONCURRENCY_TESTRUNTIME_MLC_SURFACE.md)) |
