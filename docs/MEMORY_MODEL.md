...
2. **Thread safety ([TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) MVP +
   ...
   **Send / Sync (checker predicates in `send_safe.mlc`):**
   ...
   | `Isolate[State, Msg]` | no | no | !Send/!Sync; MLC `Isolate.start` / `.send` / `.shutdown` ([TRACK_CONCURRENCY_ISOLATE_MLC_SURFACE](agent/TRACK_CONCURRENCY_ISOLATE_MLC_SURFACE.md)) |
   ...
   **Isolate (MLC):** `Isolate.start(state, capacity, handler)` → `Isolate[State, Msg]`;
   ...
   | `Isolate<State,Msg>` | N/A | N/A | `isolate.hpp` + MLC `Isolate.start` / `.send` / `.shutdown` / `.state_after_shutdown`; single owner + bounded mailbox; serial handler; Block overflow; !Send/!Sync; Msg Send on `.send` (**E092**); ([TRACK_CONCURRENCY_ISOLATE_MLC_SURFACE](agent/TRACK_CONCURRENCY_ISOLATE_MLC_SURFACE.md)) |
