...
2. **Thread safety ([TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) MVP +
   ...
   **Spawn capture (checker, [TRACK_CONCURRENCY_MUTABLE_CAPTURE_CHECK](archive/tracks/TRACK_CONCURRENCY_MUTABLE_CAPTURE_CHECK.md)
   ...
   + [TRACK_CONCURRENCY_MOVE_TRACKING](agent/TRACK_CONCURRENCY_MOVE_TRACKING.md)):**
   ...
   Array / Map still need `move`. `move x` → `std::move`; later free use after
   `move` into bare `spawn` **or** `TaskScope.spawn` → `error[E088]`
   (`move_check.mlc`: ExprSpawn + ExprMethod `"spawn"` export marks; general
   ExprLambda does not). Immutable capture OK **only if** the binding's type is Send;
   ...
   | `TaskScope` | N/A | N/A | MLC `scope \|binder\| do … end` + `.spawn do … end`; dtor cancel+join; E087 on free mut capture into `.spawn`; E092 on free `!Send` capture into `.spawn`; E088 on use after `move` into `.spawn` |
