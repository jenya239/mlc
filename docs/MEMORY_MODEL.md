...
2. **Thread safety ([TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) MVP +
   ...
   further: [CONCURRENCY_V2.md](CONCURRENCY_V2.md) phase 11 / TestRuntime):**
   ...
   **Send / Sync (checker predicates in `send_safe.mlc`):**
   ...
   | `Supervisor` | no | no | !Send/!Sync; MLC `Supervisor.new` / `.add` / `.start` / `.stop`; `RestartPolicy` Send+Sync ([TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE](agent/TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE.md)) |
   ...
   **Supervisor (MLC):** `Supervisor.new()` / `.add(name, RestartPolicy, handler)` /
   ...
   | `Supervisor` | N/A | N/A | `supervisor.hpp` + MLC `Supervisor.new` / `.add` / `.start` / `.stop`; `RestartPolicy`; one_for_one; storm intensity; !Send/!Sync; ([TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE](agent/TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE.md)) |

   Runtime: `runtime/include/mlc/concurrency/` (`channel.hpp`, `spawn.hpp`,
   `arc.hpp`, `mutex.hpp`, `stop.hpp`, `task_scope.hpp`, `thread_pool.hpp`,
   `isolate.hpp`, `supervisor.hpp`, `atomic.hpp`).
