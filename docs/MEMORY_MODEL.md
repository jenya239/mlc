...
2. **Thread safety (TRACK_CONCURRENCY closed — MVP; развитие: [CONCURRENCY_V2.md](CONCURRENCY_V2.md), [agent/TRACK_CONCURRENCY_V2.md](agent/TRACK_CONCURRENCY_V2.md)):**

   Строка `Array<T>`/`HashMap<K,V>` ниже ("mutate only inside `Mutex<T>.lock`
   или single-thread") — сегодня это соглашение, **не проверяется checker'ом**.
   `spawn { array.push(x) }` из двух потоков на COW-расшаренной переменной —
   реальный, воспроизводимый data race (COW detach неатомарен), а не только
   теоретический риск. `CONCURRENCY_V2.md` §1/§4 делает это ошибкой компиляции.

   | Type | Refcount | COW detach | Cross-thread |
   |------|----------|------------|--------------|
   | `Array<T>` / `HashMap<K,V>` | atomic (`shared_ptr`) | **not atomic** | mutate only inside `Mutex<T>.lock` or single-thread |
   | `String` | atomic on heap | no in-place shared heap mutation | read-only share OK |
   | `Shared<T>` | atomic | N/A | not `Send`; do not pass on `Channel` |
   | `Channel<T>` | internal mutex | copy in/out at send/recv | `Send`-safe payload only (checker E082) |
   | `Arc<T>` | atomic (`Arc` control block) | N/A | read-share; `Send`-safe inner only |
   | `Mutex<T>` | N/A | N/A | scoped `lock(callback)`; inner may be `Array`/`Map` |

   Runtime: `runtime/include/mlc/concurrency/` (`channel.hpp`, `spawn.hpp`, `arc.hpp`, `mutex.hpp`).
   COW helper: `runtime/include/mlc/core/cow_detach.hpp`.
   Smoke: `runtime/test/run_concurrency_smoke.sh`; stress: `stress_channel.cpp`; optional TSAN: `MLC_TSAN=1 runtime/test/run_concurrency_smoke.sh`.
