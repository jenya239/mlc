...
2. **Thread safety ([TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) MVP +
   [TRACK_CONCURRENCY_V2](archive/tracks/TRACK_CONCURRENCY_V2.md) phases 1–4 closed;
   further: [CONCURRENCY_V2.md](CONCURRENCY_V2.md) phases 5–11):**
   ...
   **Spawn capture (checker):** free use of enclosing `let mut` inside `spawn` →
   `error[E087]`. `move x` transfers ownership (`std::move`); later use →
   `error[E088]`. Immutable capture OK. Sync-safe shared capture without `move`
   still incomplete (phases 5+).
   ...
   | `Channel<T>` | internal mutex | copy in/out at send/recv | `Send`-safe payload; capacity 0 = rendezvous; `Sender`/`Receiver` split |
   | `Arc<T>` | atomic (`Arc` control block) | N/A | read-share; `Send`/`Sync` follow `T` |
   | `Mutex<T>` | N/A | N/A | scoped `lock(callback)`; always Sync; Send iff `T` Send |
   | `StopSource` / `StopToken` | N/A | N/A | `request()` / `requested()`; wake-on-cancel for channel/sleep not yet |

   Runtime: `runtime/include/mlc/concurrency/` (`channel.hpp`, `spawn.hpp`,
   `arc.hpp`, `mutex.hpp`, `stop.hpp`).
