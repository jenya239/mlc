...
2. **Thread safety ([TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) MVP +
   ...
   [TRACK_CONCURRENCY_TASKSCOPE](archive/tracks/TRACK_CONCURRENCY_TASKSCOPE.md) closed;
   further: [CONCURRENCY_V2.md](CONCURRENCY_V2.md) phases 8–11 / Isolate):**
   ...
   **Send / Sync (checker predicates in `send_safe.mlc`):**
   ...
   | `Channel` / `Sender` / `Receiver` | payload must be Send | — | E082 on unsafe send |
   | `StopSource` / `StopToken` | runtime only | — | cooperative cancel; no MLC surface yet |

   **Spawn capture (checker):** free use of enclosing `let mut` inside `spawn` →
   ...
   `error[E088]`. Immutable capture OK.

   | Type | Refcount | COW detach | Cross-thread |
   |------|----------|------------|--------------|
   | `Array<T>` / `HashMap<K,V>` | atomic (`shared_ptr`) | **not atomic** | mutate only inside `Mutex<T>.lock` or single-thread; free mut across `spawn` → E087 |
   ...
   | `Channel<T>` | internal mutex | copy in/out at send/recv | `Send`-safe payload; capacity 0 = rendezvous; `Sender`/`Receiver` split; `send`/`receive(StopToken)` → `Cancelled` on cancel wake |
   | `Arc<T>` | atomic (`Arc` control block) | N/A | read-share; `Send`/`Sync` follow `T`; Sync-safe mut capture across `spawn` OK |
   | `Mutex<T>` | N/A | N/A | scoped `lock(callback)`; always Sync; Send iff `T` Send; Sync-safe mut capture across `spawn` OK |
   | `StopSource` / `StopToken` | N/A | N/A | `request()` / `requested()`; channel wait wakes via `stop_callback` |
   | `TaskScope` | N/A | N/A | C++ runtime: spawn / spawn_with_token / join; dtor cancel+join; no MLC `scope` syntax yet |

   Runtime: `runtime/include/mlc/concurrency/` (`channel.hpp`, `spawn.hpp`,
   `arc.hpp`, `mutex.hpp`, `stop.hpp`, `task_scope.hpp`).
