...
`AtomicBool` / `AtomicI32` / `AtomicI64` / `AtomicU64`: `load`/`store`/`exchange`/
...
```mlc
fn handle(state: RoomState, msg: RoomMsg) -> unit = do
  match msg {
    Join(user, client) => ()
    Leave(user) => ()
    Post(user, text) => ()
...
let room = Isolate.start(RoomState.new(), 1024usize, handle)
...
senders: `runtime/test/test_isolate.cpp` (MLC multi-thread share deferred — Isolate not copyable).

## 17. `Isolate` mailbox обязан быть bounded

```mlc
Isolate.start(initial_state, 1024usize, handle)
```

Overflow policy v1: **Block** only (`send` waits). `DropNewest | DropOldest | Reject` —
deferred (not in C++/MLC yet).

**Статус:** bounded mailbox **done**; Drop*/Reject **deferred**.
...
**Статус:** **done** C++ + MLC method API ([TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE](archive/tracks/TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE.md)):
...
| 10 | `Supervisor` | library | **есть** C++ + MLC ([TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE](archive/tracks/TRACK_CONCURRENCY_SUPERVISOR_MLC_SURFACE.md)): `Supervisor.new`/`add`/`start`/`stop`; `RestartPolicy`; one_for_one + storm intensity; block sugar deferred |
