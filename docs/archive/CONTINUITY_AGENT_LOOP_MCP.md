> **Archived 2026-07-09.** Snapshot of `docs/agent/CONTINUITY.md` from the `agent-loop` MCP / `cr` orchestrator era (tokens, CDP, watchdog, overnight guard). That approach is discontinued; current workflow uses the plain Cursor queue — see `docs/agent/CONTINUITY.md`. Kept for reference only.
