**Status: won't-do (2026-07-11).** Рассмотрен и отклонён в тот же день, когда
создан. Причина: нет текущей проблемы синхронизации (одна реализация, не
как Ruby↔mlcc), рантайм — горячий путь (COW/growth policy/atomics),
переписывание — максимальный риск (весь компилятор зависит от
`Array`/`String`/`Shared`) без доказанной выгоды. Прецедент: ни один
мейнстримный компилируемый/транспилируемый язык не переписывает этот слой
на себе (V8 `Array` — C++, JVM core-классы — native, CPython `list`/`dict`
— C) — рантайм в языке более низкого уровня — стандартная практика, не
признак "языка без смысла". «Без hand-written C++» относится к FFI-
адаптерам/бизнес-логике (см. `FFI_LAYER.md` §8), не к рантайму. См.
`docs/PLAN.md` — заменено строкой "won't-do" в §20.
