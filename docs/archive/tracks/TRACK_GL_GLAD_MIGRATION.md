...
**Critic 2026-07-11:** STEP=1–7 vs `0b613af4`…`855c7485`; dispatch/shim
absent; glad trees + `glad_*.mlc`/`glad_*_abi`/`glfw_window_gl` present;
`build_bin.sh` links `gl.c` (or egl/gles2 if no glfw); smokes
glad_link/loader/renderer/dispatch_smoke exit 0; PLAN/archive sync.
**reopen: none**. Residuals: smoke still named `glfw_gl_dispatch_*`; archived
TRACK inventory prose stale; desktop+egl glad not co-linked; §20 parent row
still «open» while 20a–e done (Planner).

**Driver 2026-07-11 STEP=7:** Docs `TEXT_RENDERING.md` §3.3/§5.2/§8, `GUI.md`,
`FFI_LAYER.md` §8 → GLAD2; TRACK archived. Close → Critic.

**Driver 2026-07-11 STEP=6:** `regression_gate` 20/0; self-host p1↔p2 DIFF 0
(`mlcc`→`mlcc2`); GUI smokes green (glad_link, gl_loader, gl_renderer,
gui_input, text_renderer_native, glfw_gl_dispatch).

**Driver 2026-07-11 STEP=5:** Deleted `glfw_gl_dispatch.*` + `loader_shim.*`.
Window/input → `glfw_window_gl.*`; GL → `glad_gl_abi` (+ compat `mlc::gl::gl_*`).
`gl_loader_smoke` = GLFW+glad genBuffers. Smokes green.

**Driver 2026-07-11 STEP=4:** misc/gui on glad.
**Driver 2026-07-11 STEP=3:** glad `.c` linked.
**Driver 2026-07-11 STEP=2:** MLC glad extern.
**Driver 2026-07-11 STEP=1:** vendored glad.
...
| 2 | `extern fn` для всех используемых функций (desktop + headless). | **done** |
| 3 | Линковка сгенерированных `.c` в `build_bin.sh`. | **done** |
| 4 | Переключить вызывающий MLC-код (те же имена где возможно). | **done** |
| 5 | Удалить `glfw_gl_dispatch.*`/`loader_shim.*`. | **done** |
| 6 | Regression (existing smokes) + self-host diff + `regression_gate.sh`. | **done** |
| 7 | Docs update; close. | **done** |
