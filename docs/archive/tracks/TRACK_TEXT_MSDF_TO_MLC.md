...
**Critic 2026-07-11:** STEP=1–6 vs `7b45afb2`…`633b9c2d`; PLAN/archive sync;
shim absent; re-ran MAE=0 + glyph smoke. **reopen: none**. Residuals: R=G=B
(not true multi-channel msdfgen); `dump_msdf_golden --check` → MAE gate only.

**Driver 2026-07-11:** STEP=6 — `regression_gate` 20/0; self-host DIFF_EXIT=0;
TEXT_RENDERING §3.2/§5.3 updated; TRACK archived. Close → Critic.

**Driver 2026-07-11:** STEP=5 — `msdf_bridge` (FreeType mask + RGB cache);
`msdf_generate` in `misc/gui/msdf.mlc`; retargeted glyph/renderer/render_mode/
text_window smokes; deleted `msdf_shim.cpp`/`.hpp`; `build_bin.sh` → bridge.
Smokes + MAE PASS.
...
## Проблема (как было до порта)

`msdf_shim.cpp` (бывшие ~253 строки) держал EDT/SDF на C++. После STEP=5
удалён; math в `misc/gui/msdf.mlc`, FreeType mask в `msdf_bridge`.
...
| 5 | Переключить вызывающий код; удалить `msdf_shim.*`. | **done** (2026-07-11: bridge+MLC generate; shim deleted; smokes OK) |
| 6 | Self-host diff + regression_gate; docs; close. | **done** (2026-07-11: REG 20/0; DIFF_EXIT=0; closed) |
