...
**Driver 2026-07-10:** STEP=2 — `mir_lower_if_statement` + discard Block/`()`; mid-block if.
**Driver 2026-07-10:** STEP=3 — `mir_lower_if_to_local` / `if_into_local` (shared local + continue).
**Driver 2026-07-10:** STEP=4 — self-host DIFF identical; regression_gate 20/0; VM corpus (elif6, depth, examples 28, single 18, cpp_diff 18).
...
| 2 | Gap 2.1: `mir_lower_if_statement` (if как statement, значение отбрасывается) — новая функция по аналогии с `mir_lower_while_statement`. Regression: repro выше + существующий корпус. | **done** (2026-07-10: CondJump+continue; discard Block/Unit/empty Tuple; `vm_if_stmt` exit=1, `vm_if_stmt_else` exit=2; elif gate 6 ok) |
| 3 | Gap 2.2: `if` как rvalue (shared local + continue-block) — в `mir_lower_rvalue_from_expression`. Более дорогой шаг, может быть отдельным STEP при необходимости декомпозиции. | **done** (2026-07-10: `mir_lower_if_to_local`/`into_local`; wired operand+rvalue+expression_to_local; `vm_if_rvalue`=1, `_else`=2, `_block`=11; nested=3; elif6 ok) |
| 4 | Verify-gate + close: self-host (`mlcc`→`mlcc2`→`diff`), `regression_gate.sh`, полный VM-корпус. | **done** (2026-07-10: DIFF identical; regression_gate 20/0; elif6; depth; examples 28; single 18; cpp_diff 18) |
