...
**Critic 2026-07-11:** STEP=1–6 vs `9d690c96`…`495f6679`; PLAN/archive sync;
`env.hpp`/`log.hpp`/`validate.hpp` absent; gates mlcc (no Ruby stdlib-only);
re-ran `run_env_log_gate`/`run_validate_gate` OK. **reopen: none**. Residuals:
Ruby `import Log/Validate` still maps to `mlc::log::*`/`mlc::validate::*`
without body emit; Validate Ok is `ValidateSuccess {}` not `()` (mlcc
`Ok<void>`); module-local `Result` sum; thin `env_abi`/`log_abi` fwrite remain.
...
`test_driver` asserts; mlcc `from 'Env'|'Log'|'Validate'` smoke exit 0;
self-host DIFF 0.

**Driver 2026-07-11:** STEP=4 — `validate.hpp` **deleted**; pure MLC
`validate/validate.mlc` (ValidateSuccess + local Result); mlcc smoke exit 0
(11 fixtures). Decision amended: Ok payload `ValidateSuccess {}` not `()`.

**Driver 2026-07-11:** STEP=3 — `log.hpp` **deleted**; JSON escape/format in
`log/logger.mlc`; thin `log_abi.hpp` (fwrite only); file `logger.mlc` avoids
mlcc `namespace log` vs math.h; mlcc stderr JSON lines match prior C++ wire.

**Driver 2026-07-11:** STEP=2 — `env.hpp` **deleted**; thin `env_abi.hpp`
(`get`/`get_or`/`has`, by-value String); `env.mlc` `from "mlc/env/env_abi.hpp"`;
Ruby codegen + runtime smoke OK; mlcc `misc/examples/env_mlc_abi_smoke.mlc` exit 0.
...
| Public Validate API | Keep `non_empty` / `min_length` / `max_length` / `range_i32`; error messages unchanged. Ok payload is `ValidateSuccess {}` (not `()` — mlcc `Ok<void>` broken); module exports local `Result` sum for codegen usings |
| Validate impl | Pure MLC bodies; **delete** `validate.hpp` |
| Bare names (STEP=5) | `"Env"` → `env/env.mlc`, `"Log"` → `log/logger.mlc`, `"Validate"` → `validate/validate.mlc` in `path_normalize.mlc` (same table as Tcp/HttpServer). Note: file is `logger.mlc` not `log.mlc` — mlcc TU namespace `log` clashes with math.h `log`. |
...
| 2 | `Env` → прямой `extern fn`, удалить `env.hpp`. | **done** (2026-07-11: `env_abi.hpp` + mlcc smoke) |
