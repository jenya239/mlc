...
разблокирован закрытием [TRACK_FFI_LAYER](TRACK_FFI_LAYER.md)
(STEP=1–8 closed 2026-07-09). Не поднимать выше `TRACK_MIR_VM_FULL` без
явной команды.

## Status: **closed** (2026-07-10) — STEP=0–8 **done**
...
**Driver 2026-07-10:** STEP=7.1 — `msdf_shim.cpp`: FT mask → EDT SDF → RGB8
(SDF-in-RGB, msdfgen-shaped API); `msdf_glyph_smoke` numerical extrema.
...
**Driver 2026-07-10:** STEP=8 — self-host DIFF identical; regression_gate 20/0;
`TEXT_RENDERING.md` §6 MAE ≤ 8.0/255; track archived.
...
-->
| 8 | Verify-gate + close: self-host (`mlcc`→`mlcc2`→`diff`), `regression_gate.sh`, критерий приёмки `TEXT_RENDERING.md` §6 (фаза 1-2 headless) + tolerance-порог golden-тестов зафиксировать в критерии приёмки явным числом. | **done** (2026-07-10: DIFF identical; regression 20/0; MAE ≤ 8.0/255 in §6) |
