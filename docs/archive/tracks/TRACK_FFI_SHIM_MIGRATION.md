...
**Critic 2026-07-11:** `critique-audit` — commits `8ffe67b8`…`8b21220a`
match STEP 1–7; named shim `postgres.hpp`/`sodium.hpp`/`tcp.hpp` absent;
`*_abi.hpp`/`*_bridge.hpp` + smokes on disk; PLAN §20a → archive; re-ran
pg/crypto/tcp mlc abi smokes + pg/crypto gates + mlcc tcp echo EXIT 0.
**reopen: none.** Residuals (accepted vs Decision interim): Goal «direct
`extern` to vendor headers / all bookkeeping on MLC» only partial — abi+bridge
C++ remains (~1k LOC); Tcp still `export extern` Option (mlcc Option segfault);
TcpStream in `tcp_bridge.hpp` for websocket; Ruby mirrors in bridges.
