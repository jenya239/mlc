...
**Critic 2026-07-10:** STEP=1–6 vs commits `501badc6`…`d4a2917c`; gates re-OK;
PLAN §8c/archive aligned; **reopen: none**. Residual: prose still said
`import Tcp::{…}` in places — fixed to `from 'Tcp'`.
...
One `.mlc` with `import { … } from 'Tcp'` + `spawn` compiles under **`mlcc`**,
links, runs multi-client echo without `serve_http_with_thread_pool`.
...
- Общий package-manager / module-registry для произвольных зависимостей
  ([TRACK_PACKAGE_MANAGER](../../agent/TRACK_PACKAGE_MANAGER.md) — design-only,
  не трогать).
