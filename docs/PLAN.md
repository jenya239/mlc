...
| **8** Concurrency v2 (Send/Sync, structured concurrency) | **partial** | [CONCURRENCY_V2.md](CONCURRENCY_V2.md); V2/TASKSCOPE/ISOLATE **closed** 2026-07-09. HARNESS T1–T5 done, T6 deferred. **Next:** [TRACK_LANG_SPAWN_DOUBLE_EXEC](agent/TRACK_LANG_SPAWN_DOUBLE_EXEC.md) STEP=4 (verify-gate). SUPERVISOR deferred. MVP: [TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) closed |
| **8a** `spawn do <tail-call> end` выполняет тело дважды (codegen) | **open, критический** | [TRACK_LANG_SPAWN_DOUBLE_EXEC](agent/TRACK_LANG_SPAWN_DOUBLE_EXEC.md) STEP=1–3 **done**; STEP=4 next (self-host + regression_gate) |
