...
| **44** | Codegen: eliminate string-concatenation, CppAST only (audit 2026-07-17 — §1/§7 "0% string bridges" not reached; `GenStmtsResult`/`GenModuleOut` string-typed by design, not edge cases) | **active** (2026-07-17) STEP=1 done; STEP=test-fix in progress (past BinaryRule string+char; still red: Ruby→C++ namespace errors linking run_tests) | [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CODEGEN_CPPAST_ONLY (§44, **active**; STEP=1 done (`5c20ffa9`);
      STEP=test-fix in progress — past string+char; still red (Ruby→C++
      namespace errors on `run_tests` link); then STEP=2;
