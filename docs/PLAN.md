...
| **7** Language design audit (2026-07) | **partial** | [LANGUAGE_AUDIT_2026_07.md](LANGUAGE_AUDIT_2026_07.md); 7/8 треков closed; [TRACK_LANG_REGION_ARENA](agent/TRACK_LANG_REGION_ARENA.md) **active** (STEP=1–9 done; STEP=10 optional hot-path next) — Decisions 1–3 closed; Steps 1-10 |
...
| **30** | HarfBuzz/FreeType шимы: §8 «без hand-written C++» пропустил их — face/font handle-кеш и pitch-copy loop остаются ручным C++ | **open, queued** (после REGION; TEXT_GL Critic OK) | [TRACK_TEXT_SHIM_TO_MLC](agent/TRACK_TEXT_SHIM_TO_MLC.md) — bookkeeping (handle-кеш по `(font_path,pixel_size)`) и glyph pitch-copy на MLC поверх тонкого `*_abi.hpp`; bearing ABI не дублируется (уже STEP=7 в GL_PERF_BASELINE) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **LANG_REGION_ARENA (**active** 2026-07-12: STEP=1–9 done — ExprRegion +
      RegionHandle/r.alloc + E091 + pmr + escape/cycle e2e; mlcc→mlcc2 identical;
      REG 20/0; sweep ok; MLC.md §C1 region docs; STEP=10 optional hot-path next;
  ...
  → **TEXT_SHIM_TO_MLC (open, queued after REGION; TEXT_GL Critic OK;
