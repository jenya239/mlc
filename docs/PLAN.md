...
| **30** | HarfBuzz/FreeType шимы: §8 «без hand-written C++» пропустил их — face/font handle-кеш и pitch-copy loop остаются ручным C++ | **active** (STEP=5 done; STEP=6 demos) | [TRACK_TEXT_SHIM_TO_MLC](agent/TRACK_TEXT_SHIM_TO_MLC.md) — vs_shim golden ok; Steps 6–10 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_SHIM_TO_MLC (**active** 2026-07-12: STEP=1–5 done — abi +
      `text_shaping.mlc` + vs_shim golden; STEP=6 demos next;
