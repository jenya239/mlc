...
| **36** | Stdlib IO/FS: `list_dir` / `is_directory` / `is_regular_file` / `create_directories` + editor tree wire | **active** (2026-07-16) STEP=3 **done**; **STEP=4** create_directories next | [TRACK_STDLIB_IO_FS](agent/TRACK_STDLIB_IO_FS.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **STDLIB_IO_FS (§36, **active**; STEP=3 safe_list_dir done; **STEP=4** create_directories next;
