...
| **22** | Дублирующийся `extern fn ... from "<header>"` в графе импортов → clang error вместо диагностики mlcc | **active** (2026-07-12) | [TRACK_FFI_EXTERN_DEDUP](agent/TRACK_FFI_EXTERN_DEDUP.md) **active** STEP=1 done (Hybrid); STEP=2 next — repro fixture |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **FFI_EXTERN_DEDUP (active: STEP=1 Hybrid done; STEP=2 repro next;
