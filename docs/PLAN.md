...
| **10a** Text rendering целиком на MLC + окно (фундамент GUI-фреймворка) | **open, active** | [TRACK_TEXT_RENDERING_NATIVE](agent/TRACK_TEXT_RENDERING_NATIVE.md) **active** STEP=8 (verify-gate); docs §8 in [TEXT_RENDERING.md](TEXT_RENDERING.md) |
| **10b** GUI framework (layout/widgets/easing) | **backlog** | [TRACK_GUI_FRAMEWORK](agent/TRACK_GUI_FRAMEWORK.md) — activate after 10a close |
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET/POSTGRES/CRYPTO/WEBSOCKET/JOB_QUEUE/ENV_LOGGING/VALIDATION **closed**. Active Driver: [TRACK_TEXT_RENDERING_NATIVE](agent/TRACK_TEXT_RENDERING_NATIVE.md) STEP=8 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → TEXT_RENDERING_NATIVE (**active** STEP=7 — docs
