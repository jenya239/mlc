...
| **10b** GUI framework (layout/widgets/easing) | **backlog** | [TRACK_GUI_FRAMEWORK](agent/TRACK_GUI_FRAMEWORK.md) — NATIVE Critic OK 2026-07-11; activate via Planner |
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET/POSTGRES/CRYPTO/WEBSOCKET/JOB_QUEUE/ENV_LOGGING/VALIDATION **closed**. Next: Planner (GUI promote or low-pri tracks) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → GUI_FRAMEWORK (**backlog** — NATIVE Critic OK; Planner promote
