...
| **109** (authorized 2026-08-03 as **queue head** — user: «тормоза всё такие же, пока это не вылечим дальше не идём»; suspends §103) | **Editor perf dogfood** — gates green (§106–§108) but interactive lag unchanged; §108d L2 measured cpu=0 under probe (false comfort); §107d PERF_FULL ~7.3e6µs/5 frames on 10k lines still unusable. Sub-tracks: §109a baseline (visible window + demo_live.mlc) → §109b content-frame budget → §109c visible hover gate → §109d chrome row cost. Hard stop: no §103f+ / §104 W2 until close criteria Critic-audited | **open** — queue head §109a Driver STEP=0 | [agent/TRACK_EDITOR_PERF_DOGFOOD](agent/TRACK_EDITOR_PERF_DOGFOOD.md) |
