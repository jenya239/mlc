...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CODEGEN_CPPAST_ONLY (§44, **active**; STEP=1/test-fix/2/3/4/5/6/7a–7b3c5/8a/8b1 **done**;
      Tier A/B: no Ruby rebuild — `dev_gate_fast` green; STEP=8b2 next;
...
| Codegen: строки vs CppAST | Decl Fragments gone; `expr.mlc` 80 exports left (`cpp_*` types → type_gen) → 8b2/8c. См. [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) | 0% string bridges |
