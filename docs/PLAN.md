...
| **16** Integer overflow semantics | **open** | [TRACK_LANG_INT_OVERFLOW](agent/TRACK_LANG_INT_OVERFLOW.md) — STEP=1–4 **done**; STEP=5 next (tests + verify-gate + close) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **LANG_INT_OVERFLOW STEP=1 (**done** 2026-07-10: signed debug-panic/release-UB;
    ...
    STEP=4 (**done** 2026-07-10: `mlc::int_arith` i32 wrap/check/sat);
    STEP=5 (**next** — tests + verify-gate + close)
