...
| **30** | HarfBuzz/FreeType шимы: §8 «без hand-written C++» пропустил их — face/font handle-кеш и pitch-copy loop остаются ручным C++ | **active** (STEP=4 done; STEP=5 golden) | [TRACK_TEXT_SHIM_TO_MLC](agent/TRACK_TEXT_SHIM_TO_MLC.md) — `text_shaping.mlc` + abi; Steps 5–10 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_SHIM_TO_MLC (**active** 2026-07-12: STEP=1–4 done — abi +
      `text_shaping.mlc` cache/pitch-copy; STEP=5 golden next;
