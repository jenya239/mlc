...
| **13a-3** VM lowering: unary операторы, `if` не в tail-позиции | **open, высокий приоритет** | [TRACK_VM_LOWERING_GAPS](agent/TRACK_VM_LOWERING_GAPS.md) — STEP=1 next (после closed 13a-2) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **VM_LOWERING_GAPS STEP=1 (**next**, высокий приоритет) → STEP=2–4** —
