...
| **18** Package manager | **active** (STEP=1 done; STEP=2 import-path design) | [TRACK_PACKAGE_MANAGER](agent/TRACK_PACKAGE_MANAGER.md) — `mlc.json`+git+SHA frozen; Steps 2–4 design then 5–10 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **PACKAGE_MANAGER (**active** 2026-07-13: STEP=1 design deps model next;
      → [agent/TRACK_PACKAGE_MANAGER.md](agent/TRACK_PACKAGE_MANAGER.md)):**
    Steps 1–4 design (`mlc.json`+git+rev, `.mlc_packages/`) then 5–10 impl
