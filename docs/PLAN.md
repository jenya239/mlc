...
| **16** Integer overflow semantics | **open** | [TRACK_LANG_INT_OVERFLOW](agent/TRACK_LANG_INT_OVERFLOW.md) — STEP=1 **done** (signed debug-panic / release-UB; unsigned wrap; div0 panic); STEP=2 next (`MLC.md`) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_RENDERING STEP=1 (**done** 2026-07-10: §5.1 RawPointer+length view; §5.3 msdf_shim at STEP=7; §5.2 → STEP=5);
    ...
    STEP=4.1 (**done** 2026-07-10: GlyphAtlas shelf pack + smoke);
    STEP=4.2 (**done** 2026-07-10: GlyphCache LRU + smoke);
    STEP=4.3 (**done** 2026-07-10: text shim ASan/UBSan gate);
