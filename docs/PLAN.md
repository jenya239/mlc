...
| **20a** Postgres/Crypto/Tcp — прямой `extern fn` вместо `.hpp`-shim | **open, active** | [TRACK_FFI_SHIM_MIGRATION](agent/TRACK_FFI_SHIM_MIGRATION.md) **active** STEP=3 (Postgres MLC+abi done; Crypto next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → FFI_SHIM_MIGRATION (**active** STEP=3 — Crypto after Postgres MLC+abi
