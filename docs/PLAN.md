...
| **30** | HarfBuzz/FreeType шимы: §8 «без hand-written C++» пропустил их — face/font handle-кеш и pitch-copy loop остаются ручным C++ | **active** (STEP=2 done; STEP=3 harfbuzz_abi) | [TRACK_TEXT_SHIM_TO_MLC](agent/TRACK_TEXT_SHIM_TO_MLC.md) — `freetype_abi` shipped; Steps 3–10 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_SHIM_TO_MLC (**active** 2026-07-12: STEP=1–2 done — Decision +
      `freetype_abi` (raw pitch slot); STEP=3 harfbuzz_abi next;
