...
| **13a-2** MIR block-id collision на вложенном `if` (VM зависает) | **open, КРИТИЧЕСКИЙ, разбирать первым** | [TRACK_VM_BLOCK_ID_COLLISION](agent/TRACK_VM_BLOCK_ID_COLLISION.md) — найдено 2026-07-10; root cause подтверждён `--dump-mir` (дублирующийся block id), фикс — одна строка `compiler/mir/lower_fn.mlc:780`, не закоммичен; только `mlcc --run`/VM, обычная компиляция (g++) не задета |
| **13a-3** VM lowering: unary операторы, `if` не в tail-позиции | **open, высокий приоритет** | [TRACK_VM_LOWERING_GAPS](agent/TRACK_VM_LOWERING_GAPS.md) — после 13a-2 |
| **13b** `mlcc --run` stdin (crash fix + `-` convention) | **open, высокий приоритет** | [TRACK_CLI_STDIN](agent/TRACK_CLI_STDIN.md) — STEP=1 **done** (streambuf in `file.hpp`), STEP=2 next (`read_all` builtin); note PLAN queue: after [TRACK_VM_BLOCK_ID_COLLISION](agent/TRACK_VM_BLOCK_ID_COLLISION.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **VM_BLOCK_ID_COLLISION STEP=1 (**next, КРИТИЧЕСКИЙ**) → STEP=2–5** —
    elif-цепочка/вложенный `if` внутри ветки другого `if` вешает VM
    (дублирующийся block id, найдено `--dump-mir`); root cause + однострочный
    фикс готовы, не закоммичены; только `--run`, не обычная компиляция
  ...
  → CLI_STDIN STEP=1 (**done** 2026-07-10: streambuf `file.hpp`; `/dev/stdin`
    ...
    `module_loader.mlc`; маленький, изолированный)
