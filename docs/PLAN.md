...
| **20b** MSDF (EDT/SDF) алгоритм — порт на MLC | **open, active** | [TRACK_TEXT_MSDF_TO_MLC](agent/TRACK_TEXT_MSDF_TO_MLC.md) **active** STEP=4 (STEP=3 done: `msdf.mlc`; golden MAE next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_MSDF_TO_MLC (**active** STEP=4 — MLC EDT port done; golden MAE vs C++ next;
