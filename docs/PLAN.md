...
| **32** | `text_ide_panels_demo` ~72ms/frame — `GlyphCache.glyph_cache_get` O(n) scan+rebuild на каждый глиф | **active** (STEP=1 done 14.4s; STEP=2 layout cache next) | [TRACK_TEXT_GLYPH_CACHE_SCALING](agent/TRACK_TEXT_GLYPH_CACHE_SCALING.md) — HashMap+FIFO; residual = reshape |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_GLYPH_CACHE_SCALING (**active** 2026-07-13: STEP=1 done (21.65→14.40s);
