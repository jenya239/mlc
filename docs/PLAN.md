...
| **10b** GUI framework (layout/widgets/easing) | **open, active** | [TRACK_GUI_FRAMEWORK](agent/TRACK_GUI_FRAMEWORK.md) **active** STEP=4 (GLFW input); layout+hit-test done |
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET/POSTGRES/CRYPTO/WEBSOCKET/JOB_QUEUE/ENV_LOGGING/VALIDATION **closed**. Active Driver: [TRACK_GUI_FRAMEWORK](agent/TRACK_GUI_FRAMEWORK.md) STEP=4 |
