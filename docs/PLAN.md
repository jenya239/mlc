...
| **44** | Codegen: eliminate string-concatenation, CppAST only (audit 2026-07-17 — §1/§7 "0% string bridges" not reached; `GenStmtsResult`/`GenModuleOut` string-typed by design, not edge cases) | **active** (2026-07-17) STEP=1/test-fix/2/3/4 **done**; STEP=5 next | [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CODEGEN_CPPAST_ONLY (§44, **active**; STEP=1/test-fix/2/3/4 **done**;
      Tier A/B: no Ruby rebuild — `dev_gate_fast` green; STEP=5 next;
...
| Codegen: строки vs CppAST | Аудит 2026-07-17: CppAST — только листовые expressions; `GenStmtsResult.parts` уже `[Shared<CppStatement>]` (STEP=4); `GenModuleOut` и `gen_expr`/`gen_stmts_str` ещё string. См. [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) | 0% string bridges |
