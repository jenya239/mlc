...
| **10c** Масштабируемый canvas со связями (Figma/blueprint-подобное поле) | **open, design-only, низкий приоритет** | [TRACK_GUI_CANVAS_GRAPH](agent/TRACK_GUI_CANVAS_GRAPH.md) — другая архитектура, не расширение v0 (camera/zoom, spatial culling, instancing, retained scene graph); активировать явной командой, когда GUI_FRAMEWORK v1 актуален |
...
| **20a** Postgres/Crypto/Tcp — прямой `extern fn` вместо `.hpp`-shim | **open, active** | [TRACK_FFI_SHIM_MIGRATION](agent/TRACK_FFI_SHIM_MIGRATION.md) **active** STEP=2 (Decision locked; Postgres RawPointer next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → FFI_SHIM_MIGRATION (**active** STEP=2 — Postgres RawPointer after Decision
