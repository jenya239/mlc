...
| **8c** Слить `Tcp` + `spawn` в одном компиляторе (многопоточный HTTP-сервер целиком на MLC) | **open, максимальный приоритет** | [TRACK_PIPELINE_MERGE_TCP_SPAWN](agent/TRACK_PIPELINE_MERGE_TCP_SPAWN.md) **open** STEP=4 (Tcp mlcc echo OK; Tcp+spawn e2e) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **PIPELINE_MERGE_TCP_SPAWN (open STEP=4 — Tcp mlcc echo OK; Tcp+spawn e2e;
  ...
  → STDLIB_WEBSOCKET (**open** STEP=2 — paused until PIPELINE_MERGE closes
  ...
  → TEXT_RENDERING_NATIVE (open, средний приоритет, личный проект пользователя,
      ...
      GLFW-окно; демо с разным текстом + анимация; фундамент будущего
      GUI-фреймворка (не открывать тот трек сейчас)
      → [agent/TRACK_TEXT_RENDERING_NATIVE.md](agent/TRACK_TEXT_RENDERING_NATIVE.md))
