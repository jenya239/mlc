...
| **22** | Дублирующийся `extern fn ... from "<header>"` в графе импортов → clang error вместо диагностики mlcc | **active** (2026-07-12) | [TRACK_FFI_EXTERN_DEDUP](agent/TRACK_FFI_EXTERN_DEDUP.md) **active** STEP=2 done (clang enclose-namespace baseline); STEP=3 next — Hybrid impl |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **FFI_EXTERN_DEDUP (active: STEP=2 repro done; STEP=3 Hybrid impl next;
      → [agent/TRACK_FFI_EXTERN_DEDUP.md](agent/TRACK_FFI_EXTERN_DEDUP.md)):**
