...
| **26** | Concurrency test harness T6 (nightly fuzz) + T7 (`TestRuntime` MLC-level) | **active** (2026-07-12) | [TRACK_CONCURRENCY_TEST_HARNESS](agent/TRACK_CONCURRENCY_TEST_HARNESS.md) **active** T1–T5 done; STEP=6 (T6) next |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CONCURRENCY_TEST_HARNESS (active 2026-07-12: T1–T5 done; STEP=6 T6 fuzz nightly next;
      → [agent/TRACK_CONCURRENCY_TEST_HARNESS.md](agent/TRACK_CONCURRENCY_TEST_HARNESS.md)):**
    Layer 4 seed fuzz + corpus; then T7 Decision (MLC vs C++-only)
