...
| **21** | Compile-smoke coverage для `misc/examples`/`misc/gui` (regression молчала на `gui_button_demo.mlc`) | **active** (2026-07-12) | [TRACK_EXAMPLES_CI](agent/TRACK_EXAMPLES_CI.md) **active** STEP=7 done (cluster A imports); STEP=8 cluster B next |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **EXAMPLES_CI (active 2026-07-12: STEP=3 inventory ok=77 fail=30;
      STEP=7 fix cluster A next;
