...
| **10b** GUI framework (layout/widgets/easing) | **open, active** | [TRACK_GUI_FRAMEWORK](agent/TRACK_GUI_FRAMEWORK.md) **active** STEP=2 (extract `misc/gui/` modules); Decision locked IM+Button |
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET/POSTGRES/CRYPTO/WEBSOCKET/JOB_QUEUE/ENV_LOGGING/VALIDATION **closed**. Active Driver: [TRACK_GUI_FRAMEWORK](agent/TRACK_GUI_FRAMEWORK.md) STEP=2 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → GUI_FRAMEWORK (**active** STEP=2 — extract `misc/gui/` GlRenderer/TextRenderer
      → [agent/TRACK_GUI_FRAMEWORK.md](agent/TRACK_GUI_FRAMEWORK.md))
