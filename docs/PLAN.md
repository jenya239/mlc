...
| **46** | Editor UX backlog — #1…#1d+#2+#3+#4 done; next **#5 `EDITOR_SHIFT_CLICK_EXTEND`** (Planner pick-next) | **active** (2026-07-18) #4 closed; #5 pending | [TRACK_EDITOR_UX_BACKLOG](agent/TRACK_EDITOR_UX_BACKLOG.md) · [archive/tracks/TRACK_EDITOR_MOUSE_WORD_LINE_SELECT](archive/tracks/TRACK_EDITOR_MOUSE_WORD_LINE_SELECT.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **EDITOR_UX_BACKLOG (§46, **active**; #1…#1d+#2+#3+#4 closed; next #5 `EDITOR_SHIFT_CLICK_EXTEND`;
