...
| **30** | HarfBuzz/FreeType шимы: §8 «без hand-written C++» пропустил их — face/font handle-кеш и pitch-copy loop остаются ручным C++ | **active** (STEP=1 done; STEP=2 freetype_abi) | [TRACK_TEXT_SHIM_TO_MLC](agent/TRACK_TEXT_SHIM_TO_MLC.md) — Decision frozen (i64 abi + `*_byte_at`); Steps 2–10 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_SHIM_TO_MLC (**active** 2026-07-12: STEP=1 Decision frozen — i64
      abi + `ft_glyph_byte_at` (no RawPointer pitch); STEP=2 freetype_abi next;
