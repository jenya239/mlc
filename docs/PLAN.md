...
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET_SERVER + POSTGRES **closed**. **[TRACK_STDLIB_CRYPTO](agent/TRACK_STDLIB_CRYPTO.md) open** STEP=4 (gate; `crypto.mlc` done) |
