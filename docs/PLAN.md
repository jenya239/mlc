...
| **44** | Codegen: eliminate string-concatenation, CppAST only (audit 2026-07-17 — §1/§7 "0% string bridges" not reached; `GenStmtsResult`/`GenModuleOut` string-typed by design, not edge cases) | **active** (2026-07-17) STEP=1/test-fix/2/3/4/5/6/7a–7b3c5/8a/8b1/8b2 **done**; STEP=8b3 next | [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CODEGEN_CPPAST_ONLY (§44, **active**; STEP=1/test-fix/2/3/4/5/6/7a–7b3c5/8a/8b1/8b2 **done**;
      Tier A/B: no Ruby rebuild — `dev_gate_fast` green; STEP=8b3 next;
...
| Codegen: строки vs CppAST | Decl Fragments gone; `expr.mlc` 71 exports (`cpp_*`+struct/concept → type_gen) → 8b3/8c. См. [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) | 0% string bridges |
