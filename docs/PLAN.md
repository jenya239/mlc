...
| **13a** MIR VM crash на >~1500 шагов (trampoline fix) | **open, КРИТИЧЕСКИЙ приоритет** | [TRACK_VM_TRAMPOLINE](agent/TRACK_VM_TRAMPOLINE.md) — STEP=1–3 **done** (trampoline + corpus + 100k depth gate), STEP=4 next (re-bench); root cause: `vm_run_frames` host recursion per MIR step |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **VM_TRAMPOLINE STEP=1–3 (**done** 2026-07-10: trampoline + corpus
    + `vm_deep_loop` 100k / depth gate ~0.34s @ ulimit -s=8192) → STEP=4
    (**next** — re-bench) → STEP=5** — host loop replaces per-step
