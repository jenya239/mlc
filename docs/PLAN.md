...
| **32** | `text_ide_panels_demo` ~72ms/frame — `GlyphCache` O(n) + per-frame reshape | **done** (2026-07-13) Critic OK; STEP=1–3; corpus PASS. **GUI restart 2026-09-20** (editor product failed; this is the last stable fast-many-glyph OpenGL point) | [TRACK_TEXT_GLYPH_CACHE_SCALING](archive/tracks/TRACK_TEXT_GLYPH_CACHE_SCALING.md) — HashMap+FIFO + layout cache; `c323556f`…`14972c49`; [archive/EDITOR_EXPERIMENT_FAILED.md](archive/EDITOR_EXPERIMENT_FAILED.md) |
| **33** | Native code editor MVP + mlc-support preflight; editor-first GUI architecture frozen | **failed experiment** (2026-09-20) — product (`misc/editor/`, `demo_live`) abandoned; MVP close 2026-07-14 is historical only. Do not reopen. Restart from §32 | [EDITOR.md](EDITOR.md) + [GUI_ARCHITECTURE.md](GUI_ARCHITECTURE.md) + [TRACK_EDITOR_MVP](archive/tracks/TRACK_EDITOR_MVP.md) **closed**; [archive/EDITOR_EXPERIMENT_FAILED.md](archive/EDITOR_EXPERIMENT_FAILED.md) |
...
| **109** (authorized 2026-08-03/04 as **queue head** — user hard stop on editor lag; «хватит запускать, запланируй большой трек…»; suspends §103) | **Editor perf — audit / fix / harden.** Inventory H1–H15 (wake-on-hover 32% CPU over text on `demo_live.mlc`, false-green §108d cpu=0, PERF_FULL ~1.5s/frame, glyph/spans/snapshot/tree/minimap/startup). Strict sub-tracks §109a baseline → §109b wake-on-hover → §109c gate honesty → §109d content-frame budget → §109e glyph layer → §109f spans visible-only → §109g snapshot edit coverage → §109h chrome tree visible → §109i minimap sample → §109j startup open → §109k regression suite. Measure via scripts only (no manual launch-as-verify). Hard stop: no §103f+ / §104 W2 until epic Critic-closed | **CLOSED** 2026-08-05 — §109a–k Critic OK; epic dogfood gate ×2 audited | [agent/TRACK_EDITOR_PERF_DOGFOOD](agent/TRACK_EDITOR_PERF_DOGFOOD.md) |
| **110** (authorized 2026-08-04 as **path**, user: «не прошу сразу, главное путь»; STEPs blocked until §109 Critic-closed) | **Editor frame / paint architecture path** — destination: dirty generations → layout → paint list → batch/stream GL (not per-widget GL / O(doc) idle). Phases: 0 finish §109 → A Xvfb isolate → B frame ownership → C paint list → D batch/stream → E glyph damage residual → F optional overlay → G docs handoff. Harness-first; dogfood gate stays green. After §109: this ahead of §103f+ / §104 W2 under standing perf/arch priority | **CLOSED** 2026-08-06 — §110a–g Critic OK; epic handoff complete | [agent/TRACK_EDITOR_FRAME_ARCHITECTURE](agent/TRACK_EDITOR_FRAME_ARCHITECTURE.md) |
| **111** (authorized 2026-08-06, user: faster but crooked; why demo_live; agents need clear non-overlapping UI — study first) | **Editor layout integrity / product shell** — geometry contract for agents: `EditorShellLayout` + partition/no-overlap L1 + live one-map wire + bleed scenarios + rename `demo_live`→product entry + agent checklist. Not paint rewrite (§110 done). Phases §111a0 scroll/content_dirty (**CLOSED**) → §111a1 S1+S3 clamp/ensure note + glyph adjust_x (**CLOSED** 2026-08-07) → §111a study → b partition → c live wire → d bleed → e rename → f gate | **aborted** 2026-09-20 — editor product failed experiment; do not resume §111. Restart from §32 | [agent/TRACK_EDITOR_LAYOUT_INTEGRITY](agent/TRACK_EDITOR_LAYOUT_INTEGRITY.md); [archive/EDITOR_EXPERIMENT_FAILED.md](archive/EDITOR_EXPERIMENT_FAILED.md) |
| **112** (authorized 2026-09-20, user: new GUI in a separate folder, file manager first, follow Cairo playground, 100% MLC, STEP=0 via mlc-support) | **Text UI kit + file manager** in `misc/textui/`. Substrate = PLAN §32 (`text_renderer` GlyphCache + `text_shaping` + GLFW/GL). Contracts from `ruby-playground-2026` `experiments/cairo_ui` (retained, ListView as data, overflow=hit+paint, focus≠selection). Not `misc/editor/`. STEP=0 Opus map **done**; STEP=1 slice 0 **done** 2026-09-20 (`run_textui_slice0_smoke.sh`); STEP=2 = 01-label | **open** STEP=2 | [agent/TRACK_TEXTUI_FILE_MANAGER](agent/TRACK_TEXTUI_FILE_MANAGER.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **Native editor §33–§111 FAILED EXPERIMENT (2026-09-20).** Do not
      resume `misc/editor/` / `demo_live`. GUI restart = §32 text stack
      (`c323556f`…`14972c49`, `text_ide_panels_demo` + HashMap GlyphCache).
      → [archive/EDITOR_EXPERIMENT_FAILED.md](archive/EDITOR_EXPERIMENT_FAILED.md)
  → **TEXTUI_FILE_MANAGER (§112, user-authorized 2026-09-20) STEP=2**
      01-label (`TextLayout` measure + column). Slice 0 **done**
      (`scripts/run_textui_slice0_smoke.sh`).
      → [agent/TRACK_TEXTUI_FILE_MANAGER](agent/TRACK_TEXTUI_FILE_MANAGER.md)
  → **§104 Wave 2 compiler QUEUE HEAD (§104-6 slice 24 Red done; Green next)**
  ...
  → **EDITOR_FRAME_ARCHITECTURE (§110 — CLOSED 2026-08-06; §110a–g Critic OK
