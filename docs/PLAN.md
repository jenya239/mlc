...
| **20a** Postgres/Crypto/Tcp — прямой `extern fn` вместо `.hpp`-shim | **open, active** | [TRACK_FFI_SHIM_MIGRATION](agent/TRACK_FFI_SHIM_MIGRATION.md) **active** STEP=5 (Tcp fd-as-token+abi done; delete `.hpp` next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → FFI_SHIM_MIGRATION (**active** STEP=5 — delete `.hpp` after Tcp fd-as-token
