...
| **98** | Editor newline auto-indent without full stringify (§93 / #1d residual) | **open** STEP=2 done; Critic STEP=3 next | [agent/TRACK_EDITOR_NEWLINE_INDENT_NO_FULL_STRINGIFY](agent/TRACK_EDITOR_NEWLINE_INDENT_NO_FULL_STRINGIFY.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **EDITOR_NEWLINE_INDENT_NO_FULL_STRINGIFY (§98, open Critic STEP=3 next:
      green via line-start scan + document_byte_slice; [TRACK_EDITOR_NEWLINE_INDENT_NO_FULL_STRINGIFY](agent/TRACK_EDITOR_NEWLINE_INDENT_NO_FULL_STRINGIFY.md))**
```
