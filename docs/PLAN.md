...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CODEGEN_CPPAST_ONLY (§44, **active**; STEP=1/test-fix/2/3/4/5/6/7a/7b1/7b2 **done**;
      Tier A/B: no Ruby rebuild — `dev_gate_fast` green; STEP=7b3 next;
...
| Codegen: строки vs CppAST | `gen_expr`/`gen_stmts_str`/`GenModuleOut`/guarded-match body — AST; per-arm guarded Fragment + FFI/GenStmtResult/JSON → 7b3. См. [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) | 0% string bridges |
