...
| **20d** Env/Log/Validation — mlcc-пайплайн + порт логики | **open, active** | [TRACK_STDLIB_LOGIC_TO_MLC](agent/TRACK_STDLIB_LOGIC_TO_MLC.md) **active** STEP=3 (STEP=2 done: env.hpp→env_abi; Log MLC next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **STDLIB_LOGIC_TO_MLC (**active** STEP=3 — env.hpp deleted; Log→MLC next;
