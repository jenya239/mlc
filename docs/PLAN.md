...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **EDITOR_HYGIENE (§107, open 2026-08-01 — QUEUE HEAD, authorized ahead of §103a/§104 W2;
      ...
      §107a CLOSED; §107b EDITOR_SESSION_ORIGINAL_PATHS (Driver STEP=0 next)
        → §107b EDITOR_SESSION_ORIGINAL_PATHS
        → §107c EDITOR_VISIBLE_ROWS_PREFIX_JUMP
        → §107d EDITOR_PERF_SMOKE_FULL_PATH
        → §107e EDITOR_DOCUMENT_VERSION
        → §107f EDITOR_INCREMENTAL_LINE_INDEX
        → §107g EDITOR_TERMINAL_TEARDOWN
        → §107h EDITOR_SHAPE_SEGMENT_BUDGET
        → §107i EDITOR_SPANS_TICK_UNDER_LAYOUT_SKIP
        → §107j EDITOR_VISIBLE_ROWS_SINGLE_COLLECT
        → §107k EDITOR_TERMINAL_DAMAGE_REPAINT
        → §107l EDITOR_INDENT_REPLACE_NO_STRINGIFY
        → §107m EDITOR_OPEN_SIZE_GUARD
        → §107n EDITOR_PROBE_GEOMETRY_PARITY
        → §107o EDITOR_VIEWPORT_RECT_SINGLE
        → §107p EDITOR_TOOLBAR_COMMAND_TABLE
        → §107q EDITOR_DRAW_OPS
