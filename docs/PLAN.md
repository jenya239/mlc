...
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); [TRACK_STDLIB_NET_SERVER](agent/TRACK_STDLIB_NET_SERVER.md) **open** STEP=1 **done**, STEP=2 next (`tcp.hpp`). Postgres/crypto/WS треки — по мере старта (§5) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **STDLIB_NET_SERVER STEP=1 (**done** 2026-07-10: opaque TcpListener/Stream,
      Result<_,string>, sync handlers, HTTP caps); STEP=2 (**next** — `tcp.hpp`);
