...
| **22** | Дублирующийся `extern fn ... from "<header>"` в графе импортов → clang error вместо диагностики mlcc | **active** (2026-07-12) | [TRACK_FFI_EXTERN_DEDUP](agent/TRACK_FFI_EXTERN_DEDUP.md) **active** STEP=6 done (self-host DIFF=0); STEP=7 next — regression_gate |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **FFI_EXTERN_DEDUP (active: STEP=5 sweep done; STEP=6 self-host next;
