...
| **10b** GUI framework (layout/widgets/easing) | **backlog** | [TRACK_GUI_FRAMEWORK](agent/TRACK_GUI_FRAMEWORK.md) — activate after Critic OK on 10a / Planner promote |
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET/POSTGRES/CRYPTO/WEBSOCKET/JOB_QUEUE/ENV_LOGGING/VALIDATION **closed**. Next after Critic on 10a: Planner / GUI backlog |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → GUI_FRAMEWORK (**backlog** after Critic OK / Planner promote
