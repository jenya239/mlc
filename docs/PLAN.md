...
| **8** Concurrency v2 (Send/Sync, structured concurrency) | **partial** | [CONCURRENCY_V2.md](CONCURRENCY_V2.md); V2/TASKSCOPE/ISOLATE **closed** 2026-07-09. SPAWN_DOUBLE_EXEC **closed**. **Next:** [TRACK_CONCURRENCY_RUBY_PARITY](agent/TRACK_CONCURRENCY_RUBY_PARITY.md) STEP=3 (docs). SUPERVISOR deferred. MVP: [TRACK_CONCURRENCY](archive/tracks/TRACK_CONCURRENCY.md) closed |
...
| **8b** `spawn`/`Mutex`/`Channel` только self-hosted; `Tcp` stdlib только Ruby; `block_on` не в checker | **open** | [TRACK_CONCURRENCY_RUBY_PARITY](agent/TRACK_CONCURRENCY_RUBY_PARITY.md) STEP=1–2 **done** (Decision C; `block_on`/`is_ready`); STEP=3 **next** (MLC.md matrix) |
...
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET_SERVER **closed**. Postgres/crypto **после** [TRACK_CONCURRENCY_RUBY_PARITY](agent/TRACK_CONCURRENCY_RUBY_PARITY.md) design (STEP=1) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → CONCURRENCY_RUBY_PARITY STEP=1 (**done** 2026-07-10: Decision **C**
      document split; A reject; B deferred follow-up);
    STEP=2 (**next** — `block_on`/`is_ready` in self-hosted checker)
  → Postgres/crypto треки (после RUBY_PARITY design) → WebSocket/job-queue/
      config/logging (см. STDLIB_BACKEND.md §5; треки по одному перед стартом)
