...
| **100** | Editor comment toggle without full stringify (§99 / #1d residual) | **open** STEP=2 done; Critic STEP=3 next | [agent/TRACK_EDITOR_COMMENT_TOGGLE_NO_FULL_STRINGIFY](agent/TRACK_EDITOR_COMMENT_TOGGLE_NO_FULL_STRINGIFY.md) |
| **101** (§97a residual — confirmed by code read 2026-07-28; queued after §104 Wave 1 per same-day priority override) | Root cause **confirmed**, not a hypothesis: `visual_row_index_for_caret_pixel_budget` (`layout/word_wrap.mlc:422`), called once per caret every frame from `demo_live.mlc:2212`, walks `line = 0 .. caret_line` calling `visual_rows_for_line_pixel_budget` (a font-glyph-measuring wrap computation) **for every line before the caret, uncached, every single frame** — on the 100k-line perf fixture with caret near end-of-file this is ~100k pixel-budget wrap calls/frame, accounting for essentially all of the measured `draw_us=12063257` over 30 frames (~402ms/frame, ~2.4 FPS). Same disease class as the already-fixed §36 (`document_visual_row_count` full-recompute every frame), different call site that was missed. Fix strategy: extend `EditorFrameLayout`/`wrap_count_cache` (§97b) with a per-line cumulative visual-row prefix table, invalidated on the same text/wrap-column change signal already used by `wrap_count_cache_needs_recompute`, so caret visual-row becomes an O(1)/O(log n) lookup instead of O(caret_line) — do not hand-roll a second, differently-invalidated cache next to it | **open** STEP=0 next (after §100 closes) — do not defer behind further stringify residuals | *(track file not yet created; scope: `layout/wrap_cache.mlc` + `app/frame_layout.mlc` + `demo_live.mlc:2212` call site only)* |
| **102** (authorized 2026-07-28, user: "можно добавить треки по терминалу") | **Editor terminal panel** — libvterm-backed PTY terminal as a new panel/tab inside the existing (post-§97) unified `EditorAppState`/paint path, no parallel widget system. Sub-tracks §102a `TERMINAL_LIBVTERM_FFI` → §102b `TERMINAL_PTY_SPAWN` → §102c `TERMINAL_CELL_GRID_RENDER` → §102d `TERMINAL_INPUT_FORWARD` → §102e `TERMINAL_RESIZE_SCROLLBACK` → §102f `TERMINAL_PANEL_INTEGRATION` → §102g `TERMINAL_PERF_BUDGET`, strict order, each with its own gate (see track file) | **open** — §102a Decision next, queued after §101 | [agent/TRACK_EDITOR_TERMINAL](agent/TRACK_EDITOR_TERMINAL.md) |
| **103** (authorized 2026-07-28, user: "можно добавить треки по... script-vm") | **MLC Script VM, Phase 1** — embeddable dynamic-profile VM (design doc [MLC_SCRIPT_VM.md](MLC_SCRIPT_VM.md)), HARD STOP GATE lifted for Phase 1 only (§12 фазы 2-5 remain design-only, not authorized). Sub-tracks §103a `SCRIPT_VM_VALUE_REP` → §103b `SCRIPT_VM_BYTECODE_FORMAT` → §103c `SCRIPT_VM_VERIFIER` → §103d `SCRIPT_VM_INTERPRETER_ARITHMETIC` → §103e `SCRIPT_VM_CONTROL_FLOW` → §103f `SCRIPT_VM_HEAP_GC_ARENA` → §103g `SCRIPT_VM_ARRAYS_RECORDS` → §103h `SCRIPT_VM_CLOSURES_FIBERS` → §103i `SCRIPT_VM_EMBEDDING_ABI`, strict order (verifier before interpreter, per design doc §10), each with its own gate | **open** — §103a Decision next, queued after §102 | [agent/TRACK_MLC_SCRIPT_VM](agent/TRACK_MLC_SCRIPT_VM.md) |
...
| **104** (authorized 2026-07-28, user: "отдельный трек чисто по улучшению производительности, архитектуре и тестированию"; **PRIORITY override same day**, user: "это должно быть приоритетом сейчас") | **Compiler core architecture/perf/testing hygiene** — execute the 24-step plan from the 2026-06-29 review (`mlc-support/responses/review_20260629_144027.md`), scope `compiler/**`, distinct from editor tracks (§97/§101) and new-feature epics (§102/§103). Confirmed 2026-07-28: **0 of 24 steps ever actioned**; god-files the review flagged have **grown** since — `transform.mlc` 1558→1765, `decl_cpp.mlc` 1119→1666, `match_gen.mlc` 907→1403, `registry.mlc` 870→1060, `infer.mlc` 786→962 lines. Step 17 (`CppInvokedBlock` string→AST) already done independently via §44, not reopened. **Wave 1** (§104-1/2/3 foundation, §104-12/13/14/15/16 god-file splits, §104-18/19/20 build-speed, §104-22/23 tooling) **moved to queue head, ahead of §101/§102/§103** → Wave 2 (§104-6/7/8/9/10/11 MIR-as-layer, stays after §103) → Wave 3 deferred/high-risk (§104-5/21/24, needs re-authorization at pickup) | **open** — §104-1 Decision next, queue head right after §100 closes | [agent/TRACK_COMPILER_ARCHITECTURE_HYGIENE](agent/TRACK_COMPILER_ARCHITECTURE_HYGIENE.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **EDITOR_COMMENT_TOGGLE_NO_FULL_STRINGIFY (§100, open Critic STEP=3 next:
      green landed (slice + local replace); [TRACK_EDITOR_COMMENT_TOGGLE_NO_FULL_STRINGIFY](agent/TRACK_EDITOR_COMMENT_TOGGLE_NO_FULL_STRINGIFY.md))**
  → **COMPILER_ARCHITECTURE_HYGIENE Wave 1 only (§104, PRIORITY override 2026-07-28,
      ...
      open §104-1 Decision next after §100 closes; only §104-1/2/3, §104-12..16,
      §104-18..20, §104-22/23 pulled forward here, Wave 2 (§104-6..11) stays after
  ...
  → **EDITOR_TERMINAL (§102, open §102a Decision next:
      libvterm PTY panel, 7 ordered sub-tracks; [TRACK_EDITOR_TERMINAL](agent/TRACK_EDITOR_TERMINAL.md))**
  ...
  → **COMPILER_ARCHITECTURE_HYGIENE Wave 2 (§104, MIR-as-layer, §104-6..11,
      queued after §103; Wave 3 §104-5/21/24 gated, needs re-authorization)**
```
