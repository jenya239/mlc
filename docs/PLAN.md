...
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); [TRACK_STDLIB_NET_SERVER](agent/TRACK_STDLIB_NET_SERVER.md) **open** STEP=1–4 **done**, STEP=5 next (HTTP parse). Postgres/crypto/WS треки — по мере старта (§5) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **STDLIB_NET_SERVER STEP=1 (**done** 2026-07-10: opaque TcpListener/Stream,
      Result<_,string>, sync handlers, HTTP caps);
    STEP=2 (**done** 2026-07-10: `mlc/net/tcp.hpp` + smoke 12/0);
    STEP=3 (**done** 2026-07-10: `std/net/tcp.mlc` Option+handles);
    STEP=4 (**done** 2026-07-10: MLC echo + C++ client gate);
    STEP=5 (**next** — HTTP/1.1 parse);
