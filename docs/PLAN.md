...
| **30** | HarfBuzz/FreeType шимы: §8 «без hand-written C++» пропустил их — face/font handle-кеш и pitch-copy loop остаются ручным C++ | **active** (STEP=3 done; STEP=4 text_shaping.mlc) | [TRACK_TEXT_SHIM_TO_MLC](agent/TRACK_TEXT_SHIM_TO_MLC.md) — `freetype_abi`+`harfbuzz_abi`; Steps 4–10 |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_SHIM_TO_MLC (**active** 2026-07-12: STEP=1–3 done — Decision +
      `freetype_abi` + `harfbuzz_abi`; STEP=4 `text_shaping.mlc` next;
