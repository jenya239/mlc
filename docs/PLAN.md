...
| **46** | Editor UX backlog — #1…#1d+#2+#3+#4+#5+#6+#7 done; next **#8 `EDITOR_GOTO_LINE`** (Planner pick-next) | **active** (2026-07-18) #7 closed; #8 next | [TRACK_EDITOR_UX_BACKLOG](agent/TRACK_EDITOR_UX_BACKLOG.md) · [archive/tracks/TRACK_EDITOR_FIND](archive/tracks/TRACK_EDITOR_FIND.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **EDITOR_UX_BACKLOG (§46, **active**; #1…#1d+#2+#3+#4+#5+#6+#7 closed; next #8 `EDITOR_GOTO_LINE`;
