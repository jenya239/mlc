...
| **12** API-клиенты (derive Json, OpenAPI codegen) | **in progress** | [API_CLIENT.md](API_CLIENT.md); [TRACK_API_CLIENT](agent/TRACK_API_CLIENT.md) open, STEP=3 **done** (sum tagged Json derive); STEP=4 **next** (self-hosted derive Json) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → API_CLIENT STEP=1 (**done** 2026-07-09: JsonNumber f64, JsonObject Map);
    ...
    STEP=4 (**next** — self-hosted derive Json)
