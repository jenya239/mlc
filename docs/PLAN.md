...
| **20d** Env/Log/Validation — mlcc-пайплайн + порт логики | **open, active** | [TRACK_STDLIB_LOGIC_TO_MLC](agent/TRACK_STDLIB_LOGIC_TO_MLC.md) **active** STEP=5 (STEP=4 done: validate.hpp→MLC; bare names next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **STDLIB_LOGIC_TO_MLC (**active** STEP=5 — validate.hpp deleted; bare names next;
