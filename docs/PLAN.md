...
Типы, dump, structural verifier, lowering, `--dump-mir` — все 10 шагов done. Продолжение (VM/интерпретатор без g++) — [TRACK_MIR_VM_FULL](agent/TRACK_MIR_VM_FULL.md) (open, Epic 0 A–C **done**, Epic 1 STEP=1 next).
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → MIR_VM_FULL Epic 0 STEP A–C (**done** 2026-07-09: variant ctor → `__mir_variant_new`);
    Epic 1 STEP=1 (**next** — VmVariant typed fields); Epic 1-5 — 150-250 agent-часов, не брать целиком; это НЕ ускорение сборки — интерпретация без g++, 20-80× медленнее исполнения)
