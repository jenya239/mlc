...
| **20b** MSDF (EDT/SDF) алгоритм — порт на MLC | **open, active** | [TRACK_TEXT_MSDF_TO_MLC](agent/TRACK_TEXT_MSDF_TO_MLC.md) **active** STEP=1 (Decision + read msdf_shim 1:1) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **TEXT_MSDF_TO_MLC (**active** STEP=1 — Decision + read msdf_shim.cpp 1:1;
