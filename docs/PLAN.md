...
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET/POSTGRES/CRYPTO/WEBSOCKET/JOB_QUEUE/ENV_LOGGING/VALIDATION/HTTP_MLC/SPAWN_DETACH/MSDF **closed**. Next: «без C++» §20d LOGIC (**active**) / §20e GL_GLAD |
...
| **20d** Env/Log/Validation — mlcc-пайплайн + порт логики | **open, active** | [TRACK_STDLIB_LOGIC_TO_MLC](agent/TRACK_STDLIB_LOGIC_TO_MLC.md) **active** STEP=4 (STEP=3 done: log.hpp→MLC logger; Validate MLC next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **STDLIB_LOGIC_TO_MLC (**active** STEP=4 — log.hpp deleted; Validate→MLC next;
