...
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); [TRACK_STDLIB_NET_SERVER](agent/TRACK_STDLIB_NET_SERVER.md) **open** STEP=1–3 **done**, STEP=4 next (echo gate). Postgres/crypto/WS треки — по мере старта (§5) |
