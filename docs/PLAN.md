...
| **20d** Env/Log/Validation — mlcc-пайплайн + порт логики | **open, active** | [TRACK_STDLIB_LOGIC_TO_MLC](agent/TRACK_STDLIB_LOGIC_TO_MLC.md) **active** STEP=6 (STEP=5 done: bare names; gates+close next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **STDLIB_LOGIC_TO_MLC (**active** STEP=6 — bare names done; mlcc gates+close next;
