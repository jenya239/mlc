...
Codegen строит **CppAST** (`compiler/cpp_ir/cpp_ast.mlc`) и печатает через `cpp_emit/print.mlc`. Модуль `codegen/expr/expr.mlc` удалён (STEP=8c); residual string bridges остаются в stmt/decl/`GenStmtsResult` (STEP=9 closes track). Tracks [TRACK_CPPEXPR.md](archive/tracks/TRACK_CPPEXPR.md) + [TRACK_CPPGEN.md](archive/tracks/TRACK_CPPGEN.md) **closed** (2026-05).
...
| **44** | Codegen: eliminate string-concatenation, CppAST only (audit 2026-07-17 — §1/§7 "0% string bridges" not reached; `GenStmtsResult`/`GenModuleOut` string-typed by design, not edge cases) | **active** (2026-07-17) STEP=1/test-fix/2/3/4/5/6/7a–7b3c5/8a/8b1–8b13/8c **done**; STEP=9 next | [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CODEGEN_CPPAST_ONLY (§44, **active**; STEP=1/test-fix/2/3/4/5/6/7a–7b3c5/8a/8b1–8b13/8c **done**;
      Tier A/B: no Ruby rebuild — `dev_gate_fast` green; STEP=9 next;
...
| Codegen: строки vs CppAST | `expr.mlc` deleted; residual string bridges in stmt/decl/`GenStmtsResult`. См. [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) | 0% string bridges |
