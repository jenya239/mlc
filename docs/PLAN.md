...
| **22** | Дублирующийся `extern fn ... from "<header>"` в графе импортов → clang error вместо диагностики mlcc | **active** (2026-07-12) | [TRACK_FFI_EXTERN_DEDUP](agent/TRACK_FFI_EXTERN_DEDUP.md) **active** STEP=3 done (skip emit if imported); STEP=4 next — mismatch diagnostic |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **FFI_EXTERN_DEDUP (active: STEP=3 skip-emit done; STEP=4 mismatch next;
