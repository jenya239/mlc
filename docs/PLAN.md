...
| **11** Stdlib для backend-приложений (TCP/HTTP сервер, Postgres, crypto, WS, job queue) | **partial** | [STDLIB_BACKEND.md](STDLIB_BACKEND.md); NET_SERVER + POSTGRES **closed**. **[TRACK_STDLIB_CRYPTO](agent/TRACK_STDLIB_CRYPTO.md) open** STEP=5 (docs/close; gate OK) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → STDLIB_CRYPTO (**open** STEP=5 — docs/example/close; gate OK
