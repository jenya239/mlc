...
| **44** | Codegen: eliminate string-concatenation, CppAST only (audit 2026-07-17 — §1/§7 "0% string bridges" not reached; `GenStmtsResult`/`GenModuleOut` string-typed by design, not edge cases) | **active** (2026-07-17) STEP=1/test-fix/2/3/4/5/6/7a/7b1/7b2/7b3a/7b3b/7b3c1 **done**; STEP=7b3c2 next | [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **CODEGEN_CPPAST_ONLY (§44, **active**; STEP=1/test-fix/2/3/4/5/6/7a/7b1/7b2/7b3a/7b3b/7b3c1 **done**;
      Tier A/B: no Ruby rebuild — `dev_gate_fast` green; STEP=7b3c2 next;
...
| Codegen: строки vs CppAST | `gen_expr`/`gen_stmts_str`/`GenModuleOut`/guarded-match/`#line`/`GenStmtResult`/FFI binder — AST; deleter/per-arm/JSON → 7b3c2–7b3c5. См. [TRACK_CODEGEN_CPPAST_ONLY](agent/TRACK_CODEGEN_CPPAST_ONLY.md) | 0% string bridges |
