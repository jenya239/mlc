...
| **13a-3** VM lowering: unary операторы, `if` не в tail-позиции | **open, высокий приоритет** | [TRACK_VM_LOWERING_GAPS](agent/TRACK_VM_LOWERING_GAPS.md) — STEP=1–3 **done**, STEP=4 next (verify-gate + close) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **VM_LOWERING_GAPS STEP=1 (**done** 2026-07-10: `MirRvalueUnary` + eval);
    ...
    STEP=3 (**done** 2026-07-10: if-as-rvalue shared local + continue)
    → STEP=4 (**next** — verify-gate + close)** —
