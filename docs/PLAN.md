...
| **20e** GL-вызовы через GLAD2, без ручного C++ dispatch | **open, active** | [TRACK_GL_GLAD_MIGRATION](agent/TRACK_GL_GLAD_MIGRATION.md) **active** STEP=5 (gui on glad; delete dispatch/shim next) |
...
PARSE_PROGRAM_RESULT → CODE_QUALITY → FORMATTER → PHASE26_REMAINING
  ...
  → **GL_GLAD_MIGRATION (**active** STEP=4 — glad linked; retarget misc/gui next;
