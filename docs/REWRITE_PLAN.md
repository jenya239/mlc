...
- `compiler/out/` — артефакты модульной сборки mlcc (C++ и бинарник): **в git трекаются** как снимок bootstrap; править вручную не следует, обновлять только скриптом `compiler/build.sh` после изменений в `compiler/`.
