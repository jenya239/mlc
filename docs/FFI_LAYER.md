...
| Бизнес-логика/алгоритм, написанный на C++ вместо MLC (не биндинг ни к чему) | `text/msdf_shim.cpp` (EDT/SDF-алгоритм); WebSocket protocol **ported** 2026-07-11 ([TRACK_STDLIB_WEBSOCKET_TO_MLC](archive/tracks/TRACK_STDLIB_WEBSOCKET_TO_MLC.md) closed — residual thin `websocket_bridge.hpp` handles/stubs) | **портировать на MLC целиком**, без FFI вообще для этой части |
