...
кода моделью. **Повторная верификация на `compiler/out/mlcc` (2026-07-06):**
все H1–H12 проверены или закрыты; открытый хвост — только теоретический
UTF-8+`\0` (H6, low priority).
...
| H3 (тут) | диагностика вызова не-функции (E002) | **подтверждено на бинаре** — `42()` → E002, `test_checker.mlc` |
| H4 (тут) | тернарник vs if-lambda, generic-инстанс | **подтверждено на бинаре** — E015 на `Ok(Foo<i32>)` vs `Ok(Foo<i64>)` |
| H5 (тут) | суффиксы числовых литералов неполные | **подтверждено на бинаре** → `docs/mlc2.md` H3 |
| H6 (тут) | `\0` в строках: `length()` vs `bytesize` | **ASCII OK** (`"a\0b"` → len 3); UTF-8+null — deferred |
| H7 (тут) | постфиксный `if`/`unless` отсутствует | **подтверждено на бинаре** → `docs/mlc2.md` H4 |
| H8 (тут) | if-lambda vs тернарник (ctor vs var) | **подтверждено** — if/else, валидный C++; cosmetic |
| H9 (тут) | пустой `[]` вне coercion | **подтверждено на бинаре** — typed `[]` OK; untyped `[]+[1]` → E005 |
| H10 (тут) | приоритет пайпа относительно сравнений | ложный сигнал — модель сама себя опровергла, паритет соблюдён |
| H11 (тут) | generic sum ctor/match/i32 widen | **подтверждено на бинаре** → `docs/mlc2.md` H11 |
...
**нет** для `42()` / `i32`-callee: mlcc `error[E002]: called value is not a function (got i32)` (`--check-only`, 2026-07-06). Unit: `compiler/tests/test_checker.mlc`. При `TUnknown` callee оба компилятора подавляют вторичную ошибку — паритет, не баг.
...
**нет** для краевого кейса аудита (2026-07-06): `Ok(left)` / `Ok(right)` при
`Foo<i32>` vs `Foo<i64>` — checker `error[E015]: if branch types differ` до
codegen; silent-wrong тернарник не возникает. Unit: `test_checker.mlc`.
Cosmetic: mlcc использует if-lambda вместо тернарника при разных ctor-именах —
валидный C++, не баг.
...
**нет для ASCII** (2026-07-06): `"a\0b"` → `mlc::String("a\0b", 3)`, runtime
`length()` = 3. mlcc всегда передаёт длину (Ruby — только при `\0`); для ASCII
эквивалентно. Теоретический риск: UTF-8 символ + `\0`, если `length()` ≠
byte count — не воспроизведён, low priority.
...
**принято (cosmetic)**: `if c then Ok(1) else value` → if/else в C++, не тернарник;
код корректен, линкуется (2026-07-06). Ruby может эмитить тернарник — форма
отличается, семантика та же.
...
**нет** для покрытых кейсов (2026-07-06): `let x: [i32] = []`, `fn f() -> [i32] =
[]`, `g([])` — compile+link OK. Untyped `[] + [1]` → `error[E005]` (не silent
wrong). Unit: `test_checker.mlc`.
