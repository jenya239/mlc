...
| Env config + logging | `getenv` обёртка, structured log (json lines) | **open** — [TRACK_STDLIB_ENV_LOGGING](agent/TRACK_STDLIB_ENV_LOGGING.md) STEP=5 |
...
6. **Env config + logging** — **[TRACK_STDLIB_ENV_LOGGING](agent/TRACK_STDLIB_ENV_LOGGING.md)
   open** STEP=5. Gate `run_env_log_gate.sh` OK; docs+close next.
