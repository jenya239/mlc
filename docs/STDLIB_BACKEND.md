...
| WebSocket | `runtime/include/mlc/net/websocket.hpp` | **runtime** (2026-07-10): upgrade + text frames; smoke `run_websocket_runtime_smoke.sh`. MLC module — STEP=3 [TRACK_STDLIB_WEBSOCKET](agent/TRACK_STDLIB_WEBSOCKET.md) |
...
| WebSocket | upgrade handshake, frame parsing поверх TCP сервера | **partial** — runtime done; MLC STEP=3 [TRACK_STDLIB_WEBSOCKET](agent/TRACK_STDLIB_WEBSOCKET.md) |
...
4. **WebSocket** — **[TRACK_STDLIB_WEBSOCKET](agent/TRACK_STDLIB_WEBSOCKET.md)
   open** STEP=3. Runtime `websocket.hpp` + smoke done; next MLC module.
   Text-only; standalone upgrade on Tcp stream; no WSS.
