...
2. **Postgres driver** — после `FFI_LAYER` close, closed SPAWN_DOUBLE_EXEC,
   **и** после [TRACK_CONCURRENCY_RUBY_PARITY](agent/TRACK_CONCURRENCY_RUBY_PARITY.md)
   STEP=1 design (2026-07-10 Planner). Использует FFI напрямую как первый
   proof-of-concept (см. `FFI_LAYER.md` §5).
