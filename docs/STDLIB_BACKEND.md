...
| Job queue / scheduler | периодические задачи, retry — библиотека поверх `ThreadPool`/`Channel` | **open** — [TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md) STEP=2 (C++-only v1) |
