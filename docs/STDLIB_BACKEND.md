...
| Env config + logging | `getenv` обёртка, structured log (json lines) | **open** — [TRACK_STDLIB_ENV_LOGGING](agent/TRACK_STDLIB_ENV_LOGGING.md) STEP=4 |
...
6. **Env config + logging** — **[TRACK_STDLIB_ENV_LOGGING](agent/TRACK_STDLIB_ENV_LOGGING.md)
   open** STEP=4. MLC `Env`/`Log` + runtime; gate next.
