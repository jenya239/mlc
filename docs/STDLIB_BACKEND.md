...
| БД (Postgres) | FFI-биндинг `libpq`, минимальный query API | **open** — [TRACK_STDLIB_POSTGRES](agent/TRACK_STDLIB_POSTGRES.md) STEP=5 |
...
2. **Postgres driver** — **[TRACK_STDLIB_POSTGRES](agent/TRACK_STDLIB_POSTGRES.md)
   open** STEP=4. Runtime + `std/db/postgres` done; next gate.
