...
| БД (Postgres) | FFI-биндинг `libpq`, минимальный query API | **open** — [TRACK_STDLIB_POSTGRES](agent/TRACK_STDLIB_POSTGRES.md) STEP=1 |
...
2. **Postgres driver** — **[TRACK_STDLIB_POSTGRES](agent/TRACK_STDLIB_POSTGRES.md)
   open** STEP=1 (API Decision). FFI PoC: [FFI_LAYER.md](FFI_LAYER.md) §5
   (`PQconnectdb` / `PQexec` / …). Pipeline: Ruby `common/stdlib` + C++ runtime
   (Decision C / Tcp pattern).
