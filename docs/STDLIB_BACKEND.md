...
| Concurrency | `runtime/include/mlc/concurrency/` | `channel.hpp`, `mutex.hpp`, `arc.hpp`, `stop.hpp`, `task_scope.hpp`, `thread_pool.hpp`, `job_queue.hpp`, `isolate.hpp` — фундамент есть (`CONCURRENCY_V2` closed); JobQueue v1 C++-only ([TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md)) |
...
| Job queue | `runtime/include/mlc/concurrency/job_queue.hpp` | **runtime** (2026-07-11): `JobQueue` on `ThreadPool`; gate `run_job_queue_gate.sh`; **no** MLC module — [TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md) STEP=5 |
...
| Job queue / scheduler | периодические задачи, retry — библиотека поверх `ThreadPool`/`Channel` | **partial** — C++ `job_queue.hpp` (no MLC module); gate STEP=4 [TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md) |
...
5. **Job queue/scheduler** — **[TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md)
   open** STEP=4. C++-only documented; no `std/` registry entry; next gate.
