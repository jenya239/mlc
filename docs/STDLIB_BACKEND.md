...
API-таблицы и демо-сниппеты по MLC-модулям — в
[STDLIB_REFERENCE.md](STDLIB_REFERENCE.md) (не дублировать длинные сигнатуры
здесь). Ниже — статус/файлы/ограничения.
...
| HTTP-парсер/роутер (server) | `lib/mlc/common/stdlib/net/http_server.mlc` (`HttpServer`); `Tcp.set_recv_timeout` | **есть, MLC-reachable** (HTTP/1.1; keep-alive / body limit→413 / idle timeout / `serve_static`; no TLS/HTTP/2; no route table / forever drain API). API + demos: [STDLIB_REFERENCE § HttpServer](STDLIB_REFERENCE.md#httpserver). Hardening track: [TRACK_STDLIB_HTTP_HARDENING](archive/tracks/TRACK_STDLIB_HTTP_HARDENING.md) **closed** Critic OK. Shutdown: subsection below |
| БД-драйверы | `runtime/include/mlc/db/postgres.hpp`; `std/db/postgres.mlc` | **есть** (PoC): libpq blocking, Postgres only. API: [STDLIB_REFERENCE § Postgres](STDLIB_REFERENCE.md#postgres) |
| Crypto | `runtime/include/mlc/crypto/sodium.hpp`; `std/crypto/crypto.mlc` | **есть**: SHA-256/HMAC/random/`pwhash`; JWT/TLS — out. API: [STDLIB_REFERENCE § Crypto](STDLIB_REFERENCE.md#crypto) |
| WebSocket | `std/net/websocket.mlc`; thin `websocket_bridge.hpp` | **есть, MLC** (upgrade/frames/handshake); gate `run_websocket_gate.sh`. API: [STDLIB_REFERENCE § WebSocket](STDLIB_REFERENCE.md#websocket). Residual: Ruby `:extern` stubs in bridge ([TRACK_STDLIB_WEBSOCKET_TO_MLC](archive/tracks/TRACK_STDLIB_WEBSOCKET_TO_MLC.md) **closed**) |
...
| Supervisor | `runtime/include/mlc/concurrency/supervisor.hpp` | **есть** C++ v1 (2026-07-12): `RestartPolicy` permanent/transient/temporary; one_for_one; `set_restart_intensity` storm stop; smoke `test_supervisor.cpp` / concurrency smoke + sanitize gate; **no MLC module** (JobQueue-style deferral; [TRACK_CONCURRENCY_SUPERVISOR](archive/tracks/TRACK_CONCURRENCY_SUPERVISOR.md) **closed**) |
| Validation/schema | `std/validate/validate.mlc` (pure MLC; `validate.hpp` deleted) | **есть**: predicates (`non_empty` / `range_i32` / …); gate `run_validate_gate.sh`. API: [STDLIB_REFERENCE § Validate](STDLIB_REFERENCE.md#validate). No derive/JSON Schema |
| Типизированная JSON (де)сериализация / OpenAPI | — | см. [API_CLIENT.md](API_CLIENT.md), [TRACK_API_CLIENT](archive/tracks/TRACK_API_CLIENT.md) **closed** 2026-07-09; MLC Json module: [STDLIB_REFERENCE § Json](STDLIB_REFERENCE.md#json) |
| Structured logging | `std/log/logger.mlc` + thin `log_abi.hpp` (fwrite only; `log.hpp` deleted) | **есть**: JSON lines; gate `run_env_log_gate.sh`. API: [STDLIB_REFERENCE § Log](STDLIB_REFERENCE.md#log) |
| Env/config | `runtime/include/mlc/env/env_abi.hpp`; `std/env/env.mlc` | **есть**: thin getenv; gate `run_env_log_gate.sh`. API: [STDLIB_REFERENCE § Env](STDLIB_REFERENCE.md#env) |

### HTTP accept-loop shutdown (2026-07-12, TRACK_STDLIB_HTTP_HARDENING)

Two demo patterns — **do not conflate** them in docs or tests:

| Pattern | Example | Lifecycle |
|---------|---------|-----------|
| **Forever accept** | `misc/examples/http_server_forever_demo.mlc` | `scope \|s\| { while true { accept…; s.spawn … } }` — the `scope` body never returns during normal operation, so spawned handlers are **not** joined on a clean MLC return path |
| **Bounded accept** | `misc/examples/http_scope_accept_loop_demo.mlc` | Fixed connection count, then accept loop ends and `scope` joins in-flight tasks — suitable for gates/tests |

**What “shutdown” means for the forever pattern today:** process signal
(`SIGINT` / `SIGTERM` / kill) or panic → process exit → the OS closes the
listen socket and open client fds. In-flight `scope.spawn` handlers are
**abandoned** (not joined, no drain window). There is no MLC stdlib API yet
for “stop accepting, finish in-flight requests, then exit.” Prefer the
bounded demo when a test needs deterministic join; prefer forever only for
manual long-running demos.

See also [TRACK_STDLIB_HTTP_HARDENING](archive/tracks/TRACK_STDLIB_HTTP_HARDENING.md),
[STDLIB_REFERENCE § HttpServer](STDLIB_REFERENCE.md#httpserver), and the §1 HTTP
row above for keep-alive / limits / idle timeout / `serve_static` status.
...
CONCURRENCY_V2 (closed) + ThreadPool/Channel (есть)
  ...
  → Supervisor (C++ `Supervisor` v1; MLC module deferred — same closure gap)
...
7. **Validation** — [TRACK_STDLIB_VALIDATION](archive/tracks/TRACK_STDLIB_VALIDATION.md)
   **closed** 2026-07-11; logic ported to MLC in [TRACK_STDLIB_LOGIC_TO_MLC](archive/tracks/TRACK_STDLIB_LOGIC_TO_MLC.md)
   STEP=4 (`validate.hpp` deleted). Gate `run_validate_gate.sh`; mlcc
   `misc/examples/validate_mlc_smoke.mlc`. No derive/JSON Schema.
