...
| Job queue / scheduler | периодические задачи, retry — библиотека поверх `ThreadPool`/`Channel` | **open** — [TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md) STEP=1 |
...
5. **Job queue/scheduler** — **[TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md)
   open** STEP=2. Decision: C++ `JobQueue` on `ThreadPool`; MLC deferred;
   `schedule_after` + fixed retry; `wait_idle`.
