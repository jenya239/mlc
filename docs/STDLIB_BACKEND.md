...
1. **TCP/HTTP-сервер** — наивысший приоритет, критический пробел.
   Трек: [TRACK_STDLIB_NET_SERVER](agent/TRACK_STDLIB_NET_SERVER.md) (**open**,
   STEP=1 API design). Порядок внутри трека: sockets runtime → TCP stdlib →
   echo gate → HTTP/1.1 parse → routing → ThreadPool → docs/close.
