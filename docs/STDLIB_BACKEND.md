...
| HTTP-парсер/роутер/`ThreadPool`-сервер | `http_request.hpp`, `http_router.hpp`, `http_server.hpp` | **C++-only, не MLC-reachable** (уточнено 2026-07-11: `grep` по `lib/mlc/common/stdlib/`+`misc/examples/`+`compiler/` — 0 упоминаний `HttpRequest`/`HttpRouter` вне `.hpp`/`.cpp`). `curl`-совместимый сервер на MLC невозможен до [TRACK_STDLIB_HTTP_MLC](agent/TRACK_STDLIB_HTTP_MLC.md) **open**. TLS/HTTP/2/WS — out of scope |
...
|-----------|-----------|-----------|
| HTTP/TCP сервер | listen/accept, HTTP/1.1 parsing, routing | TCP: **closed** MLC-reachable ([TRACK_STDLIB_NET_SERVER](archive/tracks/TRACK_STDLIB_NET_SERVER.md)/[TRACK_PIPELINE_MERGE_TCP_SPAWN](archive/tracks/TRACK_PIPELINE_MERGE_TCP_SPAWN.md)). HTTP parse/route: C++-only — **open**, [TRACK_STDLIB_HTTP_MLC](agent/TRACK_STDLIB_HTTP_MLC.md) |
