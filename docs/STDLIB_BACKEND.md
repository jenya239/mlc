...
| HTTP/TCP сервер | — | **не существует**: нет `listen`/`accept`/`bind` ни в runtime, ни в stdlib |
...
| Типизированная JSON (де)сериализация / OpenAPI | — | см. [API_CLIENT.md](API_CLIENT.md), [TRACK_API_CLIENT](agent/TRACK_API_CLIENT.md) — отдельный трек, не блокирован FFI_LAYER |
...
| БД (Postgres) | FFI-биндинг `libpq`, минимальный query API | высокий, разблокирован `FFI_LAYER` close |
...
1. **TCP/HTTP-сервер** — наивысший приоритет, критический пробел. Отдельный
   будущий трек `TRACK_STDLIB_NET_SERVER` (не создан, создаётся при старте):
   sockets FFI → минимальный TCP accept-loop → HTTP/1.1 parser → routing →
   интеграция с `ThreadPool`/`Isolate` для конкурентной обработки соединений.
2. **Postgres driver** — после `FFI_LAYER` close, использует его напрямую
   как первый proof-of-concept (см. `FFI_LAYER.md` §5).
