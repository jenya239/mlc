...
| Env config + logging | `getenv` обёртка, structured log (json lines) | **open** — [TRACK_STDLIB_ENV_LOGGING](agent/TRACK_STDLIB_ENV_LOGGING.md) STEP=3 |
...
6. **Env config + logging** — **[TRACK_STDLIB_ENV_LOGGING](agent/TRACK_STDLIB_ENV_LOGGING.md)
   open** STEP=3. Runtime `env.hpp`/`log.hpp` done; stdlib modules next.
