...
| Concurrency | `runtime/include/mlc/concurrency/` | `channel.hpp`, `mutex.hpp`, `arc.hpp`, `stop.hpp`, `task_scope.hpp`, `thread_pool.hpp`, `job_queue.hpp`, `isolate.hpp`, `supervisor.hpp` — фундамент есть (`CONCURRENCY_V2`); JobQueue **closed**; Supervisor C++ v1 **active track** ([TRACK_CONCURRENCY_SUPERVISOR](agent/TRACK_CONCURRENCY_SUPERVISOR.md)) |
...
| HTTP-парсер/роутер (server) | `lib/mlc/common/stdlib/net/http_server.mlc` (`HttpServer`); `Tcp.set_recv_timeout` | **есть, MLC-reachable** (2026-07-11 parse/format; **hardening 2026-07-12**): keep-alive (`http_request_wants_keep_alive` / smoke `run_http_keepalive_smoke.rb`); body limit → `413` (`HttpParseTooLarge`); idle recv timeout via `SO_RCVTIMEO` (`run_http_idle_timeout_smoke.rb`); `serve_static` (traversal/`404`/`Content-Type`, `run_http_static_smoke.sh`); load smoke `run_http_load_smoke.rb` vs forever-demo. Gates also: `run_http_server_parse_smoke.sh` / `run_http_server_curl_gate.sh`. Shutdown story: §1 subsection below. Residual: no `[HttpRoute]` table API; forever accept has no MLC drain API. TLS/HTTP/2 — out of scope. Track: [TRACK_STDLIB_HTTP_HARDENING](archive/tracks/TRACK_STDLIB_HTTP_HARDENING.md) **closed** Critic OK |
...
| Supervisor | `runtime/include/mlc/concurrency/supervisor.hpp` | **есть** C++ v1 (2026-07-12): `RestartPolicy` permanent/transient/temporary; one_for_one; `set_restart_intensity` storm stop; smoke `test_supervisor.cpp` / concurrency smoke; **no MLC module** (JobQueue-style deferral; [TRACK_CONCURRENCY_SUPERVISOR](agent/TRACK_CONCURRENCY_SUPERVISOR.md)) |
| Validation/schema | `std/validate/validate.mlc` (pure MLC; `validate.hpp` deleted) | **есть** (2026-07-11 STEP=4/6): ValidateSuccess; mlcc `run_validate_gate.sh` |
...
| Structured logging | `std/log/logger.mlc` + thin `log_abi.hpp` (fwrite only; `log.hpp` deleted) | **есть** (2026-07-11 STEP=3/6): JSON in MLC; mlcc `run_env_log_gate.sh` |
| Env/config | `runtime/include/mlc/env/env_abi.hpp`; `std/env/env.mlc` | **есть** (2026-07-11 STEP=2/6): thin getenv; mlcc `run_env_log_gate.sh` |
...
See also [TRACK_STDLIB_HTTP_HARDENING](archive/tracks/TRACK_STDLIB_HTTP_HARDENING.md) and the §1 HTTP row above for keep-alive / limits / idle timeout / `serve_static` status.
