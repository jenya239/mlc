...
2. **Postgres driver** — после `FFI_LAYER` close **и** после
   [TRACK_LANG_SPAWN_DOUBLE_EXEC](agent/TRACK_LANG_SPAWN_DOUBLE_EXEC.md)
   (критический `spawn` double-exec; 2026-07-10 Planner). Использует FFI
   напрямую как первый proof-of-concept (см. `FFI_LAYER.md` §5).
