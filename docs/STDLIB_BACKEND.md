...
«без hand-written C++» ([FFI_LAYER.md](FFI_LAYER.md) §8) — HTTP server parse/
route уже на MLC (`HttpServer`); остаток C++ HTTP только в
`websocket_http.hpp` (WS handshake) до [TRACK_STDLIB_WEBSOCKET_TO_MLC](agent/TRACK_STDLIB_WEBSOCKET_TO_MLC.md).
...
| Structured logging | `runtime/include/mlc/log/log.hpp`; `std/log/log.mlc` | **есть** (2026-07-11): JSON lines stderr; MLC `Log`; gate `run_env_log_gate.sh` ([TRACK_STDLIB_ENV_LOGGING](archive/tracks/TRACK_STDLIB_ENV_LOGGING.md) **closed**) |
| Env/config | `runtime/include/mlc/env/env.hpp`; `std/env/env.mlc` | **есть** (2026-07-11): `getenv` get/get_or/has; MLC `Env`; same track/gate as logging |
...
|-----------|-----------|-----------|
| HTTP/TCP сервер | listen/accept, HTTP/1.1 parsing, routing | TCP: **closed** MLC-reachable ([TRACK_STDLIB_NET_SERVER](archive/tracks/TRACK_STDLIB_NET_SERVER.md)/[TRACK_PIPELINE_MERGE_TCP_SPAWN](archive/tracks/TRACK_PIPELINE_MERGE_TCP_SPAWN.md)). HTTP parse/route: **MLC `HttpServer`** — [TRACK_STDLIB_HTTP_MLC](agent/TRACK_STDLIB_HTTP_MLC.md) (docs/verify pending) |
...
1. **TCP/HTTP-сервер** — [TRACK_STDLIB_NET_SERVER](archive/tracks/TRACK_STDLIB_NET_SERVER.md)
   ...
   **HTTP parse/route on MLC** — [TRACK_STDLIB_HTTP_MLC](agent/TRACK_STDLIB_HTTP_MLC.md)
   (2026-07-11): `HttpServer` module + curl demo; public C++ HTTP headers removed.
