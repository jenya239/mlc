...
| Job queue | `runtime/include/mlc/concurrency/job_queue.hpp` | **runtime** (2026-07-11): `JobQueue` on `ThreadPool`; `enqueue`/`schedule_after`/`wait_idle`; **no** `std/*.mlc` (closures) — [TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md) STEP=3 |
...
| Job queue / scheduler | периодические задачи, retry — библиотека поверх `ThreadPool`/`Channel` | **partial** — runtime done; STEP=3 [TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md) |
...
5. **Job queue/scheduler** — **[TRACK_STDLIB_JOB_QUEUE](agent/TRACK_STDLIB_JOB_QUEUE.md)
   open** STEP=3. Runtime `job_queue.hpp` + smoke done; MLC skipped (Decision).
