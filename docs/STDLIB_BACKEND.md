...
| БД (Postgres) | FFI-биндинг `libpq`, минимальный query API | **open** — [TRACK_STDLIB_POSTGRES](agent/TRACK_STDLIB_POSTGRES.md) STEP=2 |
...
2. **Postgres driver** — **[TRACK_STDLIB_POSTGRES](agent/TRACK_STDLIB_POSTGRES.md)
   open** STEP=2. Decision: Option+`i32` handles + `last_error`; `-lpq` for
   consumers; live DB env-gated. FFI PoC: [FFI_LAYER.md](FFI_LAYER.md) §5.
