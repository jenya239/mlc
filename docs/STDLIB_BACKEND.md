...
| WebSocket | — | **в работе** — [TRACK_STDLIB_WEBSOCKET](agent/TRACK_STDLIB_WEBSOCKET.md) open STEP=2 |
...
| WebSocket | upgrade handshake, frame parsing поверх TCP сервера | **open** — [TRACK_STDLIB_WEBSOCKET](agent/TRACK_STDLIB_WEBSOCKET.md) STEP=2 |
...
1. **TCP/HTTP-сервер** — [TRACK_STDLIB_NET_SERVER](archive/tracks/TRACK_STDLIB_NET_SERVER.md)
   ...
   **mlcc merge** — [TRACK_PIPELINE_MERGE_TCP_SPAWN](agent/TRACK_PIPELINE_MERGE_TCP_SPAWN.md)
   (Decision A): bare `Tcp` + `spawn` e2e; examples `tcp_echo_mlcc.mlc` /
...
3. **Crypto** — [TRACK_STDLIB_CRYPTO](archive/tracks/TRACK_STDLIB_CRYPTO.md)
   **closed** 2026-07-10. Runtime `mlc::crypto`; MLC `std/crypto/crypto`; gate
   ...
   JWT — follow-up TRACK.
4. **WebSocket** — **[TRACK_STDLIB_WEBSOCKET](agent/TRACK_STDLIB_WEBSOCKET.md)
   open** STEP=2. Decision: MLC `WebSocket` handles; text-only; standalone
   upgrade on Tcp stream; no WSS.
