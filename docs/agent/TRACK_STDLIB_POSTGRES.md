...
## Status: **open** — STEP=2 next (runtime `postgres.hpp`)
...
| 2 | Runtime: `runtime/include/mlc/db/postgres.hpp` — handle-table free fns over libpq; C++ smoke (compile+link `-lpq`; live query optional). | pending |
