...
## Status: **active** — STEP=5 next (delete dispatch GL / loader_shim)

**Driver 2026-07-11 STEP=4:** `misc/gui/{gl_renderer,text_renderer,input}.mlc`
→ `glad_gl.mlc` + `gl_window.mlc`; expanded `glad_gl_abi`;
`context_begin` calls `gladLoadGL(glfwGetProcAddress)` (+ dual old table for demos);
`GLFW_INCLUDE_NONE`. Smokes: gl_renderer, text_renderer_native, gui_input ok.

**Driver 2026-07-11 STEP=3:** glad `.c` linked in `build_bin.sh`.
**Driver 2026-07-11 STEP=2:** MLC glad extern + thin abi.
**Driver 2026-07-11 STEP=1:** vendored glad trees.
