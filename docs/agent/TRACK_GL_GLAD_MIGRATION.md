...
## Status: **active** — STEP=3 next (link glad `.c` in build)

**Driver 2026-07-11 STEP=2:** `lib/mlc/common/stdlib/gl/glad_{gl,egl,gles2}.mlc`
+ thin `runtime/include/mlc/gl/glad_*_abi.hpp` (mlcc `&symbol` vs GLAD macros).
Smoke `misc/examples/glad_bindings_smoke.mlc` — mlcc emit + g++ `-fsyntax-only` 0.
GLFW context/input + string/scratch + out-param GL still on `glfw_gl_dispatch`.

**Driver 2026-07-11 STEP=1:** Decision locked; vendored
`runtime/third_party/glad/{gl,egl}/` via `glad2==2.0.8`.
