...
## Status: **active** — STEP=4 next (retarget misc/gui to glad)

**Driver 2026-07-11 STEP=3:** `build_bin.sh` links `third_party/glad/gl/src/gl.c`
(+ `-I` + `-ldl`) when glfw3; headless egl/gles2 only if no desktop (symbol clash).
`scripts/run_glad_link_smoke.sh`; gl_renderer smoke still ok.
