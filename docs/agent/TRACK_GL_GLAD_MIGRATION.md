...
## Status: **active** — STEP=1 next (Decision + vendor glad)

**Planner 2026-07-11:** activated after STDLIB_LOGIC Critic OK (`e1ffa13e`).
Last «без C++» §20 queue item (§20e). Verified on disk:
`glfw_gl_dispatch.cpp` 812 / `.hpp` 130; `loader_shim.cpp` **117** /
`.hpp` present (TRACK previously said 232 — corrected); no
`runtime/third_party/glad/` yet; callers in `misc/gui/{gl_renderer,input}.mlc`
via `from "mlc/gl/glfw_gl_dispatch.hpp"`.
...
| MLC-уровень | `extern fn` в GUI MLC (`gl_renderer.mlc` / bindings) — публичные имена вызовов сохраняются где возможно; меняется `from`/`=` на glad | не ломать demos без нужды |
| Линковка | `glad*.c` в object list `build_bin.sh` (не `extern lib`) | lock на STEP=1 |
