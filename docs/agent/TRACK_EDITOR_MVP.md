...
- `ui/theme.mlc`: full Solarized Light palette + `theme_rgb_for_tag`.
- `tests/theme_unit.mlc` + script exit 0.
...
- `commands/bus.mlc`: CommandId, KeyBinding, FocusTree/Editor; default chords; focus filter.
- `tests/command_bus_unit.mlc` + script exit 0 (all listed commands).
...
- `workspace/tab_close.mlc`: Ctrl+W / middle-click request; dirty overlay discard/cancel.
- `tests/tab_close_unit.mlc` + script exit 0.
...
- `document/selection.mlc`: anchor/focus caret model; clamp/select_all; not in renderer.
- `tests/selection_unit.mlc` + script exit 0 (`len=12`).
