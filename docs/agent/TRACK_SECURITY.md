...
| Step | Item | Status |
|------|------|--------|
| 1 | Negative corpus expansion — parser/checker edge `.mlc` files (8→16+) | pending |
...
## Step 1 detail (Driver)

- Dir: `compiler/tests/fuzz/negative_corpus/`.
- Add cases: unclosed block, bad generic arity, orphan `end`, invalid trait syntax, empty type body, duplicate `fn` name.
- `run_negative_corpus.sh`: accept exit 0 or 1 only; no compiler source changes unless repro finds crash.
...
**STEP=1** — Negative corpus expansion.
