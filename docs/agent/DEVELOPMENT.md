...
**Queue head:** Driver STEP=0 Decision `TRACK_EDITOR_RENDER_ARCHITECTURE` §97b (`minimap_cache`→`frame_layout`).
**`TRACK_EDITOR_RENDER_ARCHITECTURE` (§97)** **open** — shared_span_cache slice closed (Critic OK); next Decision `minimap_cache` (then dual-wrap / §97c; §93 after).
