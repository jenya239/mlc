...
**Queue head:** Driver `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-12 slice 4 (`transform_method.mlc`) STEP=0 Decision — slice 3 (`transform_call_args.mlc`) closed 2026-07-28, Critic-audited (independent function/type-set diff, independent fresh mlcc translation + stray-reference grep, independent full `rake test_compiler_mlc` rerun 1471/0). Slice 4 scope (per track file): `transform_result_option_hof_method_call` + `transform_array_hof_method_call` + `transform_regular_method_call` + `transform_extend_method_as_call` + `transform_region_alloc_method_call`, needs `transform_exprs_fn` injected at 3 call sites; `transform_method_call_after_object` stays in `transform.mlc`. §104-1/2/3 found already pre-existing, see track file.
