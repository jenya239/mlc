...
**Queue head:** Planner idle (authorized queue empty after §50).
**`TRACK_EDITOR_IDLE_CARET_BLINK` (§50)** **closed** 2026-07-22 (Critic OK). Archived.
**Idle residuals (deferred, no TRACK):** wrap-budget HarfBuzz rewrite (needs user auth); live idle CPU sample (optional/sandbox-limited).
