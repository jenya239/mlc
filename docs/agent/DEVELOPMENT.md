...
**Queue head:** Driver `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-14 slice 3 Decision — `codegen/expr/match_gen.mlc` split, currently 1185 lines (still above 800, allowlisted). Slice 1 (`match_result_type.mlc`, 173 lines) and slice 2 (`match_arm_lambda.mlc`, 71 lines, 12-item pure C++ lambda-string template builders, zero imports) both **CLOSED** 2026-07-30, Critic-audited independently both times (slice 2: function/type-set diff 81/81 empty, export-status diff exactly the 8 documented, byte-level function-body diff all 12 verbatim, `test_codegen.mlc` empty diff confirming transitive forwarding, fresh mlcc translation + stray-reference/duplicate-symbol checks clean, `rake test_compiler_mlc` exit 0, mlcc2 self-host diff independently rebuilt and IDENTICAL). 1 correction found during slice 1 implementation: a name collision between the newly-exported `match_return_cpp_type` and an unrelated same-named local helper in `expr_visitor_cpp.mlc` triggered a real mlcc codegen bug (mis-qualified the local definition's namespace, duplicate-symbol risk), fixed by renaming the local helper to `match_default_block_return_cpp_type`; explicitly re-checked for zero collisions before slice 2 and none were found. Remaining groups in `match_gen.mlc`: 3 codegen strategies (std::visit string-lambda, std::visit `CppExpression`, guarded if-chain) threading the `gen_stmts`/`eval_expr_fn` injection pattern + a 3-way-shared record/ctor-field-binding helper group — need their own Decision(s) per strategy or shared-helper boundary. §104-12 (`transform.mlc` split) **CLOSED** 2026-07-30, re-confirmed by an independent Critic re-audit same day: 1765→753 lines across 5 modules (`transform_coerce/context/call_args/method/support.mlc`), allowlist entry removed. §104-13 (`codegen/decl_cpp.mlc` split) **CLOSED** 2026-07-29: 6 slices, 1666→355 lines across `decl_cpp.mlc` + 5 new modules (`decl_cpp_helpers/type/trait/fn/ffi/extend.mlc`), all ≤800, allowlist entry removed, all Critic-audited. §104-1/2/3 found already pre-existing, see track file.
