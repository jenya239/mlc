...
**Queue head:** Critic `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-13 slice 3 STEP=3 close — Decision frozen, red confirmed, and green done 2026-07-28: extracted 8-item trait decl codegen group (`gen_trait_decl_cpp`/`trait_concept_dispatch_forward_protos_cpp` + 6 internal helpers) into `decl_cpp_trait.mlc`, relocated `is_semantic_declaration_fn` into `decl_cpp_helpers.mlc` first to avoid a two-way import cycle. `decl_cpp.mlc` 1463→1360 lines. Bootstrap diff scoped to `decl_cpp.cpp/.hpp` + `decl_cpp_helpers.cpp/.hpp` (additive) + 2 new files only, `rake test_compiler_mlc` 1471+/0, mlcc2 self-host diff IDENTICAL. Slice 2 (`decl_cpp_type.mlc`) **closed** 2026-07-28, Critic-audited, `decl_cpp.mlc` 1641→1463 lines. Slice 1 (`decl_cpp_helpers.mlc`) **closed** 2026-07-28, Critic-audited, `decl_cpp.mlc` 1666→1641 lines. §104-13 itself stays open: 3 more groups surveyed after this slice (fn decl codegen, FFI/extern codegen, extend/impl codegen — largest, decl-segment orchestration). §104-12 (`transform.mlc` split) **CLOSED** 2026-07-28: 4 slices, 1765→881 lines, all Critic-audited. §104-1/2/3 found already pre-existing, see track file.
