...
**Queue head:** Driver STEP=0 Decision `TRACK_EDITOR_RENDER_ARCHITECTURE` §97b (`shared_span_cache`→`frame_layout`).
**`TRACK_EDITOR_RENDER_ARCHITECTURE` (§97)** **open** — frame_cache slice closed (Critic OK); next Decision `shared_span_cache` (then minimap / §97c; §93 after).
