...
**Queue head:** Driver `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-13 slice 3 STEP=2 green — Decision frozen and red confirmed 2026-07-28: extract 8-item trait decl codegen group (`gen_trait_decl_cpp`/`trait_concept_dispatch_forward_protos_cpp` + 6 internal helpers) into `decl_cpp_trait.mlc`; needs `is_semantic_declaration_fn` relocated to `decl_cpp_helpers.mlc` first (shared by 3 other groups, would otherwise force a two-way import cycle). Slice 2 (`decl_cpp_type.mlc`) **closed** 2026-07-28, Critic-audited, `decl_cpp.mlc` 1641→1463 lines. Slice 1 (`decl_cpp_helpers.mlc`) **closed** 2026-07-28, Critic-audited, `decl_cpp.mlc` 1666→1641 lines. §104-13 itself stays open: 3 more groups surveyed after this slice (fn decl codegen, FFI/extern codegen, extend/impl codegen — largest, decl-segment orchestration). §104-12 (`transform.mlc` split) **CLOSED** 2026-07-28: 4 slices, 1765→881 lines, all Critic-audited. §104-1/2/3 found already pre-existing, see track file.
