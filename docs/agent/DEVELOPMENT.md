...
**Queue head:** Critic STEP=3 `TRACK_EDITOR_RENDER_ARCHITECTURE` §97b (`shared_span_cache`→`frame_layout`).
**`TRACK_EDITOR_RENDER_ARCHITECTURE` (§97)** **open** — shared_span_cache STEP=2 green done; Critic STEP=3 next (then minimap / §97c; §93 after).
