...
**Queue head:** Critic `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-13 slice 4 STEP=3 close — Decision frozen, red confirmed, green done 2026-07-28: `decl_cpp_fn.mlc` created (18-item fn decl codegen group, 11 exported), `decl_cpp.mlc` 1360→1171 lines, bootstrap diff scoped to the split modules only, `rake test_compiler_mlc` 1471/0, mlcc2 self-host diff identical. Slices 1/2/3 all closed 2026-07-28, Critic-audited (`decl_cpp.mlc` 1666→1641→1463→1360 lines). §104-13 itself stays open: 2 more groups surveyed after this slice (FFI/extern codegen, extend/impl codegen — largest, decl-segment orchestration). §104-12 (`transform.mlc` split) **CLOSED** 2026-07-28: 4 slices, 1765→881 lines, all Critic-audited. §104-1/2/3 found already pre-existing, see track file.
