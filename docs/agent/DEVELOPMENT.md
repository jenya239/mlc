...
**Queue head:** Driver `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-14 slice 4 Decision — `codegen/expr/match_gen.mlc` split, currently 1129 lines (still above 800, allowlisted). Slice 1 (`match_result_type.mlc`, 173 lines) and slice 2 (`match_arm_lambda.mlc`, 71 lines) both **CLOSED** 2026-07-30, Critic-audited independently. Slice 3 (`match_field_binding.mlc`, 65 lines, 3-way-shared record/ctor-field-binding group, 2 of 4 items exported) Driver-done 2026-07-30, awaiting Critic close — no correction needed (checker resolved the relocated `RecordFieldBindAccum` type cleanly via inferred return types). 1 correction found during slice 1 implementation: a name collision between the newly-exported `match_return_cpp_type` and an unrelated same-named local helper in `expr_visitor_cpp.mlc` triggered a real mlcc codegen bug (mis-qualified the local definition's namespace, duplicate-symbol risk), fixed by renaming the local helper to `match_default_block_return_cpp_type`. Remaining groups in `match_gen.mlc`: 3 codegen strategies (std::visit string-lambda, std::visit `CppExpression`, guarded if-chain) threading the `gen_stmts`/`eval_expr_fn` injection pattern + a 2-way-shared generic-ctor-type-argument resolution group (~11 functions, used by the guarded and `CppExpression` strategies only, found during slice 3's survey) — need their own Decision(s) per strategy or shared-helper boundary. §104-12 (`transform.mlc` split) **CLOSED** 2026-07-30, re-confirmed by an independent Critic re-audit same day: 1765→753 lines across 5 modules (`transform_coerce/context/call_args/method/support.mlc`), allowlist entry removed. §104-13 (`codegen/decl_cpp.mlc` split) **CLOSED** 2026-07-29: 6 slices, 1666→355 lines across `decl_cpp.mlc` + 5 new modules (`decl_cpp_helpers/type/trait/fn/ffi/extend.mlc`), all ≤800, allowlist entry removed, all Critic-audited. §104-1/2/3 found already pre-existing, see track file.
