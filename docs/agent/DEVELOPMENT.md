...
**Queue head:** Critic `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-13 slice 2 STEP=3 close — Decision frozen, red confirmed, and green done 2026-07-28: extracted 13-item type/variant struct codegen group (`gen_type_decl_fwd_cpp`/`gen_type_decl_body_cpp` + 11 internal helpers, zero circular dependency, no injection needed) into `decl_cpp_type.mlc`, `decl_cpp.mlc` 1641→1463 lines. Bootstrap diff scoped to `decl_cpp.cpp/.hpp` + 2 new files only, `rake test_compiler_mlc` 1471+/0, mlcc2 self-host diff IDENTICAL. Slice 1 (`decl_cpp_helpers.mlc`) **closed** 2026-07-28, Critic-audited, `decl_cpp.mlc` 1666→1641 lines. §104-13 itself stays open: 4 more groups surveyed (trait/fn decl codegen, FFI/extern codegen, extend/impl codegen — largest, decl-segment orchestration). §104-12 (`transform.mlc` split) **CLOSED** 2026-07-28: 4 slices, 1765→881 lines, all Critic-audited. §104-1/2/3 found already pre-existing, see track file.
