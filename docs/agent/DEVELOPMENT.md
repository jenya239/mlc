...
**Queue head:** Critic `TRACK_COMPILER_ARCHITECTURE_HYGIENE` §104-14 slice 4 close — `codegen/expr/match_gen.mlc` split, currently 1015 lines (still above 800, allowlisted). Slices 1 (`match_result_type.mlc`, 173 lines), 2 (`match_arm_lambda.mlc`, 71 lines), and 3 (`match_field_binding.mlc`, 65 lines) all **CLOSED** 2026-07-30, Critic-audited independently every time. Slice 4 (`match_generic_ctor_type.mlc`, 125 lines, 2-way-shared generic-ctor-type-argument resolution group, 1 of 11 items exported) Driver-done 2026-07-30, awaiting Critic close — also dropped 4 now-fully-unused import names (`type_is_unknown`, `generic_type_name_from_type`, `generic_type_arguments_from_type`, `result_err_type`) from `match_gen.mlc` as part of the move. 1 correction found during slice 1 implementation: a name collision between the newly-exported `match_return_cpp_type` and an unrelated same-named local helper in `expr_visitor_cpp.mlc` triggered a real mlcc codegen bug (mis-qualified the local definition's namespace, duplicate-symbol risk), fixed by renaming the local helper to `match_default_block_return_cpp_type`. Remaining group in `match_gen.mlc`: the 3 codegen strategies (std::visit string-lambda, std::visit `CppExpression`, guarded if-chain) threading the `gen_stmts`/`eval_expr_fn` injection pattern — need their own Decision(s) per strategy. §104-12 (`transform.mlc` split) **CLOSED** 2026-07-30, re-confirmed by an independent Critic re-audit same day: 1765→753 lines across 5 modules (`transform_coerce/context/call_args/method/support.mlc`), allowlist entry removed. §104-13 (`codegen/decl_cpp.mlc` split) **CLOSED** 2026-07-29: 6 slices, 1666→355 lines across `decl_cpp.mlc` + 5 new modules (`decl_cpp_helpers/type/trait/fn/ffi/extend.mlc`), all ≤800, allowlist entry removed, all Critic-audited. §104-1/2/3 found already pre-existing, see track file.
