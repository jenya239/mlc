...
## Status: **active** (2026-07-20) — `#2 CONCURRENCY_SEND_BOUND` active (STEP=0 done → Driver STEP=1)
...
| 5 | `CONCURRENCY_CANCELLATION_WAKES_BLOCKING` | `Channel.recv`/`Channel.send`, `sleep`, `Task.join` wake on cancellation/stop-request and return a `Cancelled` outcome instead of hanging, per §8/§9 — currently no cancellation propagation exists at all | new runtime smoke: a task blocked in `Channel.recv` on an empty channel observes `Cancelled` within one scheduler tick of `stop.request()`/`TaskScope` cancellation, not on channel activity | L | pending |
| 6 | `CONCURRENCY_CHANNEL_RENDEZVOUS_UNBOUNDED` | Allow `Channel[T].bounded(0)` as a synchronous rendezvous handoff (today throws `invalid_argument`); add an explicit `.unbounded()` variant, per §10 | runtime smoke: `bounded(0)` send blocks until a matching recv, no exception; `unbounded()` accepts sends past any fixed capacity | M | pending |
| 7 | `CONCURRENCY_ATOMICS` | `Atomic[Bool/Int32/Int64/UInt64]` with `load/store/exchange/compare_exchange/fetch_add/fetch_sub`, sequentially-consistent only (no `.acquire`/`.release` in v1), per §14 | e2e: concurrent `fetch_add` from N threads yields exact expected sum, TSan-clean | M | pending |
| 8 | `CONCURRENCY_FFI_METADATA` | `extern c` type/fn concurrency attributes (`thread_safe`/`sendable`/`blocking`/`callback_concurrent`/`thread_affine(MainThread)`) + checker enforcement of thread affinity at call sites, per §26/§27 | checker test: calling a `thread_affine(MainThread)`-tagged extern fn from inside `spawn`/`TaskScope.spawn` is a compile error | M | pending |
