...
## Status: **active** (2026-07-20) — `#5 CONCURRENCY_CANCELLATION_WAKES_BLOCKING` active (STEP=1 done → Driver STEP=2)
...
| 5 | `CONCURRENCY_CANCELLATION_WAKES_BLOCKING` | MLC surface for cancel-wake on blocked `Channel.send`/`recv` (C++ `StopToken` path already green) → `Cancelled`, per §8/§9; sleep/socket deferred | MLC smoke: blocked empty-channel `recv(token)` observes `Cancelled` after `stop.request()` / `TaskScope` cancel | L | **active** — [TRACK_CONCURRENCY_CANCELLATION_WAKES_BLOCKING](TRACK_CONCURRENCY_CANCELLATION_WAKES_BLOCKING.md) STEP=1 done → Driver STEP=2 |
