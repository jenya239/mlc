...
**STEP=1** — red harness: MLC `Isolate` / `.start` / `.send` does **not** resolve today
(`E001` / parse fail); document exact failure.
...
| 1 | Red: Isolate MLC missing / not constructible | open |
