...
## Status: **open** STEP=3 (1 done modified, 2 done)
...
| 3 | ccache+PCH experiment: try `CCACHE_SLOPPINESS=pch_defines,time_macros` (+ `CCACHE_DEPEND=1`) with clang `-include-pch`; measure `ccache -s` delta on 2 consecutive `MLCC_OBJ_CLEAN=1` runs of identical content. If still 0 hits — drop PCH for the ccache-enabled dev path, or switch PCH strategy (`g++` `.gch` `-include`, not `-include-pch`) and re-measure | planned |
| 4 | Flip dev-loop default to `MLCC_DEV=1` (`-O0 -g`) in `build.sh`/`build_bin.sh`; require explicit `MLCC_OPT=2` for CI / self-host bootstrap verification / release builds. Update README + `mlcc-self-host-verification.mdc` example commands | planned |
