...
| Strategy (v1) | (1) Headless launch of existing `demo_live` binary (`MLC_GLFW_VISIBLE=0`, fixed fixture e.g. `README.md`). (2) Warm-up, then sample `/proc/<pid>/stat` `utime+stime` over N seconds with no input (min of several windows). (3) Assert CPU% = `100 * delta_jiffies / (HZ * N)` under budget (**&lt;10%**). (4) Paint-only blink: overlay caret via `solid_renderer_flush_over` (no framebuffer clear / reshape). (5) Reuse compile artifact from `run_editor_demo_live_fs_compile` when present; otherwise compile once. SKIP if glfw/font/mlcc missing |
| Primary gate | Red: stable stub has no `/proc` sample. Green: `ux_ok idle_cpu_budget_stable` EXIT=0 with measured min CPU% &lt;10; auto-discovered by `run_ux_gate` |
| Module touch | `scripts/run_ux_idle_cpu_budget_*.sh` + `misc/editor/demo_live.mlc` paint-only path |
| REG | no |
...
| 2 | Implement sample script; green under 10% | **done** — paint-only caret overlay; `/proc` min-of-rounds &lt;10%; `ux_ok idle_cpu_budget_stable` |
