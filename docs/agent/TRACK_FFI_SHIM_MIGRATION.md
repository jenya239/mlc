...
## Status: **active** — STEP=3 next (Crypto)
...
- None blocking STEP=3. Residuals: Option codegen; Ruby `extern type`/`extern
  lib`; module-level mut (table still C++); upgrade i64→RawPointer when Ruby
  parses `extern type`.
