...
- None blocking STEP=4. Residuals: Option codegen; Ruby `extern type`/`extern
  lib`; module-level mut (crypto/postgres error tables still C++); no MLC
  `\xHH` escapes; upgrade i64→RawPointer when Ruby parses `extern type`.
