...
## Status: **active** — STEP=2 next (Postgres RawPointer)
...
| Вопрос | Locked |
|--------|--------|
| `PGconn*`/`PGresult*` | `extern type` + `RawPointer[T]` from `<libpq-fe.h>`; no C++ handle-table |
| Владение | Explicit `PQfinish`/`PQclear` in MLC wrappers for now (`owned` return-marker still deferred in FFI_LAYER). Declare `drop "PQfinish"`/`"PQclear"` on types for when `owned` lands — do not rely on auto-drop in STEP=2 |
| Публичный API | **Freeze** `Option<i32>` handles + `last_error(): string` for Postgres/Crypto/Tcp (gates/demos). Bookkeeping = MLC `Array` of `RawPointer` (token = index+1), not C++ |
| `last_error` | Keep function in public API; implement as MLC module state (no C++ `std::mutex`). Gates are sync/single-threaded; multi-thread safety of last_error = residual, not blocker |
| hex-encode | Pure MLC (`Array<i32>`/`string` → lowercase hex); remove C++ `to_hex` |
| Линковка | `extern lib "pq"` in postgres module; `extern lib "sodium"` in crypto module → `mlc_link_libs.txt` |
...
| File | Change |
|------|--------|
| `lib/mlc/common/stdlib/db/postgres.mlc` | STEP=2: `extern type`/`fn`/`lib "pq"` + MLC handle table |
| `lib/mlc/common/stdlib/crypto/crypto.mlc` | STEP=3: sodium externs + MLC hex + hmac statebytes |
| `lib/mlc/common/stdlib/net/tcp.mlc` | STEP=4: libc socket externs; drop `from "mlc/net/tcp.hpp"` |
| `runtime/include/mlc/{db,crypto,net}/*.hpp` | STEP=5 delete (+ bridges) |
...
- None blocking STEP=2. Residual: `last_error` thread-safety under concurrent
  callers (public API freeze); optional ≤20-LOC sockaddr helper C file only if
  STEP=4 proves libc-only accessors insufficient.
