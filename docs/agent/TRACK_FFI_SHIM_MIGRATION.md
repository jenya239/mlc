...
## Status: **active** — STEP=5 next (delete three `.hpp`)
...
- None blocking STEP=5. Residuals: Option codegen (Tcp still extern Option);
  Ruby `extern type`/`extern lib`; module-level mut; TcpListener/TcpStream +
  `.hpp` until STEP=5 (websocket still needs classes); no MLC `\xHH`.
