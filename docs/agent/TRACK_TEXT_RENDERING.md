...
| 4 | `GlyphAtlas` (shelf bin-pack) + `GlyphCache` (LRU) — чистый MLC, без FFI. Юнит-тесты на packing/eviction (тест-матрикс п.5 выше). **Плюс: ASan/UBSan гейт на shim-код с этого шага** (Test strategy п.1). | **in progress** (2026-07-10: 4.1 GlyphAtlas shelf pack + `run_glyph_atlas_smoke.sh` ok; next 4.2 GlyphCache LRU) |
<!-- sub-steps STEP=4:
