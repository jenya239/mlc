...
## Status: **open** — STEP=0–6 **done**; STEP=7.1 **done**; STEP=7.2 next
...
-->
| 7 | MSDF-генерация (§3.2) + RGB8 atlas page + MSDF шейдер + `RenderMode` переключение по pixel size. Численное сравнение distance-field с эталоном (без GPU для самой генерации). | **partial** (7.1 done: SDF RGB8 shim; 7.2 pending) |
<!-- sub-steps STEP=7:
  1) msdf_shim: FT→EDT SDF→RGB8 API + numerical smoke — done
  2) RGB8 atlas pack of MSDF glyph + MSDF median shader FBO smoke
