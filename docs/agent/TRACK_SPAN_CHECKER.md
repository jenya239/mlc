...
| Step | Item | Status |
|------|------|--------|
| 1 | `checker/check/` — `check.mlc`, `call_argument_unify.mlc` real spans on diagnostics | done (`COMMIT_HASH`) |
| 2 | `checker/transform/trait_param_expand.mlc` — E073 on decl name span | pending |
| 3 | `checker/transform/` — `transform.mlc`, `param_destructure_expand.mlc` audit + fix where span available | pending |
...
**STEP=2** — `trait_param_expand.mlc` E073 decl name span.
