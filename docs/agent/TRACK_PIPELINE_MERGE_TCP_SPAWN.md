...
## Status: **open** — STEP=2 next (named-module resolution in mlcc)
...
- Stdlib root: relative to compiler/repo — prefer path next to `mlcc` or
  env/`--stdlib-root`; STEP=2 must document the concrete rule used in
  `module_loader` / `path_normalize`.
...
- Full stdlib directory scan vs hardcoded `Tcp` table — **deferred to STEP=2
  implementation detail**; acceptance only needs `Tcp` resolving.
