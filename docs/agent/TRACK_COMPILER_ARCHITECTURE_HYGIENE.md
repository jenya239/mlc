...
## Status: **open** — Wave 1 CLOSED; **queue head §104-6 slice 24 Decision frozen** (Lambda→ConstStr + visit_int mop-up; gate LEC=0). Prior: s23 CLOSED (LEC=10); §100 closed 2026-07-28, §104-1/2/3 found already
...
- Harness: `compiler/scripts/mir-coverage_s8_red.sh` → exit 1 `no byte_substring native (Red expected)`
...
#### Critic audit (2026-08-06), §104-6 slice 8 CLOSED

Independent re-run:
- coverage: `lower_error_count=646` (<649); hist clean of byte_substring
- Wiring: whitelist + native (`text.byte_substring`, not `substring`) + runtime
- Sab1: hist clean of byte_substring
- Sab2: LEC drop load-bearing (649→646)
- Red after Green: exit 1 `__mir_string_byte_substring already in VM`
- VM smoke: independent `hello_world` fixture exit 0
- `dev_gate_fast` 1471/0

Residuals: parent open until LEC=0; HOF fold/map deferred; next non-HOF: operand-context=34, `type_is_unknown`=26, `unsupported statement`=19; CppIR `make_*` deferred.

### Slice 9 — I64/U8/Usize literals as operand/rvalue (`ConstStr`)

#### Pre-cut facts (audit 2026-08-06, post-s8)

| Fact | Evidence |
|------|----------|
| Baseline | `mir-coverage`: `lower_error_count=646` `mir_functions=2498` |
| Hist head | `fold=125`, `map=93`, `make_identifier_cpp_expression=49`, `any=36`, **operand-context=34**, `type_is_unknown`=26, `unsupported statement`=19, …, **rvalue-context=5** |
| Operand/rvalue gap | After s5 Char/Float + s7 Match: still missing `SemanticExpressionI64` / `U8` / `Usize` (string-payload literals, same IR shape as Float/Char). Also still missing Lambda/With/RecordUpdate/Extern (deferred) |
| Precedent | Char/Float → `MirOperandConstStr(value)` in both operand+rvalue |
| Deferred | HOF; CppIR `make_*`; Lambda/With/RecordUpdate; LetPattern/Break/Continue; `type_is_unknown` (needs Type as VM value) |
...
#### Critic audit (2026-08-06), §104-6 slice 9 CLOSED

Independent re-run:
- coverage: `lower_error_count=646` (≤646); operand=34; rvalue=5 — **Decision correction confirmed** (zero residual drop)
- Wiring: I64/U8/Usize → `MirOperandConstStr` / `MirRvalueUse(ConstStr)` in operand+rvalue; Char/Float/Match intact
- Sab1: LEC no regression; hist unchanged proves residual was never these literals
- Sab2: arms are ConstStr (not empty stubs)
- Red after Green: exit 1 `operand already has I64/U8/Usize arms`
- VM smoke: independent `100i64`/`1u8`/`9usize` exit 0
- `dev_gate_fast` 1471/0
...
#### Green measured (§104-6 slice 10)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_let_pattern_statement` + helpers; `SemanticStatementLetPattern` arm in `mir_lower_statement` |
| `mir-coverage.sh` | `lower_error_count=638` (<646); hist `unsupported statement=11` (<19); `mir_functions=2509` |
| Smoke | `--run` `.tmp/s10_let_pattern_smoke.mlc` (`let { a, b } = Pair {…}`) exit 0 |
...
| 2 | Green: stack+arms; LEC < 638; hist statement < 11 | **done** 2026-08-07 (reopen fix) |
| 3 | Critic | **done** 2026-08-07 — CLOSED |

#### Red measured (§104-6 slice 11)

- Harness: `compiler/scripts/mir-coverage_s11_red.sh` → exit 1 `no Break/Continue arms / loop_break_targets (Red expected)`
- `mir_lower_statement`: no `SemanticStatementBreak` / `Continue` arms
- No `loop_break_targets` on `MirLowerState`
- Coverage baseline: `lower_error_count=638`; hist `unsupported statement=11`

#### Green measured (§104-6 slice 11)

| Check | Result |
|-------|--------|
| Wiring | `loop_break_targets`/`loop_continue_targets` on `MirLowerState`; Break/Continue → `MirJump`; while/for push/pop; for `for_continue` block (index++ then Jump header); finish_current_block uses latest allocate state (id_pool) |
| `mir-coverage.sh` | `lower_error_count=630` (<638); hist **no** `unsupported statement`; `mir_functions=2525` |
| Smoke | `--run` if+break → exit 7; while+continue odd-sum → exit 25 |
...
#### Green reopen measured (§104-6 slice 11) — 2026-08-07

| Check | Result |
|-------|--------|
| Fix | `const for_blocks_allocated_state = continue_block_step.state` then `finish(for_blocks_allocated_state, …)` — avoids codegen misbind of `continue_block_step.state`→`loop_header_step.state` |
| Emit | `lower_fn.cpp` has `finish(for_blocks_allocated_state, …)` (not `loop_header_step.state`) |
| for+if | `--run` exit 16; MIR ids unique `[0,1,2,5,6,7,4,3]` dups=0 |
| for+break | `--run` exit 3 |
| for+continue | `--run` exit 25 |
| while+break | `--run` exit 7 (regression) |
| Coverage | LEC=630; hist statement absent |
| Red after | exit 1 `already has Break/Continue arms` |
| Self-host | retranslate IDENTICAL (`diff -rq` empty excl. obj) |
| `dev_gate_fast` | 1471/0 |

#### Critic re-audit (2026-08-07), §104-6 slice 11 CLOSED

Independent fixtures (not Driver's):
- for+if `[10,20,30]`+`x==20` → exit **160**; MIR ids unique dups=0
- for+break `[2,4,6,8]` break at 6 → exit **6**
- for+continue skip multiples of 3 → exit **12**
- while+continue → exit **2**
- Fresh emit: `finish(for_blocks_allocated_state, …)` present; no `finish(loop_header_step.state, …)` on for prep
- Coverage LEC=630; hist statement absent; red-after trips; `dev_gate_fast` 1471/0
...
- Harness: `compiler/scripts/mir-coverage_s12_red.sh` → exit 1 `no index_of/trim/drop natives (Red expected)`
...
#### Critic audit (2026-08-07), §104-6 slice 12 CLOSED

Independent re-audit (not a re-read of Driver log):

| Check | Result |
|-------|--------|
| Wiring | maps at `lower_fn.mlc:280-282`; dispatch+bodies in `native.mlc`; allowlist `runtime.mlc:54-56`; tip `2eac9dc3` matches |
| MIR dump | combined fixture: `call … = __mir_string_index_of(2)`, `__mir_string_trim(1)`, `__mir_array_drop(2)` |
| Smokes | independent `.tmp/critic_s12_{index_of,trim,drop,combined}.mlc` all `--run` exit 0 |
| Coverage | LEC=620 (<630); Δ=10 vs Red baseline 630 (= hist `4+3+3`); hist lines for three **absent** |
| Red after | `mir-coverage_s12_red.sh` exit 1 `natives already in VM` |
| Self-host | fresh p1→`critic_mlcc2`→p2; `diff -r --exclude=obj` IDENTICAL |
...
- Harness: `compiler/scripts/mir-coverage_s13_red.sh` → exit 1 `no byte_size/upper/to_lower/take natives (Red expected)`
...
#### Critic audit (2026-08-07), §104-6 slice 13 CLOSED

Independent re-audit (not a re-read of Driver log):

| Check | Result |
|-------|--------|
| Wiring | maps at `lower_fn.mlc:283-286`; dispatch+bodies in `native.mlc`; allowlist `runtime.mlc:57-60`; tip `3ab1d73e` matches |
| MIR dump | combined: `__mir_string_byte_size(1)`, `__mir_string_upper(1)`, `__mir_string_to_lower(1)`, `__mir_array_take(2)` |
| Smokes | independent UTF-8 byte_size + case + take + combined `--run` exit 0 |
| Coverage | LEC=616 (<620); Δ=4 vs Red baseline 620 (= hist `1+1+1+1`); hist four leaves **absent** |
| Red after | `mir-coverage_s13_red.sh` exit 1 `natives already in VM` |
| Self-host | fresh p1→`critic_mlcc2_s13`→p2; `diff -r --exclude=obj` IDENTICAL |
| Gate | `dev_gate_fast` 1471/0 |
...
|------|------|------|
| 0 | Decision freeze | **done** 2026-08-07 |
| 1 | Red: no any/all HOF desugar | **done** 2026-08-07 |
| 2 | Green: desugar; LEC < 580; hist any/all absent | **done** 2026-08-07 |
| 3 | Critic | **done** 2026-08-07 — CLOSED |

#### Red measured (§104-6 slice 14)

- Harness: `compiler/scripts/mir-coverage_s14_red.sh` → exit 1 `no any/all HOF desugar (Red expected)`
- No `fn mir_lower_array_predicate_hof_to_local`
- `mir_lower_method_to_local`: no `any`/`all` special-case
- Coverage baseline: `lower_error_count=616`; hist `any=36`, `all=9`

#### Green measured (§104-6 slice 14)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_array_predicate_hof_to_local` + `mir_lower_resolve_predicate_callback`; `method_to_local` special-cases `any`/`all` |
| Coverage | `lower_error_count=574` (<580); hist `any`/`all` **absent** |
| Smoke | `--run` any / all / combined exit 0; MIR shows `hof_predicate_*` blocks |
| Red after Green | exit 1 `mir_lower_array_predicate_hof_to_local already present` |
| Self-host | `diff -r` s14_p1/s14_p2 `--exclude=obj` IDENTICAL |
| `dev_gate_fast` | 1471/0 |

Residuals: parent open; LEC=574; hist head `fold=127`/`map=93`/CppIR/`operand-context=36` — next HOF after Critic likely `filter` or `map`.

#### Critic audit (2026-08-07), §104-6 slice 14 CLOSED

Independent re-audit (not a re-read of Driver log):

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_resolve_predicate_callback` + `mir_lower_array_predicate_hof_to_local`; `method_to_local` any/all arity-1; tip `9e6ea5f8` |
| MIR dump | nested smoke: nested `hof_predicate_*` + `__mir_length`/`__mir_array_get` |
| Smokes | independent any/all/nested `--run` exit 0 |
| Coverage | LEC=574 (<580); Δ=42 vs Red 616; hist any/all **absent**; residual `unknown lambda expression_is_named_argument=1` (Ident free-fn, Decision out of scope) |
| Red after | exit 1 `mir_lower_array_predicate_hof_to_local already present` |
| Self-host | fresh p1→`critic_mlcc2_s14`→p2; `diff -r --exclude=obj` IDENTICAL |
| Gate | `dev_gate_fast` 1471/0 |

No false-done. Slice 14 CLOSED. Parent remains open (LEC≠0).

### Slice 15 — array `filter` HOF desugar (predicate + push; after any/all)

#### Pre-cut facts (audit 2026-08-07, post-s14)

| Fact | Evidence |
|------|----------|
| Baseline | `mir-coverage`: `lower_error_count=574` `mir_functions=2590` |
| Hist | `fold=127`, `map=93`, CppIR `make_identifier…=49`, operand=36, …, **`filter=14`**, `flat_map=18` |
| Why filter next | Same boolean-predicate shape as s14 `any`/`all`; builds output array via existing `__mir_array_empty`/`__mir_array_push` — smaller than `map`/`fold` for one Green |
| Reuse | `mir_lower_resolve_predicate_callback`; for-like index walk; `mir_lower_lambda_inline_body`; array empty/push natives |
| Deferred | `map`/`fold`/`flat_map`; Option/Result `.filter` (non-array receiver — may surface as non-`unsupported method filter` residuals); CppIR; operand Lambda |

#### Decision (**frozen** 2026-08-07, Driver STEP=0)

| Item | Choice |
|------|--------|
| Problem | After s14 predicates, next small array HOF gap is `filter` (hist 14) |
| Fix | (1) Special-case `filter` arity 1 in `mir_lower_method_to_local`. (2) New `mir_lower_array_filter_hof_to_local`: lower receiver to array local; `result = __mir_array_empty()`; index loop; get element; inline predicate; if true → `result = __mir_array_push(result, element)`; yield result local. Callback = Lambda or lambda-bound Ident (reuse `mir_lower_resolve_predicate_callback`). **No new VM natives** |
| Gate | Red: no filter special-case / no `mir_lower_array_filter_hof_to_local`. Green: wired; `lower_error_count` **≤ 565** (Decision correction 2026-08-07: nested `map` +5 offsets 574−14); hist `unsupported method filter` **absent**; smoke `--run` filter; `dev_gate_fast`; self-host IDENTICAL |
| Sabotage | Claim Green while hist still lists filter; LEC>565; smoke fails |
| REG | no |
| Out of scope | `map`/`fold`/`flat_map`; Option/Result filter semantics; `lower_error_count=0` |

#### Steps (§104-6 slice 15)
...
- Harness: `compiler/scripts/mir-coverage_s15_red.sh` → exit 1 `no filter HOF desugar (Red expected)`
- No `fn mir_lower_array_filter_hof_to_local` in `lower_fn.mlc`
- `mir_lower_method_to_local`: no `method_name == 'filter'` special-case
- TRACK: no Green measured counters for slice 15

#### Green measured (§104-6 slice 15)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_array_filter_hof_to_local`; `method_to_local` special-cases `filter` arity 1; reuses `mir_lower_resolve_predicate_callback` |
| Coverage | `lower_error_count=565` (≤565); hist `filter` **absent**; `map` 93→98 (+5 nested reach) |
| Smoke | `--run` `.tmp/s15_filter_smoke.mlc` exit 0; MIR `hof_filter_*` + `__mir_array_empty`/`push` |
| Red after Green | exit 1 `mir_lower_array_filter_hof_to_local already present` |
| Self-host | `diff -r` s15_p1/s15_p2 `--exclude=obj` IDENTICAL |
| `dev_gate_fast` | 1471/0 |
...
- Harness: `compiler/scripts/mir-coverage_s16_red.sh` → exit 1 `no map HOF desugar (Red expected)`
- No `fn mir_lower_array_map_hof_to_local` in `lower_fn.mlc`
- `mir_lower_method_to_local`: no `method_name == 'map'` special-case
- TRACK: no Green measured counters for slice 16

#### Green measured (§104-6 slice 16)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_array_map_hof_to_local`; `method_to_local` special-cases `map` arity 1; reuses resolve callback |
| Coverage | `lower_error_count=477` (≤520); hist `map` **absent**; `fold` 127→130 (+3 nested) |
| Smoke | `--run` `.tmp/s16_map_smoke.mlc` exit 0; MIR `hof_map_*` + empty/push |
| Red after Green | exit 1 `mir_lower_array_map_hof_to_local already present` |
| Self-host | `diff -r` s16_p1/s16_p2 `--exclude=obj` IDENTICAL |
| `dev_gate_fast` | 1471/0 |
...
### Slice 17 — array `fold` HOF desugar (accumulator + 2-arg callback; after map)

#### Pre-cut facts (audit 2026-08-07, post-s16)

| Fact | Evidence |
|------|----------|
| Baseline | `mir-coverage`: `lower_error_count=477` `mir_functions=2689` |
| Hist | **`fold=130`**, CppIR `make_identifier…=50`, operand=36, `type_is_unknown=26`, **`flat_map=18`**, … |
| Why fold next | Hist head; last major array HOF after any/all/filter/map. Shape: `arr.fold(init, (acc, elem) => …)` — arity 2 |
| Reuse | for-like index walk; `mir_lower_lambda_inline_body` (already multi-param); `__mir_length`/`__mir_array_get`; resolve Lambda/Ident |
| Deferred | `flat_map`; CppIR `make_*`; free-fn Ident non-lambda; `lower_error_count=0` |

#### Decision (**frozen** 2026-08-07, Driver STEP=0)

| Item | Choice |
|------|--------|
| Problem | After s16 map, dominant residual is `fold` (hist 130) |
| Fix | (1) Special-case `fold` arity 2 in `mir_lower_method_to_local`. (2) New `mir_lower_array_fold_hof_to_local`: lower receiver + init into locals; index loop; get element; inline 2-param callback with `[acc, element]` into next-acc temp; assign acc ← next; yield acc. Callback = Lambda or lambda-bound Ident (reuse/extend resolve helper; require `parameter_names.length() == 2`). **No new VM natives** |
| Gate | Red: no fold special-case / no `mir_lower_array_fold_hof_to_local`. Green: wired; `lower_error_count` **≤ 400** (477−130=347 plus nested-reach buffer — fold-heavy codegen); hist `unsupported method fold` **absent**; smoke `--run` fold; `dev_gate_fast`; self-host IDENTICAL |
| Sabotage | Claim Green while hist still lists fold; LEC>400; smoke fails |
| REG | no |
| Out of scope | `flat_map`; CppIR; `lower_error_count=0` |

#### Steps (§104-6 slice 17)

| Step | Item | Gate |
|------|------|------|
| 0 | Decision freeze | **done** 2026-08-07 |
| 1 | Red: no fold HOF desugar | **done** 2026-08-07 |
| 2 | Green: desugar; LEC ≤ 400; hist fold absent | **done** 2026-08-09 |
| 3 | Critic | **done** 2026-08-09 — CLOSED |

#### Red evidence (§104-6 slice 17)

- Harness: `compiler/scripts/mir-coverage_s17_red.sh` → exit 1 `no fold HOF desugar (Red expected)`
- No `fn mir_lower_array_fold_hof_to_local` in `lower_fn.mlc`
- `mir_lower_method_to_local`: no `method_name == 'fold'` special-case
- TRACK: no Green measured counters for slice 17

#### Green measured (§104-6 slice 17)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_array_fold_hof_to_local`; `method_to_local` special-cases `fold` arity 2; reuses resolve callback (2-param) |
| Coverage | `lower_error_count=354` (≤400); hist `fold` **absent**; nested +7 vs 477−130 |
| Smoke | `--run` `.tmp/s17_fold_smoke.mlc` exit 0; MIR `hof_fold_*` |
| Red after Green | exit 1 `mir_lower_array_fold_hof_to_local already present` |
| Self-host | `diff -r` s17_p1/s17_p2 `--exclude=obj` IDENTICAL |
| `dev_gate_fast` | 1471/0 |

Residuals: parent open; LEC=354; hist head CppIR/`flat_map=19`/operand — next after Critic likely `flat_map` or CppIR leaf.

#### Critic audit (2026-08-09), §104-6 slice 17 CLOSED

Independent re-audit (not a re-read of Driver log):

| Check | Result |
|-------|--------|
| Wiring | tip `997edf7e`: `mir_lower_array_fold_hof_to_local` + `method_to_local` fold arity-2; init + 2-arg callback; no new VM natives |
| Smokes | independent fold + nested `map.fold` `--run` exit 0 |
| MIR | nested: `hof_map_*` → `hof_fold_*` |
| Coverage | LEC=354 (≤400); Δ=123 vs Red 477; hist fold **absent**; flat_map=19 |
| Red after | exit 1 `mir_lower_array_fold_hof_to_local already present` |
| Self-host | fresh critic_s17_p1 vs `mlcc2_s17` p2; `diff -r --exclude=obj` IDENTICAL; binaries `cmp` equal |
| Gate | `dev_gate_fast` 1471/0 |
...
#### Green measured (§104-6 slice 18)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_array_flat_map_hof_to_local`; `method_to_local` special-cases `flat_map` arity 1; `__mir_array_concat` join (no new natives) |
| Coverage | `lower_error_count=336` (≤340); Δ=18 vs Red 354; hist `flat_map` **absent** |
| Smoke | `--run` `.tmp/s18_flat_map_smoke.mlc` + nested map/filter exit 0; MIR `hof_flat_map_*` + `__mir_array_concat` |
...
- Harness: `compiler/scripts/mir-coverage_s19_red.sh` → exit 1 `no module-qualified call / File natives (Red expected)`
- No `mir_lower_module_qualified_method_to_local` / static-module helper
- No `__mir_file_*` in `lower_fn` / `native.mlc` / `runtime.mlc`
- `mir_lower_method_to_local`: no File/lookup_local static path
- `mir_lower_method_native_name`: no exists/read/write maps
- TRACK: no Green measured counters for slice 19
- Coverage (`MLCC=.tmp/mlcc2_s18`): `mir_functions=2832` `lower_error_count=336`; hist `make_identifier_cpp_expression=50` `type_is_unknown=26`

#### Green measured (§104-6 slice 19)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_module_qualified_method_to_local` + `mir_lower_file_static_method_to_local`; Ident not-in-locals → Call/File natives |
| VM | `__mir_file_exists`/`read`/`write` + `file_abi.hpp` host externs; runtime allowlist |
| Coverage | `lower_error_count=95` (≤220); Δ=241 vs Red 336; hist make_identifier + type_is_unknown **absent** |
| Smoke | `--run` `.tmp/s19_file_smoke.mlc` exit 0 (write/exists/read) |
...
- Harness: `compiler/scripts/mir-coverage_s20_red.sh` → exit 1 `no record_with_field / RecordUpdate arms / Field mutate path (Red expected)`
- No `mir_lower_record_update_to_local`
- No `__mir_record_with_field` in `lower_fn` / `native.mlc` / `runtime.mlc`
- No `__mir_file_make_temp_directory`
- No `SemanticExpressionRecordUpdate` arms in `lower_fn.mlc`
- `mir_lower_statement`: mutating path Ident-only (`mutating method requires identifier receiver`); no Field write-back
...
**Kind probe (temporary instrument on `_` arms, probe binary discarded, `lower_fn.mlc`
restored to tip):** operand **While=13**, **Lambda=9**; rvalue **While=4**;
**For=0**. So 17/26 of the opaque operand+rvalue residual is While used as a
value (block tail / let RHS / nested expression), not statement-shaped
`SemanticStatementExpr` (already wired via `mir_lower_while_statement`).

**Why this slice:** Highest clearable leaf without new VmValue representation.
Reuse existing CFG while/for lowerers; yield **Unit** local (same shape as
With/Extern). Lambda-as-value (9) and unknown-ident funrefs (22) stay deferred
(need closure / funref).

**Approach (Green):**
1. Add `mir_lower_while_to_local` / `mir_lower_for_to_local`: run
   `mir_lower_while_statement` / `mir_lower_for_statement`, then allocate a
   local assigned `MirOperandUnit`.
2. Wire **While** (+ **For** for completeness) in operand + rvalue +
   `expression_to_local` to those helpers.

**Expected Δ:** −17 (13+4 While). **Gate = LEC ≤ 35** (49−17=32 + margin).
Sabotage: LEC>35, or opaque operand/rvalue hist still contains While (kind
probe or residual count ≥ pre-slice While share).

**Non-goals:** Lambda-as-VmValue; unknown-ident funrefs; `visit_int`; claiming
`lower_error_count=0`; Wave 3.
...
- Harness: `compiler/scripts/mir-coverage_s22_red.sh` → exit 1 `no While/For to_local / operand|rvalue|expr While|For arms (Red expected)`
- No `mir_lower_while_to_local` / `mir_lower_for_to_local`
...
2. Coverage: LEC≤35; While absent from opaque operand/rvalue residual (or
...
#### Green measured (§104-6 slice 23)

| Check | Result |
|-------|--------|
| Wiring | `mir_lower_funref_ident_to_local` + operand/rvalue/`expression_to_local` Ident failure → ConstStr(name) |
| Coverage | `lower_error_count=10` (≤12); Δ=22 vs Red 32; hist `unknown identifier` **absent**; operand=9 Lambda residual; `visit_int`=1 |
| Smoke | C++ path: `apply_once(add_one, 41)` → binary exit 0 (`--run` VM rejects ConstStr funref — expected; Decision is C++ `--run` path) |
| Red after | exit 1 `funref Ident helper already present` |
| Self-host | `diff -r` p2/p3 `--exclude=obj` empty; `mlcc_s23`/`mlcc2_s23` `cmp` equal |
| `dev_gate_fast` | 1471/0 |
