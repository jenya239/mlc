...
## Status: **open** — STEP=1–2 **done**; STEP=3 next (tcp.mlc stdlib)

**Driver 2026-07-10:** STEP=2 — `mlc/net/tcp.hpp` (`TcpListener`/`TcpStream`,
`Result` errors, RAII, `SO_REUSEADDR`); `result::ok`/`err` move; C++ smoke
`runtime/test/test_tcp.cpp` 12/0; include in `mlc.hpp`.
