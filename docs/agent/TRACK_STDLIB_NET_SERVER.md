...
## Status: **open** — STEP=1–3 **done**; STEP=4 next (echo gate)

**Driver 2026-07-10:** STEP=3 — `std/net/tcp.mlc` + registry/scanner; free fns
`bind`/`accept`/`read`/`write_all`/… with opaque i32 handles + `Option`/
`last_error` (Result extern deferred); include in codegen; `tcp_stdlib_test` 1/18.
