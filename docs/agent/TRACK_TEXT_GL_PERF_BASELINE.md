...
**STEP=1** — Process-local `(font_path, pixel_size)` cache for `FT_Library` /
`FT_Face` in `freetype_shim.cpp` and `hb_font_t*` in `harfbuzz_shim.cpp`
(create once, reuse; no per-call `FT_Init_FreeType`/`FT_New_Face`).
...
| Step | Item | Status |
|------|------|--------|
| 7 | `freetype_shim.hpp`/`.cpp`: expose bearing — add `glyph_bearing_x()`/`glyph_bearing_y()` (mirroring the existing `glyph_width()`/`glyph_rows()` pattern) returning `face->glyph->bitmap_left`/`bitmap_top` from the last rendered glyph, same caching slot as `glyph_width`/`glyph_rows` | pending |
...
| 12 | Docs: `TEXT_RENDERING.md` — note the bearing fix + which files were affected; `GUI.md` if it references glyph positioning assumptions | pending |
