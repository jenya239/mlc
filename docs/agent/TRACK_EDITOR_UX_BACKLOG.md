...
## Status: **active** (2026-07-18) — #1 done; **#1b `EDITOR_LIVE_SOLARIZED_TEXT` opened** (STEP=0 done)
...
| 1c | `EDITOR_STALE_HELP_TEXT` | Found 2026-07-17: `demo_live.mlc` `sample_readme_text()` (shown in-app as `editor_live_README.md` tab) says "Tree: click folder to expand/collapse; click file to open" — but `demo_live` never calls `file_tree_expand`/`file_tree_collapse` (§38 `TRACK_EDITOR_FOLDER_BROWSER` Decision replaced expand-in-tree with breadcrumbs, intentionally, not a bug). Fix the in-app copy to describe actual breadcrumb+back/forward behavior | n/a — copy-only fix, no scenario required unless bundled with behavior change | S | **active** — [TRACK_EDITOR_STALE_HELP_TEXT](TRACK_EDITOR_STALE_HELP_TEXT.md) STEP=2 Critic (STEP=1 done) |
