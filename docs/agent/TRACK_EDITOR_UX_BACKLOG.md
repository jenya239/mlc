...
## Status: **active** (2026-07-18) — #1+#1b+#1c done; **#1d `EDITOR_LARGE_FILE_NO_FULL_STRINGIFY` opened** (STEP=0 done)
...
| 1d | `EDITOR_LARGE_FILE_NO_FULL_STRINGIFY` (pulled forward from #22, 2026-07-17 priority rule above) | Remove per-frame full-buffer `document_to_string` in demo — direct frame-latency regression on large files, Sublime-parity blocker | `large_file_no_full_stringify` (L2) | M | **active** — [TRACK_EDITOR_LARGE_FILE_NO_FULL_STRINGIFY](TRACK_EDITOR_LARGE_FILE_NO_FULL_STRINGIFY.md) STEP=3 Critic (STEP=2 done) |
