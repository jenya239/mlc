...
**STEP=1** — Red: assert `editor_ux_open_dropped_path` still calls `document_to_string`; stable stub `not implemented`.

### STEPs done in git

| Step | Commit (abbrev) | Notes |
|------|-----------------|-------|
| 0 | (this) | Decision: open dropped path without `document_to_string` |
...
| 1 | Red: drop_file still stringifies | pending |
| 2 | Open without stringify; green | pending |
| 3 | Critic: stable + full `run_ux_gate` | pending |

<!-- STEP=1 sub-steps: 1) red asserts document_to_string in drop_file.mlc; 2) stable stub; 3) docs -->
<!-- STEP=2 sub-steps: 1) tab_set_open_buffer or empty+update; 2) drop import; 3) green stable -->
