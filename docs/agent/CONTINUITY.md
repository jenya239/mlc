...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_DEMO_ORCHESTRATOR` STEP=3** (§46 #1; Standing discipline; no `compiler/`).
...
| **`TRACK_EDITOR_DEMO_ORCHESTRATOR` (§46 #1)** | **queue head** — STEP=0–1 done; STEP=2 tab strip → `ux/tab_strip`. Behavior-preserving; existing UX gates + `demo_live_fs_compile`. No `compiler/` |
