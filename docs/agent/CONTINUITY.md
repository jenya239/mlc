...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `TRACK_EDITOR_SOLID_PASS_CONTENT_CLIP` (§49 STEP=2). Priority override `#36` **closed** 2026-07-20. **Override 2026-07-22:** §46/#47/#48 done — §49 solid-pass minimap clip residual.
...
| **`TRACK_EDITOR_SOLID_PASS_CONTENT_CLIP` (PLAN §49)** | **active** 2026-07-22 — STEP=1 done → Driver STEP=2. Red: solid still scissors `editor_rect` |
