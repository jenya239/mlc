...
| **`TRACK_EDITOR_RENDER_ARCHITECTURE` (PLAN §97)** | **open** — §97b shared_span_cache slice closed; next Decision `minimap_cache`. Priority override. Sub-tracks §97a→§97b→§97c |
...
| **Authorized queue** | **§97b** Decision STEP=0 (`minimap_cache`→layout) — then dual-wrap / §97c; §93 after; SCRIPT_VM / MIR Epic 5 / LANG_AUTO_CYCLE still gated |
