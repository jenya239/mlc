...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Critic** `TRACK_EDITOR_CRLF_PRESERVE` (§46 `#12`).
...
| **`TRACK_EDITOR_CRLF_PRESERVE` (§46 #12)** | **active** 2026-07-18. STEP=2 done. Next Critic close |
