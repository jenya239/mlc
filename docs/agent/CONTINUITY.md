...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Critic** `TRACK_EDITOR_WRAPPED_TEXT_BLEEDS_INTO_MINIMAP` (§46 `#37`). Priority override `#36` **closed** 2026-07-20. **Override 2026-07-22:** `#38` idle CPU **closed**; `#37` minimap bleed open.
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | #1…#36 closed (#22 superseded). **`#38` closed**. **`#37 EDITOR_WRAPPED_TEXT_BLEEDS_INTO_MINIMAP` active** (STEP=3 → Critic). Product ceiling: **Sublime Text**, stability/speed beats feature breadth |
| **`TRACK_EDITOR_WRAPPED_TEXT_BLEEDS_INTO_MINIMAP` (§46 #37)** | **active** 2026-07-22 — STEP=3 done → Critic. Glyph batch uses `content_clip` |
