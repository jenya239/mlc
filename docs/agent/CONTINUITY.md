...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Planner** → §46 `#2 EDITOR_KEYBOARD_NAV_WIRE`.
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1+#1b+#1c+#1d done. Next: `#2 EDITOR_KEYBOARD_NAV_WIRE`. Product ceiling: **Sublime Text** |
