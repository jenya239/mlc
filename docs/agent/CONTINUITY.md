...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_MOUSE_WORD_LINE_SELECT` STEP=1** (§46 #4).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1…#1d+#2+#3 done. **#4 active:** [TRACK_EDITOR_MOUSE_WORD_LINE_SELECT](TRACK_EDITOR_MOUSE_WORD_LINE_SELECT.md) STEP=1. Then `#5`. Product ceiling: **Sublime Text** |
...
| **`TRACK_EDITOR_MOUSE_WORD_LINE_SELECT` (§46 #4)** | **queue head** — STEP=0 done 2026-07-18 (Decision frozen); STEP=1 L2 scenarios |
