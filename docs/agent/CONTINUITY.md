...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Planner** `TRACK_MLC_CONCURRENCY_REFINEMENT` (§47 open `#10 CONCURRENCY_SUPERVISOR_MLC_SURFACE`). Priority override `#36` **closed** 2026-07-20.
...
| **`TRACK_MLC_CONCURRENCY_REFINEMENT` (PLAN §47)** | Umbrella. `#1`–`#8` done. Next Planner: **`#9 CONCURRENCY_ISOLATE_MLC_SURFACE`**. Order fixed per `CONCURRENCY_V2.md` §20. `Future`/`async`/`await`/`select` out of scope |
