...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Planner** → §46 `#1c EDITOR_STALE_HELP_TEXT`.
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1+#1b done. Next: `#1c EDITOR_STALE_HELP_TEXT` →`#1d`→`#2`. Product ceiling: **Sublime Text** |
