...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_DRAG_AUTOSCROLL` STEP=1** (§46 #9).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1…#1d+#2+#3+#4+#5+#6+#7+#8 done. **#9 active:** [TRACK_EDITOR_DRAG_AUTOSCROLL](TRACK_EDITOR_DRAG_AUTOSCROLL.md) STEP=1. Then `#10`. Product ceiling: **Sublime Text** |
...
| **`TRACK_EDITOR_DRAG_AUTOSCROLL` (§46 #9)** | **queue head** — STEP=0 done 2026-07-18 (Decision); STEP=1 L1.5 scenario |
