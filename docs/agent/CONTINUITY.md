...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_GOTO_LINE` Critic** (§46 #8).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1…#1d+#2+#3+#4+#5+#6+#7 done. **#8 active:** [TRACK_EDITOR_GOTO_LINE](TRACK_EDITOR_GOTO_LINE.md) Critic. Then `#9`. Product ceiling: **Sublime Text** |
...
| **`TRACK_EDITOR_GOTO_LINE` (§46 #8)** | **queue head** — STEP=2 done 2026-07-18; Critic next |
