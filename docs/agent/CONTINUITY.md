...
| **`CONCURRENCY_SPAWN_DETACH`** (PLAN.md §11b) | Пользователь 2026-07-11: эмпирически найден блокирующий баг в `spawn` (fire-and-forget сериализует, не параллелит) — блокирует реальный многопоточный сервер, до этого не пойман ни одним треком, включая закрытый `STDLIB_HTTP_MLC`. Высокий приоритет — Planner берёт следующим после текущего активного трека, раньше остальной «без hand-written C++» группы |
