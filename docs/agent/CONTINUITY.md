...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `STEP=2` `TRACK_EDITOR_TRAILING_WS_VIZ` (§46 `#24`).
...
| **`TRACK_EDITOR_TRAILING_WS_VIZ` (§46 #24)** | **active** 2026-07-19. STEP=0 Decision frozen. Next STEP=1 L2 red harness. Do not skip Decision |
