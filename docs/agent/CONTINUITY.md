...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Planner pick-next** (§46 `#3 EDITOR_WORD_BOUNDARIES`).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1…#1d+#2 done. Next: Planner → `#3 EDITOR_WORD_BOUNDARIES`. Product ceiling: **Sublime Text** |
