...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `STEP=1` `TRACK_EDITOR_TREE_EXPAND_COLLAPSE` (§46 #33).
...
| **`TRACK_EDITOR_TREE_EXPAND_COLLAPSE` (§46 #33)** | **active** — nested tree rows + single-click toggle expand; keep enter-dir double-click. No `compiler/`. Do not skip Critic |
