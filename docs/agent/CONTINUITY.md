...
| **`TRACK_EDITOR_MVP` (PLAN §33) — PRIORITY OVERRIDE 2026-07-13** | Пользователь: editor MVP + mlc-support preflight — **голова очереди**. STEP=pre **done** (2026-07-13). Далее `ROLE=Driver` **STEP=0** scaffold `misc/editor/`, затем E1…E6. **Не** открывать `TRACK_GUI_SCENE_PHASE_B`, пока EDITOR не closed или пользователь явно не вернул canvas. Одноразовый override (как TEXT_GL_PERF), не общий паттерн |
