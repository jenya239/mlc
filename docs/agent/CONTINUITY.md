...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_UNDO_COALESCE` STEP=3 Critic** (§46 #6).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1…#1d+#2+#3+#4+#5 done. **#6 active:** [TRACK_EDITOR_UNDO_COALESCE](TRACK_EDITOR_UNDO_COALESCE.md) STEP=3 Critic. Then `#7`. Product ceiling: **Sublime Text** |
...
| **`TRACK_EDITOR_UNDO_COALESCE` (§46 #6)** | **queue head** — STEP=2 done 2026-07-18 (coalesce + app wire); STEP=3 Critic |
