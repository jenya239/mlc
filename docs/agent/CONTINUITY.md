...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `STEP=2` `TRACK_EDITOR_BRACKET_AUTOCLOSE` (§46 `#18`).
...
| **`TRACK_EDITOR_BRACKET_AUTOCLOSE` (§46 #18)** | **active** 2026-07-19. STEP=1 done (L1 red). Next STEP=2 API + wire. Do not skip Decision |
