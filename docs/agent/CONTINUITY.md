...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Planner** `pick-next` `PLAN_QUEUE` (§46 next `#27 EDITOR_DROP_FILE`).
...
| **`TRACK_EDITOR_INDENT_MODEL` (§46 #26)** | **active** 2026-07-19. STEP=3 done (demo_live Tab wire). Next Critic. Do not skip Decision |
