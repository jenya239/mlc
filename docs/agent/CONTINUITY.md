...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Critic** `TRACK_CONCURRENCY_MUTABLE_CAPTURE_CHECK` (§47 #1).
...
| **`TRACK_MLC_CONCURRENCY_REFINEMENT` (PLAN §47)** | Umbrella. **`#1 CONCURRENCY_MUTABLE_CAPTURE_CHECK` active** (STEP=2 done → Driver STEP=3). Order fixed per `CONCURRENCY_V2.md` §20. `Future`/`async`/`await`/`select` out of scope |
| **`TRACK_CONCURRENCY_MUTABLE_CAPTURE_CHECK` (§47 #1)** | **active** — STEP=0–2 done; next Driver STEP=3 MEMORY_MODEL. E087 on `TaskScope.spawn`. Touches `compiler/`. Do not skip Critic |
