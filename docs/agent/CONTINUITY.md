...
| **`TRACK_EDITOR_MVP` (PLAN §33) — PRIORITY OVERRIDE 2026-07-13** | Пользователь: editor MVP + mlc-support (1M/Opus preflight) — **голова очереди**. После Critic Phase A → `ROLE=Driver` **TRACK_EDITOR_MVP STEP=pre** (bundle+dry-run+Opus per LARGE_CONTEXT §7), затем STEP=0+. **Не** открывать `TRACK_GUI_SCENE_PHASE_B` и не продолжать GUI Phase B–D, пока EDITOR не closed или пользователь явно не вернул canvas. Одноразовый override (как TEXT_GL_PERF), не общий паттерн |
| **`TRACK_EDITOR_MVP` placement** | Design: `docs/EDITOR.md`, track `docs/agent/TRACK_EDITOR_MVP.md`. Код → `misc/editor/`. Не Script VM, не `text_ide_panels_demo` |
