...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_STALE_HELP_TEXT` STEP=2 Critic** (§46 #1c).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1+#1b done. **#1c active:** [TRACK_EDITOR_STALE_HELP_TEXT](TRACK_EDITOR_STALE_HELP_TEXT.md) STEP=2 Critic. Then `#1d`→`#2`. Product ceiling: **Sublime Text** |
...
| **`TRACK_EDITOR_STALE_HELP_TEXT` (§46 #1c)** | **queue head** — STEP=1 done 2026-07-18 (sample_* copy); STEP=2 Critic. No `compiler/` |
