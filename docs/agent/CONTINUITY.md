...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `TRACK_CONCURRENCY_MOVE_TRACKING` (§47 #3) STEP=1. Priority override `#36 EDITOR_WRAP_PER_FRAME_ON_LARGE_FILE` **closed** 2026-07-20 (Critic OK) — resume §47 `#3`.
