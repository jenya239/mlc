...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `STEP=1` `TRACK_EDITOR_SESSION_CARET_RESTORE` (§46 `#20`).
...
| **`TRACK_EDITOR_SESSION_CARET_RESTORE` (§46 #20)** | **active** 2026-07-19. STEP=0 Decision frozen. Next STEP=1 L1 red. Do not skip Decision |
