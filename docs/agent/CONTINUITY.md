...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Planner pick-next §46 `#6 EDITOR_UNDO_COALESCE`**.
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1…#1d+#2+#3+#4+#5 done. Next `#6 EDITOR_UNDO_COALESCE`. Product ceiling: **Sublime Text** |
