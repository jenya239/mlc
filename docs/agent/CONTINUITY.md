...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `STEP=2` `TRACK_EDITOR_DROP_FILE` (§46 `#27`).
...
| **`TRACK_EDITOR_DROP_FILE` (§46 #27)** | **active** 2026-07-19. STEP=0 Decision frozen. Next STEP=1 L1 `drop_file_opens_tab`. Do not skip Decision |
