...
**INSTRUCTIONS_REV:** `2026-07-16-editor-folder-browser` — bump when workflow/rules change.
...
Перед работой: `git status` + `git log --oneline -15`. Чужой uncommitted diff (`compiler/out/mlcc`, SCRIPT_VM design-only, `.tmp/**`, `compiler/out/extern_concurrency_lint.*`) — не удалять чужое; `git add` явным списком. **`misc/editor/ux/folder_panel.mlc` + demo_live/tab_strip/live-demo script — in scope для TRACK_EDITOR_FOLDER_BROWSER**. Не трогать `compiler/` в этом треке. Анти-false-done / анти-stale-docs — как в CONTINUITY.md. После правок `lib/mlc/` — `scripts/regression_gate.sh` перед Critic close.
...
| **`TRACK_EDITOR_FOLDER_BROWSER` (PLAN §38) — queue head** | Folder browser + tab close; absorb foreign WIP. Prefer `misc/editor/**`; no `compiler/` |
