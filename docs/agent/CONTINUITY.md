...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Driver** `TRACK_CONCURRENCY_MUTABLE_CAPTURE_CHECK` STEP=1 (§47 #1).
...
| **`TRACK_MLC_CONCURRENCY_REFINEMENT` (PLAN §47)** | Umbrella. **`#1 CONCURRENCY_MUTABLE_CAPTURE_CHECK` active** (STEP=0 → Driver STEP=1). Order fixed per `CONCURRENCY_V2.md` §20. `Future`/`async`/`await`/`select` out of scope |
| **`TRACK_CONCURRENCY_MUTABLE_CAPTURE_CHECK` (§47 #1)** | **active** — STEP=0 Decision frozen; next Driver STEP=1 red test. Gap: `TaskScope.spawn` skips E087 (bare `spawn` already OK). Touches `compiler/`. Self-host + Tier B required |
