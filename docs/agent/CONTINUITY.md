...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). As of 2026-07-17 test-fix Decision: Tier A no longer Ruby-rebuilds — `dev_gate_fast` green; STEP=2 **done**, proceed to STEP=3. `TRACK_EDITOR_FOLDER_NAV` STEP=3 Critic remains separately pending.
...
| **`TRACK_CODEGEN_CPPAST_ONLY` (PLAN §44) — queue head** | Eliminate string-concat codegen, CppAST only. STEP=1/test-fix/2 **done** (`CppInvokedBlock*` body → `[Shared<CppStatement>]`). STEP=3 next (`While`/`For`/`WithBlock`). Touches `compiler/` — Tier B + self-host every numbered STEP |
