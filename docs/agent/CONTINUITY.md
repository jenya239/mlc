...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **Planner** `TRACK_MLC_CONCURRENCY_REFINEMENT` (§47 open `#9 CONCURRENCY_ISOLATE_MLC_SURFACE`). Priority override `#36` **closed** 2026-07-20.
