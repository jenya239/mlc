...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). As of 2026-07-17 test-fix Decision: Tier A/B no longer Ruby-rebuilds — `dev_gate_fast` green; STEP=8b6 **done**, proceed to STEP=8b7. `TRACK_EDITOR_FOLDER_NAV` STEP=3 Critic remains separately pending.
...
| **`TRACK_CODEGEN_CPPAST_ONLY` (PLAN §44) — queue head** | Eliminate string-concat codegen, CppAST only. STEP=1/test-fix/2/3/4/5/6/7a–7b3c5/8a/8b1–8b6 **done** (`expr.mlc` 56 exports). STEP=8b7 next. Touches `compiler/` — Tier B + self-host every numbered STEP |
