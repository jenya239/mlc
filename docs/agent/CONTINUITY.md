...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_STALE_HELP_TEXT` STEP=1** (§46 #1c; copy-only; no `compiler/`).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1+#1b done. **#1c active:** [TRACK_EDITOR_STALE_HELP_TEXT](TRACK_EDITOR_STALE_HELP_TEXT.md) STEP=1 next. Then `#1d`→`#2`. Product ceiling: **Sublime Text** |
...
| **`TRACK_EDITOR_STALE_HELP_TEXT` (§46 #1c)** | **queue head** — STEP=0 Decision done 2026-07-18; STEP=1 fix sample_* copy (breadcrumb/nav). No `compiler/` |
