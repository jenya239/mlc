...
**`test_gate=fail` → `ROLE=Driver STEP=test-fix` before TRACK feature STEPs** (rotation table). Tier A (`dev_gate_fast`) green as of 2026-07-17 test-fix Decision. Queue head: **`TRACK_EDITOR_KEYBOARD_NAV_WIRE` STEP=3 Critic** (§46 #2).
...
| **`TRACK_EDITOR_UX_BACKLOG` (PLAN §46)** | Umbrella. #1…#1d done. **#2 active:** [TRACK_EDITOR_KEYBOARD_NAV_WIRE](TRACK_EDITOR_KEYBOARD_NAV_WIRE.md) STEP=3 Critic. Then `#3`. Product ceiling: **Sublime Text** |
...
| **`TRACK_EDITOR_KEYBOARD_NAV_WIRE` (§46 #2)** | **queue head** — STEP=2 done 2026-07-18 (GLFW nav keys + apply + demo_live); STEP=3 Critic |
