...
| **`TRACK_EDITOR_RENDER_ARCHITECTURE` (PLAN §97)** | **open** — §97c Critic NOT OK (stale UX greps); Driver retarget next. Priority override. Sub-tracks §97a→§97b→§97c |
...
| **Authorized queue** | **§97c** Driver retarget stale UX harness greps → re-Critic STEP=3; then §96 add-on → §93; SCRIPT_VM / MIR Epic 5 / LANG_AUTO_CYCLE still gated |
