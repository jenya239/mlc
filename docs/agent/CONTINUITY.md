...
| **`TRACK_EDITOR_MVP` (PLAN §33) — PRIORITY OVERRIDE 2026-07-13** | Пользователь: editor MVP — **голова очереди**. STEP=pre+0 **done**. Далее `ROLE=Driver` **STEP=1+** (E1…). **Не** открывать `TRACK_GUI_SCENE_PHASE_B`, пока EDITOR не closed или пользователь явно не вернул canvas. Одноразовый override (как TEXT_GL_PERF), не общий паттерн |
