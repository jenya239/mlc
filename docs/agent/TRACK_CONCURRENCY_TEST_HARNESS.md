...
## Status: **open** — STEP=3 done; STEP=4 (T4) next

**Driver 2026-07-09:** STEP=3 — `stress_mutex`/`stress_arc`/`stress_spawn` +
smoke wire; fixed `spawn_task` eager future + decay-copy.
...
| 4 (T4) | `scripts/concurrency_sanitize_gate.sh` (asan/ubsan/tsan матрица по всем stress-тестам) + wiring в `.github/workflows/ci.yml` как обязательная job (не опциональный ручной `MLC_TSAN=1`). | **next** |
