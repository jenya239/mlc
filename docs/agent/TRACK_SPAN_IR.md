...
```
bundle exec rake test_compiler_mlc   # 751 pass (baseline post BUILD)
...
| Step | Item | Status |
|------|------|--------|
| 1 | `SDeclFn` + `Span`; propagate in `transform_decl.mlc`; test | pending |
...
**STEP=1** — `SDeclFn` span + transform propagation + test.
