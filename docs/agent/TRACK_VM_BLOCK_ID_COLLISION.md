...
Related but distinct: [../archive/tracks/TRACK_VM_TRAMPOLINE.md](../archive/tracks/TRACK_VM_TRAMPOLINE.md)
...
## Status: **open, критический приоритет внутри VM-трека** — выше
`TRACK_CLI_STDIN`, наравне/выше остатка `TRACK_MIR_VM_FULL` Epic 4 work;
это не масштаб/скорость (как был `VM_TRAMPOLINE`), это **неверный CFG**,
который либо тихо портит логику, либо вешает процесс в бесконечном цикле.
...
| Step | Item | Status |
|------|------|--------|
| 1 | Применить фикс (строка 780, `then_block_step.state` → `else_block_step.state`). Пересобрать `mlcc`. | pending |
| 2 | Regression: repro выше (`classify(0/1/5)`) — все три случая корректны, без зависания. Добавить как постоянную фикстуру в `compiler/tests/` (класс бага никогда не должен вернуться незаметно). | pending |
| 3 | Проверить глубже вложенные цепочки (elif × 5+, `if` вложенный в `then`-ветку, не только `else`) — тот же баг теоретически может проявляться и там (см. анализ выше — риск не ограничен именно `else`-веткой, зависит от того, что аллокация из "забытого" состояния происходит первой). | pending |
| 4 | Прогнать весь существующий VM-корпус (`vm_cpp_diff_programs.txt`, `run_single_file_vm_gate.sh`, `run_examples_vm_gate.sh`, `run_vm_trampoline_depth_gate.sh`) — чистый рефактор одной строки, не должно быть регрессий. | pending |
| 5 | Verify-gate + close: self-host (`mlcc`→`mlcc2`→`diff`), `regression_gate.sh`. Обновить `TRACK_MIR_VM_FULL.md` — отметить, что elif/nested-if был сломан до этого фикса. | pending |
