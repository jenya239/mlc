...
| 7b2 | Convert `gen_match_guarded_body` → `[Shared<CppStatement>]`; kill Fragment wrappers in guarded-match / `gen_match_default_block_cpp`; string_match string-path Fragments | pending |
| 7b3 | Remaining: `#line` / FFI binder+deleter / `GenStmtResult` string bridge / `derive_methods_cpp` JSON Fragments; delete unused `cpp_*_from_string_output` | pending |
