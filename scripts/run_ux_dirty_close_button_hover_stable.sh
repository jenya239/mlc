...
if ! grep -A24 'dirty_close_overlay_is_visible(app.overlay) then' "$DEMO" | grep -q 'editor_ux_chrome_hover_draw_entry'; then
...
if ! grep -A24 'dirty_close_overlay_is_visible(app.overlay) then' "$DEMO" | grep -q 'theme.danger_red'; then
  echo "[ux dirty_close_button_hover_stable] FAIL: discard missing danger base" >&2
  exit 1
fi
if ! grep -A24 'dirty_close_overlay_is_visible(app.overlay) then' "$DEMO" | grep -q 'theme.accent_red'; then
...
if ! grep -A24 'dirty_close_overlay_is_visible(app.overlay) then' "$DEMO" | grep -q 'dirty_close_overlay_discard_rect'; then
  echo "[ux dirty_close_button_hover_stable] FAIL: missing discard_rect helper" >&2
  exit 1
fi
if ! grep -A24 'dirty_close_overlay_is_visible(app.overlay) then' "$DEMO" | grep -q 'dirty_close_overlay_cancel_rect'; then
