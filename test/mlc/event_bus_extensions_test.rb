...
class EventBusExtensionsTest < Minitest::Test
  ...
  def test_bus_level_filtering_info
    bus = MLC::Common::Diagnostics::EventBus.new(min_level: :info)
    events = []
    bus.subscribe(:test) { |payload| events << payload }
  ...
  def test_bus_level_filtering_warning
    bus = MLC::Common::Diagnostics::EventBus.new(min_level: :warning)
    events = []
    bus.subscribe(:test) { |payload| events << payload }
  ...
  def test_bus_level_filtering_error
    bus = MLC::Common::Diagnostics::EventBus.new(min_level: :error)
    events = []
    bus.subscribe(:test) { |payload| events << payload }
  ...
  def test_structured_event
    ...
    assert_equal 1, events.size
    event = events.first
    assert_instance_of MLC::Common::Diagnostics::EventBus::Event, event
