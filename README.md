...
- Container/Context pattern для dependency injection
- Rule-based система для обработки различных конструкций языка
- Services для изолированной бизнес-логики
...
1. Установить зависимости: `bundle install` (при необходимости задать `BUNDLE_PATH=vendor/bundle`).
2. Запустить быстрые тесты: `bundle exec rake test_fast` (около 2–3 минут).
3. При необходимости полной проверки запустить весь набор: `bundle exec rake test` (~20 минут).
4. Убедиться, что бинарь доступен: `bin/mlc --help` возвращает справку.
5. При наличии дополнительных интеграционных окружений (GPU/OpenGL) прогнать соответствующие интеграционные тесты.
...
**Язык реализации компилятора:** Ruby 4.0+
...
**Container pattern:**
Используется для dependency injection и управления жизненным циклом сервисов в бэкенде.
...
```bash
bundle install
bundle exec rake test_unit   # ~30 сек, быстрая проверка
bundle exec rake test_fast   # ~2-3 мин, без E2E
bundle exec rake test        # ~20 мин, полный набор
```
...
```
2577 runs, 6561 assertions, 0 failures, 0 errors, 2 skips
Line Coverage: 84.27%, Branch Coverage: 58.82%
```

### Типы тестов

- **Unit тесты** (`rake test_unit`, ~30 сек): парсер, семантический анализ, codegen, C++ AST DSL
- **Integration тесты** (`rake test_integration`): полный pipeline MLC → C++
- **E2E тесты** (`rake test_e2e`): компиляция MLC → C++ → бинарник → запуск
- **Полный набор** (`rake test`, ~20 мин): все вышеперечисленное
...
Модель владения (Shared/Weak/Owned, move semantics, use-after-move detection) описана в документации, но семантический анализатор не проверяет ownership. Borrow checking отсутствует.

**Async/Await:**
Парсинг и AST узлы реализованы. Lowering в C++20 coroutines отсутствует — async код не компилируется в работающий C++.

**FFI:**
Отсутствует интеграция с внешними C/C++ библиотеками. Нет `extern` деклараций.

**Оптимизация:**
Базовые пассы (constant folding, dead code elimination) реализованы на уровне SemanticIR. Нет MIR/CFG/SSA для сложных оптимизаций.

**Инкрементальная компиляция:**
Отсутствует. Каждый запуск компилирует всю программу заново.

**Система типов:**
Нет tuple types, type aliases, const/constexpr, newtype pattern, полноценного dynamic dispatch для traits.

**Инфраструктура:**
Нет CI/CD, нет LSP, нет REPL, нет package manager.
