        ...
        module Services
          ...
          class ASTFactory
            def int_literal(value:, origin: nil)
              MLC::Source::AST::IntLit.new(value: value, origin: origin)
            end

            def float_literal(value:, origin: nil)
              MLC::Source::AST::FloatLit.new(value: value, origin: origin)
            end

            def string_literal(value:, origin: nil)
              MLC::Source::AST::StringLit.new(value: value, origin: origin)
            end

            def regex_literal(pattern:, flags: "", origin: nil)
              MLC::Source::AST::RegexLit.new(pattern: pattern, flags: flags, origin: origin)
            end

            def unit_literal(origin: nil)
              MLC::Source::AST::UnitLit.new(origin: origin)
            end

            def var_ref(name:, origin: nil)
              MLC::Source::AST::VarRef.new(name: name, origin: origin)
            end

            def member_access(object:, member:, origin: nil)
              MLC::Source::AST::MemberAccess.new(object: object, member: member, origin: origin)
            end

            def index_access(object:, index:, origin: nil)
              MLC::Source::AST::IndexAccess.new(object: object, index: index, origin: origin)
            end
            ...
            def lambda(params:, body:, origin: nil)
              MLC::Source::AST::Lambda.new(params: params, body: body, origin: origin)
            end

            def lambda_param(name:, type: nil, origin: nil)
              MLC::Source::AST::LambdaParam.new(name: name, type: type, origin: origin)
            end

            def let(name:, value:, body:, mutable: false, type: nil, origin: nil)
              MLC::Source::AST::Let.new(name: name, value: value, body: body, mutable: mutable, type: type, origin: origin)
            end

            def record_literal(type_name:, fields:, origin: nil)
              MLC::Source::AST::RecordLit.new(type_name: type_name, fields: fields, origin: origin)
            end

            def array_literal(elements:, origin: nil)
              MLC::Source::AST::ArrayLiteral.new(elements: elements, origin: origin)
            end

            def generator(var_name:, iterable:, origin: nil)
              MLC::Source::AST::Generator.new(var_name: var_name, iterable: iterable, origin: origin)
            end

            def list_comprehension(output_expr:, generators:, filters: [], origin: nil)
              MLC::Source::AST::ListComprehension.new(output_expr: output_expr, generators: generators, filters: filters, origin: origin)
            end

            def block_expr(statements:, result_expr:, origin: nil)
              MLC::Source::AST::BlockExpr.new(statements: statements, result_expr: result_expr, origin: origin)
            end
            ...
            def return_stmt(expr: nil, origin: nil)
              MLC::Source::AST::Return.new(expr: expr, origin: origin)
            end

            def do_expr(body:, origin: nil)
              MLC::Source::AST::DoExpr.new(body: body, origin: origin)
            end

            def assignment(target:, value:, origin: nil)
              MLC::Source::AST::Assignment.new(target: target, value: value, origin: origin)
            end

            def break_stmt(origin: nil)
              MLC::Source::AST::Break.new(origin: origin)
            end

            def continue_stmt(origin: nil)
              MLC::Source::AST::Continue.new(origin: origin)
            end

            def for_loop(var_name:, iterable:, body:, origin: nil)
              MLC::Source::AST::ForLoop.new(var_name: var_name, iterable: iterable, body: body, origin: origin)
            end

            def while_loop(condition:, body:, origin: nil)
              MLC::Source::AST::WhileLoop.new(condition: condition, body: body, origin: origin)
            end

            def block(statements:, origin: nil)
              MLC::Source::AST::Block.new(stmts: statements, origin: origin)
            end

            def if_stmt(condition:, then_branch:, else_branch: nil, origin: nil)
              MLC::Source::AST::IfStmt.new(condition: condition, then_branch: then_branch, else_branch: else_branch, origin: origin)
            end

            def match_expr(scrutinee:, arms:, origin: nil)
              MLC::Source::AST::MatchExpr.new(scrutinee: scrutinee, arms: arms, origin: origin)
            end
            ...
            def pattern_wildcard(origin: nil)
              MLC::Source::AST::Pattern.new(kind: :wildcard, data: {}, origin: origin)
            end

            def pattern_literal(value:, origin: nil)
              MLC::Source::AST::Pattern.new(kind: :literal, data: { value: value }, origin: origin)
            end

            def pattern_var(name:, origin: nil)
              MLC::Source::AST::Pattern.new(kind: :var, data: { name: name }, origin: origin)
            end
